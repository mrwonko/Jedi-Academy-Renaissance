...
namespace jar
{
    ...
    **/
    ...
    public:
		 ...
		 **/
		 static const std::vector<std::string> GetFilesInDirectory(const std::string& directoryPath);
		...
		/** \brief Returns all directories in a given directory except those starting with ".", relative to the g_rootDir, in alphabetical order
            \return vector of filenames (without path)
		 **/
		 static const std::vector<std::string> GetDirectoriesInDirectory(const std::string& directoryPath);
