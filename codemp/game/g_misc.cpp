...
static void misc_lightstyle_set ( gentity_t *ent)
{
	...
	{	//turn off
		...
		{
			char lightstyle[32];
			g_trap->GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle*3)+0, lightstyle, 32);
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+0, lightstyle);

			g_trap->GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle*3)+1, lightstyle, 32);
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+1, lightstyle);

			g_trap->GetConfigstring(CS_LIGHT_STYLES + (mLightOffStyle*3)+2, lightstyle, 32);
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+2, lightstyle);
		}else
		{
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+0, "a");
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+1, "a");
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+2, "a");
		}
	}
	else
	{	//Turn myself on now
		...
		{
			char lightstyle[32];
			g_trap->GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle*3)+0, lightstyle, 32);
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+0, lightstyle);

			g_trap->GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle*3)+1, lightstyle, 32);
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+1, lightstyle);

			g_trap->GetConfigstring(CS_LIGHT_STYLES + (mLightSwitchStyle*3)+2, lightstyle, 32);
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+2, lightstyle);
		}
		else
		{
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+0, "z");
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+1, "z");
			g_trap->SetConfigstring(CS_LIGHT_STYLES + (mLightStyle*3)+2, "z");
		}
	}
}
...
void TeleportPlayer( gentity_t *player, vec3_t origin, vec3_t angles ) {
	...
	g_trap->UnlinkEntity ((sharedEntity_t *)player);
	...
	if ( player->client->sess.sessionTeam != TEAM_SPECTATOR ) {
		g_trap->LinkEntity ((sharedEntity_t *)player);
	}
}
...
void misc_model_breakable_gravity_init( gentity_t *ent, qboolean dropToFloor )
{
	...
	if ( dropToFloor )
	{
		...
		g_trap->Trace( &tr, top, ent->r.mins, ent->r.maxs, bottom, ent->s.number, MASK_NPCSOLID, qfalse, 0, 0 );
...
void SP_misc_G2model( gentity_t *ent ) {
	...
	g_trap->G2API_InitGhoul2Model(&ent->s, name1, G_ModelIndex( name1 ), 0, 0, 0, 0);
	g_trap->G2API_SetBoneAnim(ent->s.ghoul2, 0, "model_root", 0, 12, BONE_ANIM_OVERRIDE_LOOP, 1.0f, level.time, -1, -1);
	ent->s.radius = 150;
//	VectorSet (ent->r.mins, -16, -16, -16);
//	VectorSet (ent->r.maxs, 16, 16, 16);
	g_trap->LinkEntity ((sharedEntity_t *)ent);
...
void locateCamera( gentity_t *ent ) {
	...
	if ( !owner ) {
		g_trap->Print( "Couldn't find target for misc_partal_surface\n" );
		G_FreeEntity( ent );
		return;
	}
...
void SP_misc_bsp(gentity_t *ent)
{
	...
	g_trap->SetBrushModel( (sharedEntity_t *)ent, temp );  // SV_SetBrushModel -- sets mins and maxs
	G_BSPIndex(temp);
	...
	*/
	...
	g_trap->SetActiveSubBSP(ent->s.modelindex);
	G_SpawnEntitiesFromString(qtrue);
	g_trap->SetActiveSubBSP(-1);
...
void G_PortalifyEntities(gentity_t *ent)
{
	...
	while (i < MAX_GENTITIES)
	{
		...
		if (scan && scan->inuse && scan->s.number != ent->s.number && g_trap->InPVS(ent->s.origin, scan->r.currentOrigin))
		{
			trace_t tr;

			g_trap->Trace(&tr, ent->s.origin, vec3_origin, vec3_origin, scan->r.currentOrigin, ent->s.number, CONTENTS_SOLID, qfalse, 0, 0);
...
void SP_misc_skyportal (gentity_t *ent)
{
	...
	g_trap->SetConfigstring( CS_SKYBOXORG, va("%.2f %.2f %.2f %.1f %i %.2f %.2f %.2f %i %i", ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], fov_x, (int)isfog, fogv[0], fogv[1], fogv[2], fogn, fogf ) );
...
void HolocronTouch(gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	{ //once we pick up this holocron we'll have all of them, so give us super special best prize!
		//g_trap->Print("You deserve a pat on the back.\n");
	}
...
void SP_misc_holocron(gentity_t *ent)
{
	...
	if ( tr.startsolid )
	{
		g_trap->Print ("SP_misc_holocron: misc_holocron startsolid at %s\n", vtos(ent->s.origin));
		G_FreeEntity( ent );
		return;
	}
...
void InitShooter( gentity_t *ent, int weapon ) {
	...
	RegisterItem( BG_FindItemForWeapon( static_cast< weapon_t >( weapon ) ) );
...
void SP_misc_ammo_floor_unit(gentity_t *ent)
{
	...
	if ( tr.startsolid )
	{
		g_trap->Print ("SP_misc_ammo_floor_unit: misc_ammo_floor_unit startsolid at %s\n", vtos(ent->s.origin));
		G_FreeEntity( ent );
		return;
	}
...
void SP_misc_shield_floor_unit( gentity_t *ent )
{
	...
	if ( tr.startsolid )
	{
		g_trap->Print ("SP_misc_shield_floor_unit: misc_shield_floor_unit startsolid at %s\n", vtos(ent->s.origin));
		G_FreeEntity( ent );
		return;
	}
...
{
	BG_EvaluateTrajectory< moduleType::game >( &ent->s.pos, level.time, ent->r.currentOrigin );
...
void maglock_link( gentity_t *self )
{
	...
	g_trap->Trace( &trace, start, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void misc_faller_create( gentity_t *ent, gentity_t *other, gentity_t *activator )
{
	...
	g_trap->LinkEntity((sharedEntity_t *)faller);
}
...
//Maybe I should use my g_trap->TrueMalloc/g_trap->TrueFree stuff with this.
...
void SP_misc_weapon_shooter( gentity_t *self )
{
	...
	self->client = G_ClientForShooter();//(gclient_s *)g_trap->Malloc(sizeof(gclient_s), TAG_G_ALLOC, qtrue);
	...
	RegisterItem(BG_FindItemForWeapon(static_cast< weapon_t >( self->s.weapon )));
