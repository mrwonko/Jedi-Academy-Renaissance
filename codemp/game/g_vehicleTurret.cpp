...
//-----------------------------------------------------
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		g_trap->Trace( &tr, org2, NULL, NULL, org, parent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
//-----------------------------------------------------
{
	...
	if ( turretEnemy != NULL )
	{
		if ( turretEnemy->health > 0 )
		{
			...
			{
				// was in valid radius
				if ( g_trap->InPVS( pVeh->m_vMuzzlePos[curMuzzle], turretEnemy->r.currentOrigin ) )
				{
					...
					g_trap->Trace( &tr, start, NULL, NULL, end, parent->s.number, MASK_SHOT, qfalse, 0, 0 );
