		...
		)
	{
			  ...
			  (pVeh->m_pPilot->playerState->weapon == WP_SABER && BG_SabersOff( pVeh->m_pPilot->playerState ) ))) )
		{
			if ((curTime - pVeh->m_iTurboTime)>pVeh->m_pVehicleInfo->turboRecharge)
			{
				...
				if ( mod == moduleType::game && pVeh->m_pVehicleInfo->iTurboStartFX)
				{
					int i;
					for (i=0; (i<MAX_VEHICLE_EXHAUSTS && pVeh->m_iExhaustTag[i]!=-1); i++)
					{
						...
						{ //fine, I'll use a tempent for this, but only because it's played only once at the start of a turbo.
							...
							g_trap->G2API_GetBoltMatrix(pVeh->m_pParentEntity->ghoul2, 0, pVeh->m_iExhaustTag[i], &boltMatrix, boltDir, pVeh->m_pParentEntity->playerState->origin, level.time, NULL, pVeh->m_pParentEntity->modelScale);
...
void AnimateRiders( Vehicle_t *pVeh )
{
	...
	if ( pVeh->m_iBoarding != 0 )
	{
		...
		if ( pVeh->m_iBoarding < 0 )
		{
			...
			pVeh->m_iBoarding = BG_GetTime< mod >() + iAnimLen;
			...
			BG_SetAnim< mod >(pVeh->m_pPilot->playerState, bgAllAnims[pVeh->m_pPilot->localAnimIndex].anims,
...
void G_SetSpeederVehicleFunctions( vehicleInfo_t *pVehInfo )
{
	if( mod == moduleType::game ) {
		...
		pVehInfo->AnimateRiders = AnimateRiders< mod >;
...
template void G_SetSpeederVehicleFunctions< moduleType::game >( vehicleInfo_t *pVehInfo );
template void G_SetSpeederVehicleFunctions< moduleType::cgame >( vehicleInfo_t *pVehInfo );
...
template void G_CreateSpeederNPC< moduleType::game >( Vehicle_t **pVeh, const char *strType );
template void G_CreateSpeederNPC< moduleType::cgame >( Vehicle_t **pVeh, const char *strType );
