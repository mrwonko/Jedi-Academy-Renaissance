...
void BG_SiegeParseClassFile(const char *filename, siegeClassDesc_t *descBuffer)
{
	...
	if (BG_SiegeGetPairedValue(classInfo, "uishader", parseBuf))
	{
		if( mod == moduleType::ui ) {
			bgSiegeClasses[ bgNumSiegeClasses ].uiPortraitShader = re->RegisterShaderNoMip( parseBuf );
	...
	{
		bgSiegeClasses[ bgNumSiegeClasses ].classShader = mod == moduleType::game ? 0 : re->RegisterShaderNoMip( parseBuf );
		assert( mod == moduleType::game || bgSiegeClasses[bgNumSiegeClasses].classShader );
		if ( mod != moduleType::game && !bgSiegeClasses[bgNumSiegeClasses].classShader )
		{
...
void BG_SiegeLoadClasses(siegeClassDesc_t *descBuffer)
{
	...
	for (i = 0; i < numFiles; i++, fileptr += filelen+1)
	{
		...
		if (descBuffer)
		{
			BG_SiegeParseClassFile< mod >(filename, &descBuffer[i]);
		}
		else
		{
			BG_SiegeParseClassFile< mod >(filename, NULL);
		}
	}
}
template void BG_SiegeLoadClasses< moduleType::game >( siegeClassDesc_t *descBuffer );
template void BG_SiegeLoadClasses< moduleType::cgame >( siegeClassDesc_t *descBuffer );
template void BG_SiegeLoadClasses< moduleType::ui>( siegeClassDesc_t *descBuffer );
//======================================
...
void BG_SiegeParseTeamFile(const char *filename)
{
	...
	if( mod == moduleType::cgame ) {
		if( BG_SiegeGetPairedValue( teamInfo, "FriendlyShader", parseBuf ) )
			bgSiegeTeams[ bgNumSiegeTeams ].friendlyShader = re->RegisterShaderNoMip( parseBuf );
	} else {
...
void BG_SiegeLoadTeams(void)
{
	...
	for (i = 0; i < numFiles; i++, fileptr += filelen+1)
	{
		...
		Q_strcat(filename, sizeof(filename), fileptr);
		BG_SiegeParseTeamFile< mod >(filename);
	}
}
template void BG_SiegeLoadTeams< moduleType::game >( void );
template void BG_SiegeLoadTeams< moduleType::cgame >( void );
template void BG_SiegeLoadTeams< moduleType::ui >( void );
//======================================
	...
	if (t)
	{
		...
		while (i < t->numClasses)
		{
			...
			while (sNum < MAX_SABERS)
			{
				...
				{
					WP_SaberParseParms< mod >(saberName, &saber);
					if (!Q_stricmp(saberName, saber.name))
					{ //found the matching saber
						if (saber.model[0])
						{
							BG_ModelCache< mod >(saber.model, NULL);
						}
					}
				}
...
template void BG_PrecacheSabersForSiegeTeam< moduleType::game >( int team );
template void BG_PrecacheSabersForSiegeTeam< moduleType::cgame >( int team );
