...
//		assert(0);
	...
	g_trap->EA_SelectWeapon(client, weapon);
}
...
void BotReportStatus(bot_state_t *bs)
{
	...
	{
		g_trap->EA_SayTeam(bs->client, teamplayStateDescriptions[bs->teamplayState]);
	}
	else if (level.gametype == GT_SIEGE)
	{
		g_trap->EA_SayTeam(bs->client, siegeStateDescriptions[bs->siegeState]);
	}
	else if (level.gametype == GT_CTF || level.gametype == GT_CTY)
	{
		g_trap->EA_SayTeam(bs->client, ctfStateDescriptions[bs->ctfState]);
	}
}
...
int BotAI_GetSnapshotEntity( int clientNum, int sequence, entityState_t *state ) {
	...
	entNum = g_trap->BotGetSnapshotEntity( clientNum, sequence );
	if ( entNum == -1 ) {
		memset(state, 0, sizeof(entityState_t));
		return -1;
	}
...
void BotEntityInfo(int entnum, aas_entityinfo_t *info) {
	g_trap->AAS_EntityInfo(entnum, info);
}
...
void BotChangeViewAngles(bot_state_t *bs, float thinktime) {
	...
	g_trap->EA_View(bs->client, bs->viewangles);
}
...
void BotUpdateInput(bot_state_t *bs, int time, int elapsed_time) {
	...
	g_trap->EA_GetInput(bs->client, (float) time / 1000, &bi);
	//respawn hack
...
int BotAI(int client, float thinktime) {
	...
	while( g_trap->BotGetServerCommand(client, buf, sizeof(buf)) ) {
...
#ifdef _DEBUG
	start = g_trap->Milliseconds();
#endif
	StandardBotAI(bs, thinktime);
#ifdef _DEBUG
	end = g_trap->Milliseconds();

	g_trap->Cvar_Update(&bot_debugmessages);
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	bs->gs = g_trap->BotAllocGoalState(client);

	//allocate a weapon state
	bs->ws = g_trap->BotAllocWeaponState();

	bs->inuse = qtrue;
	bs->entitynum = client;
	bs->setupcount = 4;
	bs->entergame_time = FloatTime();
	bs->ms = g_trap->BotAllocMoveState();
	numbots++;
...
int BotAIShutdownClient(int client, qboolean restart) {
	...
	g_trap->BotFreeMoveState(bs->ms);
	//free the goal state`
	g_trap->BotFreeGoalState(bs->gs);
	//free the weapon weights
	g_trap->BotFreeWeaponState(bs->ws);
	//
	//clear the bot state
...
void BotResetState(bot_state_t *bs) {
	...
	if (bs->ms) g_trap->BotResetMoveState(bs->ms);
	if (bs->gs) g_trap->BotResetGoalState(bs->gs);
	if (bs->ws) g_trap->BotResetWeaponState(bs->ws);
	if (bs->gs) g_trap->BotResetAvoidGoals(bs->gs);
	if (bs->ms) g_trap->BotResetAvoidReach(bs->ms);
}
...
int OrgVisible(vec3_t org1, vec3_t org2, int ignore)
{
	...
	g_trap->Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_SOLID, qfalse, 0, 0 );
...
int WPOrgVisible(gentity_t *bot, vec3_t org1, vec3_t org2, int ignore)
{
	...
	if (tr.fraction == 1)
	{
		g_trap->Trace(&tr, org1, NULL, NULL, org2, ignore, MASK_PLAYERSOLID, qfalse, 0, 0);
...
int OrgVisibleBox(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2, int ignore)
{
	...
	{
		g_trap->Trace(&tr, org1, mins, maxs, org2, ignore, MASK_SOLID, qfalse, 0, 0);
	}
...
int CheckForFunc(vec3_t org, int ignore)
{
	...
	g_trap->Trace(&tr, org, NULL, NULL, under, ignore, MASK_SOLID, qfalse, 0, 0);
...
int BotTrace_Strafe(bot_state_t *bs, vec3_t traceto)
{
	...
	g_trap->Trace(&tr, from, playerMins, playerMaxs, to, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
...
int BotTrace_Jump(bot_state_t *bs, vec3_t traceto)
{
	...
	g_trap->Trace(&tr, bs->origin, mins, maxs, traceto_mod, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	g_trap->Trace(&tr, tracefrom_mod, mins, maxs, traceto_mod, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	while (i < level.num_entities)
	{
		...
		{ //try to escape from anything with a non-0 s.weapon and non-0 damage. This hopefully only means dangerous projectiles.
		  //Or a sentry gun if bolt_Head == 1000. This is a terrible hack, yes.
			...
			{
				g_trap->Trace(&tr, bs->origin, NULL, NULL, ent->s.pos.trBase, bs->client, MASK_SOLID, qfalse, 0, 0);
...
{ //get the nearest possible waypoint to the flag since it's not in its original position
	...
	{
		g_trap->Trace(&tr, wp->origin, mins, maxs, flagEnt->s.pos.trBase, flagEnt->s.number, MASK_SOLID, qfalse, 0, 0);
	...
	while (i < gWPNum)
	{
		...
		{
			g_trap->Trace(&tr, gWPArray[i]->origin, mins, maxs, flagEnt->s.pos.trBase, flagEnt->s.number, MASK_SOLID, qfalse, 0, 0);
...
hasPoint:
	...
	if (bs->touchGoal)
	{
		//g_trap->Print("Please, master, let me touch it!\n");
	...
	if (bs->siegeState == SIEGESTATE_ATTACKER)
	{
		...
		{ //looks like we have no goals other than to keep the other team from completing objectives
			...
			if (bs->shootGoal)
			{
				...
				{
					g_trap->Trace(&tr, bs->origin, NULL, NULL, dif, bs->client, MASK_SOLID, qfalse, 0, 0);
...
void GetIdealDestination(bot_state_t *bs)
{
...
#ifdef _DEBUG
	g_trap->Cvar_Update(&bot_nogoals);
	...
	if (badthing)
	{
		...
		if (gWPArray[tempInt] && gWPArray[tempInt]->inuse && bs->escapeDirTime < level.time)
		{
			...
			if (plusLen < minusLen)
			{
				...
				//g_trap->Print("Escaping from scary bad thing [%s]\n", badthing->classname);
			}
		}
		//g_trap->Print("Run away run away run away!\n");
		return;
	}
	...
	{
		//g_trap->Print("I need something to do\n");
		idleWP = GetBestIdleGoal(bs);
...
void MeleeCombatHandling(bot_state_t *bs)
{
	...
	g_trap->Trace(&tr, usethisvec, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0, 0);
	...
	g_trap->Trace(&tr, bs->origin, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0, 0);
	...
	g_trap->Trace(&tr, midorg, mins, maxs, downvec, -1, MASK_SOLID, qfalse, 0, 0);
		...
		if (bs->currentEnemy && bs->currentEnemy->client)
		{
			...
			else if (bs->currentEnemy->client->ps.weapon == WP_SABER && bs->frame_Enemy_Len < 80 && ((Q_irand(1, 10) < 8 && bs->saberBFTime < level.time) || bs->saberBTime > level.time || BG_SaberInKata(bs->currentEnemy->client->ps.saberMove) || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK || bs->currentEnemy->client->ps.saberMove == LS_SPINATTACK_DUAL))
			{
				...
				while (checkIncr < idealDist)
				{
					...
					g_trap->Trace(&tr, bs->goalPosition, NULL, NULL, groundcheck, bs->client, MASK_SOLID, qfalse, 0, 0);
...
void BotAimLeading(bot_state_t *bs, vec3_t headlevel, float leadAmount)
{
	...
	//g_trap->Print("Leadin target with a velocity total of %f\n", vtotal);
...
int BotFallbackNavigation(bot_state_t *bs)
{
	...
	g_trap->Trace(&tr, bs->origin, mins, maxs, trto, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
void StrafeTracing(bot_state_t *bs)
{
	...
	g_trap->Trace(&tr, bs->origin, mins, maxs, rorg, bs->client, MASK_SOLID, qfalse, 0, 0);
	...
	g_trap->Trace(&tr, rorg, NULL, NULL, drorg, bs->client, MASK_SOLID, qfalse, 0, 0);
...
gentity_t *CheckForFriendInLOF(bot_state_t *bs)
{
	...
	g_trap->Trace(&tr, trfrom, mins, maxs, trto, bs->client, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	if (flagRed && flagBlue)
	{
			...
			bs->wpDestination == flagBlue)
		{
			...
			if (diddrop && desiredDrop)
			{
				...
				if (VectorLength(a) <= BOT_FLAG_GET_DISTANCE)
				{
					g_trap->Trace(&tr, bs->origin, mins, maxs, desiredDrop->s.pos.trBase, bs->client, MASK_SOLID, qfalse, 0, 0);
...
int BotSurfaceNear(bot_state_t *bs)
{
	...
	g_trap->Trace(&tr, bs->origin, NULL, NULL, fwd, bs->client, MASK_SOLID, qfalse, 0, 0);
...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	{ //stupid vehicle debug, I tire of having to connect another client to test passengers.
		...
		{ //in a vehicle, so...
			...
			{
				g_trap->EA_MoveForward(bs->client);

				if (bot_getinthecarrr.integer == 3)
				{ //use alt fire
					g_trap->EA_Alt_Attack(bs->client);
				}
			}
		}
		else
		{ //find one, get in
			...
			{ //broke before end so we must've found something
				...
				g_trap->EA_Move(bs->client, v, 5000.0f);
	...
	if (bot_forgimmick.integer)
	{
		...
		{ //for testing cpu usage moving around rmg terrain without AI
			...
			g_trap->EA_Attack(bs->client);
			g_trap->EA_Move(bs->client, mdir, 5000);
		}
		...
		if (bs->forceMove_Right)
		{
			...
			{
				g_trap->EA_MoveRight(bs->client);
			}
			else
			{
				g_trap->EA_MoveLeft(bs->client);
			}
		}
		if (bs->forceMove_Up)
		{
			g_trap->EA_Jump(bs->client);
		}
		return;
	}
	...
	{
		if (bs->currentEnemy && bs->currentEnemy->client &&
			!ForcePowerUsableOn( &g_entities[bs->client], bs->currentEnemy, static_cast<forcePowers_t>( level.clients[bs->client].ps.fd.forcePowerSelected) ) )
		{
			useTheForce = 0;
			forceHostile = 0;
		}
	...
	else if (bs->doChat && bs->chatTime <= level.time)
	{
		if (bs->chatTeam)
		{
			g_trap->EA_SayTeam(bs->client, bs->currentChat);
			bs->chatTeam = 0;
		}
		else
		{
			g_trap->EA_Say(bs->client, bs->currentChat);
		}
		if (bs->doChat == 2)
		{
	...
	if (bs->beStill < level.time && !WaitingForNow(bs, bs->goalPosition) && !fjHalt)
	{
		...
		{
		//	g_trap->EA_Move(bs->client, bs->origin, 5000);
			bs->beStill = level.time + 200;
		}
		else
		{
			g_trap->EA_Move(bs->client, bs->goalMovedir, 5000);
		}
	...
	if (useTheForce)
	{
		...
		if (bs->forceJumpChargeTime > level.time)
		{
			...
			g_trap->EA_ForcePower(bs->client);
		}
		else
		{
#endif
...
//	static int botlib_residual;
	...
	{
		g_trap->Cvar_Update(&bot_pvstype);
		g_trap->Cvar_Update(&bot_camp);
		g_trap->Cvar_Update(&bot_attachments);
		g_trap->Cvar_Update(&bot_forgimmick);
		g_trap->Cvar_Update(&bot_honorableduelacceptance);
#ifndef FINAL_BUILD
		g_trap->Cvar_Update(&bot_getinthecarrr);
#endif
	...
	if (gBotEdit)
	{
		g_trap->Cvar_Update(&bot_wp_info);
		BotWaypointRender();
	}
	...
	for( i = 0; i < MAX_CLIENTS; i++ ) {
		...
		g_trap->BotUserCommand(botstates[i]->client, &botstates[i]->lastucmd);
	}
...
int BotAISetup( int restart ) {
	//rww - new bot cvars..
	g_trap->Cvar_Register(&bot_forcepowers, "bot_forcepowers", "1", CVAR_CHEAT);
	g_trap->Cvar_Register(&bot_forgimmick, "bot_forgimmick", "0", CVAR_CHEAT);
	g_trap->Cvar_Register(&bot_honorableduelacceptance, "bot_honorableduelacceptance", "0", CVAR_CHEAT);
	g_trap->Cvar_Register(&bot_pvstype, "bot_pvstype", "1", CVAR_CHEAT);
#ifndef FINAL_BUILD
	g_trap->Cvar_Register(&bot_getinthecarrr, "bot_getinthecarrr", "0", 0);
#endif

#ifdef _DEBUG
	g_trap->Cvar_Register(&bot_nogoals, "bot_nogoals", "0", CVAR_CHEAT);
	g_trap->Cvar_Register(&bot_debugmessages, "bot_debugmessages", "0", CVAR_CHEAT);
#endif

	g_trap->Cvar_Register(&bot_attachments, "bot_attachments", "1", 0);
	g_trap->Cvar_Register(&bot_camp, "bot_camp", "1", 0);

	g_trap->Cvar_Register(&bot_wp_info, "bot_wp_info", "1", 0);
	g_trap->Cvar_Register(&bot_wp_edit, "bot_wp_edit", "0", CVAR_CHEAT);
	g_trap->Cvar_Register(&bot_wp_clearweight, "bot_wp_clearweight", "1", 0);
	g_trap->Cvar_Register(&bot_wp_distconnect, "bot_wp_distconnect", "1", 0);
	g_trap->Cvar_Register(&bot_wp_visconnect, "bot_wp_visconnect", "1", 0);

	g_trap->Cvar_Update(&bot_forcepowers);
	//end rww
