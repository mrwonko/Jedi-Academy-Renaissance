...
void Rancor_SetBolts( gentity_t *self )
{
	if ( self && self->client )
	{
		...
		ri->torsoBolt = g_trap->G2API_AddBolt( self->ghoul2, 0, "jaw_bone" );
	}
}
...
void Rancor_CheckDropVictim( void )
{
	...
	g_trap->Trace( &trace, start, mins, maxs, end, NPCS.NPC->activator->s.number, NPCS.NPC->activator->clipmask, qfalse, 0, 0 );
