...
void Boba_FireFlameThrower( gentity_t *self )
{
	...
	g_trap->G2API_GetBoltMatrix( self->ghoul2, 0, self->client->renderInfo.handLBolt,
	...
	g_trap->Trace( &tr, start, traceMins, traceMaxs, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void Boba_StartFlameThrower( gentity_t *self )
{
	...
	if ( fire != NULL )
	{
		mdxaBone_t	boltMatrix;
		vec3_t		org, dir, ang;
		g_trap->G2API_GetBoltMatrix( NPC->ghoul2, NPC->playerModel, NPC->handRBolt,
				&boltMatrix, NPC->r.currentAngles, NPC->r.currentOrigin, (cg.time?cg.time:level.time),
				NULL, NPC->s.modelScale );

		g_trap->G2API_GiveMeVectorFromMatrix( boltMatrix, ORIGIN, org );
		g_trap->G2API_GiveMeVectorFromMatrix( boltMatrix, NEGATIVE_Y, dir );
		vectoangles( dir, ang );
	...
	*/
	...
	g_trap->G2API_GetBoltMatrix(NPCS.NPC->ghoul2, 0, NPCS.NPC->client->renderInfo.handRBolt, &boltMatrix, NPCS.NPC->r.currentAngles,
...
void Boba_FireDecide( void )
{
	...
	if ( TIMER_Done( NPCS.NPC, "nextAttackDelay" ) && TIMER_Done( NPCS.NPC, "flameTime" ) )
	{
		...
		else if ( g_trap->InPVS( NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
		{
			NPCS.NPCInfo->enemyLastSeenTime = level.time;
				...
				if ( level.time - NPCS.NPCInfo->enemyLastSeenTime < 10000 )//we have seem the enemy in the last 10 seconds
				{
					if ( !Q_irand( 0, 10 ) )
					{
						...
						{//never checked ShotEntity this frame, so must do a trace...
							...
							g_trap->Trace( &tr, muzzle, vec3_origin, vec3_origin, end, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
...
static qboolean Jedi_ClearPathToSpot( vec3_t dest, int impactEntNum )
{
	...
	g_trap->Trace( &trace, NPCS.NPC->r.currentOrigin, mins, NPCS.NPC->r.maxs, dest, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
	...
	{//FIXME: does this check the last spot, too?  We're assuming that should be okay since the enemy is there?
		...
		g_trap->Trace( &trace, start, mins, NPCS.NPC->r.maxs, end, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );//NPC->r.mins?
...
qboolean NPC_MoveDirClear( int forwardmove, int rightmove, qboolean reset )
{
	...
	*/
	...
	g_trap->Trace( &trace, NPCS.NPC->r.currentOrigin, mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask|CONTENTS_BOTCLIP, qfalse, 0, 0 );
	if ( trace.allsolid || trace.startsolid )
	...
	g_trap->Trace( &trace, trace.endpos, mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
...
evasionType_t Jedi_CheckFlipEvasions( gentity_t *self, float rightdot, float zdiff )
{
		...
		&& !BG_SaberInSpecialAttack( self->client->ps.torsoAnim ) )
	{
		...
		g_trap->Trace( &trace, self->r.currentOrigin, mins, maxs, traceto, self->s.number, CONTENTS_SOLID|CONTENTS_MONSTERCLIP|CONTENTS_BOTCLIP, qfalse, 0, 0 );
...
static qboolean Jedi_SaberBlock( int saberNum, int bladeNum ) //saberNum = 0, bladeNum = 0
{
	...
	*/
	...
	trace_t	tr;
	g_trap->Trace( &tr, saberPoint, vec3_origin, vec3_origin, axisPoint, NPC->enemy->s.number, MASK_SHOT, G2_RETURNONHIT, 10 );
...
//	VectorCopy(saberTip, saberTipOld);
	...
	//get the actual point of impact
	g_trap->Trace( &tr, saberPoint, saberMins, saberMaxs, hitloc, NPCS.NPC->enemy->s.number, CONTENTS_BODY, qfalse, 0, 0 );//, G2_RETURNONHIT, 10 );
...
gentity_t *Jedi_FindEnemyInCone( gentity_t *self, gentity_t *fallback, float minDot )
{
	...
	for ( e = 0 ; e < numListedEntities ; e++ )
	{
		...
		if ( !g_trap->InPVS( check->r.currentOrigin, self->r.currentOrigin ) )
		...
		g_trap->Trace( &tr, self->r.currentOrigin, vec3_origin, vec3_origin, check->r.currentOrigin, self->s.number, MASK_SHOT, qfalse, 0, 0 );
		...
		while ( hitCount < maxHits )
		{
			...
			{//do a rough trace of the path
				...
				for ( elapsedTime = timeStep; elapsedTime < floor(travelTime)+timeStep; elapsedTime += timeStep )
				{
					...
					BG_EvaluateTrajectory< moduleType::game >( &tr, level.time + elapsedTime, testPos );
					if ( testPos[2] < lastPos[2] )
					{//going down, ignore botclip
						g_trap->Trace( &trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
					}
					else
					{//going up, check for botclip
						g_trap->Trace( &trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask|CONTENTS_BOTCLIP, qfalse, 0, 0 );
					}
					...
					{//reached end, all clear
						...
						{//hmm, make sure we'll land on the ground...
							...
							bottom[2] -= 128;
							g_trap->Trace( &trace, trace.endpos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, bottom, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
							if ( trace.fraction >= 1.0f )
							{//would fall too far
								blocked = qtrue;
							}
						...
						{//Fake a force-jump
							...
							if ( goal == NPCS.NPC->enemy )
							{
								...
								{//FIXME: make it so it doesn't try the same spot again?
									...
									bottom[2] -= 128;
									g_trap->Trace( &trace, dest, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, bottom, goal->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
									if ( trace.fraction < 1.0f )
...
static void Jedi_CheckJumps( void )
{
	...
	for ( elapsedTime = 500; elapsedTime <= 4000; elapsedTime += 500 )
	{
		...
		{//going down, don't check for BOTCLIP
			g_trap->Trace( &trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );//FIXME: include CONTENTS_BOTCLIP?
		}
		else
		{//going up, check for BOTCLIP
			g_trap->Trace( &trace, lastPos, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, testPos, NPCS.NPC->s.number, NPCS.NPC->clipmask|CONTENTS_BOTCLIP, qfalse, 0, 0 );
	...
	{//not gripping
		...
		{//hunt him down
			...
			if ( (NPC_ClearLOS4( NPCS.NPC->enemy )||NPCS.NPCInfo->enemyLastSeenTime>level.time-500) && NPC_FaceEnemy( qtrue ) )//( NPCInfo->rank == RANK_CREWMAN || NPCInfo->rank > RANK_LT_JG ) &&
			{
				...
				g_trap->Trace( &trace, NPC->r.currentOrigin, NPC->r.mins, NPC->r.maxs, end, NPC->s.number, NPC->clipmask|CONTENTS_BOTCLIP );
	...
	*/		else
			{//FIXME: try to find a waypoint that can see enemy, jump from there
				...
				{//try to jump to the blockedDest
					...
					g_trap->LinkEntity( (sharedEntity_t *)tempGoal );
...
//		if ( NPC->client->ps.powerups[PW_CLOAKED] || g_crosshairEntNum != NPC->s.number )
		...
		{//if I'm not cloaked and the player's crosshair is on me, I will wake up, otherwise do this stuff down here...
			if ( !g_trap->InPVS( player->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
...
static void Jedi_Patrol( void )
{
	...
	{//look for enemies
		...
		for ( i = 0; i < ENTITYNUM_WORLD; i++ )
		{
			...
			{
				if ( g_trap->InPVS( NPCS.NPC->r.currentOrigin, enemy->r.currentOrigin ) )
