...
float trap_Cvar_VariableValue( const char *var_name ) {
	...
	g_trap->Cvar_VariableStringBuffer(var_name, buf, sizeof(buf));
	return atof(buf);
}
...
void G_LoadArenasFromFile( char *filename ) {
	...
	len = g_trap->FS_Open( filename, &f, FS_READ );
	if ( !f ) {
		g_trap->Print( S_COLOR_RED "file not found: %s\n", filename );
		return;
	}
	if ( len >= MAX_ARENAS_TEXT ) {
		g_trap->Print( S_COLOR_RED "file too large: %s is %i, max allowed is %i\n", filename, len, MAX_ARENAS_TEXT );
		g_trap->FS_Close( f );
		return;
	}

	g_trap->FS_Read( buf, len, f );
	buf[len] = 0;
	g_trap->FS_Close( f );
...
void G_LoadArenas( void ) {
	...
	numdirs = g_trap->FS_GetFileList("scripts", ".arena", dirlist, 1024 );
	dirptr  = dirlist;
...
//	g_trap->Print( "%i arenas parsed\n", level.arenas.num );
	...
	// get all arenas from .arena files
	numFiles = g_trap->FS_GetFileList("scripts", ".arena", filelist, ARRAY_LEN(filelist) );
...
static void PlayerIntroSound( const char *modelAndSkin ) {
	...
	g_trap->SendConsoleCommand( EXEC_APPEND, va( "play sound/player/announce/%s.wav\n", skin ) );
}
#endif
...
void G_AddRandomBot( int team ) {
	...
	for ( n = 0; n < level.bots.num ; n++ ) {
		...
		if (i >= sv_maxclients->integer) {
			num--;
			if (num <= 0) {
				skill = g_trap->Cvar_VariableIntegerValue( "g_npcspskill" );
				if (team == TEAM_RED) teamstr = "red";
				else if (team == TEAM_BLUE) teamstr = "blue";
				else teamstr = "";
				Q_strncpyz(netname, value, sizeof(netname));
				Q_CleanStr(netname);
				g_trap->SendConsoleCommand( EXEC_INSERT, va("addbot \"%s\" %.2f %s %i\n", netname, skill, teamstr, 0) );
				return;
			}
		}
	}
...
void G_CheckMinimumPlayers( void ) {
	...
	g_trap->Cvar_Update(&bot_minplayers);
...
void G_CheckBotSpawn( void ) {
	...
	for( n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++ ) {
		...
		if( level.gametype == GT_SINGLE_PLAYER ) {
			g_trap->GetUserinfo( botSpawnQueue[n].clientNum, userinfo, sizeof(userinfo) );
...
static void AddBotToSpawnQueue( int clientNum, int delay ) {
	...
	g_trap->Print( S_COLOR_YELLOW "Unable to delay spawn\n" );
...
qboolean G_BotConnect( int clientNum, qboolean restart ) {
	...
	if (!BotAISetupClient( clientNum, &settings, restart )) {
		g_trap->DropClient( clientNum, "BotAISetupClient failed" );
		return qfalse;
	}
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	clientNum = g_trap->BotAllocateClient();
	if ( clientNum == -1 ) {
//		g_trap->Print( S_COLOR_RED "Unable to add bot.  All player slots are in use.\n" );
//		g_trap->Print( S_COLOR_RED "Start server with more 'open' slots.\n" );
	...
	if ( !botinfo ) {
		g_trap->Print( S_COLOR_RED "Error: Bot '%s' not defined\n", name );
		g_trap->BotFreeClient( clientNum );
		return;
	}
...
void Svcmd_AddBot_f( void ) {
	...
	if ( !name[0] ) {
		g_trap->Print( "Usage: Addbot <botname> [skill 1-5] [team] [msec delay] [altname]\n" );
		return;
	}

	// skill
	g_trap->Argv( 2, string, sizeof( string ) );
	if ( !string[0] ) {
		skill = 4;
	}
	else {
	...
	// team
	g_trap->Argv( 3, team, sizeof( team ) );

	// delay
	g_trap->Argv( 4, string, sizeof( string ) );
	if ( !string[0] ) {
		delay = 0;
	}
	else {
	...
	// alternative name
	g_trap->Argv( 5, altname, sizeof( altname ) );
	...
	if ( level.time - level.startTime > 1000 &&
		g_trap->Cvar_VariableIntegerValue( "cl_running" ) ) {
		g_trap->SendServerCommand( -1, "loaddefered\n" );	// FIXME: spelled wrong, but not changing for demo
	}
}
...
void Svcmd_BotList_f( void ) {
	...
	g_trap->Print("name             model            personality              funname\n");
...
static void G_SpawnBots( char *botList, int baseDelay ) {
	...
	skill = g_trap->Cvar_VariableIntegerValue( "g_npcspskill" );
	if( skill < 1 ) {
		g_trap->Cvar_Set( "g_npcspskill", "1" );
		skill = 1;
	}
	else if ( skill > 5 ) {
		g_trap->Cvar_Set( "g_npcspskill", "5" );
		skill = 5;
	}
	...
	while( *p ) {
		...
		// does "Bad Things"
		g_trap->SendConsoleCommand( EXEC_INSERT, va("addbot \"%s\" %f free %i\n", bot, skill, delay) );
...
static void G_LoadBots( void ) {
	...
	g_trap->Cvar_Register( &botsFile, "g_botsFile", "", CVAR_INIT|CVAR_ROM );
	if( *botsFile.string ) {
	...
	numdirs = g_trap->FS_GetFileList("scripts", ".bot", dirlist, 1024 );
	dirptr  = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
...
char *G_GetBotInfoByNumber( int num ) {
	if( num < 0 || num >= level.bots.num ) {
		g_trap->Print( S_COLOR_RED "Invalid bot number: %i\n", num );
		return NULL;
	}
...
void G_InitBots( void ) {
	...
	g_trap->Cvar_Register( &bot_minplayers, "bot_minplayers", "0", CVAR_SERVERINFO );
