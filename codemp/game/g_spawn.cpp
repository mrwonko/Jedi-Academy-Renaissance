...
//		g_trap->Error( ERR_DROP, "G_SpawnString() called while not spawning" );
	}
...
qboolean	G_SpawnVector( const char *key, const char *defaultString, float *out ) {
	...
	if ( sscanf( s, "%f %f %f", &out[0], &out[1], &out[2] ) != 3 ) {
		g_trap->Print( "G_SpawnVector: Failed sscanf on %s (default: %s)\n", key, defaultString );
		VectorClear( out );
		return qfalse;
	}
	return present;
...
qboolean G_CallSpawn( gentity_t *ent ) {
	...
	if ( !ent->classname ) {
		g_trap->Print( "G_CallSpawn: NULL classname\n" );
		return qfalse;
	}
	...
	g_trap->Print( "%s doesn't have a spawn function\n", ent->classname );
	return qfalse;
}
...
static int fieldcmp( const void *a, const void *b ) {
	return Q_stricmp( (const char *)a, ((game::field_t*)b)->name );
}
...
void G_ParseField( const char *key, const char *value, gentity_t *ent )
{
	...
	f = (game::field_t *)Q_LinearSearch( key, fields, ARRAY_LEN( fields ), sizeof( game::field_t ), fieldcmp );
	if ( f )
	{// found it
		b = (byte *)ent;
		...
		case F_VECTOR:
			if ( sscanf( value, "%f %f %f", &vec[0], &vec[1], &vec[2] ) == 3 ) {
				...
				g_trap->Print( "G_ParseField: Failed sscanf on F_VECTOR (key/value: %s/%s)\n", key, value );
...
#define ADJUST_AREAPORTAL() \
	...
	{ \
		...
		g_trap->AdjustAreaPortalState((sharedEntity_t *)ent, qtrue); \
	}
...
void G_SpawnGEntityFromSpawnVars( qboolean inSubBSP ) {
	...
	if ( g_trap->ICARUS_ValidEnt( (sharedEntity_t *)ent ) )
	{
...
char *G_AddSpawnVarToken( const char *string ) {
	...
	if ( level.numSpawnVarChars + l + 1 > MAX_SPAWN_VARS_CHARS ) {
		g_trap->Error( ERR_DROP, "G_AddSpawnVarToken: MAX_SPAWN_VARS_CHARS" );
	}
...
static void HandleEntityAdjustment(void)
{
	...
	if (Q_stricmp(value, NOVALUE) != 0)
	{
		if ( sscanf( value, "%f %f %f", &origin[0], &origin[1], &origin[2] ) != 3 ) {
			g_trap->Print( "HandleEntityAdjustment: failed sscanf on 'origin' (%s)\n", value );
			VectorClear( origin );
		}
	}
	else
	...
	if (Q_stricmp(value, NOVALUE) != 0)
	{
		if ( sscanf( value, "%f %f %f", &angles[0], &angles[1], &angles[2] ) != 3 ) {
			g_trap->Print( "HandleEntityAdjustment: failed sscanf on 'angles' (%s)\n", value );
			VectorClear( angles );
		}
	...
	{
		if ( sscanf( value, "%f %f %f", &angles[0], &angles[1], &angles[2] ) != 3 ) {
			g_trap->Print( "HandleEntityAdjustment: failed sscanf on 'direction' (%s)\n", value );
			VectorClear( angles );
		}
	}
	else
...
qboolean G_ParseSpawnVars( qboolean inSubBSP ) {
	...
	if ( !g_trap->GetEntityToken( com_token, sizeof( com_token ) ) ) {
		// end of spawn string
		return qfalse;
	}
	if ( com_token[0] != '{' ) {
		g_trap->Error( ERR_DROP, "G_ParseSpawnVars: found %s when expecting {",com_token );
	}

	// go through all the key / value pairs
	while ( 1 ) {
		// parse key
		if ( !g_trap->GetEntityToken( keyname, sizeof( keyname ) ) ) {
			g_trap->Error( ERR_DROP, "G_ParseSpawnVars: EOF without closing brace" );
		}
		...
		if ( com_token[0] == '}' ) {
			g_trap->Error( ERR_DROP, "G_ParseSpawnVars: closing brace without data" );
		}
		if ( level.numSpawnVars == MAX_SPAWN_VARS ) {
			g_trap->Error( ERR_DROP, "G_ParseSpawnVars: MAX_SPAWN_VARS" );
		}
...
CGhoul2Info_v *precachedKyle = nullptr;
...
void SP_worldspawn( void )
{
	...
	g_trap->SetServerCull(g_cullDistance);

	G_SpawnString( "classname", "", &text );
	if ( Q_stricmp( text, "worldspawn" ) ) {
		g_trap->Error( ERR_DROP, "SP_worldspawn: The first entity isn't 'worldspawn'" );
	}
	...
	if (!BGPAFtextLoaded)
	{
		BG_ParseAnimationFile< moduleType::game >("models/players/_humanoid/animation.cfg", bgHumanoidAnimations, qtrue);
	}
	...
	if (!precachedKyle)
	{
		...
		re->G2API_InitGhoul2Model(&precachedKyle, "models/players/kyle/model.glm", 0, 0, -20, 0, 0);

		if (precachedKyle)
		{
			defSkin = g_trap->R_RegisterSkin("models/players/kyle/model_default.skin");
			g_trap->G2API_SetSkin(precachedKyle, 0, defSkin, defSkin);
		}
	}
	...
	g_trap->SetConfigstring( CS_GAME_VERSION, GAME_VERSION );

	g_trap->SetConfigstring( CS_LEVEL_START_TIME, va("%i", level.startTime ) );

	G_SpawnString( "music", "", &text );
	g_trap->SetConfigstring( CS_MUSIC, text );

	G_SpawnString( "message", "", &text );
	g_trap->SetConfigstring( CS_MESSAGE, text );				// map specific message

	g_trap->SetConfigstring( CS_MOTD, g_motd.string );		// message of the day

	G_SpawnString( "gravity", "800", &text );
	g_trap->Cvar_Set( "g_gravity", text );
	g_trap->Cvar_Update( &g_gravity );

	G_SpawnString( "enableBreath", "0", &text );

	G_SpawnString( "soundSet", "default", &text );
	g_trap->SetConfigstring( CS_GLOBAL_AMBIENT_SET, text );
	...
	g_trap->SetConfigstring( CS_WARMUP, "" );
	if ( g_restarted.integer ) {
		g_trap->Cvar_Set( "g_restarted", "0" );
	...
	g_trap->SetConfigstring(CS_LIGHT_STYLES+(LS_STYLES_START*3)+0, defaultStyles[0][0]);
	g_trap->SetConfigstring(CS_LIGHT_STYLES+(LS_STYLES_START*3)+1, defaultStyles[0][1]);
	g_trap->SetConfigstring(CS_LIGHT_STYLES+(LS_STYLES_START*3)+2, defaultStyles[0][2]);
...
void G_LinkLocations( void ) {
	...
	g_trap->SetConfigstring( CS_LOCATIONS, "unknown" );

	for ( i=0, n=1; i<level.locations.num; i++ ) {
		level.locations.data[i].cs_index = n;
		g_trap->SetConfigstring( CS_LOCATIONS + n, level.locations.data[i].message );
		n++;
	}
...
void G_SpawnEntitiesFromString( qboolean inSubBSP ) {
	...
	if ( !G_ParseSpawnVars(qfalse) ) {
		g_trap->Error( ERR_DROP, "SpawnEntities: no entities" );
	}
	...
	{//World has a spawn script, but we don't want the world in ICARUS and running scripts,
		...
		if ( script_runner )
		{
			...
			{
				g_trap->ICARUS_InitEnt( (sharedEntity_t *)script_runner );
			}
		}
	}
