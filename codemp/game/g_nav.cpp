...
void NPC_SetMoveGoal( gentity_t *ent, vec3_t point, int radius, qboolean isNavGoal, int combatPoint, gentity_t *targetEnt )
{
	...
	g_trap->LinkEntity( (sharedEntity_t *)ent->NPC->goalEntity );
}
...
//	return NAV_CheckAhead( self, point, trace, clipmask|CONTENTS_BOTCLIP );
	...
	if ( !g_trap->InPVS( self->r.currentOrigin, point ) )
		return qfalse;
	...
	if ( self->flags & FL_NAVGOAL )
	{
		...
		g_trap->Trace( &trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask|CONTENTS_MONSTERCLIP|CONTENTS_BOTCLIP)&~CONTENTS_BODY, qfalse, 0, 0 );
		if ( trace.startsolid&&(trace.contents&CONTENTS_BOTCLIP) )
		{//started inside do not enter, so ignore them
			clipmask &= ~CONTENTS_BOTCLIP;
			g_trap->Trace( &trace, point, mins, maxs, self->r.currentOrigin, self->parent->s.number, (clipmask|CONTENTS_MONSTERCLIP)&~CONTENTS_BODY, qfalse, 0, 0 );
		}
	...
	{
		g_trap->Trace( &trace, self->r.currentOrigin, mins, maxs, point, self->s.number, clipmask|CONTENTS_MONSTERCLIP|CONTENTS_BOTCLIP, qfalse, 0, 0);
		if ( trace.startsolid&&(trace.contents&CONTENTS_BOTCLIP) )
		{//started inside do not enter, so ignore them
			clipmask &= ~CONTENTS_BOTCLIP;
			g_trap->Trace( &trace, self->r.currentOrigin, mins, maxs, point, self->s.number, clipmask|CONTENTS_MONSTERCLIP, qfalse, 0, 0);
		}
...
int NAV_FindClosestWaypointForEnt( gentity_t *ent, int targWp )
{
	...
	return g_trap->Nav_GetNearestNode( (sharedEntity_t *)ent, ent->waypoint, NF_CLEAR_PATH, targWp );
}
...
int NAV_FindClosestWaypointForPoint( gentity_t *ent, vec3_t point )
{
	...
	bestWP = g_trap->Nav_GetNearestNode( (sharedEntity_t *)marker, marker->waypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
...
qboolean NAV_CheckAhead( gentity_t *self, vec3_t end, trace_t *trace, int clipmask )
{
	...
	g_trap->Trace( trace, self->r.currentOrigin, mins, self->r.maxs, end, self->s.number, clipmask, qfalse, 0, 0 );
...
qboolean NAV_StackedCanyon( gentity_t *self, gentity_t *blocker, vec3_t pathDir )
{
	...
	g_trap->Trace( &tr, test, self->r.mins, self->r.maxs, test, self->s.number, self->clipmask|extraClip, qfalse, 0, 0 );
...
{//check only against architectrure
	...
	g_trap->Nav_GetNodePosition( endID, end );
	...
	if ( trace.entityNum < ENTITYNUM_WORLD )
	{
		...
		{//special case: doors are architecture, but are dynamic, like entitites
			...
			{//a locked door!
				...
				if ( failEdge )
				{
					g_trap->Nav_AddFailedEdge( self->s.number, startID, endID );
				}
			}
...
{
	return g_trap->Nav_GetNearestNode( (sharedEntity_t *)self, lastNode, NF_CLEAR_PATH, WAYPOINT_NONE );
}
...
int	NAV_MoveToGoal( gentity_t *self, navInfo_t *info )
{
	...
	bestNode = g_trap->Nav_GetBestNode( self->waypoint, self->NPC->goalEntity->waypoint, NODE_NONE );
	...
	if ( bestNode == WAYPOINT_NONE )
	{
		if ( NAVDEBUG_showEnemyPath )
		{
			...
			g_trap->Nav_GetNodePosition( self->NPC->goalEntity->waypoint, torigin );
			g_trap->Nav_GetNodePosition( self->waypoint, neworigin );
	...
	//Get this position
	g_trap->Nav_GetNodePosition( bestNode, origin );
	g_trap->Nav_GetNodePosition( self->waypoint, end );
	...
	if ( NAVDEBUG_showEnemyPath )
	{
		...
		//Get the positions
		g_trap->Nav_GetNodePosition( self->NPC->goalEntity->waypoint, dest );
		g_trap->Nav_GetNodePosition( bestNode, start );

		//Draw the route
		G_DrawNode( start, NODE_START );
		G_DrawNode( dest, NODE_GOAL );
		g_trap->Nav_ShowPath( self->waypoint, self->NPC->goalEntity->waypoint );
	}
...
//	VectorMA( origin, MAX_RADIUS_CHECK, trace_dir, test_pos );
	...
	g_trap->Trace( &tr, origin, mins, maxs, test_pos, ENTITYNUM_NONE, ( CONTENTS_SOLID | CONTENTS_MONSTERCLIP | CONTENTS_BOTCLIP ), qfalse, 0, 0 );
...
void SP_waypoint ( gentity_t *ent )
{
	if ( navCalculatePaths )
	{
		...
		ent->health = g_trap->Nav_AddRawPoint( ent->r.currentOrigin, ent->spawnflags, radius );
...
void SP_waypoint_small (gentity_t *ent)
{
	if ( navCalculatePaths )
	{
		...
		ent->health = g_trap->Nav_AddRawPoint( ent->r.currentOrigin, ent->spawnflags, 2 );
		NAV_StoreWaypoint( ent );
		G_FreeEntity(ent);
		return;
	}
...
{
	char cmd[1024];
	g_trap->Argv( 1, cmd, 1024 );

	if ( Q_stricmp( cmd, "show" ) == 0 )
	{
		g_trap->Argv(2, cmd, 1024);
	...
	else if ( Q_stricmp( cmd, "set" ) == 0 )
	{
		...
		{
			NAVDEBUG_curGoal = g_trap->Nav_GetNearestNode( (sharedEntity_t *)&g_entities[0], g_entities[0].waypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
		}
	}
...
void NAV_StoreWaypoint( gentity_t *ent )
{
	...
	if ( !tempWaypointList )
	{
		//tempWaypointList = (waypointData_t *) g_trap->Malloc(sizeof(waypointData_t)*MAX_STORED_WAYPOINTS, TAG_TEMP_WORKSPACE, qtrue);

		int size = sizeof(waypointData_t)*MAX_STORED_WAYPOINTS;
		g_trap->TrueMalloc((void **)&tempWaypointList, size);
	...
	*/
	...
	{
		//g_trap->Error( ERR_DROP, "Too many waypoints!  (%d > %d)\n", numStoredWaypoints, MAX_STORED_WAYPOINTS );
...
#endif
			{
				g_trap->Nav_HardConnect( tempWaypointList[i].nodeID, tempWaypointList[target].nodeID );
			}
		}
...
//			if ( !NAV_WaypointsTooFar( ent, target ) )
	...
	//g_trap->Free(tempWaypointList);
	/*
	g_trap->TrueFree((void **)&tempWaypointList);
	tempWaypointList=0;
	*/

	//Now check all blocked edges, mark failed ones
	g_trap->Nav_CheckBlockedEdges();

	g_trap->Nav_SetPathsCalculated(qfalse);
	//navigator.pathsCalculated = qfalse;

	//Calculate the paths based on the supplied waypoints
	//g_trap->Nav_CalculatePaths();

	//Save the resulting information
	/*
	if ( g_trap->Nav_Save( filename, checksum ) == qfalse )
	{
...
void NAV_ShowDebugInfo( void )
{
	...
	{
		g_trap->Nav_ShowNodes();
	}

	if ( NAVDEBUG_showEdges )
	{
		g_trap->Nav_ShowEdges();
	}

	if ( NAVDEBUG_showTestPath )
	{
		//Get the nearest node to the player
		int	nearestNode = g_trap->Nav_GetNearestNode( (sharedEntity_t *)&g_entities[0], g_entities[0].waypoint, NF_ANY, WAYPOINT_NONE );
		int	testNode = g_trap->Nav_GetBestNode( nearestNode, NAVDEBUG_curGoal, NODE_NONE );
		vec3_t	dest, start;
		...
		//Get the positions
		g_trap->Nav_GetNodePosition( NAVDEBUG_curGoal, dest );
		g_trap->Nav_GetNodePosition( nearestNode, start );

		G_DrawNode( start, NODE_START );
		G_DrawNode( dest, NODE_GOAL );
		g_trap->Nav_ShowPath( nearestNode, NAVDEBUG_curGoal );
	}
...
{
	g_entities[clNum].waypoint = g_trap->Nav_GetNearestNode( (sharedEntity_t *)&g_entities[clNum], g_entities[clNum].lastWaypoint, NF_CLEAR_PATH, WAYPOINT_NONE );
}
