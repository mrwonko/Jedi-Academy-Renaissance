...
void NPC_SetPainEvent( gentity_t *self )
{
	if ( !self->NPC || !(self->NPC->aiFlags&NPCAI_DIE_ON_IMPACT) )
	{
		...
		if (!g_trap->ICARUS_TaskIDPending((sharedEntity_t *)self, TID_CHAN_VOICE) && self->client)
		{
	...
	{//I am dead and carrying a key
		...
		{//player touched me
			...
			*/
			...
			//FIXME: temp message
			g_trap->SendServerCommand( NULL, text );
			*/
		}
	}
...
void NPC_UseResponse( gentity_t *self, gentity_t *user, qboolean useWhenDone )
{
	...
	/*
	if ( g_trap->VoiceVolume[self->s.number] )
	{//I'm talking already
		if ( !useWhenDone )
...
/*
		...
		{//Heal me NOW, dammit!
			...
			else if ( !self->enemy && activator->s.number == 0 && !g_trap->VoiceVolume[self->s.number] && !(self->NPC->scriptFlags&SCF_NO_RESPONSE) )
