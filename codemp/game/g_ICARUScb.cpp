...
void Q3_TaskIDClear( int *taskID ); // in Q3_Interface.cpp
...
int Q3_PlaySound( int taskID, int entID, const char *name, const char *channel )
{
		...
		)	// paranoia towards project end <g>
	{
		...
		if (g_subtitles->integer == 1 || (ent->NPC && (ent->NPC->scriptFlags & SCF_USE_SUBTITLES) ) ) // Show all text
		{
			...
			{
				g_trap->SendServerCommand( -1, va("ct \"%s\" %i", finalName, soundHandle) );
			}
	...
	*/
	...
	if ( type_voice )
	{
		...
		g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_CHAN_VOICE, taskID );
...
void Q3_Play( int taskID, int entID, const char *type, const char *name )
{
	...
	if ( !Q_stricmp( type, "PLAY_ROFF" ) )
	{
		...
		ent->roffid = g_trap->ROFF_Cache((char*)name);
		if ( ent->roffid )
		{
			...
			g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_MOVE_NAV, taskID );
			...
			g_trap->ROFF_Play(ent->s.number, ent->roffid, qtrue);
		}
	}
}
...
{
	//Complete the task
	g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)ent, TID_ANGLE_FACE );
...
{	//complete the task
	g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)ent, TID_MOVE_NAV );
...
void Q3_Lerp2Pos( int taskID, int entID, vec3_t origin, vec3_t angles, float duration )
{
	...
	if ( angles != NULL )
	{
		...
		g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_ANGLE_FACE, taskID );
	}
	else
	{
...
int Q3_GetFloat( int entID, int type, const char *name, float *value )
{
	...
	default:
		if ( g_trap->ICARUS_VariableDeclared( name ) != VTYPE_FLOAT )
			return 0;

		return g_trap->ICARUS_GetFloatVariable( name, value );
	}
...
int Q3_GetVector( int entID, int type, const char *name, vec3_t value )
{
	...
	default:
		...
		if ( g_trap->ICARUS_VariableDeclared( name ) != VTYPE_VECTOR )
			return 0;

		return g_trap->ICARUS_GetVectorVariable( name, value );
	}
...
int Q3_GetString( int entID, int type, const char *name, char **value )
{
	...
	default:
		...
		if ( g_trap->ICARUS_VariableDeclared( name ) != VTYPE_STRING )
			return 0;

		return g_trap->ICARUS_GetStringVariable( name, (const char *) *value );
	}
...
void MoveOwner( gentity_t *self )
{
	...
	else
	{
		...
		g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)owner, TID_MOVE_NAV );
	}
}
...
{
	g_trap->Cvar_Set("timescale", data);
}
...
void SolidifyOwner( gentity_t *self )
{
	...
	{
		g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)owner, TID_RESIZE );
	}
}
...
static qboolean Q3_SetBehaviorSet( int entID, int toSet, const char *scriptname)
{
	...
	if(!Q_stricmp("NULL", scriptname))
	{
		...
		{
//			g_trap->TagFree( ent->behaviorSet[bSet] );
		}
...
static void Q3_ScrollText ( const char *id)
{
	...
	//g_trap->SendServerCommand( -1, va("st \"%s\"", id));
...
{
	G_DebugPrint( WL_WARNING, "Q3_ScrollText: NOT SUPPORTED IN MP\n");
	//g_trap->SendServerCommand( -1, va("lt \"%s\"", id));
...
qboolean Q3_Set( int taskID, int entID, const char *type_name, const char *data )
{
	...
	case SET_ANIM_UPPER:
		if ( Q3_SetAnimUpper( entID, (char *) data ) )
		{
			...
			g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_ANIM_UPPER, taskID );
			return qfalse;	//Don't call it back
		}
		break;
	...
	case SET_ANIM_LOWER:
		if ( Q3_SetAnimLower( entID, (char *) data ) )
		{
			...
			g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_ANIM_LOWER, taskID );
			return qfalse;	//Don't call it back
		}
		break;
	...
	case SET_ANIM_BOTH:
		{
			...
			if ( both >= 2 )
			{
				g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_ANIM_BOTH, taskID );
			}
			if ( both )
			{
				return qfalse;	//Don't call it back
	...
	case SET_BEHAVIOR_STATE:
		...
		{
			g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_BSTATE, taskID );
	...
	case SET_LOCATION:
		...
		{
			g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_LOCATION, taskID );
			return qfalse;
		}
		break;
	...
	case SET_SOLID:
		if(!Q_stricmp("true", ((char *)data)))
		{
			...
			{
				g_trap->ICARUS_TaskIDSet( (sharedEntity_t *)ent, TID_RESIZE, taskID );
				return qfalse;
			}
		}
		else
...
//		if ( g_timescale->value <= 1.0f )
	...
	case SET_LOADGAME:
		//g_trap->SendConsoleCommand( va("load %s\n", (const char *) data ) );
...
//		//can never be set
	...
	default:
		...
		g_trap->ICARUS_SetVar( taskID, entID, type_name, data );
		break;
	}
