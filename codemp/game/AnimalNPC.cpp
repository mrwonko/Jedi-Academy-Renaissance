...
/*		else
		{
			...
			if ( pVeh->m_pVehicleInfo->explodeFX )
			{
				...
				bottom[2] -= 80;
				g_trap->trace( &trace, parent->currentOrigin, vec3_origin, vec3_origin, bottom, parent->s.number, CONTENTS_SOLID );
			...
			if ( pVeh->m_pVehicleInfo->explosionRadius > 0 && pVeh->m_pVehicleInfo->explosionDamage > 0 )
			{
				...
				g_trap->trace( &trace, parent->currentOrigin, mins, maxs, bottom, parent->s.number, CONTENTS_SOLID );
...
//		pVehInfo->UpdateRider				=		UpdateRider;
	...
	if( mod == moduleType::cgame ) { //cgame prediction attachment func
...
// explicitly instantiate for all necessary modules
template void G_SetAnimalVehicleFunctions<moduleType::game>(vehicleInfo_t*);
template void G_SetAnimalVehicleFunctions<moduleType::cgame>(vehicleInfo_t*);
...
// explicitly instantiate for all necessary modules
template void G_CreateAnimalNPC<moduleType::game>(Vehicle_t**, const char*);
template void G_CreateAnimalNPC<moduleType::cgame>(Vehicle_t**, const char*);
