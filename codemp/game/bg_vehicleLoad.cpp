...
//These funcs are actually shared in both game and cgame
...
typedef enum {
	...
	VF_SHADER,	// (cgame only) take the string, call g_trap->R_RegisterShader
	VF_SHADER_NOMIP,// (cgame only) take the string, call g_trap->R_RegisterShaderNoMip
...
static qboolean BG_ParseVehWeaponParm( vehWeaponInfo_t *vehWeapon, const char *parmName, char *pValue )
{
	...
	case VF_STRING:	// string on disk, pointer in memory
		...
		{ //just use 1024 bytes in case we want to write over the string
			*(char **)(b+vehWeaponField->ofs) = (char *)BG_Alloc< mod >(1024);//(char *)BG_Alloc(strlen(value));
	...
	case VF_MODEL:// take the string, get the G_ModelIndex
		*(int *)( b + vehWeaponField->ofs ) = mod == moduleType::game ? G_ModelIndex( value ) : re->RegisterModel( value );
		break;
	...
	case VF_SHADER:	// (cgame only) take the string, call trap_R_RegisterShader
		if( mod == moduleType::ui ) {
			*(int *)( b + vehWeaponField->ofs ) = re->RegisterShaderNoMip( value );
		} else if( mod == moduleType::cgame ) {
			*(int *)( b + vehWeaponField->ofs ) = re->RegisterShader( value );
		}
		break;
	...
	case VF_SOUND:	// take the string, get the G_SoundIndex
		*(int *)( b + vehWeaponField->ofs ) = mod == moduleType::game ? G_SoundIndex( value ) : S_RegisterSound( value );
		break;
...
{//load up specified vehWeapon and save in array: g_vehWeaponInfo
	...
	while ( 1 )
	{
		...
		{
			if ( !BG_ParseVehWeaponParm< mod >( vehWeapon, parmName, value ) )
			{
...
int VEH_VehWeaponIndexForName( const char *vehWeaponName )
{
	...
	vw = VEH_LoadVehWeapon< mod >( vehWeaponName );
...
void BG_SetSharedVehicleFunctions( vehicleInfo_t *pVehInfo ) {
	if( mod == moduleType::game ) {
		...
		G_SetSharedVehicleFunctions< mod >( pVehInfo );
	}

	if( mod != moduleType::ui ) {
		switch( pVehInfo->type ) {
		case VH_SPEEDER:
			G_SetSpeederVehicleFunctions< mod >( pVehInfo );
			break;
		case VH_ANIMAL:
			G_SetAnimalVehicleFunctions< mod >( pVehInfo );
			break;
		case VH_FIGHTER:
			G_SetFighterVehicleFunctions< mod >( pVehInfo );
			break;
		case VH_WALKER:
			G_SetWalkerVehicleFunctions< mod >( pVehInfo );
			break;
		default:
			break;
		}
...
static qboolean BG_ParseVehicleParm( vehicleInfo_t *vehicle, const char *parmName, char *pValue )
{
	...
	case VF_STRING:	// string on disk, pointer in memory
		...
		{ //just use 128 bytes in case we want to write over the string
			*(char **)(b+vehField->ofs) = (char *)BG_Alloc< mod >(128);//(char *)BG_Alloc(strlen(value));
	...
	case VF_ANIM:
		...
		break;
	case VF_WEAPON:	// take string, resolve into index into VehWeaponParms
		*(int *)(b+vehField->ofs) = VEH_VehWeaponIndexForName< mod >( value );
		break;
	case VF_MODEL:	// take the string, get the G_ModelIndex
		*(int *)( b + vehField->ofs ) = mod == moduleType::game ? G_ModelIndex( value ) : re->RegisterModel( value );
		break;
	case VF_MODEL_CLIENT:	// (MP cgame only) take the string, get the G_ModelIndex
		if( mod != moduleType::game ) {
			*(int *)( b + vehField->ofs ) = re->RegisterModel( value );
		}
		break;
	case VF_EFFECT:	// take the string, get the G_EffectIndex
		if( mod == moduleType::game ) {
	...
	case VF_SHADER:	// (cgame only) take the string, call trap_R_RegisterShader
		if( mod == moduleType::ui ) {
			*(int *)( b + vehField->ofs ) = re->RegisterShaderNoMip( value );
		} else if( mod == moduleType::cgame ) {
			*(int *)( b + vehField->ofs ) = re->RegisterShader( value );
		}
		break;
	case VF_SHADER_NOMIP:// (cgame only) take the string, call trap_R_RegisterShaderNoMip
		if( mod != moduleType::game ) {
		...
		break;
	case VF_SOUND:	// take the string, get the G_SoundIndex
		*(int *)(b+vehField->ofs) = mod == moduleType::game ? G_SoundIndex( value ) : S_RegisterSound( value );
		break;
...
{//load up specified vehicle and save in array: g_vehicleInfo
	...
	while ( 1 )
	{
		...
		{
			if ( !BG_ParseVehicleParm< mod >( vehicle, parmName, value ) )
			{
#ifndef FINAL_BUILD
	...
	if ( weap1[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weap1", weap1 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weap2[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weap2", weap2 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle1[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle1", weapMuzzle1 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle2[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle2", weapMuzzle2 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle3[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle3", weapMuzzle3 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle4[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle4", weapMuzzle4 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle5[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle5", weapMuzzle5 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle6[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle6", weapMuzzle6 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle7[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle7", weapMuzzle7 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle8[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle8", weapMuzzle8 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle9[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle9", weapMuzzle9 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( weapMuzzle10[0] )
	{
		if ( !BG_ParseVehicleParm< mod >( vehicle, "weapMuzzle10", weapMuzzle10 ) )
		{
#ifndef FINAL_BUILD
	...
	if ( vehicle->model )
	{
		vehicle->modelIndex = mod == moduleType::game ? G_ModelIndex( va( "models/players/%s/model.glm", vehicle->model ) ) : re->RegisterModel( va( "models/players/%s/model.glm", vehicle->model ) );
	}

	if( mod != moduleType::game ) {
		if( VALIDSTRING( vehicle->skin ) )
			re->RegisterSkin( va( "models/players/%s/model_%s.skin", vehicle->model, vehicle->skin ) );
	}

	//sanity check and clamp the vehicle's data
	BG_VehicleClampData( vehicle );
	// Setup the shared function pointers.
	BG_SetSharedVehicleFunctions< mod >( vehicle );
	...
	if( mod == moduleType::game ) {
		...
		re->RegisterShader( "gfx/menus/radar/bracket" );
		re->RegisterShader( "gfx/menus/radar/lead" );
		re->RegisterShaderNoMip( "gfx/menus/radar/asteroid" );
		...
		if( vehicle->hideRider ) {
			re->RegisterShaderNoMip( "gfx/menus/radar/circle_base" );
			re->RegisterShaderNoMip( "gfx/menus/radar/circle_base_frame" );
			re->RegisterShaderNoMip( "gfx/menus/radar/circle_base_shield" );
		}
	}
...
int VEH_VehicleIndexForName( const char *vehicleName )
{
	...
	v = VEH_LoadVehicle< mod >( vehicleName );
 	if ( v == VEHICLE_NONE )
	{
...
void BG_VehWeaponLoadParms( void )
{
	...
	*marker = 0;
	...
	fileCnt = g_trap->FS_GetFileList("ext_data/vehicles/weapons", ".vwp", vehWeaponExtensionListBuf, sizeof(vehWeaponExtensionListBuf) );

	holdChar = vehWeaponExtensionListBuf;

	tempReadBuffer = (char *)BG_TempAlloc< mod >(MAX_VEH_WEAPON_DATA_SIZE);
...
//		Com_Printf( "Parsing %s\n", holdChar );
		...
		len = g_trap->FS_Open(va( "ext_data/vehicles/weapons/%s", holdChar), &f, FS_READ);
		...
		{
			g_trap->FS_Read(tempReadBuffer, len, f);
	...
	BG_TempFree< mod >(MAX_VEH_WEAPON_DATA_SIZE);
}
...
//	const char	*filename = "ext_data/vehicles.dat";
	...
	*marker = 0;
	...
	//now load in the extra .veh extensions
	fileCnt = g_trap->FS_GetFileList("ext_data/vehicles", ".veh", vehExtensionListBuf, sizeof(vehExtensionListBuf) );

	holdChar = vehExtensionListBuf;

	tempReadBuffer = (char *)BG_TempAlloc< mod >(MAX_VEHICLE_DATA_SIZE);
...
//		Com_Printf( "Parsing %s\n", holdChar );
		...
		len = g_trap->FS_Open(va( "ext_data/vehicles/%s", holdChar), &f, FS_READ);
	...
	BG_TempFree< mod >(MAX_VEHICLE_DATA_SIZE);
	...
	// Setup the shared function pointers.
	BG_SetSharedVehicleFunctions< mod >( &g_vehicleInfo[VEHICLE_BASE] );

	//Load the Vehicle Weapons data, too
	BG_VehWeaponLoadParms< mod >();
}

template< moduleType mod >
int BG_VehicleGetIndex( const char *vehicleName )
{
	return VEH_VehicleIndexForName< mod >( vehicleName );
}
template int BG_VehicleGetIndex< moduleType::game >( const char* vehicleName );
template int BG_VehicleGetIndex< moduleType::cgame >( const char* vehicleName );
...
void BG_GetVehicleModelName(char *modelName, const char *vehicleName, size_t len)
{
	...
	int vIndex = BG_VehicleGetIndex< mod >(vehName);
...
template void BG_GetVehicleModelName< moduleType::game >( char *modelName, const char *vehicleName, size_t len );
template void BG_GetVehicleModelName< moduleType::cgame >( char *modelName, const char *vehicleName, size_t len );
...
template void BG_GetVehicleSkinName< moduleType::cgame >( char *skinname, int len );
...
void AttachRidersGeneric( Vehicle_t *pVeh )
{
	...
	if ( pVeh->m_pPilot )
	{
		...
		int crotchBolt = g_trap->G2API_AddBolt(parent->ghoul2, 0, "*driver");
		...
		// Get the driver tag.
		g_trap->G2API_GetBoltMatrix( parent->ghoul2, 0, crotchBolt, &boltMatrix, yawOnlyAngles, parent->playerState->origin, BG_GetTime< mod >(), NULL, parent->modelScale );
		BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, pilot->playerState->origin );
	}
}
template void AttachRidersGeneric< moduleType::game >( Vehicle_t *pVeh );
template void AttachRidersGeneric< moduleType::cgame >( Vehicle_t *pVeh );
