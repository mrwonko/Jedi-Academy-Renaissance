...
void NPC_ChangeWeapon( int newWeapon )
{
	...
	{
		g_trap->G2API_RemoveGhoul2Model( NPC->ghoul2, NPC->weaponModel[0] );
	}
...
qboolean ShotThroughGlass (trace_t *tr, gentity_t *target, vec3_t spot, int mask)
{
	...
	{//ok to shoot through breakable glass
		...
		g_trap->Trace (tr, muzzle, NULL, NULL, spot, skip, mask, qfalse, 0, 0 );
		return qtrue;
	}
...
qboolean CanShoot ( gentity_t *ent, gentity_t *shooter )
{
	...
	g_trap->Trace ( &tr, muzzle, NULL, NULL, spot, shooter->s.number, MASK_SHOT, qfalse, 0, 0 );
			...
			if( newenemy->health > 0 )
			{
				...
				{//FIXME:  check for range and FOV or vis?
					...
					{//Make sure we're not just going back and forth here
						if ( g_trap->InPVS(newenemy->r.currentOrigin, NPCS.NPC->r.currentOrigin) )
						{
		...
		if ( ally->client )
		{
			if ( ally->health > 0 )
			{
				...
				{//if on same team or if player is disguised as your team
					...
					if(!g_trap->InPVS(ally->r.currentOrigin, NPCS.NPC->r.currentOrigin))
					{
						continue;
					}
...
gentity_t *NPC_CheckEnemy( qboolean findNew, qboolean tooFarOk, qboolean setEnemy )
{
	...
	*/
	...
	if ( NPCS.NPC->enemy )
	{
		...
		else if ( !g_trap->InPVS(NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->r.currentOrigin ) )
...
//	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
	if( NPCS.NPC->s.weapon == WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/ ) // any other guns to check for?
	{
		...
		g_trap->Trace ( &tr, muzzle, mins, maxs, ent->r.currentOrigin, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
	else
	{
		g_trap->Trace ( &tr, muzzle, NULL, NULL, ent->r.currentOrigin, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
...
//	if ( NPC->client->playerTeam == TEAM_SCAVENGERS )
	if( NPCS.NPC->s.weapon == WP_BLASTER /*|| NPC->s.weapon == WP_BLASTER_PISTOL*/ ) // any other guns to check for?
	{
		...
		g_trap->Trace ( &tr, muzzle, mins, maxs, targ, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
	else
	{
		g_trap->Trace ( &tr, muzzle, NULL, NULL, targ, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
...
//	qboolean	duck_ok = qfalse;
	...
	{//He's in our FOV
		...
		if(attack_ok)
		{
			...
			g_trap->Trace( &tr, muzzle, NULL, NULL, hitspot, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
			ShotThroughGlass( &tr, NPCS.NPC->enemy, hitspot, MASK_SHOT );
			/*
			//OLD: trace regardless of facing
			g_trap->Trace ( &tr, muzzle, NULL, NULL, enemy_org, NPC->s.number, MASK_SHOT );
...
static int NPC_CollectCombatPoints( const vec3_t origin, const float radius, combatPt_t *points, const int flags )
{
	...
	for ( i = 0; i < level.numCombatPoints; i++ )
	{
		...
		{//must not be within PVS of mu current origin
			if ( g_trap->InPVS( origin, level.combatPoints[i].origin ) )
			{
				continue;
			}
		}
...
int NPC_FindCombatPoint( const vec3_t position, const vec3_t avoidPosition, vec3_t enemyPosition, const int flags, const float avoidDist, const int ignorePoint )
{
	...
	for ( j = 0; j < numPoints; j++ )
	{
		...
		if ( flags & CP_AVOID_ENEMY  )
		{
			...
			{
				g_trap->Nav_GetNodePosition( NPC->waypoint, wpOrg );
			}
			else
			*/
			{
		...
		g_trap->Trace( &tr, level.combatPoints[i].origin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, level.combatPoints[i].origin, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
		...
		if ( flags & CP_HAS_ROUTE )
		{
			...
			*/
			...
			if ( waypoint == WAYPOINT_NONE || level.combatPoints[i].waypoint == WAYPOINT_NONE || g_trap->Nav_GetBestNodeAltRoute2( waypoint, level.combatPoints[i].waypoint, NODE_NONE ) == WAYPOINT_NONE )
		...
		{
			cost = g_trap->Nav_GetPathCost( waypoint, level.combatPoints[i].waypoint );
			if ( cost < bestCost )
			{
				bestCost = cost;
				best = i;
...
//		}
		...
		{
			if ( g_trap->InPVS( found->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
			{
				dist = DistanceSquared( found->r.currentOrigin, NPCS.NPC->r.currentOrigin );
				if ( dist < bestDist )
				{
					if ( !g_trap->Nav_GetBestPathBetweenEnts( (sharedEntity_t *)NPCS.NPC, (sharedEntity_t *)found, NF_CLEAR_PATH )
						|| g_trap->Nav_GetBestNodeAltRoute2( NPCS.NPC->waypoint, found->waypoint, NODE_NONE ) == WAYPOINT_NONE )
					{//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
...
void NPC_CheckGetNewWeapon( void )
{
	...
	{//if running away because dropped weapon...
		...
		{//need a weapon, any lying around?
			...
			{//try to nav to it
				/*
				if ( !g_trap->Nav_GetBestPathBetweenEnts( NPC, foundWeap, NF_CLEAR_PATH )
					|| g_trap->Nav_GetBestNodeAltRoute( NPC->waypoint, foundWeap->waypoint ) == WAYPOINT_NONE )
				{//can't possibly have a route to any OR can't possibly have a route to this one OR don't have a route to this one
