...
//	qboolean	shoot = qfalse;
	...
	if ( NPCS.NPC->enemy && !(NPCS.NPCInfo->scriptFlags&SCF_FORCED_MARCH) )
	{
		...
		if ( NPCS.NPC->client->leader
			...
			&& !g_trap->ICARUS_TaskIDPending( (sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV ) )
		{
			NPC_ClearGoal();
		}
			NPC_BSST_Attack();
...
/*
		...
		enemyPVS = g_trap->inPVS( NPC->enemy->r.currentOrigin, NPC->r.currentOrigin );
		...
		{//in the pvs
			...
			g_trap->trace ( &tr, muzzle, vec3_origin, vec3_origin, enemyHead, NPC->s.number, MASK_SHOT );
