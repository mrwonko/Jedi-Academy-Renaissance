...
{
	g_trap->Trace(results, start, tMins, tMaxs, end, passEntityNum, contentmask, qfalse, 0, 0);
}
...
void G_AttachToVehicle( gentity_t *pEnt, usercmd_t **ucmd )
{
	...
	crotchBolt = g_trap->G2API_AddBolt(vehEnt->ghoul2, 0, "*driver");

	// Get the driver tag.
	g_trap->G2API_GetBoltMatrix( vehEnt->ghoul2, 0, crotchBolt, &boltMatrix,
		...
		else if ( pVeh->m_iNumPassengers < pVeh->m_pVehicleInfo->maxPassengers )
		{
			...
			for ( i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++ )
			{
				if ( pVeh->m_ppPassengers[i] == NULL )
				{
					...
					if ( mod == moduleType::game && ent->client )
					{
...
qboolean Eject( Vehicle_t *pVeh, bgEntity_t *pEnt, qboolean forceEject )
{
	...
	{
		G_EjectDroidUnit< mod >( pVeh, qfalse );
		return qtrue;
	}
...
getItOutOfMe:
	...
	if ( (gentity_t *)pVeh->m_pPilot == ent )
	{
		...
		while (j < pVeh->m_iNumPassengers)
		{
			if (pVeh->m_ppPassengers[j])
			{
				...
				if ( mod == moduleType::game && ((gentity_t *)pVeh->m_ppPassengers[j])->client )
				{
				...
				while (k < pVeh->m_iNumPassengers)
				{
					...
					{ //move down
						...
						if ( mod == moduleType::game && pVeh->m_ppPassengers[k-1] && ((gentity_t *)pVeh->m_ppPassengers[k-1])->client )
						{
	...
	else
	{
		...
		for ( i = 0; i < pVeh->m_pVehicleInfo->maxPassengers; i++ )
		{
			...
			{
				//Server just needs to tell client he's not a passenger anymore
				if ( mod == moduleType::game && ((gentity_t *)pVeh->m_ppPassengers[i])->client )
				{
...
/*	if ( !ent->s.number && ent->client->ps.weapon != WP_SABER
	...
	{
		g_trap->cvar_set( "cg_thirdperson", "0" );
	}*/
...
qboolean EjectAll( Vehicle_t *pVeh )
{
	...
	{
		G_EjectDroidUnit< mod >( pVeh, pVeh->m_pVehicleInfo->killRiderOnDeath );
	}
...
qboolean Initialize( Vehicle_t *pVeh )
{
	...
	*/
	...
	//Initialize to landed (wings closed, gears down) animation
	{
		...
		pVeh->m_ulFlags |= VEH_GEARSOPEN;
		BG_SetAnim< moduleType::game >(pVeh->m_pParentEntity->playerState,
...
static qboolean Update( Vehicle_t *pVeh, const usercmd_t *pUmcd )
{
	...
	curTime = mod == moduleType::game ? level.time : pm->cmd.serverTime;
	...
	if (mod == moduleType::game && parent->spawnflags & 1)
	{
		...
		&& pVeh->m_pVehicleInfo->type != VH_WALKER  )
	{
		...
		if ( pUmcd->upmove > 0 )
		{
			...
			if ( pVeh->m_pVehicleInfo->Eject( pVeh, pRider, qfalse ) )
			{
				...
				if (!g_trap->ICARUS_TaskIDPending((sharedEntity_t *)rider, TID_CHAN_VOICE))
				{
...
static void AttachRiders( Vehicle_t *pVeh )
{
	...
	AttachRidersGeneric< moduleType::game >(pVeh);
	...
	if (pVeh->m_pPilot)
	{
		...
		g_trap->LinkEntity( (sharedEntity_t *)pilot );
	}
	...
	if (pVeh->m_pOldPilot)
	{
		...
		g_trap->LinkEntity( (sharedEntity_t *)oldpilot );
	}
	...
	while (i < pVeh->m_iNumPassengers)
	{
		if (pVeh->m_ppPassengers[i])
		{
			...
			crotchBolt = g_trap->G2API_AddBolt(parent->ghoul2, 0, "*driver");
			...
			// Get the driver tag.
			g_trap->G2API_GetBoltMatrix( parent->ghoul2, 0, crotchBolt, &boltMatrix,
		...
		&& pVeh->m_iDroidUnitTag != -1)
	{
		...
		if ( droid->client )
		{
			...
			// Get the droid tag.
			g_trap->G2API_GetBoltMatrix( parent->ghoul2, 0, pVeh->m_iDroidUnitTag, &boltMatrix,
			...
			g_trap->LinkEntity( (sharedEntity_t *)droid );
...
void G_VehicleDamageBoxSizing(Vehicle_t *pVeh)
{
	...
	g_trap->Trace(&trace, parent->client->ps.origin, back, nose, parent->client->ps.origin, parent->s.number, parent->clipmask, qfalse, 0, 0);
...
int G_FlyVehicleImpactDir(gentity_t *veh, trace_t *trace)
{
	...
	g_trap->Trace(&localTrace, veh->client->ps.origin, testMins, testMaxs, fPos, veh->s.number, veh->clipmask, qfalse, 0, 0);
	...
	{ //if nose is clear check for tearing the wings off
			...
			!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_F))
		{
			...
			g_trap->Trace(&localTrace, rWing, testMins, testMaxs, fPos, veh->s.number, veh->clipmask, qfalse, 0, 0);
			if (localTrace.startsolid || localTrace.allsolid || localTrace.fraction != 1.0f)
			{ //impact
			...
			!(pVeh->m_iRemovedSurfaces & SHIPSURF_BROKEN_D))
		{
			...
			g_trap->Trace(&localTrace, lWing, testMins, testMaxs, fPos, veh->s.number, veh->clipmask, qfalse, 0, 0);
			if (localTrace.startsolid || localTrace.allsolid || localTrace.fraction != 1.0f)
			{ //impact
...
//	pVehInfo->AnimateRiders					=		AnimateRiders;
	...
	pVehInfo->Board							=		Board< mod >;
	pVehInfo->Eject							=		Eject< mod >;
	pVehInfo->EjectAll						=		EjectAll< mod >;
...
template void G_SetSharedVehicleFunctions< moduleType::game >( vehicleInfo_t *pVehInfo );
template void G_SetSharedVehicleFunctions< moduleType::cgame >( vehicleInfo_t *pVehInfo );