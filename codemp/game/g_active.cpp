	...
	*/
	...
	if ( !self->client || self->client->ps.lastOnGround+300<level.time || ( self->client->ps.lastOnGround+100 < level.time && easyBreakBrush ) )
	{
		...
		if ( magnitude >= 100 && other->s.number < ENTITYNUM_WORLD )
		{
			...
			cont = g_trap->PointContents( other->r.absmax, other->s.number );
...
void	G_TouchTriggers( gentity_t *ent ) {
	...
	for ( i=0 ; i<num ; i++ ) {
		...
		if ( hit->s.eType == ET_ITEM ) {
			if ( !BG_PlayerTouchesItem< moduleType::game >( &ent->client->ps, &hit->s, level.time ) ) {
				continue;
			}
		} else {
			if ( !g_trap->EntityContact( mins, maxs, (sharedEntity_t *)hit, qfalse ) ) {
				continue;
			}
		}
...
static void SV_PMTrace( trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask ) {
	g_trap->Trace( results, start, mins, maxs, end, passEntityNum, contentMask, qfalse, 0, 10 );
}
...
void SpectatorThink( gentity_t *ent, usercmd_t *ucmd ) {
	...
	if ( client->sess.spectatorState != SPECTATOR_FOLLOW ) {
		...
		pmove.pointcontents = g_trap->PointContents;
...
qboolean ClientInactivityTimer( gclient_t *client ) {
	if ( ! g_inactivity.integer ) {
		...
		if ( level.time > client->inactivityTime ) {
			g_trap->DropClient( client - level.clients, "Dropped due to inactivity" );
			return qfalse;
		}
		if ( level.time > client->inactivityTime - 10000 && !client->inactivityWarning ) {
			client->inactivityWarning = qtrue;
			g_trap->SendServerCommand( client - level.clients, "cp \"Ten seconds until inactivity drop!\n\"" );
		}
	}
	return qtrue;
}
...
void G_VehicleAttachDroidUnit( gentity_t *vehEnt )
{
	if ( vehEnt && vehEnt->m_pVehicle && vehEnt->m_pVehicle->m_pDroidUnit != NULL )
	{
		...
		g_trap->G2API_GetBoltMatrix(vehEnt->ghoul2, 0, vehEnt->m_pVehicle->m_iDroidUnitTag, &boltMatrix, vehEnt->r.currentAngles, vehEnt->r.currentOrigin, level.time,
		...
		g_trap->LinkEntity( (sharedEntity_t *)droidEnt );
...
void G_HeldByMonster( gentity_t *ent, usercmd_t *ucmd )
{
		...
		&& ent->client->ps.hasLookTarget )//NOTE: lookTarget is an entity number, so this presumes that client 0 is NOT a Rancor...
	{
		...
		if ( monster && monster->client )
		{
			...
			g_trap->LinkEntity( (sharedEntity_t *)ent );//redundant?
		}
	}
...
//		g_trap->Print("serverTime >>>>>\n" );
	...
	if ( game::pmove_msec.integer < 8 ) {
		g_trap->Cvar_Set("pmove_msec", "8");
	}
	else if (game::pmove_msec.integer > 33) {
		g_trap->Cvar_Set("pmove_msec", "33");
	}

	if ( game::pmove_fixed.integer || client->pers.pmoveFixed ) {
		ucmd->serverTime = ((ucmd->serverTime + game::pmove_msec.integer-1) / game::pmove_msec.integer) * game::pmove_msec.integer;
...
#define	MIN_NPC_SPEED	16.0f
	...
	if (client->bodyGrabIndex != ENTITYNUM_NONE)
	{
		...
		else
		{
			...
			g_trap->G2API_GetBoltMatrix(ent->ghoul2, 0, 0, &rhMat, tAng, ent->client->ps.origin, level.time,
	...
	if (ent->client->beingThrown > level.time)
	{
			...
			thrower->client->throwingIndex != ent->s.number)
		{
			...
			g_trap->G2API_HaveWeGhoul2Models(thrower->ghoul2))
		{
#if 0
			int lHandBolt = g_trap->G2API_AddBolt(thrower->ghoul2, 0, "*l_hand");
			int pelBolt = g_trap->G2API_AddBolt(thrower->ghoul2, 0, "pelvis");
...
#endif
			{
				...
				g_trap->G2API_GetBoltMatrix(thrower->ghoul2, 0, lHandBolt, &boltMatrix, tAngles, thrower->client->ps.origin, level.time, 0, thrower->modelScale);
				boltOrg[0] = boltMatrix.matrix[0][3];
				boltOrg[1] = boltMatrix.matrix[1][3];
				boltOrg[2] = boltMatrix.matrix[2][3];

				g_trap->G2API_GetBoltMatrix(thrower->ghoul2, 0, pelBolt, &pBoltMatrix, tAngles, thrower->client->ps.origin, level.time, 0, thrower->modelScale);
				...
				{ //see if we can move to be next to the hand.. if it's not clear, break the throw.
					...
					g_trap->Trace(&tr, intendedOrigin, ent->r.mins, ent->r.maxs, intendedOrigin, ent->s.number, ent->clipmask, qfalse, 0, 0);
					g_trap->Trace(&tr2, ent->client->ps.origin, ent->r.mins, ent->r.maxs, intendedOrigin, ent->s.number, CONTENTS_SOLID, qfalse, 0, 0);
...
//	WP_SaberPositionUpdate(ent, ucmd); //check the server-side saber point, do apprioriate server-side actions (effects are cs-only)
	...
	pmove.pmove_fixed = game::pmove_fixed.integer | static_cast< int > ( client->pers.pmoveFixed );
	pmove.pmove_msec = game::pmove_msec.integer;
	pmove.pmove_float = game::pmove_float.integer;
	...
	if (ent->ghoul2)
	{
		...
		else
		{
			...
			pmove.g2Bolts_LFoot = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*l_leg_foot");
			pmove.g2Bolts_RFoot = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*r_leg_foot");
		}
	}
	...
	if (level.intermissionQueued != 0 && g_singlePlayer.integer) {
		if ( level.time - level.intermissionQueued >= 1000  ) {
			...
			if ( level.time - level.intermissionQueued >= 2000 && level.time - level.intermissionQueued <= 2500 ) {
				g_trap->SendConsoleCommand( EXEC_APPEND, "centerview\n");
			}
...
void ClientThink( int clientNum, usercmd_t *ucmd ) {
	...
	{
		g_trap->GetUsercmd( clientNum, &ent->client->pers.cmd );
	}
...
*/
	if ( !(ent->r.svFlags & SVF_BOT) && !game::g_synchronousClients.integer ) {
...
void G_RunClient( gentity_t *ent ) {
	...
	if ( !(ent->r.svFlags & SVF_BOT) && g_forceClientUpdateRate.integer && ent->client->lastCmdTime < level.time - g_forceClientUpdateRate.integer ) {
		g_trap->GetUsercmd( ent-g_entities, &ent->client->pers.cmd );
...
//	i = g_trap->AAS_PointReachabilityAreaIndex( ent->client->ps.origin );
