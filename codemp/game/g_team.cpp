...
void QDECL PrintMsg( gentity_t *ent, const char *fmt, ... ) {
	...
	if (vsprintf (msg, fmt, argptr) > sizeof(msg)) {
		g_trap->Error( ERR_DROP, "PrintMsg overrun" );
	}
	va_end (argptr);
	...
	g_trap->SendServerCommand ( ( (ent == NULL) ? -1 : ent-g_entities ), va("print \"%s\"", msg ));
}
*/
...
void Team_SetFlagStatus( int team, flagStatus_t status ) {
	...
	if( modified ) {
		...
		g_trap->SetConfigstring( CS_FLAGSTATUS, st );
	}
}
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker)
{
	...
	if ( ( ( VectorLength(v1) < CTF_TARGET_PROTECT_RADIUS &&
		g_trap->InPVS(flag->r.currentOrigin, targ->r.currentOrigin ) ) ||
		( VectorLength(v2) < CTF_TARGET_PROTECT_RADIUS &&
		g_trap->InPVS(flag->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
	...
	if (carrier && carrier != attacker) {
		...
		if ( ( ( VectorLength(v1) < CTF_ATTACKER_PROTECT_RADIUS &&
			g_trap->InPVS(carrier->r.currentOrigin, targ->r.currentOrigin ) ) ||
			( VectorLength(v2) < CTF_ATTACKER_PROTECT_RADIUS &&
				g_trap->InPVS(carrier->r.currentOrigin, attacker->r.currentOrigin ) ) ) &&
			attacker->client->sess.sessionTeam != targ->client->sess.sessionTeam) {
...
void Team_ReturnFlagSound( gentity_t *ent, int team ) {
	...
	if (ent == NULL) {
		g_trap->Print ("Warning:  NULL passed to Team_ReturnFlagSound\n");
		return;
	}
...
void Team_TakeFlagSound( gentity_t *ent, int team ) {
	...
	if (ent == NULL) {
		g_trap->Print ("Warning:  NULL passed to Team_TakeFlagSound\n");
		return;
	}
...
void Team_CaptureFlagSound( gentity_t *ent, int team ) {
	...
	if (ent == NULL) {
		g_trap->Print ("Warning:  NULL passed to Team_CaptureFlagSound\n");
		return;
	}
...
locationData_t *Team_GetLocation(gentity_t *ent)
{
	...
	for ( i=0; i<level.locations.num; i++ ) {
		...
		if ( !g_trap->InPVS( origin, loc->origin ) ) {
			continue;
		}
...
void TeamplayInfoMessage( gentity_t *ent ) {
	...
	g_trap->SendServerCommand( ent-g_entities, va("tinfo %i %s", cnt, string) );
}
