...
void SaberUpdateSelf(gentity_t *ent)
{
	...
	{ //Standard contents (saber is active)
		...
		if (g_saberDebugBox.integer == 1|| g_saberDebugBox.integer == 4)
		{
			...
			G_DebugBoxLines(dbgMins, dbgMaxs, (10.0f/(float)game::sv_fps.integer)*100);
		}
#endif
...
static QINLINE void G_G2PlayerAngles( gentity_t *ent, matrix3_t legs, vec3_t legsAngles)
{
	...
	{ //sort of hacky, but it saves a pretty big load off the server
		...
		while (i < MAX_CLIENTS)
		{
			...
			if (clEnt && clEnt->inuse && clEnt->client &&
				g_trap->InPVS(clEnt->client->ps.origin, ent->client->ps.origin))
	...
	{ //don't do these things on non-humanoids
		...
		{ //then put our arm in this client's hand
			...
			{
				lHandBolt = g_trap->G2API_AddBolt(other->ghoul2, 0, "*l_hand");
			}
			else
			...
			if (lHandBolt)
			{
				...
				g_trap->G2API_GetBoltMatrix(other->ghoul2, 0, lHandBolt, &boltMatrix, tAngles, other->client->ps.origin, level.time, 0, other->modelScale);
		...
		{ //make sure we aren't IKing if we don't have anyone to hold onto us.
			...
			{
				lHandBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*l_hand");
			}
			else
...
static QINLINE qboolean WP_SabersCheckLock2( gentity_t *attacker, gentity_t *defender, sabersLockMode_t lockMode )
{
	...
	g_trap->Trace( &trace, attacker->r.currentOrigin, attacker->r.mins, attacker->r.maxs, newOrg, attacker->s.number, attacker->clipmask, qfalse, 0, 0 );
	if ( !trace.startsolid && !trace.allsolid )
	{
		...
		g_trap->LinkEntity( (sharedEntity_t *)attacker );
	}
	...
	g_trap->Trace( &trace, defender->r.currentOrigin, defender->r.mins, defender->r.maxs, newOrg, defender->s.number, defender->clipmask, qfalse, 0, 0 );
	if ( !trace.startsolid && !trace.allsolid )
	{
		if (defender->client)
		{
		...
		g_trap->LinkEntity( (sharedEntity_t *)defender );
	}
...
static QINLINE int G_GetAttackDamage(gentity_t *self, int minDmg, int maxDmg, float multPoint)
{
	...
	BG_SaberStartTransAnim< moduleType::ui >(self->s.number, self->client->ps.fd.saberAnimLevel, self->client->ps.weapon, self->client->ps.torsoAnim, &animSpeedFactor, self->client->ps.brokenLimbs);
...
{ //Hit the ent with the normal trace, try the collision trace.
	...
	if (g2Hit && g2Hit->inuse && g2Hit->ghoul2)
	{
		...
		if (game::com_optvehtrace.integer &&
			g2Hit->s.eType == ET_NPC &&
			g2Hit->s.NPC_class == CLASS_VEHICLE &&
			g2Hit->m_pVehicle)
		{
			g_trap->G2API_CollisionDetectCache ( G2Trace, g2Hit->ghoul2, angles, g2HitOrigin, level.time, g2Hit->s.number, lastValidStart, lastValidEnd, g2Hit->modelScale, 0, g_g2TraceLod.integer, fRadius );
		}
		else
		{
			g_trap->G2API_CollisionDetect ( G2Trace, g2Hit->ghoul2, angles, g2HitOrigin, level.time, g2Hit->s.number, lastValidStart, lastValidEnd, g2Hit->modelScale, 0, g_g2TraceLod.integer, fRadius );
		}
...
void WP_SaberRadiusDamage( gentity_t *ent, vec3_t point, float radius, int damage, float knockBack )
{
	...
	else
	{
		...
		numEnts = g_trap->EntitiesInBox( mins, maxs, radiusEnts, 128 );
...
static QINLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	while (!saberTraceDone)
	{
		...
		{ //This didn't quite work out like I hoped. But it's better than nothing. Sort of.
			...
			g_trap->Trace(&tr, saberEnd, saberTrMins, saberTrMaxs, saberStart, self->s.number, trMask, qfalse, 0, 0);
		...
		else
		{
			...
			g_trap->Trace(&tr, saberStart, saberTrMins, saberTrMaxs, saberEndExtrapolated, self->s.number, trMask, qfalse, 0, 0);
...
void WP_SaberStartMissileBlockCheck( gentity_t *self, usercmd_t *ucmd  )
{
	...
	numListedEntities = g_trap->EntitiesInBox( mins, maxs, entityList, MAX_GENTITIES );
	...
	for ( e = 0 ; e < numListedEntities ; e++ )
	{
		...
		{ //seems like a valid enemy to look at.
			...
			if (lookTDist == -1 || vecLen < lookTDist)
			{
				...
				g_trap->Trace(&tr, myEyes, NULL, NULL, ent->client->ps.origin, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
		...
		if ( dist < closestDist )
		{
			...
			g_trap->Trace( &trace, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, traceTo, ent->s.number, ent->clipmask, qfalse, 0, 0 );
...
static QINLINE qboolean CheckThrownSaberDamaged(gentity_t *saberent, gentity_t *saberOwner, gentity_t *ent, int dist, int returning, qboolean noDCheck)
{
	...
	if (ent && ent->client && ent->inuse && ent->s.number != saberOwner->s.number &&
		...
		g_trap->InPVS(ent->client->ps.origin, saberent->r.currentOrigin) &&
		ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
	...
	{ //hit a client
		...
		{ //within range
			...
			g_trap->Trace(&tr, saberent->r.currentOrigin, NULL, NULL, ent->client->ps.origin, saberent->s.number, MASK_SHOT, qfalse, 0, 0);
		...
		ent->s.number != saberent->s.number && (noDCheck ||g_trap->InPVS(ent->r.currentOrigin, saberent->r.currentOrigin)))
	{ //hit a non-client
		...
		if (veclen < dist)
		{
			...
			g_trap->Trace(&tr, saberent->r.currentOrigin, NULL, NULL, entOrigin, saberent->s.number, MASK_SHOT, qfalse, 0, 0);
...
static QINLINE void saberMoveBack( gentity_t *ent, qboolean goingBack )
{
	...
	BG_EvaluateTrajectory< moduleType::game >( &ent->s.pos, level.time, origin );
	//Get current angles?
	BG_EvaluateTrajectory< moduleType::game >( &ent->s.apos, level.time, ent->r.currentAngles );
	...
	{ //acts as a fallback in case touch code fails, keeps saber from going through things between predictions
		...
		g_trap->Trace(&tr, oldOrg, mins, maxs, compensatedOrigin, ent->r.ownerNum, MASK_PLAYERSOLID, qfalse, 0, 0);
...
{	//spawn a "dead" saber entity here so it looks like the saber fell out of the air.
	...
	g_trap->Trace(&tr, startorg, saberent->r.mins, saberent->r.maxs, startorg, saberent->s.number, saberent->clipmask, qfalse, 0, 0);
	...
	g_trap->LinkEntity((sharedEntity_t *)saberent);
}
...
void saberKnockDown(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other)
{
	...
	//(normally at the player's right hand) could result in the saber being stuck
	//in the the map and then freaking out.
	g_trap->Trace(&tr, saberent->r.currentOrigin, saberent->r.mins, saberent->r.maxs, saberent->r.currentOrigin, saberent->s.number, saberent->clipmask, qfalse, 0, 0);
	if(tr.startsolid || tr.fraction != 1)
	{//bad position, try popping our origin up a bit
...
void WP_SaberRemoveG2Model( gentity_t *saberent )
{
	if ( saberent->ghoul2 )
	{
		g_trap->G2API_RemoveGhoul2Models( &saberent->ghoul2 );
	}
}
...
void WP_SaberAddG2Model( gentity_t *saberent, const char *saberModel, qhandle_t saberSkin )
{
	...
	re->G2API_InitGhoul2Model( &saberent->ghoul2, saberModel, saberent->s.modelindex, saberSkin, 0, 0, 0 );
}
...
void saberFirstThrown(gentity_t *saberent)
{
	...
	{ //if owner is rank 3 in saber throwing, the saber goes where he points
		...
		{ //if highest saber throw rank, we can direct the saber toward players directly by looking at them
			g_trap->Trace(&tr, traceFrom, NULL, NULL, traceTo, saberOwn->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
		}
		else
		{
			g_trap->Trace(&tr, traceFrom, NULL, NULL, traceTo, saberOwn->s.number, MASK_SOLID, qfalse, 0, 0);
		}
...
void UpdateClientRenderBolts(gentity_t *self, vec3_t renderOrigin, vec3_t renderAngles)
{
	...
	{
		//head
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->headBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
		ri->headPoint[0] = boltMatrix.matrix[0][3];
		ri->headPoint[1] = boltMatrix.matrix[1][3];
		ri->headPoint[2] = boltMatrix.matrix[2][3];

		//right hand
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->handRBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
		ri->handRPoint[0] = boltMatrix.matrix[0][3];
		ri->handRPoint[1] = boltMatrix.matrix[1][3];
		ri->handRPoint[2] = boltMatrix.matrix[2][3];

		//left hand
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->handLBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
		ri->handLPoint[0] = boltMatrix.matrix[0][3];
		ri->handLPoint[1] = boltMatrix.matrix[1][3];
		ri->handLPoint[2] = boltMatrix.matrix[2][3];

		//chest
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->torsoBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
		ri->torsoPoint[0] = boltMatrix.matrix[0][3];
		ri->torsoPoint[1] = boltMatrix.matrix[1][3];
		ri->torsoPoint[2] = boltMatrix.matrix[2][3];

		//crotch
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->crotchBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
		ri->crotchPoint[0] = boltMatrix.matrix[0][3];
		ri->crotchPoint[1] = boltMatrix.matrix[1][3];
		ri->crotchPoint[2] = boltMatrix.matrix[2][3];

		//right foot
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->footRBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
		ri->footRPoint[0] = boltMatrix.matrix[0][3];
		ri->footRPoint[1] = boltMatrix.matrix[1][3];
		ri->footRPoint[2] = boltMatrix.matrix[2][3];

		//left foot
		g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->footLBolt, &boltMatrix, renderAngles, renderOrigin, level.time, NULL, self->modelScale);
...
void UpdateClientRenderinfo(gentity_t *self, vec3_t renderOrigin, vec3_t renderAngles)
{
	...
	if ( ri->mPCalcTime < level.time )
	{
		...
		{ //the g2 instance changed, so update all the bolts.
			...
			{
				ri->headBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "*head_eyes");
				ri->handRBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "*r_hand");
				ri->handLBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "*l_hand");
				ri->torsoBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "thoracic");
				ri->crotchBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "pelvis");
				ri->footRBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "*r_leg_foot");
				ri->footLBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "*l_leg_foot");
				ri->motionBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, "Motion");
			}
			else
			{
				ri->headBolt = -1;
...
static gentity_t *G_KickTrace( gentity_t *ent, vec3_t kickDir, float kickDist, vec3_t kickEnd, int kickDamage, float kickPush )
{
	...
	if (d_saberKickTweak.integer)
	{
		g_trap->Trace( &trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
	}
	else
	{
		g_trap->Trace( &trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
...
static void G_GrabSomeMofos(gentity_t *self)
{
	...
	g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, ri->handRBolt, &boltMatrix, flatAng, self->client->ps.origin,
	...
	g_trap->Trace( &trace, self->client->ps.origin, grabMins, grabMaxs, pos, self->s.number, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
...
{ //rww - keep the saber position as updated as possible on the server so that we can try to do realistic-looking contact stuff
  //Note that this function also does the majority of working in maintaining the server g2 client instance (updating angles/anims/etc)
		...
		else
		{
			...
			else
			{
				...
				{ //grapplee
					...
					g_trap->Trace(&trace, self->client->ps.origin, self->r.mins, self->r.maxs, idealSpot, self->s.number, self->clipmask, qfalse, 0, 0);
	...
	clientOverride = g_trap->G2API_OverrideServer(self->ghoul2);
...
nextStep:
	...
	//fVSpeed *= 0.08;
	fVSpeed *= 1.6f/game::sv_fps.value;
	...
	g_trap->G2API_GetBoltMatrix(self->ghoul2, 1, 0, &boltMatrix, properAngles, properOrigin, level.time, NULL, self->modelScale);
...
*/
	...
	*/
	...
	if (!BG_SabersOff(&self->client->ps))
	{
			...
			while (rBladeNum < self->client->saber[rSaberNum].numBlades)
			{
				...
				if ( rSaberNum == 0 && self->client->ps.saberInFlight )
				{
					...
					else
					{
						...
						if ( saberent->s.saberInFlight )
						{//spinning
							BG_EvaluateTrajectory< moduleType::game >( &saberEnt->s.pos, level.time+50, saberOrg );
							BG_EvaluateTrajectory< moduleType::game >( &saberEnt->s.apos, level.time+50, saberAngles );
						}
						else
						{//coming right back
							vec3_t saberDir;
							BG_EvaluateTrajectory< moduleType::game >( &saberEnt->s.pos, level.time, saberOrg );
							VectorSubtract( self->r.currentOrigin, saberOrg, saberDir );
							vectoangles( saberDir, saberAngles );
						}
						g_trap->G2API_GetBoltMatrix(saberEnt->ghoul2, 0, rBladeNum, &boltMatrix, saberAngles, saberOrg, level.time, NULL, self->modelScale);
				...
				{
					g_trap->G2API_GetBoltMatrix(self->ghoul2, rSaberNum+1, rBladeNum, &boltMatrix, properAngles, properOrigin, level.time, NULL, self->modelScale);
					BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint);
					BG_GiveMeVectorFromMatrix(&boltMatrix, NEGATIVE_Y, self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir);
					VectorCopy( self->client->saber[rSaberNum].blade[rBladeNum].muzzlePoint, boltOrigin );
					VectorMA( boltOrigin, self->client->saber[rSaberNum].blade[rBladeNum].lengthMax, self->client->saber[rSaberNum].blade[rBladeNum].muzzleDir, end );
		...
		{
			g_trap->LinkEntity((sharedEntity_t *)mySaber);
		}
