...
/*		VectorSet( endPos, NPC->r.currentOrigin[0], NPC->r.currentOrigin[1], NPC->r.currentOrigin[2] - 512 );
		g_trap->Trace( &trace, NPC->r.currentOrigin, NULL, NULL, endPos, NPC->s.number, MASK_SOLID );
...
//	static	vec3_t	muzzle;
	...
	genBolt1 = g_trap->G2API_AddBolt(NPCS.NPC->ghoul2, 0, "*flash");
...
void NPC_Probe_Pain(gentity_t *self, gentity_t *attacker, int damage)
{
	...
	if ( self->health < 30 || mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT ) // demp2 always messes them up real good
	{
		...
		g_trap->Trace( &trace, self->r.currentOrigin, NULL, NULL, endPos, self->s.number, MASK_SOLID, qfalse, 0, 0 );
...
void ImperialProbe_Wait(void)
{
	if ( NPCS.NPCInfo->localState == LSTATE_DROP )
	{
		...
		g_trap->Trace( &trace, NPCS.NPC->r.currentOrigin, NULL, NULL, endPos, NPCS.NPC->s.number, MASK_SOLID, qfalse, 0, 0 );
