...
static qboolean StringToFilter( char *s, ipFilter_t *f ) {
	...
	for ( i=0; i<4; i++ ) {
		if ( *s < '0' || *s > '9' ) {
			...
			g_trap->Print( "Bad filter address: %s\n", s );
			return qfalse;
		}
...
static void UpdateIPBans( void ) {
	...
	*iplist_final = 0;
	...
	g_trap->Cvar_Set( "g_banIPs", iplist_final );
}
...
static void AddIP( char *str ) {
	...
	if ( i == numIPFilters ) {
		if ( numIPFilters == MAX_IPFILTERS ) {
			g_trap->Print( "IP filter list is full\n" );
			return;
		}
		numIPFilters++;
	}
...
void Svcmd_AddIP_f (void)
{
	...
	if ( g_trap->Argc() < 2 ) {
		g_trap->Print("Usage: addip <ip-mask>\n");
		return;
	}

	g_trap->Argv( 1, str, sizeof( str ) );
...
void Svcmd_RemoveIP_f (void)
{
	...
	if ( g_trap->Argc() < 2 ) {
		g_trap->Print("Usage: removeip <ip-mask>\n");
		return;
	}
...
void	Svcmd_EntityList_f (void) {
	...
	for (e = 0; e < level.num_entities ; e++, check++) {
		...
		case ET_GENERAL:
			g_trap->Print("ET_GENERAL          ");
			break;
		case ET_PLAYER:
			g_trap->Print("ET_PLAYER           ");
			break;
		case ET_ITEM:
			g_trap->Print("ET_ITEM             ");
			break;
		case ET_MISSILE:
			g_trap->Print("ET_MISSILE          ");
			break;
		case ET_SPECIAL:
			g_trap->Print("ET_SPECIAL          ");
			break;
		case ET_HOLOCRON:
			g_trap->Print("ET_HOLOCRON         ");
			break;
		case ET_MOVER:
			g_trap->Print("ET_MOVER            ");
			break;
		case ET_BEAM:
			g_trap->Print("ET_BEAM             ");
			break;
		case ET_PORTAL:
			g_trap->Print("ET_PORTAL           ");
			break;
		case ET_SPEAKER:
			g_trap->Print("ET_SPEAKER          ");
			break;
		case ET_PUSH_TRIGGER:
			g_trap->Print("ET_PUSH_TRIGGER     ");
			break;
		case ET_TELEPORT_TRIGGER:
			g_trap->Print("ET_TELEPORT_TRIGGER ");
			break;
		case ET_INVISIBLE:
			g_trap->Print("ET_INVISIBLE        ");
			break;
		case ET_NPC:
			g_trap->Print("ET_NPC              ");
			break;
		case ET_BODY:
			g_trap->Print("ET_BODY             ");
			break;
		case ET_TERRAIN:
			g_trap->Print("ET_TERRAIN          ");
			break;
		case ET_FX:
			g_trap->Print("ET_FX               ");
			break;
		default:
			g_trap->Print("%-3i                ", check->s.eType);
			break;
		}

		if ( check->classname ) {
			g_trap->Print("%s", check->classname);
		}
		g_trap->Print("\n");
	}
}
...
gclient_t	*ClientForString( const char *s ) {
	...
	g_trap->Print( "User %s is not on the server\n", s );
	return NULL;
}
...
void	Svcmd_ForceTeam_f( void ) {
	...
	if ( g_trap->Argc() < 3 ) {
		g_trap->Print("Usage: forceteam <player> <team>\n");
		return;
	}
...
void Svcmd_Say_f( void ) {
	...
	g_trap->SendServerCommand( -1, va("print \"server: %s\n\"", text ) );
}
