...
int PM_GetSaberStance(void)
{
	...
	saberInfo_t *saber1 = BG_MySaber< mod >( pm->ps->clientNum, 0 );
	saberInfo_t *saber2 = BG_MySaber< mod >( pm->ps->clientNum, 1 );
			...
			{//moving at a decent speed
				...
				{//splash
					...
					else
					{
					 ...
					 //yeah, this is kind of crappy and makes no use of prediction whatsoever
					if( mod == moduleType::game && pVeh->m_pVehicleInfo->iWakeFX ) {
...
void PM_AddEvent( int newEvent ) {
	BG_AddPredictableEventToPlayerstate< mod >( newEvent, 0, pm->ps );
}

template< moduleType mod >
void PM_AddEventWithParm( int newEvent, int parm )
{
	BG_AddPredictableEventToPlayerstate< mod >( newEvent, parm, pm->ps );
}
...
static void PM_JumpForDir( void )
{
	...
	{
		PM_SetAnim< mod >(SETANIM_LEGS,anim,SETANIM_FLAG_OVERRIDE);
	}
}
...
qboolean PM_AdjustAngleForWallRun( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//wall-running and not at end of anim
		...
		{//stop it
			if ( (ps->legsAnim) == BOTH_WALL_RUN_RIGHT )
			{
				PM_SetAnim< mod >(SETANIM_BOTH, BOTH_WALL_RUN_RIGHT_STOP, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			}
			else if ( (ps->legsAnim) == BOTH_WALL_RUN_LEFT )
			{
				PM_SetAnim< mod >(SETANIM_BOTH, BOTH_WALL_RUN_LEFT_STOP, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			}
		}
	}
	...
	{//wall-running up
		...
		{//hmm, some room, see if there's a floor right here
			...
			{//cool, do the alt-flip and land on whetever it is we just scaled up
				...
				PM_SetAnim< mod >(SETANIM_BOTH, BOTH_FORCEWALLRUNFLIP_ALT, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
		...
		{//stop it
			...
			//why?!?#?#@!%$R@$KR#F:Hdl;asfm
			PM_SetAnim< mod >(SETANIM_BOTH, BOTH_FORCEWALLRUNFLIP_END, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
...
qboolean PM_AdjustAngleForWallJump( playerState_t *ps, usercmd_t *ucmd, qboolean doMove )
{
	...
	{//hugging wall, getting ready to jump off
		...
		{//uber-skillz
			...
			{//hold on until you let go manually
				...
				{//if got to hold part of anim, play hold anim
					...
					{
						ps->saberHolstered = 2;
						PM_SetAnim< mod >( SETANIM_BOTH, BOTH_FORCEWALLRELEASE_FORWARD+(ps->legsAnim-BOTH_FORCEWALLHOLD_FORWARD), SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
		...
		{//jump off
			...
			{
				//PM_JumpForDir();
				PM_SetAnim< mod >(SETANIM_LEGS,BOTH_FORCEJUMP1,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD|SETANIM_FLAG_RESTART);
			}
...
{//NOTE!!! assumes an appropriate anim is being passed in!!!
	PM_SetAnim< mod >( SETANIM_BOTH, anim, SETANIM_FLAG_RESTART|SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
	PM_AddEvent< mod >( EV_JUMP );//make sound for grab
	pm->ps->pm_flags |= PMF_STUCK_TO_WALL;
}
...
static qboolean PM_CheckJump( void )
{
	...
	{ //Forced jump anim
		...
		PM_SetAnim< mod >( parts, anim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
						...
						{ //jump is already active (the anim has started)
							...
							{//not in the middle of a legsAnim
								...
								if ( newAnim != -1 )
								{
									...
									PM_SetAnim< mod >( parts, newAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
								}
							}
						}
					}
				...
				if ( allowWallRuns )
				{
					...
					if (1) //sure, we have it! Because I SAID SO.
					{
						...
						{//there is a wall there
							...
							//animate me
							PM_SetAnim< mod >( parts, wallWalkAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
	//						pm->ps->pm_flags |= PMF_JUMPING|PMF_SLOW_MO_FALL;
...
static void PM_FlyVehicleMove( void )
{
	...
	{//NPC
		...
		{
			//g_trap->Printf("Generating MoveDir\n");
			for ( i = 0 ; i < 3 ; i++ )
			{
...
static int PM_TryRoll( void )
{
	...
	if ( pm->ps->weapon == WP_SABER )
	{
		saberInfo_t *saber = BG_MySaber< mod >( pm->ps->clientNum, 0 );
		if ( saber
			&& (saber->saberFlags&SFL_NO_ROLLS) )
		{
			return 0;
		}
		saber = BG_MySaber< mod >( pm->ps->clientNum, 1 );
		if ( saber
			&& (saber->saberFlags&SFL_NO_ROLLS) )
		{
			return 0;
...
static void PM_CrashLand( void ) {
		...
		pm->ps->legsAnim == BOTH_A7_KICK_L_AIR)
	{
		...
		if ( landAnim != -1 )
		{
			...
			{
				PM_SetAnim< mod >(SETANIM_BOTH, landAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			}
			else
			{
				PM_SetAnim< mod >(SETANIM_LEGS, landAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			}
		}
	}
			...
			pm->ps->legsAnim == BOTH_FORCEJUMP1)
	{
		...
		PM_SetAnim< mod >(SETANIM_BOTH, fjAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
	}
	...
	{ //only play a land animation if we transitioned into an in-air animation while off the ground
		if (!BG_SaberInSpecial(pm->ps->saberMove))
		{
			if ( pm->ps->pm_flags & PMF_BACKWARDS_JUMP ) {
				PM_ForceLegsAnim< mod >( BOTH_LANDBACK1 );
			} else {
				PM_ForceLegsAnim< mod >( BOTH_LAND1 );
			}
		}
	}
	...
	{ //saber handles its own anims
		...
		{
			PM_StartTorsoAnim< mod >( TORSO_WEAPONREADY4 );
		}
		else
		{
			if (pm->ps->weapon == WP_EMPLACED_GUN)
			{
				PM_StartTorsoAnim< mod >( BOTH_GUNSIT1 );
			}
			else
			{
				PM_StartTorsoAnim< mod >( WeaponReadyAnim[pm->ps->weapon] );
			}
		}
	}
	...
	if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		...
		{//roll!
			...
			if (PM_InRollComplete(pm->ps, pm->ps->legsAnim))
			{
				...
				pm->ps->legsTimer = 0;
				pm->ps->legsAnim = 0;
				PM_SetAnim< mod >(SETANIM_BOTH,BOTH_LAND1,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
				pm->ps->legsTimer = TIMER_LAND;
			}
			...
			{//absorb some impact
				...
				PM_SetAnim< mod >(SETANIM_BOTH,anim,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
				didRoll = qtrue;
			}
		}
	}
	...
	if ( !(pml.groundTrace.surfaceFlags & SURF_NODAMAGE) )  {
		if (delta > 7)
		{
			...
			{ //Add the appropriate event..
				PM_AddEventWithParm< mod >( EV_ROLL, delta_send );
			}
			else
			{
				PM_AddEventWithParm< mod >( EV_FALL, delta_send );
			}
		}
		else
		{
			if (didRoll)
			{
				PM_AddEventWithParm< mod >( EV_ROLL, 0 );
			}
			else
			{
				PM_AddEventWithParm< mod >( EV_FOOTSTEP, PM_FootstepForSurface() );
			}
		}
	}
...
static void PM_GroundTraceMissed( void ) {
	...
	if ( pm->ps->pm_type == PM_FLOAT )
	{
		...
		PM_SetAnim< mod >(parts, BOTH_CHOKE3, SETANIM_FLAG_OVERRIDE);
	}
	else if ( pm->ps->pm_type == PM_JETPACK )
	{//jetpacking
		//rww - also don't use SETANIM_FLAG_HOLD, it will cause the legs to float around a bit before going into
	...
	else if ( pm->ps->groundEntityNum != ENTITYNUM_NONE || (pm->ps->legsAnim) == BOTH_CHOKE3 )
	{
		...
		if ( trace.fraction == 1.0 || pm->ps->pm_type == PM_FLOAT ) {
			if ( pm->ps->velocity[2] <= 0 && !(pm->ps->pm_flags&PMF_JUMP_HELD))
			{
				...
				PM_SetAnim< mod >(SETANIM_LEGS,BOTH_INAIR1,0);
				pm->ps->pm_flags &= ~PMF_BACKWARDS_JUMP;
			}
			else if ( pm->cmd.forwardmove >= 0 )
			{
				PM_SetAnim< mod >(SETANIM_LEGS,BOTH_JUMP1,SETANIM_FLAG_OVERRIDE);
				pm->ps->pm_flags &= ~PMF_BACKWARDS_JUMP;
			}
			else
			{
				PM_SetAnim< mod >(SETANIM_LEGS,BOTH_JUMPBACK1,SETANIM_FLAG_OVERRIDE);
				pm->ps->pm_flags |= PMF_BACKWARDS_JUMP;
			}
	...
	{ //Client won't catch an animation restart because it only checks frame against incoming frame, so if you roll when you land after rolling
	  //off of something it won't replay the roll anim unless we switch it off in the air. This fixes that.
		PM_SetAnim< mod >(SETANIM_BOTH,BOTH_INAIR1,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
		pm->ps->inAirAnim = qtrue;
	}
...
static void PM_GroundTrace( void ) {
	...
	if ( pm->ps->velocity[2] > 0 && DotProduct( pm->ps->velocity, trace.plane.normal ) > 10 ) {
		...
		if ( pm->cmd.forwardmove >= 0 ) {
			PM_ForceLegsAnim< mod >( BOTH_JUMP1 );
			pm->ps->pm_flags &= ~PMF_BACKWARDS_JUMP;
		} else {
			PM_ForceLegsAnim< mod >( BOTH_JUMPBACK1 );
			pm->ps->pm_flags |= PMF_BACKWARDS_JUMP;
		}
...
void PM_FootSlopeTrace( float *pDiff, float *pInterval )
{
	...
	g_trap->G2API_GetBoltMatrix( pm->ghoul2, 0, pm->g2Bolts_LFoot, &boltMatrix, G2Angles, pm->ps->origin, pm->cmd.serverTime, NULL, pm->modelScale );
	footLPoint[0] = boltMatrix.matrix[0][3];
	footLPoint[1] = boltMatrix.matrix[1][3];
	footLPoint[2] = boltMatrix.matrix[2][3];

	g_trap->G2API_GetBoltMatrix( pm->ghoul2, 0, pm->g2Bolts_RFoot, &boltMatrix, G2Angles, pm->ps->origin, pm->cmd.serverTime, NULL, pm->modelScale );
...
qboolean PM_AdjustStandAnimForSlope( void )
{
	...
	PM_ContinueLegsAnim< mod >(destAnim);
...
static void PM_Footsteps( void ) {
	...
	{
		PM_ContinueLegsAnim< mod >( pm->ps->torsoAnim );
	}
		...
		if ( pm->waterlevel > 1 )
		{
			if (pm->xyspeed > 60)
			{
				PM_ContinueLegsAnim< mod >( BOTH_SWIMFORWARD );
			}
			else
			{
				PM_ContinueLegsAnim< mod >( BOTH_SWIM_IDLE1 );
			}
		}
		return;
	}
	// if not trying to move
		...
		if (  pm->xyspeed < 5 ) {
				...
				pm_entSelf->s.NPC_class == CLASS_RANCOR )
			{
				...
				{//holding someone
					PM_ContinueLegsAnim< mod >( BOTH_STAND4 );
				...
				{//have an enemy or have had one since we spawned
					PM_ContinueLegsAnim< mod >( BOTH_STAND2 );
					//PM_SetAnim(pm,SETANIM_LEGS,BOTH_STAND2,SETANIM_FLAG_NORMAL);
				}
				else
				{//just stand there
					PM_ContinueLegsAnim< mod >( BOTH_STAND1 );
				...
				pm_entSelf->s.NPC_class == CLASS_WAMPA )
			{
				...
				{
					PM_SetAnim< mod >(SETANIM_LEGS, BOTH_CROUCH1IDLE, setAnimFlags);
				}
				else
				{
					PM_ContinueLegsAnim< mod >( BOTH_CROUCH1IDLE );
				}
			} else {
				if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
				{
					...
					PM_ContinueLegsAnim< mod >( TORSO_WEAPONREADY4 );
				}
				else
				{
					if (pm->ps->weapon == WP_SABER && BG_SabersOff( pm->ps ) )
					{
						if (!PM_AdjustStandAnimForSlope< mod >())
						{
							//PM_ContinueLegsAnim( BOTH_STAND1 );
							PM_ContinueLegsAnim< mod >(PM_LegsSlopeBackTransition(BOTH_STAND1));
						}
					}
					else
					{
						if (pm->ps->weapon != WP_SABER || !PM_AdjustStandAnimForSlope< mod >())
						{
							if (pm->ps->weapon == WP_SABER)
							{
								PM_ContinueLegsAnim< mod >(PM_LegsSlopeBackTransition(PM_GetSaberStance< mod >()));
							}
							else
							{
								PM_ContinueLegsAnim< mod >(PM_LegsSlopeBackTransition(WeaponReadyLegsAnim[pm->ps->weapon]));
							}
						}
					}
				}
	...
	else if ( pm->ps->pm_flags & PMF_DUCKED )
	{
		...
		{ //if the roll failed or didn't attempt, do standard crouching anim stuff.
			if ( pm->ps->pm_flags & PMF_BACKWARDS_RUN ) {
				...
				{
					PM_SetAnim< mod >(SETANIM_LEGS, BOTH_CROUCH1WALKBACK, setAnimFlags);
				}
				else
				{
					PM_ContinueLegsAnim< mod >( BOTH_CROUCH1WALKBACK );
				}
			}
			else {
				if ((pm->ps->legsAnim) != BOTH_CROUCH1WALK)
				{
					PM_SetAnim< mod >(SETANIM_LEGS, BOTH_CROUCH1WALK, setAnimFlags);
				}
				else
				{
					PM_ContinueLegsAnim< mod >( BOTH_CROUCH1WALK );
				}
			}
		}
		else
		{ //otherwise send us into the roll
			pm->ps->legsTimer = 0;
			pm->ps->legsAnim = 0;
			PM_SetAnim< mod >(SETANIM_BOTH,rolled,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			PM_AddEventWithParm< mod >( EV_ROLL, 0 );
			pm->maxs[2] = pm->ps->crouchheight;//CROUCH_MAXS_2;
			pm->ps->viewheight = DEFAULT_VIEWHEIGHT;
			pm->ps->pm_flags &= ~PMF_DUCKED;
	...
	else
	{
		...
		if (desiredAnim != -1)
		{
			...
			{
				PM_SetAnim< mod >(SETANIM_LEGS, desiredAnim, setAnimFlags);
			}
			else
			{
				PM_ContinueLegsAnim< mod >(ires);
			}
		}
	}
	...
	if ( ( ( old + 64 ) ^ ( pm->ps->bobCycle + 64 ) ) & 128 )
	{
		...
		if ( pm->waterlevel == 1 ) {
			// splashing
			PM_AddEvent< mod >( EV_FOOTSPLASH );
		} else if ( pm->waterlevel == 2 ) {
			// wading / swimming at surface
			PM_AddEvent< mod >( EV_SWIM );
...
static void PM_WaterEvents( void ) {		// FIXME?
	...
	if (!pml.previous_waterlevel && pm->waterlevel) {
		...
		PM_AddEvent< mod >( EV_WATER_TOUCH );
	}
	...
	if (pml.previous_waterlevel && !pm->waterlevel) {
		...
		PM_AddEvent< mod >( EV_WATER_LEAVE );
	}

	if ( mod == moduleType::game && impact_splash )
	{
	...
	if (pml.previous_waterlevel != 3 && pm->waterlevel == 3) {
		PM_AddEvent< mod >( EV_WATER_UNDER );
	}
	...
	//
	if (pml.previous_waterlevel == 3 && pm->waterlevel != 3) {
		PM_AddEvent< mod >( EV_WATER_CLEAR );
	}
}
...
void PM_BeginWeaponChange( int weapon ) {
	...
	PM_AddEventWithParm< mod >( EV_CHANGE_WEAPON, weapon );
	pm->ps->weaponstate = WEAPON_DROPPING;
	pm->ps->weaponTime += 200;
	//PM_StartTorsoAnim( TORSO_DROPWEAP1 );
	PM_SetAnim< mod >(SETANIM_TORSO, TORSO_DROPWEAP1, SETANIM_FLAG_OVERRIDE);
...
void PM_FinishWeaponChange( void ) {
	...
	if (weapon == WP_SABER)
	{
		PM_SetSaberMove< mod >(LS_DRAW);
	}
	else
	{
		//PM_StartTorsoAnim( TORSO_RAISEWEAP1);
		PM_SetAnim< mod >(SETANIM_TORSO, TORSO_RAISEWEAP1, SETANIM_FLAG_OVERRIDE);
	}
	pm->ps->weapon = weapon;
...
template int BG_VehTraceFromCamPos< moduleType::game >( trace_t *camTrace, bgEntity_t *bgEnt, const vec3_t entOrg, const vec3_t shotStart, const vec3_t end, vec3_t newEnd, vec3_t shotDir, float bestDist );
...
void PM_RocketLock( float lockDist, qboolean vehicleLock )
{
	...
	{//vehicles also do a trace from the camera point if the main one misses
		if ( tr.fraction >= 1.0f )
		{
			...
			if ( BG_VehTraceFromCamPos< mod >( &camTrace, PM_BGEntForNum(pm->ps->clientNum), pm->ps->origin, muzzlePoint, tr.endpos, newEnd, shotDir, (tr.fraction*lockDist) ) )
			{
	...
	if ( charging )
	{
		if ( altFire )
		{
			...
			//	Com_Printf("Starting charge\n");
				...
				BG_AddPredictableEventToPlayerstate< mod >(EV_WEAPON_CHARGE_ALT, pm->ps->weapon, pm->ps);
			}
		...
		else
		{
			...
			//	Com_Printf("Starting charge\n");
#endif
				BG_AddPredictableEventToPlayerstate< mod >(EV_WEAPON_CHARGE, pm->ps->weapon, pm->ps);
			}
...
int PM_ItemUsable(playerState_t *ps, int forcedUse)
{
	...
	case HI_SEEKER:
		...
		{
			PM_AddEventWithParm< mod >(EV_ITEMUSEFAIL, SEEKER_ALREADYDEPLOYED);
			return 0;
		}
	...
	case HI_SENTRY_GUN:
		...
		{
			PM_AddEventWithParm< mod >(EV_ITEMUSEFAIL, SENTRY_ALREADYPLACED);
			return 0;
		}
		...
		{
			PM_AddEventWithParm< mod >(EV_ITEMUSEFAIL, SENTRY_NOROOM);
			return 0;
		}
	...
	case HI_SHIELD:
		...
		PM_AddEventWithParm< mod >(EV_ITEMUSEFAIL, SHIELD_NOROOM);
		return 0;
...
backAgain:
		...
		switch ( pm->ps->weapon )
		{
			case WP_SABER:
				...
				{ //we'll be starting a new attack
					PM_AddEvent< mod >(EV_SABER_ATTACK);
				}
	...
	{ //override it
		...
		PM_SetAnim< mod >(SETANIM_BOTH, Anim, iFlags);
	}
}
...
static void PM_Weapon( void )
{
	...
	{ //reset into weapon stance
		...
		{ //saber handles its own anims
			if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
			{
				...
				PM_StartTorsoAnim< mod >( TORSO_RAISEWEAP1);
			}
			else
			{
				if (pm->ps->weapon == WP_EMPLACED_GUN)
	...
	{ //nothing else should be allowed to happen during this time, including weapon fire
		...
		{ //of seperateOnTorso, handle it after setting the legs
			PM_SetAnim< mod >(SETANIM_TORSO, desiredAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			pm->ps->torsoTimer = 1;
		}

		if (playFullBody)
		{ //sorry if all these exceptions are getting confusing. This one just means play on both legs and torso.
			PM_SetAnim< mod >(SETANIM_BOTH, desiredAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
		...
		{ //special case, play dodge anim on whole body, choke anim too if off ground
			if (seperateOnTorso)
			{
				PM_SetAnim< mod >(SETANIM_LEGS, desiredAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
				pm->ps->legsTimer = 1;

				PM_SetAnim< mod >(SETANIM_TORSO, desiredOnTorso, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
				pm->ps->torsoTimer = 1;
			}
			else
			{
	...
	if ( pm->cmd.buttons & BUTTON_USE_HOLDABLE ) {
		...
		if ( ! ( pm->ps->pm_flags & PMF_USE_ITEM_HELD ) ) {
			...
			else
			{
				...
				pm->ps->pm_flags |= PMF_USE_ITEM_HELD;
				PM_AddEvent< mod >( EV_USE_ITEM0 + bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag );
	...
	*/
		...
		if ( pm->ps->clientNum < MAX_CLIENTS && pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] != -1 )
		{
			...
			{ //the weapon is out of ammo essentially because it cannot fire primary or secondary, so do the switch
			  //regardless of if the player is attacking or not
				PM_AddEventWithParm< mod >( EV_NOAMMO, WP_NUM_WEAPONS+pm->ps->weapon );
	...
	if ( pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING ) {
		if ( pm->ps->weapon != pm->cmd.weapon ) {
			PM_BeginWeaponChange< mod >( pm->cmd.weapon );
		}
	}
		...
		pm->ps->zoomMode == 1)
	{
		if (pm_cancelOutZoom)
		{
			...
			pm->ps->zoomLocked = qfalse;
			pm->ps->zoomLockTime = 0;
			PM_AddEvent< mod >( EV_DISRUPTOR_ZOOMSOUND );
			return;
		}
	...
	if ( pm->ps->weaponstate == WEAPON_RAISING ) {
		...
		if (PM_CanSetWeaponAnims())
		{
			if ( pm->ps->weapon == WP_SABER )
			{
				PM_StartTorsoAnim< mod >( PM_GetSaberStance< mod >() );
			}
			else if (pm->ps->weapon == WP_MELEE || PM_IsRocketTrooper())
			{
				PM_StartTorsoAnim< mod >( pm->ps->legsAnim );
			}
			else
			{
				if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
		...
		pm->ps->weapon == WP_MELEE)
	{
			...
			pm->ps->forceHandExtend == HANDEXTEND_NONE)
		{
			...
			{ //don't do this while holding attack
				...
				{
					PM_StartTorsoAnim< mod >( desTAnim );
				}
			}
		}
	}
	...
	if ( PM_DoChargedWeapons< mod >(vehicleRocketLock, veh))
	{
	...
	if (pm->ps->weapon == WP_EMPLACED_GUN)
	{
		...
		if ( (pm->cmd.buttons & BUTTON_ALT_ATTACK) )
		{
			PM_AddEvent< mod >( EV_ALT_FIRE );
		}
		else
		{
			PM_AddEvent< mod >( EV_FIRE_WEAPON );
		}
		return;
	}
	...
	{ //a vehicle NPC that has a pilot
		...
		if( mod == moduleType::game ) { //hack, only do it game-side. vehicle weapons don't really need predicting I suppose.
			if( ( pm->cmd.buttons & BUTTON_ALT_ATTACK ) ) {
	...
	if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
	{
		PM_StartTorsoAnim< mod >( BOTH_ATTACK4 );
	}
			...
			{ //kicks
					...
					!BG_KickingAnim(pm->ps->legsAnim))
				{
					...
					if (kickMove != -1)
					{
						...
						if (kickAnim != -1)
						{
							PM_SetAnim< mod >(SETANIM_BOTH, kickAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
				...
				if (pm->ps->torsoAnim != pm->ps->legsAnim)
				{
					PM_SetAnim< mod >(SETANIM_BOTH, pm->ps->legsAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
				}
				pm->ps->weaponTime = 0;
				return;
			}
	...
	{
		PM_StartTorsoAnim< mod >( WeaponAttackAnim[pm->ps->weapon] );
	}
...
{
	if ( game::bg_fighterAltControl.integer
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime)
{
	...
	saber = BG_MySaber< mod >( ps->clientNum, 0 );
	if ( saber
		&& saber->moveSpeedScale != 1.0f )
	{
		ps->speed *= saber->moveSpeedScale;
	}
	saber = BG_MySaber< mod >( ps->clientNum, 1 );
	if ( saber
		&& saber->moveSpeedScale != 1.0f )
	{
		ps->speed *= saber->moveSpeedScale;
					 ...
					 vec3_t origin, vec3_t angles, vec3_t scale, int blendTime, qboolean forceHalt)
{
	...
	if (!*ikInProgress && !forceHalt)
	{
		...
		if (!g_trap->G2API_SetBoneIKState(ghoul2, time, NULL, IKS_DYNAMIC, &ikP))
		{
			assert(!"Failed to init IK system for g2 instance!");
		}

		//Now, create our IK bone state.
		if (g_trap->G2API_SetBoneIKState(ghoul2, time, "lhumerus", IKS_DYNAMIC, &ikP))
		{
			//restrict the elbow joint
			VectorSet(ikP.pcjMins,-90.0f,-20.0f,-20.0f);
			VectorSet(ikP.pcjMaxs,30.0f,20.0f,-20.0f);

			if (g_trap->G2API_SetBoneIKState(ghoul2, time, "lradius", IKS_DYNAMIC, &ikP))
	...
	{ //actively update our ik state.
		...
		g_trap->G2API_GetBoltMatrix(ghoul2, 0, lHandBolt, &lHandMatrix, tAngles, origin, time, 0, scale);
		...
		ikM.boneName[0] = 0;
		if (g_trap->G2API_IKMove(ghoul2, time, &ikM))
		{
			...
			g_trap->G2API_AnimateG2Models(ghoul2, time, &tuParms);
		}
		else
		{
			*ikInProgress = qfalse;
	...
	{ //kill it
		...
		g_trap->G2API_SetBoneIKState(ghoul2, time, "lhumerus", IKS_NONE, NULL);
		g_trap->G2API_SetBoneIKState(ghoul2, time, "lradius", IKS_NONE, NULL);

		//then reset the angles/anims on these PCJs
		g_trap->G2API_SetBoneAngles(ghoul2, 0, "lhumerus", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, time);
		g_trap->G2API_SetBoneAngles(ghoul2, 0, "lradius", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, time);

		//Get the anim/frames that the pelvis is on exactly, and match the left arm back up with them again.
		g_trap->G2API_GetBoneAnim(ghoul2, "pelvis", (const int)time, &cFrame, &sFrame, &eFrame, &flags, &animSpeed, 0, 0);
		g_trap->G2API_SetBoneAnim(ghoul2, 0, "lhumerus", sFrame, eFrame, flags, animSpeed, time, sFrame, 300);
		g_trap->G2API_SetBoneAnim(ghoul2, 0, "lradius", sFrame, eFrame, flags, animSpeed, time, sFrame, 300);

		//And finally, get rid of all the ik state effector data by calling with null bone name (similar to how we init it).
		g_trap->G2API_SetBoneIKState(ghoul2, time, NULL, IKS_NONE, NULL);
...
static void BG_G2ClientNeckAngles( void *ghoul2, int time, const vec3_t lookAngles, vec3_t headAngles, vec3_t neckAngles, vec3_t thoracicAngles, vec3_t headClampMinAngles, vec3_t headClampMaxAngles )
{
	...
	*/
	...
	g_trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", headAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
	g_trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", neckAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
	g_trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", thoracicAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
}
							...
							vec3_t ulAngles, vec3_t llAngles, vec3_t modelScale, float *tPitchAngle, float *tYawAngle, int *corrTime )
{
	...
	{//FIXME: no need to do this if legs and torso on are same frame
		...
		g_trap->G2API_GetBoltMatrix_NoRecNoRot( ghoul2, 0, motionBolt, &boltMatrix, vec3_origin, cent_lerpOrigin, time, 0, modelScale);
...
//	static int			movementOffsets[8] = { 0, 22, 45, -22, 0, 22, -45, -22 };
	...
	{ //a vehicle or riding a vehicle - in either case we don't need to be in here
		...
		{
			g_trap->G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			g_trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			g_trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			g_trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			g_trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
		}
		return;
	}
		...
		{ //misc emplaced
			...
			*/
			...
			{ //try to adjust so it doesn't look wrong
				...
				g_trap->G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", llAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
				g_trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", ulAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
				g_trap->G2API_SetBoneAngles(ghoul2, 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			...
			VectorScale(facingAngles, 0.6f, facingAngles);	g_trap->G2API_SetBoneAngles(ghoul2, 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			VectorScale(facingAngles, 0.8f, facingAngles);	g_trap->G2API_SetBoneAngles(ghoul2, 0, "upper_lumbar", facingAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			VectorScale(facingAngles, 0.8f, facingAngles);	g_trap->G2API_SetBoneAngles(ghoul2, 0, "thoracic", facingAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
			...
			g_trap->G2API_SetBoneAngles(ghoul2, 0, "cervical", facingAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time);
...
{//																							up			right		fwd
	g_trap->G2API_SetBoneAngles( ghoul2, 0, "thoracic", cent_lerpAngles, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, 0, 0, time );
}
...
void PmoveSingle (pmove_t *pmove) {
		...
		&& BG_FullBodyTauntAnim( pm->ps->torsoAnim ) )
	{
		...
		{//stop the anim
			...
			{
				PM_SetAnim< mod >( SETANIM_BOTH, BOTH_MEDITATE_END, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
			}
			else
			{
				pm->ps->legsTimer = pm->ps->torsoTimer = 0;
	...
	*/
	...
	{ //not allowed to move while charging the disruptor
		...
		{ //get out
			...
			PM_AddEventWithParm< mod >(EV_WEAPON_CHARGE, WP_DISRUPTOR); //cut the weapon charge sound
			pm->cmd.upmove = 0;
		}
	}
	else if (pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomMode == 1)
	...
	PM_CmdForSaberMoves< mod >(&pm->cmd);

	BG_AdjustClientSpeed< mod >(pm->ps, &pm->cmd, pm->cmd.serverTime);
...
//	PM_AdjustAnglesForStabDown( pm->ps, &pm->cmd );
	PM_AdjustAngleForWallJump< mod >( pm->ps, &pm->cmd, qtrue );
	PM_AdjustAngleForWallRunUp< mod >( pm->ps, &pm->cmd, qtrue );
	PM_AdjustAngleForWallRun< mod >( pm->ps, &pm->cmd, qtrue );
	...
	*/
	...
	if (pm->ps->pm_type == PM_JETPACK)
	{
		if (pm->cmd.rightmove > 0)
		{
			PM_ContinueLegsAnim< mod >(BOTH_INAIRRIGHT1);
		}
		else if (pm->cmd.rightmove < 0)
		{
            PM_ContinueLegsAnim< mod >(BOTH_INAIRLEFT1);
		}
		else if (pm->cmd.forwardmove > 0)
		{
			PM_ContinueLegsAnim< mod >(BOTH_INAIR1);
		}
		else if (pm->cmd.forwardmove < 0)
		{
			PM_ContinueLegsAnim< mod >(BOTH_INAIRBACK1);
		}
		else
		{
			PM_ContinueLegsAnim< mod >(BOTH_INAIR1);
...
*/
			...
			if( mod == moduleType::game ) {
				...
				if( !veh->playerState->vehBoarding )//|| veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER)
				{
					if( veh->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER ) { //client must explicitly call this for prediction
						BG_FighterUpdate< mod >( veh->m_pVehicle, &veh->m_pVehicle->m_ucmd, pm->mins, pm->maxs, self->playerState->gravity, pm->trace );
					}
	...
	if ( !pm->pmove_float )
		g_trap->SnapVector( pm->ps->velocity );
...
template void Pmove< moduleType::game >( pmove_t *pmove );
template void Pmove< moduleType::cgame >( pmove_t *pmove );
