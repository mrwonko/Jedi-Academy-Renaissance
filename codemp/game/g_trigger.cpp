...
void InitTrigger( gentity_t *self ) {
	...
	self->r.contents = CONTENTS_TRIGGER;		// replaces the -1 from g_trap->SetBrushModel
...
void multi_trigger_run( gentity_t *ent )
{
	...
	{
		g_trap->SetConfigstring( CS_GLOBAL_AMBIENT_SET, ent->soundSet );
	}
...
void multi_trigger( gentity_t *ent, gentity_t *activator )
{
	...
	{ //has "teambalance" property
		...
		numEnts = g_trap->EntitiesInBox( ent->r.absmin, ent->r.absmax, entityList, MAX_GENTITIES );
		while (i < numEnts)
		{
...
void Do_Strike(gentity_t *ent)
{
	...
	g_trap->Trace(&localTrace, strikeFrom, NULL, NULL, strikePoint, ent->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
void shipboundary_touch( gentity_t *self, gentity_t *other, trace_t *trace )
{
	...
	if (!ent || !ent->inuse)
	{ //this is bad
		g_trap->Error( ERR_DROP, "trigger_shipboundary has invalid target '%s'\n", self->target );
		return;
	}
...
void shipboundary_think(gentity_t *ent)
{
	...
	numListedEntities = g_trap->EntitiesInBox( ent->r.absmin, ent->r.absmax, iEntityList, MAX_GENTITIES );
...
void SP_trigger_shipboundary(gentity_t *self)
{
	...
	{
		g_trap->Error( ERR_DROP, "trigger_shipboundary without a target." );
	}
	G_SpawnInt("traveltime", "0", &self->genericValue1);

	if (!self->genericValue1)
	{
		g_trap->Error( ERR_DROP, "trigger_shipboundary without traveltime." );
	}
	...
	{//already hyperspacing, just keep us moving
		...
		{//they've started the hyperspace but haven't been teleported yet
			...
			{//half-way, now teleport them!
				...
				if (!ent || !ent->inuse)
				{ //this is bad
					g_trap->Error( ERR_DROP, "trigger_hyperspace has invalid target '%s'\n", self->target );
					return;
				}
				...
				if (!ent || !ent->inuse)
				{ //this is bad
					g_trap->Error( ERR_DROP, "trigger_hyperspace has invalid target2 '%s'\n", self->target2 );
					return;
				}
...
void SP_trigger_hyperspace(gentity_t *self)
{
	...
	if (!self->target || !self->target[0])
	{
		g_trap->Error( ERR_DROP, "trigger_hyperspace without a target." );
	}
	if (!self->target2 || !self->target2[0])
	{
		g_trap->Error( ERR_DROP, "trigger_hyperspace without a target2." );
	}
...
void SP_func_timer( gentity_t *self ) {
	...
	if ( self->random >= self->wait ) {
		...
		g_trap->Print( "func_timer at %s has random >= wait\n", vtos( self->s.origin ) );
	}
