...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		...
		g_trap->Trace( &tr, ent->client->ps.origin, ent->r.mins, vMax, ent->client->ps.origin, ent->s.number, mask, qfalse, 0, 0 );
	} else {
		g_trap->Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask, qfalse, 0, 0 );
	}
...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	*/
	...
	if (pushed_p > &pushed[MAX_GENTITIES]) {
		g_trap->Error( ERR_DROP, "pushed_p > &pushed[MAX_GENTITIES]" );
	}
	pushed_p->ent = check;
	...
	if (!block) {
		...
		g_trap->LinkEntity ((sharedEntity_t *)check);
		return qtrue;
	}
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	g_trap->UnlinkEntity( (sharedEntity_t *)pusher );

	listedEntities = g_trap->EntitiesInBox( totalMins, totalMaxs, entityList, MAX_GENTITIES );

	// move the pusher to it's final position
	VectorAdd( pusher->r.currentOrigin, move, pusher->r.currentOrigin );
	VectorAdd( pusher->r.currentAngles, amove, pusher->r.currentAngles );
	g_trap->LinkEntity( (sharedEntity_t *)pusher );
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		*obstacle = check;
		...
		for ( p=pushed_p-1 ; p>=pushed ; p-- ) {
			...
			g_trap->LinkEntity ((sharedEntity_t *)p->ent);
		}
		return qfalse;
	}
...
void G_MoverTeam( gentity_t *ent ) {
	...
	for (part = ent ; part ; part=part->teamchain) {
		...
		BG_EvaluateTrajectory< moduleType::game >( &part->s.pos, level.time, origin );
		BG_EvaluateTrajectory< moduleType::game >( &part->s.apos, level.time, angles );
	...
	if (part) {
		...
		for ( part = ent ; part ; part = part->teamchain ) {
			...
			BG_EvaluateTrajectory< moduleType::game >( &part->s.pos, level.time, part->r.currentOrigin );
			BG_EvaluateTrajectory< moduleType::game >( &part->s.apos, level.time, part->r.currentAngles );
			g_trap->LinkEntity( (sharedEntity_t *)part );
		}
...
void Reached_BinaryMover( gentity_t *ent )
{
	...
	{
		g_trap->Error( ERR_DROP, "Reached_BinaryMover: bad moverState" );
	}
}
...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	g_trap->Trace(&tr, origin, pMins, pMaxs, origin, other->s.number, other->clipmask, qfalse, 0, 0);
	if (!tr.startsolid &&
		!tr.allsolid &&
		tr.fraction == 1.0f &&
...
void Think_SpawnNewDoorTrigger( gentity_t *ent )
{
	...
	g_trap->LinkEntity ((sharedEntity_t *)other);
...
void SP_func_door (gentity_t *ent)
{
	...
	g_trap->SetBrushModel( (sharedEntity_t *)ent, ent->model );
...
void SpawnPlatTrigger( gentity_t *ent ) {
	...
	g_trap->LinkEntity ((sharedEntity_t *)trigger);
}
...
void Think_SetupTrainTargets( gentity_t *ent ) {
	...
	for ( path = ent->nextTrain ; path != start ; path = next ) {
		...
		if ( !path->target ) {
//			g_trap->Printf( "Train corner at %s without a target\n",
//				vtos(path->s.origin) );
			//end of path
			break;
		}
		...
		do {
			...
			if ( !next ) {
//				g_trap->Printf( "Train corner at %s without a target path_corner\n",
//					vtos(path->s.origin) );
				//end of path
				break;
			}
...
void SP_path_corner( gentity_t *self ) {
	if ( !self->targetname ) {
		g_trap->Print ("path_corner with no targetname at %s\n", vtos(self->s.origin));
		G_FreeEntity( self );
		return;
	}
...
void SP_func_train (gentity_t *self) {
	...
	if ( !self->target ) {
		g_trap->Print ("func_train without a target at %s\n", vtos(self->r.absmin));
		G_FreeEntity( self );
		return;
	}

	g_trap->SetBrushModel( (sharedEntity_t *)self, self->model );
	InitMover( self );
...
void funcBBrushDieGo (gentity_t *self)
{
	...
	g_trap->AdjustAreaPortalState( (sharedEntity_t *)self, qtrue );
...
void SP_func_breakable( gentity_t *self )
{
	...
	if ( self->team && self->team[0] )
	{
		...
		{
			g_trap->Error( ERR_DROP, "team name %s not recognized\n", self->team );
		}
	}
	*/
	...
	self->team = NULL;
	if (!self->model) {
		g_trap->Error( ERR_DROP, "func_breakable with NULL model\n" );
	}
	InitBBrush( self );
...
{
	//g_trap->Print("Mr. Glass says: PLZ NO IT HURTS\n");
	//Make "cracking" sound?
}
