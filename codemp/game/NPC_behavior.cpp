		...
		if(attack_ok)
		{
			...
			{//He's in our FOV
				...
				if(attack_ok)
				{
					...
					g_trap->Trace ( &tr, muzzle, NULL, NULL, enemy_org, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
					...
					{//no, so shoot for the head
						attack_scale *= 0.75;
						g_trap->Trace ( &tr, muzzle, NULL, NULL, enemy_head, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
						traceEnt = &g_entities[tr.entityNum];
					}
...
//			NPC->cantHitEnemyCounter++;
	...
	{//We reached our captureGoal
		if(g_trap->ICARUS_IsInitialized(NPCS.NPC->s.number))
		{
			g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)NPCS.NPC, TID_BSTATE );
		}
	}
}
...
qboolean NPC_CheckInvestigate( int alertEventNum )
{
	...
	//if(!g_trap->InPVSIgnorePortals(ent->r.currentOrigin, NPC->r.currentOrigin))//should we be able to hear through areaportals?
	if ( !g_trap->InPVS( soundPos, NPCS.NPC->r.currentOrigin ) )
...
//		Com_Printf( "%s jumping %s, gravity at %4.0f percent\n", NPC->targetname, vtos(NPC->client->ps.velocity), NPC->client->ps.gravity/8.0f );
	...
	case JS_LANDING:
		...
		else
		{
			...
			g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV );
...
void NPC_BSRemove (void)
{
	...
	if( !g_trap->InPVS( NPCS.NPC->r.currentOrigin, g_entities[0].r.currentOrigin ) )//FIXME: use cg.vieworg?
	...
	else
	{
		...
		if ( NPCS.NPCInfo->investigateDebounceTime > level.time )
		{
			...
			if ( NPCS.NPCInfo->tempGoal->waypoint != WAYPOINT_NONE )
			{
				if ( !Q_irand( 0, 30 ) )
				{
					int	numEdges = g_trap->Nav_GetNodeNumEdges( NPCS.NPCInfo->tempGoal->waypoint );
					...
					if ( numEdges != WAYPOINT_NONE )
					{
						...
						int nextWp = g_trap->Nav_GetNodeEdge( NPCS.NPCInfo->tempGoal->waypoint, branchNum );
						g_trap->Nav_GetNodePosition( nextWp, branchPos );
		...
		{//Just finished waiting
			...
			if ( NPCS.NPC->waypoint == NPCS.NPCInfo->homeWp )
			{
				...
				if ( numEdges != WAYPOINT_NONE )
				{
					...
					int nextWp = g_trap->Nav_GetNodeEdge( NPCS.NPCInfo->homeWp, branchNum );
					g_trap->Nav_GetNodePosition( nextWp, NPCS.NPCInfo->tempGoal->r.currentOrigin );
			...
			{//At a branch, so return home
				g_trap->Nav_GetNodePosition( NPCS.NPCInfo->homeWp, NPCS.NPCInfo->tempGoal->r.currentOrigin );
...
void NPC_BSSearchStart( int homeWp, bState_t bState )
{
	...
	NPCS.NPCInfo->aiFlags |= NPCAI_ENROUTE_TO_HOMEWP;
	NPCS.NPCInfo->investigateDebounceTime = 0;
	g_trap->Nav_GetNodePosition( homeWp, NPCS.NPCInfo->tempGoal->r.currentOrigin );
...
{//FIXME: don't actually go all the way to the next waypoint, just move in fits and jerks...?
	...
	else
	{
		...
		{//Just finished waiting
			...
			if ( NPCS.NPC->waypoint != WAYPOINT_NONE )
			{
				int	numEdges = g_trap->Nav_GetNodeNumEdges( NPCS.NPC->waypoint );

				if ( numEdges != WAYPOINT_NONE )
				{
					int branchNum = Q_irand( 0, numEdges - 1 );

					int nextWp = g_trap->Nav_GetNodeEdge( NPCS.NPC->waypoint, branchNum );
					g_trap->Nav_GetNodePosition( nextWp, NPCS.NPCInfo->tempGoal->r.currentOrigin );
					NPCS.NPCInfo->tempGoal->waypoint = nextWp;
				}
					...
					{//player is the guy I'm running from
						...
						{//player is using saber
							...
							{//they're looking at me
								...
								{//they're close
									if ( g_trap->InPVS( NPC->r.currentOrigin, NPC->enemy->r.currentOrigin ) )
		...
		if ( NPCS.NPC->waypoint != WAYPOINT_NONE )
		{
			...
			if ( numEdges != WAYPOINT_NONE )
			{
				...
				for ( branchNum = 0; branchNum < numEdges; branchNum++ )
				{
					...
					nextWp = g_trap->Nav_GetNodeEdge( NPCS.NPC->waypoint, branchNum );
					g_trap->Nav_GetNodePosition( nextWp, branchPos );
...
/*
	else if ( g_trap->InPVS( NPC->enemy->r.currentOrigin, NPC->r.currentOrigin ) )
	{
