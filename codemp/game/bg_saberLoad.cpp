...
void WP_SaberSetDefaults( saberInfo_t *saber ) {
	...
	saber->skin					= 0;
	saber->soundOn				= BG_SoundIndex< mod >( "sound/weapons/saber/enemy_saber_on.wav" );
	saber->soundLoop			= BG_SoundIndex< mod >( "sound/weapons/saber/saberhum3.wav" );
	saber->soundOff				= BG_SoundIndex< mod >( "sound/weapons/saber/enemy_saber_off.wav" );
	saber->numBlades			= 1;
...
static void Saber_ParseSoundOn( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->soundOn = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseSoundLoop( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->soundLoop = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseSoundOff( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->soundOff = BG_SoundIndex< mod >( value );
}
...
static void Saber_ParseSpinSound( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->spinSound = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseSwingSound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->swingSound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseSwingSound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->swingSound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseSwingSound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->swingSound[2] = BG_SoundIndex< mod >( value );
}
...
static void Saber_ParseG2MarksShader( saberInfo_t *saber, const char **p ) {
	...
	if( mod == moduleType::cgame ) {
		saber->g2MarksShader = re->RegisterShader( value );
	} else {
		SkipRestOfLine( p );
	}
}
...
static void Saber_ParseG2WeaponMarkShader( saberInfo_t *saber, const char **p ) {
	...
	if( mod == moduleType::cgame ) {
		saber->g2WeaponMarkShader = re->RegisterShader( value );
	} else {
		SkipRestOfLine( p );
	}
}
...
static void Saber_ParseHitSound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->hitSound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseHitSound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->hitSound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseHitSound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->hitSound[2] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBlockSound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->blockSound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBlockSound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->blockSound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBlockSound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->blockSound[2] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBounceSound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->bounceSound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBounceSound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->bounceSound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBounceSound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->bounceSound[2] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
...
static void Saber_ParseG2MarksShader2( saberInfo_t *saber, const char **p ) {
	...
	if( mod == moduleType::cgame ) {
		saber->g2MarksShader2 = re->RegisterShader( value );
	} else {
		SkipRestOfLine( p );
	}
}
...
static void Saber_ParseG2WeaponMarkShader2( saberInfo_t *saber, const char **p ) {
	...
	if( mod == moduleType::cgame ) {
		saber->g2WeaponMarkShader2 = re->RegisterShader( value );
	} else {
		SkipRestOfLine( p );
	}
}
...
static void Saber_ParseHit2Sound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->hit2Sound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseHit2Sound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->hit2Sound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseHit2Sound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->hit2Sound[2] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBlock2Sound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->block2Sound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBlock2Sound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->block2Sound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBlock2Sound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->block2Sound[2] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBounce2Sound1( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->bounce2Sound[0] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBounce2Sound2( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->bounce2Sound[1] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
static void Saber_ParseBounce2Sound3( saberInfo_t *saber, const char **p ) {
	...
	if ( COM_ParseString( p, &value ) )
		return;
	saber->bounce2Sound[2] = BG_SoundIndex< mod >( value );
}
template< moduleType mod >
...
struct saberParse {
	...
	static qboolean hashSetup;
};
template< moduleType mod >
keywordHash_t* saberParse< mod >::keywordHash[ KEYWORDHASH_SIZE ];

template< moduleType mod >
keywordHash_t saberParse< mod >::keywords[] = {
	...
	{ "soundOn",				Saber_ParseSoundOn< mod >,				NULL },
	{ "soundLoop",				Saber_ParseSoundLoop< mod >,			NULL },
	{ "soundOff",				Saber_ParseSoundOff< mod >,			NULL },
	...
	{ "spinSound",				Saber_ParseSpinSound< mod >,			NULL },
	{ "swingSound1",			Saber_ParseSwingSound1< mod >,			NULL },
	{ "swingSound2",			Saber_ParseSwingSound2< mod >,			NULL },
	{ "swingSound3",			Saber_ParseSwingSound3< mod >,			NULL },
	...
	{ "g2MarksShader",			Saber_ParseG2MarksShader< mod >,		NULL },
	{ "g2MarksShader2",			Saber_ParseG2MarksShader2< mod >,		NULL },
	{ "g2WeaponMarkShader",		Saber_ParseG2WeaponMarkShader< mod >,	NULL },
	{ "g2WeaponMarkShader2",	Saber_ParseG2WeaponMarkShader2< mod >,	NULL },
	...
	{ "hitSound1",				Saber_ParseHitSound1< mod >,			NULL },
	{ "hit2Sound1",				Saber_ParseHit2Sound1< mod >,			NULL },
	{ "hitSound2",				Saber_ParseHitSound2< mod >,			NULL },
	{ "hit2Sound2",				Saber_ParseHit2Sound2< mod >,			NULL },
	{ "hitSound3",				Saber_ParseHitSound3< mod >,			NULL },
	{ "hit2Sound3",				Saber_ParseHit2Sound3< mod >,			NULL },
	{ "blockSound1",			Saber_ParseBlockSound1< mod >,			NULL },
	{ "block2Sound1",			Saber_ParseBlock2Sound1< mod >,		NULL },
	{ "blockSound2",			Saber_ParseBlockSound2< mod >,			NULL },
	{ "block2Sound2",			Saber_ParseBlock2Sound2< mod >,		NULL },
	{ "blockSound3",			Saber_ParseBlockSound3< mod >,			NULL },
	{ "block2Sound3",			Saber_ParseBlock2Sound3< mod >,		NULL },
	{ "bounceSound1",			Saber_ParseBounceSound1< mod >,		NULL },
	{ "bounce2Sound1",			Saber_ParseBounce2Sound1< mod >,		NULL },
	{ "bounceSound2",			Saber_ParseBounceSound2< mod >,		NULL },
	{ "bounce2Sound2",			Saber_ParseBounce2Sound2< mod >,		NULL },
	{ "bounceSound3",			Saber_ParseBounceSound3< mod >,		NULL },
	{ "bounce2Sound3",			Saber_ParseBounce2Sound3< mod >,		NULL },
	{ "blockEffect",			Saber_ParseBlockEffect< mod >,			NULL },
	{ "blockEffect2",			Saber_ParseBlockEffect2< mod >,		NULL },
	{ "hitPersonEffect",		Saber_ParseHitPersonEffect< mod >,		NULL },
	{ "hitPersonEffect2",		Saber_ParseHitPersonEffect2< mod >,	NULL },
	{ "hitOtherEffect",			Saber_ParseHitOtherEffect< mod >,		NULL },
	{ "hitOtherEffect2",		Saber_ParseHitOtherEffect2< mod >,		NULL },
	{ "bladeEffect",			Saber_ParseBladeEffect< mod >,			NULL },
	{ "bladeEffect2",			Saber_ParseBladeEffect2< mod >,		NULL },
...
template< moduleType mod >
qboolean saberParse< mod >::hashSetup = qfalse;

template< moduleType mod >
static void WP_SaberSetupKeywordHash( void ) {
	int i;

	memset( saberParse< mod >::keywordHash, 0, sizeof( saberParse< mod >::keywordHash ) );
	for ( i=0; saberParse< mod >::keywords[i].keyword; i++ )
		KeywordHash_Add( saberParse< mod >::keywordHash, &saberParse< mod >::keywords[i] );

	saberParse< mod >::hashSetup = qtrue;
}
...
qboolean WP_SaberParseParms( const char *saberName, saberInfo_t *saber ) {
	...
	if ( !saberParse< mod >::hashSetup )
		WP_SaberSetupKeywordHash< mod >();

	if ( !saber )
		return qfalse;

	//Set defaults so that, if it fails, there's at least something there
	WP_SaberSetDefaults< mod >( saber );
	...
	while ( 1 ) {
		...
		key = KeywordHash_Find( saberParse< mod >::keywordHash, token );
		if ( key ) {
			key->func( saber, &p );
			continue;
		}
...
template qboolean WP_SaberParseParms< moduleType::game >( const char *saberName, saberInfo_t *saber );
template qboolean WP_SaberParseParms< moduleType::cgame >( const char *saberName, saberInfo_t *saber );
...
void WP_RemoveSaber( saberInfo_t *sabers, int saberNum )
{
	...
	WP_SaberSetDefaults< mod >( &sabers[saberNum] );
...
//	{
//		g_trap->G2API_RemoveGhoul2Model( ent->ghoul2, ent->weaponModel[saberNum] );
...
void WP_SetSaber( int entNum, saberInfo_t *sabers, int saberNum, const char *saberName )
{
	...
	if ( Q_stricmp( "none", saberName ) == 0 || Q_stricmp( "remove", saberName ) == 0 )
	{
		...
		{ //can't remove saber 0 ever
			WP_RemoveSaber< mod >( sabers, saberNum );
		}
		return;
	}
	...
	{
		WP_SaberParseParms< mod >( DEFAULT_SABER, &sabers[saberNum] );//get saber info
	}
	else
	{
		WP_SaberParseParms< mod >( saberName, &sabers[saberNum] );//get saber info
	}
	if ((sabers[1].saberFlags&SFL_TWO_HANDED))
	{//not allowed to use a 2-handed saber as second saber
		WP_RemoveSaber< mod >( sabers, 1 );
		return;
	}
...
template void WP_SetSaber< moduleType::game >( int entNum, saberInfo_t *sabers, int saberNum, const char *saberName );
template void WP_SetSaber< moduleType::cgame >( int entNum, saberInfo_t *sabers, int saberNum, const char *saberName );
...
void WP_SaberLoadParms( void )
{
	...
	*marker = 0;
	...
	fileCnt = g_trap->FS_GetFileList( "ext_data/sabers", ".sab", saberExtensionListBuf, sizeof( saberExtensionListBuf ) );

	holdChar = saberExtensionListBuf;
	for ( i=0; i<fileCnt; i++, holdChar += saberExtFNLen+1 ) {
		saberExtFNLen = strlen( holdChar );

		len = g_trap->FS_Open( va( "ext_data/sabers/%s", holdChar ), &f, FS_READ );
		...
		g_trap->FS_Read(bgSaberParseTBuffer, len, f);
...
template void WP_SaberLoadParms< moduleType::game >( void );
template void WP_SaberLoadParms< moduleType::cgame >( void );
template void WP_SaberLoadParms< moduleType::ui >( void );
