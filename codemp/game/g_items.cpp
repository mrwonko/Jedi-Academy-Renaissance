...
void ShieldGoSolid(gentity_t *self)
{
	...
	g_trap->Trace (&tr, self->r.currentOrigin, self->r.mins, self->r.maxs, self->r.currentOrigin, self->s.number, CONTENTS_BODY, qfalse, 0, 0 );
	if(tr.startsolid)
	{	// gah, we can't activate yet
		self->nextthink = level.time + 200;
		self->think = ShieldGoSolid;
		g_trap->LinkEntity((sharedEntity_t *)self);
	}
	else
	{ // get hard... huh-huh...
...
//	static int	shieldID;
	...
	g_trap->Trace (&tr, ent->r.currentOrigin, NULL, NULL, end, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	start[2] += (height>>1);
	g_trap->Trace (&tr, start, 0, 0, posTraceEnd, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
	posWidth = MAX_SHIELD_HALFWIDTH * tr.fraction;
	// negative trace
	g_trap->Trace (&tr, start, 0, 0, negTraceEnd, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
//  negWidth - 0 - 255 8 bits
	...
	// see if we're valid
	g_trap->Trace (&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ent->s.number, CONTENTS_BODY, qfalse, 0, 0 );
...
qboolean PlaceShield(gentity_t *playerent)
{
	...
	g_trap->Trace (&tr, playerent->client->ps.origin, mins, maxs, dest, playerent->s.number, MASK_SHOT, qfalse, 0, 0 );
	if (tr.fraction > 0.9)
	{//room in front
		VectorCopy(tr.endpos, pos);
		// drop to floor
		VectorSet( dest, pos[0], pos[1], pos[2] - 4096 );
		g_trap->Trace( &tr, pos, mins, maxs, dest, playerent->s.number, MASK_SOLID, qfalse, 0, 0 );
		if ( !tr.startsolid && !tr.allsolid )
		{
			...
			g_trap->LinkEntity ((sharedEntity_t *)shield);
...
//-----------------------------------------------------
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( !g_trap->InPVS( org2, target->r.currentOrigin ))
		{
			continue;
		}
		...
		g_trap->Trace( &tr, org2, NULL, NULL, org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
//---------------------------------
{
	...
	else
	{
		...
		g_trap->Trace( &tr, org2, NULL, NULL, org, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
//---------------------------------
{
	...
	numListedEntities = g_trap->EntitiesInBox( testMins, testMaxs, iEntityList, MAX_GENTITIES );
	...
	while (i < numListedEntities)
	{
		...
		{ //client stuck inside me. go nonsolid.
			...
			numListedEntities = g_trap->EntitiesInBox( g_entities[clNum].r.absmin, g_entities[clNum].r.absmax, iEntityList, MAX_GENTITIES );
...
//------------------------------------------------------------------------
{
	...
	g_trap->LinkEntity((sharedEntity_t *)sentry);
...
void ItemUse_UseDisp(gentity_t *ent, int type)
{
	...
	if (item)
	{
		...
		g_trap->LinkEntity((sharedEntity_t *)eItem);
...
void EWeb_SetBoneAngles(gentity_t *ent, char *bone, vec3_t angles)
{
	...
	g_trap->G2API_SetBoneAngles( ent->ghoul2,
					0,
					bone,
					angles,
					flags,
...
void EWeb_SetBoneAnim(gentity_t *eweb, int startFrame, int endFrame)
{
	...
	assert(eweb->ghoul2);
	g_trap->G2API_SetBoneAnim(eweb->ghoul2, 0, "model_root", startFrame, endFrame,
...
void EWebFire(gentity_t *owner, gentity_t *eweb)
{
	...
	g_trap->G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->genericValue10, &boltMatrix, eweb->s.apos.trBase, eweb->r.currentOrigin, level.time, NULL, eweb->modelScale);
...
void EWebPositionUser(gentity_t *owner, gentity_t *eweb)
{
	...
	g_trap->G2API_GetBoltMatrix(eweb->ghoul2, 0, eweb->genericValue9, &boltMatrix, eweb->s.apos.trBase, eweb->r.currentOrigin, level.time, NULL, eweb->modelScale);
	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, p);
	...
	g_trap->Trace(&tr, owner->client->ps.origin, owner->r.mins, owner->r.maxs, p, owner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	{ //all clear, we can move there
		...
		VectorCopy(p, pDown);
		pDown[2] -= 7.0f;
		g_trap->Trace(&tr, p, owner->r.mins, owner->r.maxs, pDown, owner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
gentity_t *EWeb_Create(gentity_t *spawner)
{
	...
	g_trap->Trace(&tr, s, mins, maxs, pos, spawner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	downPos[2] -= 18.0f;
	g_trap->Trace(&tr, pos, mins, maxs, downPos, spawner->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	re->G2API_InitGhoul2Model(&ent->ghoul2, modelName, 0, 0, 0, 0, 0);
	...
	ent->genericValue10 = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*cannonflash"); //muzzle bolt
	ent->genericValue9 = g_trap->G2API_AddBolt(ent->ghoul2, 0, "cannon_Yrot"); //for placing the owner relative to rotation
...
int Pickup_Powerup( gentity_t *ent, gentity_t *other ) {
    ...
    // cannot use OnSameTeam as it expects to g_entities, not clients
      ...
      continue;
		...
		g_trap->Trace( &tr, client->ps.origin, NULL, NULL, ent->s.pos.trBase, ENTITYNUM_NONE, CONTENTS_SOLID, qfalse, 0, 0 );
		if ( tr.fraction != 1.0 ) {
			continue;
		}
...
void RespawnItem( gentity_t *ent ) {
	...
	if (ent->team) {
		...
		if ( !ent->teammaster ) {
			g_trap->Error( ERR_DROP, "RespawnItem: bad teammaster");
		}
...
//		predict = qtrue;
	...
	if (predict) {
		if (other->client)
		{
			BG_AddPredictableEventToPlayerstate< moduleType::game >( EV_ITEM_PICKUP, ent->s.number, &other->client->ps);
		}
		else
		{
...
gentity_t *LaunchItem( gitem_t *item, vec3_t origin, vec3_t velocity ) {
	...
	g_trap->LinkEntity ((sharedEntity_t *)dropped);
...
/*	item = &bg_itemlist[ ent->s.modelindex ];
	...
	{
		g_trap->G2API_InitGhoul2Model(&ent->s, item->world_model[0], G_ModelIndex(item->world_model[0] ), 0, 0, 0, 0);
		ent->s.radius = 60;
	}
*/
	if ( ent->spawnflags & ITMSF_SUSPEND ) {
		...
		g_trap->Trace( &tr, ent->s.origin, ent->r.mins, ent->r.maxs, dest, ent->s.number, MASK_SOLID, qfalse, 0, 0 );
		if ( tr.startsolid ) {
			g_trap->Print ("FinishSpawningItem: %s startsolid at %s\n", ent->classname, vtos(ent->s.origin));
			G_FreeEntity( ent );
			return;
		}
...
void G_CheckTeamItems( void ) {
	...
	if( level.gametype == GT_CTF || level.gametype == GT_CTY ) {
		...
		if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
			g_trap->Print( S_COLOR_YELLOW "WARNING: No team_CTF_redflag in map\n" );
		}
		item = BG_FindItem( "team_CTF_blueflag" );
		if ( !item || !itemRegistered[ item - bg_itemlist ] ) {
			g_trap->Print( S_COLOR_YELLOW "WARNING: No team_CTF_blueflag in map\n" );
		}
	}
}
...
void RegisterItem( gitem_t *item ) {
	if ( !item ) {
		g_trap->Error( ERR_DROP, "RegisterItem: NULL" );
	}
...
//	g_trap->Print( "%i items registered\n", count );
	g_trap->SetConfigstring(CS_ITEMS, string);
}
...
int G_ItemDisabled( gitem_t *item ) {
	...
	return g_trap->Cvar_VariableIntegerValue( name );
}
...
void G_BounceItem( gentity_t *ent, trace_t *trace ) {
	...
	BG_EvaluateTrajectoryDelta< moduleType::game >( &ent->s.pos, hitTime, velocity );
...
void G_RunItem( gentity_t *ent ) {
	...
	g_trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, ent->r.ownerNum, mask, qfalse, 0, 0 );
	...
	g_trap->LinkEntity( (sharedEntity_t *)ent );	// FIXME: avoid this for stationary?
	...
	contents = g_trap->PointContents( ent->r.currentOrigin, -1 );
