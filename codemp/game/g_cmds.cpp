...
void DeathmatchScoreboardMessage( gentity_t *ent ) {
	...
	g_trap->SendServerCommand( ent-g_entities, va("scores %i %i %i%s", i,
...
int ClientNumberFromString( gentity_t *to, const char *s, qboolean allowconnecting ) {
	...
	g_trap->SendServerCommand( to-g_entities, va( "print \"User %s is not on the server\n\"", s ) );
	return -1;
}
...
void Cmd_Give_f( gentity_t *ent )
{
	...
	g_trap->Argv( 1, name, sizeof( name ) );
	G_Give( ent, name, ConcatArgs( 2 ), g_trap->Argc() );
}
...
void Cmd_GiveOther_f( gentity_t *ent )
{
	...
	if ( g_trap->Argc () < 3 ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"Usage: giveother <player id> <givestring>\n\"" );
		return;
	}

	g_trap->Argv( 1, otherindex, sizeof( otherindex ) );
	...
	if ( (otherEnt->health <= 0 || otherEnt->client->tempSpectate >= level.time || otherEnt->client->sess.sessionTeam == TEAM_SPECTATOR) )
	{
		...
		g_trap->SendServerCommand( ent-g_entities, va( "print \"%s\n\"", G_GetStringEdString( "MP_SVGAME", "MUSTBEALIVE" ) ) );
		return;
	}

	g_trap->Argv( 2, name, sizeof( name ) );

	G_Give( otherEnt, name, ConcatArgs( 3 ), g_trap->Argc()-1 );
}
...
void Cmd_LevelShot_f( gentity_t *ent )
{
	...
	{
		g_trap->SendServerCommand(ent-g_entities, "print \"The levelshot command must be executed by a local client\n\"");
		return;
	}
	...
	{
		g_trap->SendServerCommand(ent-g_entities, "print \"Must not be in singleplayer mode for levelshot\n\"" );
		return;
	}

	BeginIntermission();
	g_trap->SendServerCommand( ent-g_entities, "clientLevelShot" );
}
...
void Cmd_TeamTask_f( gentity_t *ent ) {
	...
	g_trap->GetUserinfo(client, userinfo, sizeof(userinfo));
	Info_SetValueForKey(userinfo, "teamtask", va("%d", task));
	g_trap->SetUserinfo(client, userinfo);
...
void Cmd_KillOther_f( gentity_t *ent )
{
	...
	if ( g_trap->Argc () < 2 ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"Usage: killother <player id>\n\"" );
		return;
	}
...
void BroadcastTeamChange( gclient_t *client, int oldTeam )
{
	...
	if ( client->sess.sessionTeam == TEAM_RED ) {
		g_trap->SendServerCommand( -1, va("cp \"%s" S_COLOR_WHITE " %s\n\"",
...
void SetTeam( gentity_t *ent, char *s ) {
	...
	if ( !Q_stricmp( s, "scoreboard" ) || !Q_stricmp( s, "score" )  ) {
		...
		if ( g_teamForceBalance.integer && !g_jediVmerc.integer ) {
			...
			if ( team == TEAM_RED && counts[TEAM_RED] - counts[TEAM_BLUE] > 1 ) {
				...
				{
					g_trap->SendServerCommand( ent->client->ps.clientNum,
				...
				*/
				{
					//JAC: Invalid clientNum was being used
					g_trap->SendServerCommand( ent-g_entities,
	...
	if (level.gametype == GT_SIEGE)
	{
		...
		{ //can't switch to anything in siege unless you want to switch to being a fulltime spectator
			...
			g_trap->SendServerCommand(ent-g_entities, va("sb %i", client->sess.siegeDesiredTeam));

			g_trap->SendServerCommand( ent-g_entities, va("print \"You will be on the selected team the next time the round begins.\n\"") );
			return;
		}
		*/
		...
		{ //not a spectator now, and not switching to spec, so you have to wait til you die.
			//g_trap->SendServerCommand( ent-g_entities, va("print \"You will be on the selected team the next time you respawn.\n\"") );
			qboolean doBegin;
...
//	{
//		g_trap->SendServerCommand(clientNum, va("sb %i", team));
//	}
...
void Cmd_Team_f( gentity_t *ent ) {
	...
	if ( level.gametype == GT_DUEL
		...
		g_trap->SendServerCommand( ent-g_entities, "print \"Cannot switch teams in Duel\n\"" );
		return;
	...
	{ //don't let clients change teams manually at all in powerduel, it will be taken care of through automated stuff
		g_trap->SendServerCommand( ent-g_entities, "print \"Cannot switch teams in Power Duel\n\"" );
		return;
	}

	g_trap->Argv( 1, s, sizeof( s ) );
...
void Cmd_DuelTeam_f(gentity_t *ent)
{
	...
	{
		g_trap->SendServerCommand( ent-g_entities, va("print \"You cannot change your duel team unless you are a spectator.\n\""));
		return;
	}
	*/
	...
	{ //No arg so tell what team we're currently on.
		...
		{
		case DUELTEAM_FREE:
			g_trap->SendServerCommand( ent-g_entities, va("print \"None\n\"") );
			break;
		case DUELTEAM_LONE:
			g_trap->SendServerCommand( ent-g_entities, va("print \"Single\n\"") );
			break;
		case DUELTEAM_DOUBLE:
			g_trap->SendServerCommand( ent-g_entities, va("print \"Double\n\"") );
			break;
		default:
			break;
		}
	...
	{
		g_trap->SendServerCommand( ent-g_entities, va("print \"'%s' not a valid duel team.\n\"", s) );
	}
...
void Cmd_SiegeClass_f( gentity_t *ent )
{
	...
	g_trap->Argv( 1, className, sizeof( className ) );
...
//	Cmd_Kill_f(ent);
	...
	g_trap->SendServerCommand( ent-g_entities, va("print \"%s%s\n\"", S_COLOR_GREEN, fpChStr) );
...
qboolean G_SetSaber(gentity_t *ent, int saberNum, char *saberName, qboolean siegeOverride)
{
	...
	WP_SetSaber< moduleType::game >( ent->s.number, ent->client->saber, saberNum, truncSaberName );
...
void Cmd_FollowCycle_f( gentity_t *ent, int dir ) {
	...
	if ( dir != 1 && dir != -1 ) {
		g_trap->Error( ERR_DROP, "Cmd_FollowCycle_f: bad dir %i", dir );
	}
...
static void G_SayTo( gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message, char *locMsg )
{
	...
	*/
	...
	if (locMsg)
	{
		g_trap->SendServerCommand( other-g_entities, va("%s \"%s\" \"%s\" \"%c\" \"%s\" %i",
			mode == SAY_TEAM ? "ltchat" : "lchat",
			name, locMsg, color, message, ent->s.number));
	}
	else
	{
		g_trap->SendServerCommand( other-g_entities, va("%s \"%s%c%c%s\" %i",
...
static void Cmd_Tell_f( gentity_t *ent ) {
	...
	if ( g_trap->Argc () < 3 ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"Usage: tell <player id> <message>\n\"" );
		return;
	}
...
void Cmd_GameCommand_f( gentity_t *ent ) {
	...
	if ( g_trap->Argc() != 3 ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"Usage: gc <player id> <order 0-%d>\n\"", numgc_orders - 1 ) );
		return;
	}

	g_trap->Argv( 2, arg, sizeof( arg ) );
	order = atoi( arg );

	if ( order >= numgc_orders ) {
		g_trap->SendServerCommand( ent-g_entities, va("print \"Bad order: %i\n\"", order));
		return;
	}
...
qboolean G_VoteClientkick( gentity_t *ent, int numArgs, const char *arg1, const char *arg2 ) {
	...
	if ( n < 0 || n >= level.maxclients ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"invalid client number %d.\n\"", n ) );
		return qfalse;
	}

	if ( g_entities[n].client->pers.connected == CON_DISCONNECTED ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"there is no client with the client number %d.\n\"", n ) );
		return qfalse;
	}
...
qboolean G_VoteGametype( gentity_t *ent, int numArgs, const char *arg1, const char *arg2 ) {
	...
	if ( arg2[0] && isalpha( arg2[0] ) ) {
		...
		if ( gt == -1 )
		{
			g_trap->SendServerCommand( ent-g_entities, va( "print \"Gametype (%s) unrecognised, defaulting to FFA/Deathmatch\n\"", arg2 ) );
			gt = GT_FFA;
		}
	}
	// numeric but out of range
	else if ( gt < 0 || gt >= GT_MAX_GAME_TYPE ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"Gametype (%i) is out of range, defaulting to FFA/Deathmatch\n\"", gt ) );
		gt = GT_FFA;
	}

	// logically invalid gametypes, or gametypes not fully implemented in MP
	if ( gt == GT_SINGLE_PLAYER ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"This gametype is not supported (%s).\n\"", arg2 ) );
		return qfalse;
	}
	...
	for ( i=0; i<level.arenas.num; i++ ) {
		...
		if ( G_DoesMapSupportGametype( map, level.gametype ) ) {
			...
			if ( strlen( buf ) + strlen( tmpMsg ) >= sizeof( buf ) ) {
				g_trap->SendServerCommand( ent-g_entities, va( "print \"%s\"", buf ) );
				buf[0] = '\0';
			}
...
qboolean G_VoteMap( gentity_t *ent, int numArgs, const char *arg1, const char *arg2 ) {
	...
	if ( strchr( arg2, '\\' ) ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"Can't have mapnames with a \\\n\"" );
		return qfalse;
	}

	Com_sprintf( bspName, sizeof(bspName), "maps/%s.bsp", arg2 );
	if ( g_trap->FS_Open( bspName, &fp, FS_READ ) <= 0 ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"Can't find map %s on server\n\"", bspName ) );
		if( fp != NULL_FILE )
			g_trap->FS_Close( fp );
		return qfalse;
	}
	...
	g_trap->Cvar_VariableStringBuffer( "nextmap", s, sizeof( s ) );
	if ( *s )
...
qboolean G_VoteNextmap( gentity_t *ent, int numArgs, const char *arg1, const char *arg2 ) {
	...
	g_trap->Cvar_VariableStringBuffer( "nextmap", s, sizeof( s ) );
	if ( !*s ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"nextmap not set.\n\"" );
		return qfalse;
	}
...
void Svcmd_ToggleAllowVote_f( void ) {
	if ( g_trap->Argc() == 1 ) {
		...
		for ( i = 0; i<validVoteStringsSize; i++ ) {
			if ( (g_allowVote.integer & (1 << i)) )	g_trap->Print( "%2d [X] %s\n", i, validVoteStrings[i].string );
		...
		g_trap->Cvar_Set( "g_allowVote", va( "%i", (1 << index) ^ (g_allowVote.integer & ((1 << validVoteStringsSize) - 1)) ) );
		g_trap->Cvar_Update( &g_allowVote );
...
void Cmd_CallVote_f( gentity_t *ent ) {
	...
	numArgs = g_trap->Argc();
	g_trap->Argv( 1, arg1, sizeof( arg1 ) );
	if ( numArgs > 1 )
		Q_strncpyz( arg2, ConcatArgs( 2 ), sizeof( arg2 ) );

	// filter ; \n \r
	if ( Q_strchrs( arg1, ";\r\n" ) || Q_strchrs( arg2, ";\r\n" ) ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
		return;
	}
	...
	if ( i == validVoteStringsSize ) {
		...
		int toggle = 0;
		g_trap->SendServerCommand( ent-g_entities, "print \"Invalid vote string.\n\"" );
		g_trap->SendServerCommand( ent-g_entities, "print \"Allowed vote strings are: \"" );
		for ( i=0; i<validVoteStringsSize; i++ ) {
			if ( !(g_allowVote.integer & (1<<i)) )
				continue;
...
validVote:
	...
	if ( !(vote->validGT & (1<<level.gametype)) ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"%s is not applicable in this gametype.\n\"", arg1 ) );
		return;
	}

	if ( numArgs < vote->numArgs+2 ) {
		g_trap->SendServerCommand( ent-g_entities, va( "print \"%s requires more arguments: %s\n\"", arg1, vote->shortHelp ) );
		return;
	}
	...
	if ( level.voteExecuteTime ) {
		...
		g_trap->SendConsoleCommand( EXEC_APPEND, va( "%s\n", level.voteString ) );
	}
	...
	g_trap->SetConfigstring( CS_VOTE_TIME,	va( "%i", level.voteTime ) );
	g_trap->SetConfigstring( CS_VOTE_STRING,	level.voteDisplayString );
...
qboolean G_TeamVoteLeader( gentity_t *ent, int cs_offset, team_t team, int numArgs, const char *arg1, const char *arg2 ) {
	...
	{
		g_trap->SendServerCommand( ent-g_entities, va( "print \"User %s is not on your team\n\"", arg2 ) );
		return qfalse;
	}
...
void Cmd_CallTeamVote_f( gentity_t *ent ) {
	...
	// filter ; \n \r
	if ( Q_strchrs( arg1, ";\r\n" ) || Q_strchrs( arg2, ";\r\n" ) ) {
		g_trap->SendServerCommand( ent-g_entities, "print \"Invalid team vote string.\n\"" );
		return;
	}
	...
	if ( !Q_stricmp( arg1, "leader" ) ) {
		...
		g_trap->SendServerCommand( ent-g_entities, "print \"Invalid team vote string.\n\"" );
		g_trap->SendServerCommand( ent-g_entities, va("print \"Allowed team vote strings are: ^%c%s %s\n\"", COLOR_GREEN, "leader", "<optional client name or number>" ));
		return;
	}
	...
	for ( i=0; i<level.maxclients; i++ ) {
		...
		if ( level.clients[i].sess.sessionTeam == team )
			g_trap->SendServerCommand( i, va("print \"%s^7 called a team vote (%s)\n\"", ent->client->pers.netname, level.teamVoteStringClean[cs_offset] ) );
	}
	...
	g_trap->SetConfigstring( CS_TEAMVOTE_TIME + cs_offset, va("%i", level.teamVoteTime[cs_offset] ) );
	g_trap->SetConfigstring( CS_TEAMVOTE_STRING + cs_offset, level.teamVoteDisplayString[cs_offset] );
	g_trap->SetConfigstring( CS_TEAMVOTE_YES + cs_offset, va("%i", level.teamVoteYes[cs_offset] ) );
	g_trap->SetConfigstring( CS_TEAMVOTE_NO + cs_offset, va("%i", level.teamVoteNo[cs_offset] ) );
}
...
void Cmd_SetViewpos_f( gentity_t *ent ) {
	...
	if ( g_trap->Argc() != 5 ) {
		g_trap->SendServerCommand( ent-g_entities, va("print \"usage: setviewpos x y z yaw\n\""));
		return;
	}

	VectorClear( angles );
	for ( i = 0 ; i < 3 ; i++ ) {
		g_trap->Argv( i + 1, buffer, sizeof( buffer ) );
		origin[i] = atof( buffer );
	}

	g_trap->Argv( 4, buffer, sizeof( buffer ) );
...
int G_ItemUsable(playerState_t *ps, int forcedUse)
{
	...
	case HI_SENTRY_GUN:
		...
		g_trap->Trace(&tr, ps->origin, mins, maxs, trtest, ps->clientNum, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	case HI_SHIELD:
		...
		g_trap->Trace(&tr, ps->origin, mins, maxs, dest, ps->clientNum, MASK_SHOT, qfalse, 0, 0 );
		if (tr.fraction > 0.9 && !tr.startsolid && !tr.allsolid)
		{
			VectorCopy(tr.endpos, pos);
			VectorSet( dest, pos[0], pos[1], pos[2] - 4096 );
			g_trap->Trace( &tr, pos, mins, maxs, dest, ps->clientNum, MASK_SOLID, qfalse, 0, 0 );
...
void Cmd_SaberAttackCycle_f(gentity_t *ent)
{
	...
	{
		g_trap->SendServerCommand( ent-g_entities, va( "print \"%s (saberAttackCycle)\n\"", G_GetStringEdString( "MP_SVGAME", "CANNOT_TASK_INTERMISSION" ) ) );
		return;
	}
	...
	*/
	...
	{ //no cycling for akimbo
		...
		{//can turn second saber off
			...
			{
				g_trap->SendServerCommand( ent-g_entities, va("print \"SABERSTANCEDEBUG: Attempted to toggle dual saber blade.\n\"") );
			}
			return;
		}
	}
	...
	{ //use staff stance then.
		...
		{//second blade off
			...
			{//can't turn second blade back on if it's in the air, you naughty boy!
				if (d_saberStanceDebug.integer)
				{
					g_trap->SendServerCommand( ent-g_entities, va("print \"SABERSTANCEDEBUG: Attempted to toggle staff blade in air.\n\"") );
				}
				return;
			}
			//turn it on
		...
		if (d_saberStanceDebug.integer)
		{
			g_trap->SendServerCommand( ent-g_entities, va("print \"SABERSTANCEDEBUG: Attempted to toggle staff blade.\n\"") );
		}
		return;
	}
	...
	{ //we have a flag of useable stances so cycle through it instead
		...
		if (d_saberStanceDebug.integer)
		{
			g_trap->SendServerCommand( ent-g_entities, va("print \"SABERSTANCEDEBUG: Attempted to cycle given class stance.\n\"") );
		}
	}
	else
	{
		...
		if (d_saberStanceDebug.integer)
		{
			g_trap->SendServerCommand( ent-g_entities, va("print \"SABERSTANCEDEBUG: Attempted to cycle stance normally.\n\"") );
		}
	}
/*
#ifndef FINAL_BUILD
	...
	{
	case FORCE_LEVEL_1:
		g_trap->SendServerCommand( ent-g_entities, va("print \"Lightsaber Combat Style: %sfast\n\"", S_COLOR_BLUE) );
		break;
	case FORCE_LEVEL_2:
		g_trap->SendServerCommand( ent-g_entities, va("print \"Lightsaber Combat Style: %smedium\n\"", S_COLOR_YELLOW) );
		break;
	case FORCE_LEVEL_3:
		g_trap->SendServerCommand( ent-g_entities, va("print \"Lightsaber Combat Style: %sstrong\n\"", S_COLOR_RED) );
		break;
	}
#endif
*/
...
void Cmd_EngageDuel_f(gentity_t *ent)
{
	...
	*/
	...
	g_trap->Trace(&tr, ent->client->ps.origin, NULL, NULL, fwdOrg, ent->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
void Cmd_TargetUse_f( gentity_t *ent )
{
	if ( g_trap->Argc() > 1 )
	{
		...
		g_trap->Argv( 1, sArg, sizeof( sArg ) );
...
void ClientCommand( int clientNum ) {
	...
	if ( !command )
	{
		g_trap->SendServerCommand( clientNum, va( "print \"Unknown command %s\n\"", cmd ) );
		return;
	}
	...
	{
		g_trap->SendServerCommand( clientNum, va( "print \"%s (%s)\n\"", G_GetStringEdString( "MP_SVGAME", "CANNOT_TASK_INTERMISSION" ), cmd ) );
		return;
	}
