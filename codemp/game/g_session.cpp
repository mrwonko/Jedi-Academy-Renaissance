...
void G_ReadSessionData( gclient_t *client )
{
	...
	var = va( "session%i", client - level.clients );
	g_trap->Cvar_VariableStringBuffer( var, s, sizeof(s) );
...
void G_InitWorldSession( void ) {
	...
	g_trap->Cvar_VariableStringBuffer( "session", s, sizeof(s) );
	gt = atoi( s );
	...
	if ( level.gametype != gt ) {
		...
		g_trap->Print( "Gametype changed, clearing session data.\n" );
	}
}
...
void G_WriteSessionData( void ) {
	...
	g_trap->Cvar_Set( "session", va("%i", level.gametype) );
