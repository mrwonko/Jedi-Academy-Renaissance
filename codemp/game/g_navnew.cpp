...
int NAVNEW_ClearPathBetweenPoints(vec3_t start, vec3_t end, vec3_t mins, vec3_t maxs, int ignore, int clipmask)
{
	...
	if ( !g_trap->InPVS( start, end ) )
	{
		return ENTITYNUM_WORLD;
	}

	g_trap->Trace( &trace, start, mins, maxs, end, ignore, clipmask, qfalse, 0, 0 );
...
{//try pushing blocker to one side
	...
	g_trap->Trace( &tr, blocker->r.currentOrigin, mins, blocker->r.maxs, end, blocker->s.number, blocker->clipmask|CONTENTS_BOTCLIP, qfalse, 0, 0);
...
{//trace to sides of blocker and see if either is clear
	...
	*/
	...
	if ( self->NPC->sideStepHoldTime > level.time )
	{
		...
		g_trap->Trace( &tr, self->r.currentOrigin, mins, self->r.maxs, block_pos, self->s.number, self->clipmask|CONTENTS_BOTCLIP, qfalse, 0, 0 );
...
{//see if the direct path between 2 nodes is blocked by architecture or an ent
	...
	g_trap->Nav_GetNodePosition( wp1, pos1 );
	g_trap->Nav_GetNodePosition( wp2, pos2 );
	...
	g_trap->Trace( &trace, pos1, mins, maxs, pos2, ignoreEntNum, clipmask, qfalse, 0, 0 );
...
int	NAVNEW_MoveToGoal( gentity_t *self, navInfo_t *info )
{
	...
	{//just use current waypoints
		bestNode = g_trap->Nav_GetBestNodeAltRoute2( self->waypoint, self->NPC->goalEntity->waypoint, bestNode );
	}
	//FIXME!!!!: this is making them wiggle back and forth between waypoints
	else if ( (bestNode = g_trap->Nav_GetBestPathBetweenEnts( (sharedEntity_t *)self, (sharedEntity_t *)self->NPC->goalEntity, NF_CLEAR_PATH )) == NODE_NONE )//!NAVNEW_GetWaypoints( self, qtrue ) )
	...
	while( !foundClearPath )
	{
		...
		/*
		bestNode = g_trap->Nav_GetBestNodeAltRoute( self->waypoint, self->NPC->goalEntity->waypoint, bestNode );
		*/
		...
		{//we were blocked somehow
			...
			{
				self->NPC->aiFlags |= NPCAI_BLOCKED;
				g_trap->Nav_GetNodePosition( oldBestNode, NPCInfo->blockedDest );
			}
		}
		*/
		...
		{//we haven't already tried to go straight to goal or goal's wp
			...
			{//our bestNode is the goal's wp
				if ( NAV_HitNavGoal( self->r.currentOrigin, self->r.mins, self->r.maxs, origin, g_trap->Nav_GetNodeRadius( bestNode ), FlyingCreature( self ) ) )
		...
		*/
		...
		{//not heading straight for goal
			...
			{//we know it's clear or architecture
				//g_trap->Nav_GetNodePosition( self->waypoint, origin );
				/*
				if ( NAV_HitNavGoal( self->r.currentOrigin, self->r.mins, self->r.maxs, origin, g_trap->Nav_GetNodeRadius( bestNode ), FlyingCreature( self ) ) )
			...
			{//heading to an edge off our confirmed clear waypoint... make sure it's clear
				...
				{//we fell back to our waypoint, reset the origin
					self->NPC->aiFlags |= NPCAI_BLOCKED;
					g_trap->Nav_GetNodePosition( oldBestNode, NPCS.NPCInfo->blockedDest );
					g_trap->Nav_GetNodePosition( bestNode, origin );
				}
			}
		}
		...
		{//blocked by ent!
			...
			{
				self->NPC->aiFlags |= NPCAI_BLOCKED;
				g_trap->Nav_GetNodePosition( bestNode, NPCS.NPCInfo->blockedDest );
			}
			...
			{//we headed for our goal and failed and our goal's WP and failed
				...
				{//our waypoint is our goal's waypoint, nothing we can do
					...
					g_trap->Nav_AddFailedNode( (sharedEntity_t *)self, self->waypoint );
					goto failed;
				}
				else
			...
			{//we headed toward our next waypoint (instead of our waypoint) and failed
				...
				{//mark this edge failed and try our waypoint
					...
					if ( d_patched.integer &&//use patch-style navigation
 						( !g_trap->Nav_NodesAreNeighbors( self->waypoint, bestNode )
					...
					{//the direct path between these 2 nodes is blocked by an ent
						g_trap->Nav_AddFailedEdge( self->s.number, self->waypoint, bestNode );
					}
			...
			{//we headed for *our* waypoint and couldn't get to it
				if ( d_altRoutes.integer )
				{
					...
					//if ( (bestNode = g_trap->Nav_GetBestPathBetweenEnts( self, self->NPC->goalEntity, NF_CLEAR_PATH )) == NODE_NONE )//!NAVNEW_GetWaypoints( self, qfalse ) )
...
//finish:
	...
	if ( NAVDEBUG_showEnemyPath )
	{
		...
		{
			vec3_t	wpPos;
			g_trap->Nav_GetNodePosition( self->waypoint, wpPos );
		...
		g_trap->Nav_ShowPath( bestNode, self->NPC->goalEntity->waypoint );
	}
