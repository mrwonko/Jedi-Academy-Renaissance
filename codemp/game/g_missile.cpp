	...
	*/
	...
	if (other->takedamage && !isKnockedSaber) {
		...
		if ( ent->damage ) {
			...
			BG_EvaluateTrajectoryDelta< moduleType::game >( &ent->s.pos, level.time, velocity );
...
void G_RunMissile( gentity_t *ent ) {
	...
	{
		g_trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
	...
	{
		g_trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, origin, passent, ent->clipmask, qfalse, 0, 0 );
	}

	if ( tr.startsolid || tr.allsolid ) {
		// make sure the tr.entityNum is set to the entity we're stuck in
		g_trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, passent, ent->clipmask, qfalse, 0, 0 );
		tr.fraction = 0;
	}
	else {
	...
	if (ent->s.weapon == G2_MODEL_PART && !ent->bounceCount)
	{
		...
		lowerOrg[2] -= 1;
		g_trap->Trace( &trG, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, lowerOrg, passent, ent->clipmask, qfalse, 0, 0 );
	...
	if ( tr.fraction != 1) {
		...
		{ //player or NPC, try making a mark on him
			//copy current pos to s.origin, and current projected traj to origin2
			VectorCopy(ent->r.currentOrigin, ent->s.origin);
			BG_EvaluateTrajectory< moduleType::game >( &ent->s.pos, level.time, ent->s.origin2 );
