...
//				G_PlayEffect( "small_chunks", self->playerModel, self->genericBolt1, self->s.number);
				ATST_PlayEffect( self, g_trap->G2API_AddBolt(self->ghoul2, 0, "*flash3"), "env/med_explode2" );
...
//				G_PlayEffect( "small_chunks", self->playerModel, self->genericBolt2, self->s.number);
				ATST_PlayEffect( self, g_trap->G2API_AddBolt(self->ghoul2, 0, "*flash4"), "env/med_explode2" );
...
//		NPC_ChangeWeapon( WP_ATST_SIDE );
		...
		blasterTest = g_trap->G2API_GetSurfaceRenderStatus( NPCS.NPC->ghoul2, 0, "head_light_blaster_cann" );
		chargerTest = g_trap->G2API_GetSurfaceRenderStatus( NPCS.NPC->ghoul2, 0, "head_concussion_charger" );
