...
void turretG2_set_models( gentity_t *self, qboolean dying )
{
	if ( dying )
	{
		...
		g_trap->G2API_RemoveGhoul2Model( &self->ghoul2, 0 );
		G_KillG2Queue( self->s.number );
		self->s.modelGhoul2 = 0;
		/*
		g_trap->G2API_InitGhoul2Model( &self->ghoul2,
									name2,
									0, //base->s.modelindex,
	...
	else
	{
		if ( !(self->spawnflags&SPF_TURRETG2_TURBO) )
		{
			...
			//set the new onw
			re->G2API_InitGhoul2Model( &self->ghoul2,
										name,
										0, //base->s.modelindex,
										//note, this is not the same kind of index - this one's referring to the actual
										//index of the model in the g2 instance, whereas modelindex is the index of a
		...
		{//different pitch bone and muzzle flash points
			...
			self->genericValue11 = g_trap->G2API_AddBolt( self->ghoul2, 0, "*muzzle1" );
			self->genericValue12 = g_trap->G2API_AddBolt( self->ghoul2, 0, "*muzzle2" );
		}
		else
		{
			G2Tur_SetBoneAngles(self, "Bone_body", vec3_origin);
			self->genericValue11 = g_trap->G2API_AddBolt( self->ghoul2, 0, "*flash03" );
		}
	}
}
...
//-----------------------------------------------------
{
		...
		&& self->attackDebounceTime < level.time )
	{
		...
		g_trap->G2API_GetBoltMatrix( self->ghoul2,
					0,
...
//-----------------------------------------------------
{
	...
	BG_EvaluateTrajectory< moduleType::game >( &self->s.apos, level.time, self->r.currentAngles );
...
//		diffYaw = sin( level.time * 0.0001f + self->count ) * 5.0f;	// don't do this for now since it can make it go into walls.
	...
	if ( diffPitch )
	{
		...
		/*
		g_trap->G2API_SetBoneAngles( self->ghoul2,
						0,
						"Bone_body",
						desiredAngles,
