...
//	g_trap->Print ("%i teams with %i entities\n", c, c2);
}
...
void G_CacheGametype( void )
{
	...
	if ( g_gametype.string[0] && isalpha( g_gametype.string[0] ) )
	{
		...
		if ( gt == -1 )
		{
			g_trap->Print( "Gametype '%s' unrecognised, defaulting to FFA/Deathmatch\n", g_gametype.string );
			level.gametype = GT_FFA;
		}
		else
			level.gametype = static_cast< gametype_t >( gt );
	}
	else if ( g_gametype.integer < 0 || g_gametype.integer >= GT_MAX_GAME_TYPE )
	{
		g_trap->Print( "g_gametype %i is out of range, defaulting to 0 (FFA/Deathmatch)\n", g_gametype.integer );
		level.gametype = GT_FFA;
	}
	else
		level.gametype = static_cast< gametype_t >( atoi( g_gametype.string ) );

	g_trap->Cvar_Set( "g_gametype", va( "%i", level.gametype ) );
	g_trap->Cvar_Update( &g_gametype );
}
...
void G_InitGame( int levelTime, int randomSeed, int restart ) {
	...
	g_trap->Cvar_Set("RMG", "0");
	RMG.integer = 0;

	//Clean up any client-server ghoul2 instance attachments that may still exist exe-side
	g_trap->G2API_CleanEntAttachments();

	BG_InitAnimsets(); //clear it out

	B_InitAlloc(); //make sure everything is clean

	g_trap->SV_RegisterSharedMemory( gSharedBuffer.raw );

	//Load external vehicle data
	BG_VehicleLoadParms< moduleType::game >();

	g_trap->Print ("------- Game Initialization -------\n");
	g_trap->Print ("gamename: %s\n", GAMEVERSION);
	g_trap->Print ("gamedate: %s\n", SOURCE_DATE);
	...
	//g_trap->SP_RegisterServer("mp_svgame");

	if ( g_log.string[0] )
	{
		g_trap->FS_Open( g_log.string, &level.logFile, g_logSync.integer ? FS_APPEND_SYNC : FS_APPEND );
		if ( level.logFile )
			g_trap->Print( "Logging to %s\n", g_log.string );
		else
			g_trap->Print( "WARNING: Couldn't open logfile: %s\n", g_log.string );
	}
	else
		g_trap->Print( "Not logging game events to disk.\n" );

	g_trap->GetServerinfo( serverinfo, sizeof( serverinfo ) );
	G_LogPrintf( "------------------------------------------------------------\n" );
	...
	if ( g_securityLog.integer )
	{
		if ( g_securityLog.integer == 1 )
			g_trap->FS_Open( SECURITY_LOG, &level.security.log, FS_APPEND );
		else if ( g_securityLog.integer == 2 )
			g_trap->FS_Open( SECURITY_LOG, &level.security.log, FS_APPEND_SYNC );

		if ( level.security.log )
			g_trap->Print( "Logging to " SECURITY_LOG "\n" );
		else
			g_trap->Print( "WARNING: Couldn't open logfile: " SECURITY_LOG "\n" );
	}
	else
		g_trap->Print( "Not logging security events to disk.\n" );
	...
	g_trap->LocateGameData( (sharedEntity_t *)level.gentities, level.num_entities, sizeof( gentity_t ),
		&level.clients[0].ps, sizeof( level.clients[0] ) );

	//Load sabers.cfg data
	WP_SaberLoadParms< moduleType::game >();
...
//	Com_Printf ("-----------------------------------\n");
	...
	g_trap->Cvar_Register( &mapname, "mapname", "", CVAR_SERVERINFO | CVAR_ROM );
	G_CacheMapname( &mapname );
	g_trap->Cvar_Register( &ckSum, "sv_mapChecksum", "", CVAR_ROM );

	navCalculatePaths	= ( g_trap->Nav_Load( mapname.string, ckSum.integer ) == qfalse );
	...
	{
		g_trap->SetConfigstring ( CS_CLIENT_DUELISTS, va("-1|-1|-1") );
	}
	else
	{
		g_trap->SetConfigstring ( CS_CLIENT_DUELISTS, va("-1|-1") );
	}
// nmckenzie: DUEL_HEALTH: Default.
	g_trap->SetConfigstring ( CS_CLIENT_DUELHEALTHS, va("-1|-1|!") );
	g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, va("-1") );

	SaveRegisteredItems();

	//g_trap->Print ("-----------------------------------\n");

	if( level.gametype == GT_SINGLE_PLAYER || g_trap->Cvar_VariableIntegerValue( "com_buildScript" ) ) {
		G_ModelIndex( SP_PODIUM_MODEL );
		G_SoundIndex( "sound/player/gurp1.wav" );
		G_SoundIndex( "sound/player/gurp2.wav" );
	}

	if ( g_trap->Cvar_VariableIntegerValue( "bot_enable" ) ) {
		BotAISetup( restart );
	...
	{//loaded
		...
		g_trap->Nav_SetPathsCalculated(qtrue);
		...
		{//clear all the failed edges unless we just loaded the game (which would include failed edges)
			g_trap->Nav_ClearAllFailedEdges();
		}
		*/
	...
	if ( level.gametype == GT_JEDIMASTER ) {
		...
		if ( i == level.num_entities ) {
			...
			if( !spawnpoint ) {
				g_trap->Error( ERR_DROP, "Couldn't find an FFA spawnpoint to drop the jedimaster saber at!\n" );
				return;
			}
...
//	Com_Printf("... Gameside GHOUL2 Cleanup\n");
	...
	{ //clean up all the ghoul2 instances
		...
		if (ent->client)
		{
			...
			while (j < MAX_SABERS)
			{
				if (ent->client->weaponGhoul2[j] && g_trap->G2API_HaveWeGhoul2Models(ent->client->weaponGhoul2[j]))
				{
					g_trap->G2API_CleanGhoul2Models(&ent->client->weaponGhoul2[j]);
				}
				j++;
			}
		}
	...
	if (g2SaberInstance && g_trap->G2API_HaveWeGhoul2Models(g2SaberInstance))
	{
		g_trap->G2API_CleanGhoul2Models(&g2SaberInstance);
		g2SaberInstance = NULL;
	}
	if (precachedKyle && g_trap->G2API_HaveWeGhoul2Models(precachedKyle))
	{
		re->G2API_CleanGhoul2Models(&precachedKyle);
		precachedKyle = NULL;
	}

//	Com_Printf ("... ICARUS_Shutdown\n");
	g_trap->ICARUS_Shutdown ();	//Shut ICARUS down
...
//	Com_Printf ("... Reference Tags Cleared\n");
	...
	if ( level.security.log )
	{
		...
		g_trap->FS_Close( level.security.log );
...
void AdjustTournamentScores( void ) {
		...
		level.clients[level.sortedClients[1]].pers.connected == CON_CONNECTED)
	{
		...
		if (clFailure != 2)
		{
			...
			g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, va("%i", clientNum ) );
...
//	preNumSpec = level.numNonSpectatorClients;
	...
	if ( !game::g_warmup.integer || level.gametype == GT_SIEGE )
		level.warmupTime = 0;
	...
	if ( level.gametype >= GT_TEAM ) {
		g_trap->SetConfigstring( CS_SCORES1, va("%i", level.teamScores[TEAM_RED] ) );
		g_trap->SetConfigstring( CS_SCORES2, va("%i", level.teamScores[TEAM_BLUE] ) );
	} else {
		if ( level.numConnectedClients == 0 ) {
			g_trap->SetConfigstring( CS_SCORES1, va("%i", SCORE_NOT_PRESENT) );
			g_trap->SetConfigstring( CS_SCORES2, va("%i", SCORE_NOT_PRESENT) );
		} else if ( level.numConnectedClients == 1 ) {
			g_trap->SetConfigstring( CS_SCORES1, va("%i", level.clients[ level.sortedClients[0] ].ps.persistant[PERS_SCORE] ) );
			g_trap->SetConfigstring( CS_SCORES2, va("%i", SCORE_NOT_PRESENT) );
		} else {
			g_trap->SetConfigstring( CS_SCORES1, va("%i", level.clients[ level.sortedClients[0] ].ps.persistant[PERS_SCORE] ) );
			g_trap->SetConfigstring( CS_SCORES2, va("%i", level.clients[ level.sortedClients[1] ].ps.persistant[PERS_SCORE] ) );
		}
		...
		{ //when not in duel, use this configstring to pass the index of the player currently in first place
			...
			{
				g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, va("%i", level.sortedClients[0] ) );
			}
			else
			{
				g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, "-1" );
			}
		}
	}
...
void ExitLevel (void) {
	...
	{
		g_trap->SendConsoleCommand( EXEC_APPEND, "vstr nextmap\n" );
	}
	level.changemap = NULL;
	level.intermissiontime = 0;
...
//	msec %= 1000;
	...
	if ( dedicated.integer )
		g_trap->Print( "%s", string + l );

	if ( !level.logFile )
		return;

	g_trap->FS_Write( string, strlen( string ), level.logFile );
}
/*
=================
G_SecurityLogPrintf
...
void QDECL G_SecurityLogPrintf( const char *fmt, ... ) {
	...
	if ( dedicated.integer )
		g_trap->Print( "%s", string + timeLen );

	if ( !level.security.log )
		return;

	g_trap->FS_Write( string, strlen( string ), level.security.log );
}
...
//	qboolean		won = qtrue;
	...
	g_trap->SetConfigstring( CS_INTERMISSION, "1" );
...
//		}
	...
	if (g_singlePlayer.integer) {
		...
		g_trap->SendConsoleCommand( EXEC_APPEND, (won) ? "spWin\n" : "spLose\n" );
	}
	*/
}
		...
		(level.time > level.intermissiontime + 2000) )
	{
		...
		if (!DuelLimitHit())
		{
			...
			if (level.gametype == GT_POWERDUEL)
			{
				...
				{
					g_trap->SetConfigstring ( CS_CLIENT_DUELISTS, va("%i|%i|%i", level.sortedClients[0], level.sortedClients[1], level.sortedClients[2] ) );
					g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, "-1" );
				}
			}
			else
			{
				if (level.numPlayingClients >= 2)
				{
					g_trap->SetConfigstring ( CS_CLIENT_DUELISTS, va("%i|%i", level.sortedClients[0], level.sortedClients[1] ) );
					g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, "-1" );
				}
			}
...
//				g_trap->SendServerCommand( -1, "print \"Timelimit hit.\n\"");
				g_trap->SendServerCommand( -1, va("print \"%s.\n\"",G_GetStringEdString("MP_SVGAME", "TIMELIMIT_HIT")));
				if (d_powerDuelPrint.integer)
				{
	...
	if (level.gametype == GT_POWERDUEL && level.numPlayingClients >= 3)
	{
			...
			duelists[2] != -1)
		{
			...
			{ //The lone duelist lost, give the other two wins (if applicable) and him a loss
				...
				g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, va("%i", duelists[1] ) );
			...
			{ //the coupled duelists lost, give the lone duelist a win (if applicable) and the couple both losses
				...
				g_trap->SetConfigstring ( CS_CLIENT_DUELWINNER, va("%i", duelists[0] ) );
	...
	if ( level.gametype < GT_SIEGE && fraglimit.integer ) {
		...
		if ( level.teamScores[TEAM_BLUE] >= fraglimit.integer ) {
			g_trap->SendServerCommand( -1, va("print \"Blue %s\n\"", G_GetStringEdString("MP_SVGAME", "HIT_THE_KILL_LIMIT")) );
			if (d_powerDuelPrint.integer)
			{
		...
		for ( i=0 ; i< sv_maxclients->integer ; i++ ) {
			...
			if ( (level.gametype == GT_DUEL || level.gametype == GT_POWERDUEL) && duel_fraglimit.integer && cl->sess.wins >= duel_fraglimit.integer )
			{
				...
				gDuelExit = qtrue;
				g_trap->SendServerCommand( -1, va("print \"%s" S_COLOR_WHITE " hit the win limit.\n\"",
					cl->pers.netname ) );
				return;
			}
	...
	if ( level.gametype >= GT_CTF && capturelimit.integer ) {
		...
		{
			g_trap->SendServerCommand( -1,  va("print \"%s \"", G_GetStringEdString("MP_SVGAME", "PRINTREDTEAM")));
		...
		if ( level.teamScores[TEAM_BLUE] >= capturelimit.integer ) {
			g_trap->SendServerCommand( -1,  va("print \"%s \"", G_GetStringEdString("MP_SVGAME", "PRINTBLUETEAM")));
			g_trap->SendServerCommand( -1,  va("print \"%s.\n\"", G_GetStringEdString("MP_SVGAME", "HIT_CAPTURE_LIMIT")));
			LogExit( "Capturelimit hit." );
			return;
		}
...
// nmckenzie: DUEL_HEALTH
		...
		if ( level.numPlayingClients != 2 ) {
			if ( level.warmupTime != -1 ) {
				level.warmupTime = -1;
				g_trap->SetConfigstring( CS_WARMUP, va("%i", level.warmupTime) );
				G_LogPrintf( "Warmup:\n" );
			}
			return;
		}
		...
		if ( level.warmupTime < 0 ) {
			if ( level.numPlayingClients == 2 ) {
				...
				level.warmupTime = level.time + ( game::g_warmup.integer - 1 ) * 1000;
	...
	else if (level.gametype == GT_POWERDUEL)
	{
		...
		*/
		...
		if ( level.time > level.warmupTime ) {
			level.warmupTime += 10000;
			g_trap->Cvar_Set( "g_restarted", "1" );
			g_trap->Cvar_Update( &g_restarted );
			g_trap->SendConsoleCommand( EXEC_APPEND, "map_restart 0\n" );
			level.restarted = qtrue;
			return;
		}
...
void G_KickAllBots(void)
{
	...
	for ( i=0 ; i< sv_maxclients->integer ; i++ )
	{
		...
		g_trap->SendConsoleCommand( EXEC_INSERT, va("clientkick %d\n", i) );
	}
}
...
void CheckVote( void ) {
		...
		if (level.votingGametype)
		{
			...
			{ //If we're voting to a different game type, be sure to refresh all the map stuff
				...
				{ //ok, kick all the bots, cause the aren't supported!
					...
					//g_trap->Cvar_Set("bot_minplayers", "0");
				}

				if (nextMap && nextMap[0])
				{
					g_trap->SendConsoleCommand( EXEC_APPEND, va("map %s\n", nextMap ) );
				}
			}
			else
			...
			{ //This means to auto-correct fraglimit when voting to and from duel.
				...
				if ((level.votingGametypeTo == GT_DUEL || level.votingGametypeTo == GT_POWERDUEL) && currentGT != GT_DUEL && currentGT != GT_POWERDUEL)
				{
					...
					{ //if voting to duel, and fraglimit is more than 3 (or unlimited), then set it down to 3
						g_trap->SendConsoleCommand(EXEC_APPEND, "fraglimit 3\n");
					}
					if (currentTL)
					{ //if voting to duel, and timelimit is set, make it unlimited
						g_trap->SendConsoleCommand(EXEC_APPEND, "timelimit 0\n");
					}
				}
					...
					(currentGT == GT_DUEL || currentGT == GT_POWERDUEL))
				{
					...
					{ //if voting from duel, an fraglimit is less than 20, then set it up to 20
						g_trap->SendConsoleCommand(EXEC_APPEND, "fraglimit 20\n");
					}
				}
			}
	...
	level.voteTime = 0;
	g_trap->SetConfigstring( CS_VOTE_TIME, "" );
}
...
void PrintTeam(int team, char *message) {
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) {
		if (level.clients[i].sess.sessionTeam != team)
			continue;
		g_trap->SendServerCommand( i, message );
	}
}
...
void CheckTeamVote( int team ) {
	...
	g_trap->SetConfigstring( CS_TEAMVOTE_TIME + cs_offset, "" );
}
...
void CheckCvars( void ) {
	...
	if ( g_password.modificationCount != lastMod ) {
		...
		g_trap->Cvar_Set("g_password", password );

		if ( *g_password.string && Q_stricmp( g_password.string, "none" ) ) {
			g_trap->Cvar_Set( "g_needpass", "1" );
		} else {
			g_trap->Cvar_Set( "g_needpass", "0" );
		}
	}
}
...
void G_RunThink (gentity_t *ent) {
	...
	ent->nextthink = 0;
	if (!ent->think) {
		//g_trap->Error( ERR_DROP, "NULL ent->think");
		goto runicarus;
	}
	ent->think (ent);
...
runicarus:
	if ( ent->inuse )
	{
		...
		g_trap->ICARUS_MaintainTaskManager(ent->s.number);
		RestoreNPCGlobals();
	}
}
...
void NAV_CheckCalcPaths( void )
{
	...
	{//first time we've ever loaded this map...
		...
		g_trap->Nav_CalculatePaths(qfalse);
...
#endif
		if ( g_trap->Nav_Save( mapname.string, ckSum.integer ) == qfalse )
		{
...
void G_RunFrame( int levelTime ) {
	...
	if (gDoSlowMoDuel)
	{
		if (level.restarted)
		{
			...
			g_trap->Cvar_VariableStringBuffer("timescale", buf, sizeof(buf));

			tFVal = atof(buf);

			g_trap->Cvar_Set("timescale", "1");
			if (tFVal == 1.0f)
			{
		...
		else
		{
			...
			if (timeDif < 150)
			{
				g_trap->Cvar_Set("timescale", "0.1f");
			}
			else if (timeDif < 1150)
			{
				...
				g_trap->Cvar_Set("timescale", va("%f", useDif));
			}
			else
			{
				char buf[128];
	...
	if (g_allowNPC.integer)
	{
		...
		{
			g_trap->Nav_CheckAllFailedEdges();
		}
		g_trap->Nav_ClearCheckedNodes();
		...
		for ( i = 0; i < level.num_entities ; i++)
		{
			...
			if ( d_altRoutes.integer )
			{
				g_trap->Nav_CheckFailedNodes( (sharedEntity_t *)ent );
			}
		}
...
#ifdef _G_FRAME_PERFANAL
	g_trap->PrecisionTimer_Start(&timer_ItemRun);
#endif
	//
	...
	for (i=0 ; i<level.num_entities ; i++, ent++) {
		...
		if ( ent->s.eType == ET_MOVER && level.gametype == GT_SIEGE && level.intermissiontime)
		{
			if ( !Q_stricmp("func_door", ent->classname) && ent->moverState != MOVER_POS1 )
			{
				...
				{
					g_trap->AdjustAreaPortalState( (sharedEntity_t *)ent, qfalse );
				}
...
#ifdef _G_FRAME_PERFANAL
	iTimer_ItemRun = g_trap->PrecisionTimer_End(timer_ItemRun);
#endif

	SiegeCheckTimers();

#ifdef _G_FRAME_PERFANAL
	g_trap->PrecisionTimer_Start(&timer_ROFF);
#endif
	g_trap->ROFF_UpdateEntities();
#ifdef _G_FRAME_PERFANAL
	iTimer_ROFF = g_trap->PrecisionTimer_End(timer_ROFF);
#endif



#ifdef _G_FRAME_PERFANAL
	g_trap->PrecisionTimer_Start(&timer_ClientEndframe);
#endif
...
#ifdef _G_FRAME_PERFANAL
	iTimer_ClientEndframe = g_trap->PrecisionTimer_End(timer_ClientEndframe);
#endif



#ifdef _G_FRAME_PERFANAL
	g_trap->PrecisionTimer_Start(&timer_GameChecks);
#endif
...
#ifdef _G_FRAME_PERFANAL
	iTimer_GameChecks = g_trap->PrecisionTimer_End(timer_GameChecks);
#endif



#ifdef _G_FRAME_PERFANAL
	g_trap->PrecisionTimer_Start(&timer_Queues);
#endif
...
#ifdef _G_FRAME_PERFANAL
	iTimer_Queues = g_trap->PrecisionTimer_End(timer_Queues);
#endif
...
const char *G_GetStringEdString(char *refSection, char *refName)
{
	...
	g_trap->SP_GetStringTextString(va("%s_%s", refSection, refName), text, sizeof(text));
	return text;
	*/
...
gameImport_t *g_trap = NULL;

gameExport_t* GetGameAPI( int apiVersion, gameImport_t *import )
{
	...
	if ( apiVersion != GAME_API_VERSION ) {
		g_trap->Print( "Mismatched GAME_API_VERSION: expected %i, got %i\n", GAME_API_VERSION, apiVersion );
		return NULL;
	}
