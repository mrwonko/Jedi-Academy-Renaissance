...
struct gentity_or_bgentity {
	using type = bgEntity_t;
};

template<>
struct gentity_or_bgentity< moduleType::game > {
	using type = gentity_t;
};
...
void PM_VehicleImpact(bgEntity_t *pEnt, trace_t *trace)
{
	...
	gentity_or_bgentity< mod >::type *hitEnt = nullptr;
	bool landing = false;
	if constexpr( mod == moduleType::game ) {
		hitEnt = trace != NULL ? &g_entities[ trace->entityNum ] : NULL;
		...
		{//spiralling to our deaths, explode on any solid impact
			if( hitEnt->s.NPC_class == CLASS_VEHICLE ) {//hit another vehicle, explode!
		...
		if( trace->entityNum < ENTITYNUM_WORLD
			...
			&& Q_stricmp( "func_rotating", hitEnt->classname ) == 0 ) {//hit a func_rotating that is supposed to destroy anything it touches!
	...
	{//we're landing, we're cool
		...
		*/
		...
		if( mod == moduleType::game ) {
			if( hitEnt && ( hitEnt->s.eType == ET_PLAYER || hitEnt->s.eType == ET_NPC ) && pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER ) { //always smack players
			} else {
				return;
			}
		} else {
		...
		(magnitude >= 100||forceSurfDestruction) )
	{
		...
		{//a bit of a hack, may conflict with getting shot, but...
			...
			if ( trace && !pSelfVeh->m_iRemovedSurfaces && !forceSurfDestruction )
			{
				...
				vec3_t	bounceDir;
				if constexpr( mod == moduleType::cgame ) {
				...
				else if( pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER ) {//check for impact with another fighter
					if constexpr( mod == moduleType::cgame ) {
						hitEnt = PM_BGEntForNum( trace->entityNum );
					}
				...
				{//bounce off impact surf and turn away
					...
					*/
					if constexpr( mod == moduleType::game ) { //server-side, turn the guy we hit away from us, too
							...
							&& !( hitEnt->spawnflags & 2 ) )//and not if suspended
						{
							...
							//now do pitch
							if( !bounceDir[ 2 ] ) {//shouldn't be any pitch
							} else {
								pitchTurnStrength = turnStrength*turnDelta[ PITCH ];
								if( pitchTurnStrength > MAX_IMPACT_TURN_ANGLE ) {
									pitchTurnStrength = MAX_IMPACT_TURN_ANGLE;
							...
							//now do yaw
							if( !bounceDir[ 0 ]
								&& !bounceDir[ 1 ] ) {//shouldn't be any yaw
							} else {
								yawTurnStrength = turnStrength*turnDelta[ YAW ];
								if( yawTurnStrength > MAX_IMPACT_TURN_ANGLE ) {
									yawTurnStrength = MAX_IMPACT_TURN_ANGLE;
			...
			if constexpr( mod == moduleType::game ) {
				...
				if( hitEnt && ( hitEnt->s.eType != ET_TERRAIN || !( hitEnt->spawnflags & 1 ) || pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER ) ) { //don't damage the vehicle from terrain that doesn't want to damage vehicles
					if( pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER ) { //increase the damage...
						float mult = ( pSelfVeh->m_vOrientation[ PITCH ] * 0.1f );
						if( mult < 1.0f ) {
							mult = 1.0f;
						}
						if( hitEnt->inuse && hitEnt->takedamage ) { //if the other guy takes damage, don't hurt us a lot for ramming him
				...
				if( hitEnt &&
					hitEnt->inuse &&
					hitEnt->takedamage ) { //damage this guy because we hit him
					float pmult = 1.0f;
					int finalD;
					gentity_t *attackEnt;
					...
					if( ( hitEnt->s.eType == ET_PLAYER && hitEnt->s.number < MAX_CLIENTS ) ||
						...
						if( pSelfVeh->m_pVehicleInfo->type == VH_FIGHTER ) { //player die good.. if me fighter
							pmult = 2000.0f;
						} else {
							pmult = 40.0f;
						}
						...
						if( hitEnt->client &&
							...
							G_CanBeEnemy( (gentity_t *)pEnt, hitEnt ) ) { //smash!
		...
		//it doesn't look bad though. could just use predicted events, but I'm too lazy.
				...
				if( !hitEnt || hitEnt->s.owner != pEnt->s.number ) { //don't hit your own missiles!
					AngleVectors( pSelfVeh->m_vOrientation, NULL, NULL, vehUp );
					pEnt->m_pVehicle->m_iHitDebounce = pm->cmd.serverTime + 200;
	...
	for ( bumpcount=0 ; bumpcount < numbumps ; bumpcount++ ) {
		...
		if (pm->ps->clientNum >= MAX_CLIENTS)
		{
			...
			{ //do vehicle impact stuff then
				PM_VehicleImpact< mod >(pEnt, &trace);
			}
		}
...
template qboolean	PM_SlideMove< moduleType::game >( qboolean gravity );
template qboolean	PM_SlideMove< moduleType::cgame >( qboolean gravity );
...
//	vec3_t		delta, delta2;
...
#endif
	{
		...
		if ( delta > 2 ) {
			if ( delta < 7 ) {
				PM_AddEvent< mod >( EV_STEP_4 );
			} else if ( delta < 11 ) {
				PM_AddEvent< mod >( EV_STEP_8 );
			} else if ( delta < 15 ) {
				PM_AddEvent< mod >( EV_STEP_12 );
			} else {
				PM_AddEvent< mod >( EV_STEP_16 );
			}
		}
		if ( pm->debugLevel ) {
...
template void PM_StepSlideMove< moduleType::game >( qboolean gravity );
template void PM_StepSlideMove< moduleType::cgame >( qboolean gravity );
