...
*/
	...
	if ( winAnim != -1 )
	{
		PM_SetAnim< mod >( SETANIM_BOTH, winAnim, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD );
...
int PM_SaberLockResultAnim( playerState_t *duelist, qboolean superBreak, qboolean won ) {
	...
	if( mod == moduleType::game ) {
		...
		if( duelist->clientNum == pm->ps->clientNum ) {//me
			PM_SetAnim< mod >( SETANIM_BOTH, baseAnim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD );
		} else {//other guy
	...
	{//if you lose a superbreak, you're defenseless
		...
		*/
		if ( mod == moduleType::game || duelist->clientNum == pm->ps->clientNum )
		{
...
void PM_SaberLockBreak( playerState_t *genemy, qboolean victory, int strength )
{
	...
	if ( PM_SaberLockWinAnim< mod >( victory, superBreak ) != -1 )
		PM_SaberLockLoseAnim< mod >( genemy, victory, superBreak );

	//must be a saberlock that's not between single and single...
	else {
		PM_SaberLockResultAnim< mod >( pm->ps, superBreak, qtrue );
		pm->ps->weaponstate = WEAPON_FIRING;
		PM_SaberLockResultAnim< mod >( genemy, superBreak, qfalse );
		genemy->weaponstate = WEAPON_READY;
	}
	...
	if ( !victory )
	{//no-one won
		BG_AddPredictableEventToPlayerstate< mod >(EV_JUMP, 0, genemy);
	}
	else
	{
		if ( PM_irand_timesync( 0, 1 ) )
		{
			BG_AddPredictableEventToPlayerstate< mod >(EV_JUMP, PM_irand_timesync( 0, 75 ), genemy);
		}
	}
}
...
void PM_SaberLocked( void )
{
		...
		BG_InSaberLock(genemy->torsoAnim))
	{
		...
		{//between 8 and 80 from each other
			PM_SaberLockBreak< mod >( genemy, qfalse, 0 );
			return;
		}
		/*
		...
		{//holding attack
			...
			{ //old locks
					...
					(pm->ps->torsoAnim) == BOTH_BF2LOCK )
				{
					...
					{//I won!  Break out
						PM_SaberLockBreak< mod >( genemy, qtrue, strength );
						return;
					}
					else
					{
			...
			if ( BG_InSaberLockOld( genemy->torsoAnim ) )
			{
					...
					(genemy->torsoAnim) == BOTH_BF1LOCK )
				{
					if ( !PM_irand_timesync( 0, 2 ) )
					{
						BG_AddPredictableEventToPlayerstate< mod >(EV_PAIN, floor((float)80/100*100.0f), genemy);
					}
...
saberMoveName_t PM_SaberJumpAttackMove2( void )
{
	...
	if (pm->ps->fd.saberAnimLevel == SS_DUAL)
	{
		return static_cast< saberMoveName_t >( PM_SaberDualJumpAttackMove() );
	}
	else
	{
...
saberMoveName_t PM_SaberAttackForMovement(saberMoveName_t curmove)
{
	...
	{//moving right
		if ( !noSpecials
			...
			&& BG_EnoughForcePowerForMove< mod >( SABER_ALT_ATTACK_POWER_LR ) )//have enough power
		{//cartwheel right
	...
	{//not moving left or right
		...
		{//forward= T2B slash
			if (!noSpecials&&
				...
				BG_EnoughForcePowerForMove< mod >(SABER_ALT_ATTACK_POWER_FB) )
			{ //DUAL/STAFF JUMP ATTACK
				newmove = PM_SaberJumpAttackMove2< mod >();
				if ( newmove != LS_A_T2B
					&& newmove != LS_NONE )
				{
					BG_ForcePowerDrain(pm->ps, FP_GRIP, SABER_ALT_ATTACK_POWER_FB);
			...
			{ //FLIP AND DOWNWARD ATTACK
				...
				{
					newmove = PM_SaberFlipOverAttackMove< mod >();
					if ( newmove != LS_A_T2B
						&& newmove != LS_NONE )
					{
						BG_ForcePowerDrain(pm->ps, FP_GRIP, SABER_ALT_ATTACK_POWER_FB);
			...
			{ //DFA
				...
				//if (PM_SomeoneInFront(&tr))
				{
					newmove = PM_SaberJumpAttackMove< mod >();
					if ( newmove != LS_A_T2B
						&& newmove != LS_NONE )
					{
						BG_ForcePowerDrain(pm->ps, FP_GRIP, SABER_ALT_ATTACK_POWER_FB);
				...
				BG_EnoughForcePowerForMove< mod >(SABER_ALT_ATTACK_POWER_FB))
			{ //LUNGE (weak)
				newmove = PM_SaberLungeAttackMove< mod >( noSpecials );
				if ( newmove != LS_A_T2B
					&& newmove != LS_NONE )
				{
					BG_ForcePowerDrain(pm->ps, FP_GRIP, SABER_ALT_ATTACK_POWER_FB);
			...
			{
				saberMoveName_t stabDownMove = PM_CheckStabDown< mod >();
		...
		{//backward= T2B slash//B2T uppercut?
			if (!noSpecials&&
				...
				(pm->cmd.buttons & BUTTON_ATTACK))
			{ //BACKFLIP ATTACK
				newmove = static_cast< saberMoveName_t >( PM_SaberBackflipAttackMove< mod >() );
			}
...
qboolean PM_CanDoKata( void )
{
	...
	if ( !pm->ps->saberInFlight//not throwing saber
		...
		*/
		...
		&& BG_EnoughForcePowerForMove< mod >(SABER_ALT_ATTACK_POWER) )// have enough power
...
qboolean PM_SaberPowerCheck(void)
{
	...
	{
		return BG_EnoughForcePowerForMove< mod >(forcePowerNeeded[pm->ps->fd.forcePowerLevel[FP_SABERTHROW]][FP_SABERTHROW]);
	}
...
void PM_WeaponLightsaber(void)
{
	...
	{//in knockdown
			...
			&& pm->ps->legsTimer <= 250 )
		{
			if ( (pm->cmd.buttons&BUTTON_ATTACK) )
			{
				if ( BG_EnoughForcePowerForMove< mod >(SABER_ALT_ATTACK_POWER_FB) && !pm->ps->saberInFlight )
				{
					if ( PM_CanDoRollStab< mod >() )
					{
						//make sure the saber is on for this move!
						if ( pm->ps->saberHolstered == 2 )
						{//all the way off
							pm->ps->saberHolstered = 0;
							PM_AddEvent< mod >(EV_SABER_UNHOLSTER);
						}
						PM_SetSaberMove< mod >( LS_ROLL_STAB );
						BG_ForcePowerDrain(pm->ps, FP_GRIP, SABER_ALT_ATTACK_POWER_FB);
					}
				}
			}
			...
			)
		{
				...
				pm->ps->saberLockEnemy >= 0)
			{
				...
				if (bgEnt)
				{
					...
					if (en)
					{
						PM_SaberLockBreak< mod >(en, qfalse, 0);
						return;
					}
				}
			}
			...
			{
				pm->ps->torsoTimer = 0;
				PM_SetAnim< mod >(SETANIM_TORSO,BOTH_STAND1,SETANIM_FLAG_OVERRIDE);
	...
	if (BG_SabersOff( pm->ps ))
	{
		if (pm->ps->saberMove != LS_READY)
		{
			PM_SetSaberMove< mod >( LS_READY );
		}

		if ((pm->ps->legsAnim) != (pm->ps->torsoAnim) && !BG_InSlopeAnim(pm->ps->legsAnim) &&
			pm->ps->torsoTimer <= 0)
		{
			PM_SetAnim< mod >(SETANIM_TORSO,(pm->ps->legsAnim),SETANIM_FLAG_OVERRIDE);
		}
		else if (BG_InSlopeAnim(pm->ps->legsAnim) && pm->ps->torsoTimer <= 0)
		{
			PM_SetAnim< mod >(SETANIM_TORSO,PM_GetSaberStance< mod >(),SETANIM_FLAG_OVERRIDE);
		}
		...
		{ //kick instead of doing a throw
				...
				&& !(pm->cmd.buttons&BUTTON_ATTACK) )//not trying to swing the saber
			{
				if ( (pm->cmd.forwardmove||pm->cmd.rightmove)//trying to kick in a specific direction
					&& PM_CheckAltKickAttack< mod >() )//trying to do a kick
				{//allow them to do the kick now!
					int kickMove = PM_KickMoveForConditions();
					if (kickMove != -1)
					{
						pm->ps->weaponTime = 0;
						PM_SetSaberMove< mod >( kickMove );
						return;
					}
				}
			}
	...
	{//guiding saber
		...
		{
			PM_SetAnim< mod >(SETANIM_TORSO, BOTH_SABERPULL, SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			pm->ps->torsoTimer = 1;
			return;
		}
	}
		...
		switch ( pm->ps->saberBlocked )
		{
			case BLOCKED_BOUNCE_MOVE:
				{ //act as a bounceMove and reset the saberMove instead of using a seperate value for it
					pm->ps->torsoTimer = 0;
					PM_SetSaberMove< mod >( pm->ps->saberMove );
					pm->ps->weaponTime = pm->ps->torsoTimer;
			...
			case BLOCKED_ATK_BOUNCE:
				...
				else
				{
					...
					PM_SetSaberMove< mod >( bounceMove );
			...
			case BLOCKED_UPPER_RIGHT:
				PM_SetSaberMove< mod >( LS_PARRY_UR );
				break;
			case BLOCKED_UPPER_RIGHT_PROJ:
				PM_SetSaberMove< mod >( LS_REFLECT_UR );
				break;
			case BLOCKED_UPPER_LEFT:
				PM_SetSaberMove< mod >( LS_PARRY_UL );
				break;
			case BLOCKED_UPPER_LEFT_PROJ:
				PM_SetSaberMove< mod >( LS_REFLECT_UL );
				break;
			case BLOCKED_LOWER_RIGHT:
				PM_SetSaberMove< mod >( LS_PARRY_LR );
				break;
			case BLOCKED_LOWER_RIGHT_PROJ:
				PM_SetSaberMove< mod >( LS_REFLECT_LR );
				break;
			case BLOCKED_LOWER_LEFT:
				PM_SetSaberMove< mod >( LS_PARRY_LL );
				break;
			case BLOCKED_LOWER_LEFT_PROJ:
				PM_SetSaberMove< mod >( LS_REFLECT_LL);
				break;
			case BLOCKED_TOP:
				PM_SetSaberMove< mod >( LS_PARRY_UP );
				break;
			case BLOCKED_TOP_PROJ:
				PM_SetSaberMove< mod >( LS_REFLECT_UP );
				break;
			default:
				pm->ps->saberBlocked = BLOCKED_NONE;
				break;
...
weapChecks:
	...
	if ( PM_CanDoKata< mod >() )
	{
		...
		{//not overridden
			...
			case SS_FAST:
			case SS_TAVION:
				PM_SetSaberMove< mod >( LS_A1_SPECIAL );
				break;
			case SS_MEDIUM:
				PM_SetSaberMove< mod >( LS_A2_SPECIAL );
				break;
			case SS_STRONG:
			case SS_DESANN:
				PM_SetSaberMove< mod >( LS_A3_SPECIAL );
				break;
			case SS_DUAL:
				PM_SetSaberMove< mod >( LS_DUAL_SPIN_PROTECT );//PM_CheckDualSpinProtect();
				break;
			case SS_STAFF:
				PM_SetSaberMove< mod >( LS_STAFF_SOULCAL );
				break;
			}
			pm->ps->weaponstate = WEAPON_FIRING;
		...
		{
			PM_SetSaberMove< mod >( overrideMove );
			pm->ps->weaponstate = WEAPON_FIRING;
			BG_ForcePowerDrain(pm->ps, FP_GRIP, SABER_ALT_ATTACK_POWER);
		}
	...
	{//Just selected the weapon
		...
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_WALK1,SETANIM_FLAG_NORMAL);
		}
		else if((pm->ps->legsAnim) == BOTH_RUN1 )
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_RUN1,SETANIM_FLAG_NORMAL);
		}
		else if((pm->ps->legsAnim) == BOTH_RUN2 )
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_RUN2,SETANIM_FLAG_NORMAL);
		}
		else if( pm->ps->legsAnim == BOTH_RUN_STAFF)
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_RUN_STAFF,SETANIM_FLAG_NORMAL);
		}
		else if( pm->ps->legsAnim == BOTH_RUN_DUAL)
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_RUN_DUAL,SETANIM_FLAG_NORMAL);
		}
		...
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_WALK2,SETANIM_FLAG_NORMAL);
		}
		else if( pm->ps->legsAnim == BOTH_WALK_STAFF)
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_WALK_STAFF,SETANIM_FLAG_NORMAL);
		}
		else if( pm->ps->legsAnim == BOTH_WALK_DUAL)
		{
			PM_SetAnim< mod >(SETANIM_TORSO,BOTH_WALK_DUAL,SETANIM_FLAG_NORMAL);
		}
		else
		{
			PM_SetAnim< mod >(SETANIM_TORSO,PM_GetSaberStance< mod >(),SETANIM_FLAG_NORMAL);
		}
		...
		else
		{
			...
			{//only 1 attack you can do from this anim
				...
				{//hit it early enough to do the attack
					PM_SetSaberMove< mod >( LS_LEAP_ATTACK );
				}
				return;
			}
			...
			if ( anim == -1 )
			{
				...
				{//get attack move from movement command
					...
					newmove = PM_SaberAttackForMovement< mod >( static_cast< saberMoveName_t >( curmove ) );
...
//				}
			...
			PM_SetSaberMove< mod >( newmove );

			if ( both && pm->ps->torsoAnim == anim )
			{
				PM_SetAnim< mod >(SETANIM_LEGS,anim,SETANIM_FLAG_OVERRIDE|SETANIM_FLAG_HOLD);
			}
...
template void PM_WeaponLightsaber< moduleType::game >( void );
template void PM_WeaponLightsaber< moduleType::cgame >( void );
...
void PM_SetSaberMove(short newMove)
{
	...
	*/
	...
	{ //if not riding a vehicle
		...
		PM_SetAnim< mod >(parts, anim, setflags);
			...
			{//wasn't playing that attack before
				...
				{ //randomly make pain sounds with a broken arm because we are suffering.
					...
					if (iFactor != -1)
					{
						...
						{
							BG_AddPredictableEventToPlayerstate< mod >(EV_PAIN, PM_irand_timesync( 1, 100 ), pm->ps);
						}
					}
				}
			}
...
template void PM_SetSaberMove< moduleType::game >( short newMove );
template void PM_SetSaberMove< moduleType::cgame >( short newMove );
...
template saberInfo_t *BG_MySaber< moduleType::game >( int clientNum, int saberNum );
template saberInfo_t *BG_MySaber< moduleType::cgame >( int clientNum, int saberNum );
template saberInfo_t *BG_MySaber< moduleType::ui >( int clientNum, int saberNum );
