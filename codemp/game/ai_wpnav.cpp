...
void TransferWPData(int from, int to)
{
	...
	if (!gWPArray[to])
	{
		g_trap->Print(S_COLOR_RED "FATAL ERROR: Could not allocated memory for waypoint\n");
	}
...
void CreateNewWP(vec3_t origin, int flags)
{
	if (gWPNum >= MAX_WPARRAY_SIZE)
	{
		if (!RMG.integer)
		{
			g_trap->Print(S_COLOR_YELLOW "Warning: Waypoint limit hit (%i)\n", MAX_WPARRAY_SIZE);
		}
		return;
	}
	...
	if (!gWPArray[gWPNum])
	{
		g_trap->Print(S_COLOR_RED "ERROR: Could not allocated memory for waypoint\n");
	}
...
void RemoveWP_InTrail(int afterindex)
{
	...
	{
		g_trap->Print(S_COLOR_YELLOW "Waypoint number %i does not exist\n", afterindex);
		return;
	}
	...
	if (!foundanindex)
	{
		g_trap->Print(S_COLOR_YELLOW "Waypoint index %i should exist, but does not (?)\n", afterindex);
		return;
	}
...
void WPFlagsModify(int wpnum, int flags)
{
	...
	{
		g_trap->Print(S_COLOR_YELLOW "WPFlagsModify: Waypoint %i does not exist\n", wpnum);
		return;
	}
...
int CanGetToVector(vec3_t org1, vec3_t org2, vec3_t mins, vec3_t maxs)
{
	...
	g_trap->Trace(&tr, org1, mins, maxs, org2, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
int CanGetToVectorTravel(vec3_t org1, vec3_t org2, vec3_t mins, vec3_t maxs)
{
	...
	g_trap->Trace(&tr, midpos, NULL, NULL, dmid, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
//int ExampleAnimEntMove(gentity_t *self, vec3_t moveTo, float stepSize)
{
	...
	while (!initialDone || didMove)
	{
		...
		g_trap->Trace(&tr, workingOrg, mins, maxs, stepGoal, ENTITYNUM_NONE, traceMask, qfalse, 0, 0);
		...
		{ //stair check
			...
			{
				g_trap->Trace(&tr, trFrom, mins, maxs, trTo, ENTITYNUM_NONE, traceMask, qfalse, 0, 0);
...
//		nodetable[i].index = 0;
	...
	if (!behindTheScenes)
	{
		g_trap->Print(S_COLOR_YELLOW "Point %i is not connected to %i - Repairing...\n", startindex, endindex);
	}
	...
	g_trap->Trace(&tr, startplace, NULL, NULL, starttrace, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
	...
	while (nodenum < MAX_NODETABLE_SIZE && !foundit && cancontinue)
	{
		...
		while (i < prenodestart)
		{
			if (extendednodes[i] != 1)
			{
				...
				g_trap->Trace(&tr, testspot, NULL, NULL, starttrace, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
//					nodetable[nodenum].index = nodenum;
...
#endif
		{
			g_trap->Print(S_COLOR_RED "Could not link %i to %i, unreachable by node branching.\n", startindex, endindex);
		}
		gWPArray[startindex]->flags |= WPFLAG_ONEWAY_FWD;
		gWPArray[endindex]->flags |= WPFLAG_ONEWAY_BACK;
		if (!behindTheScenes)
		{
			g_trap->Print(S_COLOR_YELLOW "Since points cannot be connected, point %i has been flagged as only-forward and point %i has been flagged as only-backward.\n", startindex, endindex);
		}
	...
	while (failsafe < MAX_NODETABLE_SIZE && i < MAX_NODETABLE_SIZE && i >= 0)
	{
		...
		if (!nodetable[nodetable[i].neighbornum].inuse || !CanGetToVectorTravel(validspotpos, /*nodetable[nodetable[i].neighbornum].origin*/nodetable[i].origin, mins, maxs) || VectorLength(a) > maxDistFactor || (!CanGetToVectorTravel(validspotpos, gWPArray[endindex]->origin, mins, maxs) && CanGetToVectorTravel(nodetable[i].origin, gWPArray[endindex]->origin, mins, maxs)) )
		{
			...
			if (!CreateNewWP_InTrail(nodetable[i].origin, nodetable[i].flags, insertindex))
			{
				if (!behindTheScenes)
				{
					g_trap->Print(S_COLOR_RED "Could not link %i to %i, waypoint limit hit.\n", startindex, endindex);
				}
				return 0;
			}
	...
	if (!behindTheScenes)
	{
		g_trap->Print(S_COLOR_YELLOW "Finished connecting %i to %i.\n", startindex, endindex);
	}
...
{ //if a door blocks the trail, we'll just have to assume the points on each side are in visibility when it's open
	...
	g_trap->Trace(&tr, gWPArray[start]->origin, NULL, NULL, gWPArray[end]->origin, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
	...
	g_trap->Trace(&tr, gWPArray[end]->origin, NULL, NULL, gWPArray[start]->origin, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
//	int ctRet;
	...
	g_trap->Cvar_Update(&bot_wp_distconnect);
	g_trap->Cvar_Update(&bot_wp_visconnect);
...
int OrgVisibleCurve(vec3_t org1, vec3_t mins, vec3_t maxs, vec3_t org2, int ignore)
{
	...
	g_trap->Trace(&tr, evenorg1, mins, maxs, org2, ignore, MASK_SOLID, qfalse, 0, 0);

	if (tr.fraction == 1 && !tr.startsolid && !tr.allsolid)
	{
		g_trap->Trace(&tr, evenorg1, mins, maxs, org1, ignore, MASK_SOLID, qfalse, 0, 0);
...
int GetNearestVisibleWPToItem(vec3_t org, int ignore)
{
	...
	while (i < gWPNum)
	{
			...
			gWPArray[i]->origin[2]+15 > org[2])
		{
			...
			if (flLen < bestdist && g_trap->InPVS(org, gWPArray[i]->origin) && OrgVisibleBox(org, mins, maxs, gWPArray[i]->origin, ignore))
			{
				bestdist = flLen;
				bestindex = i;
			}
...
{ //set waypoint weights depending on weapon and item placement
	...
	g_trap->Cvar_Update(&bot_wp_clearweight);
...
int LoadPathData(const char *filename)
{
	...
	len = g_trap->FS_Open(routePath, &f, FS_READ);

	B_TempFree(1024); //routePath

	if (!f)
	{
		g_trap->Print(S_COLOR_YELLOW "Bot route data not found for %s\n", filename);
		return 2;
	}

	if (len >= 524288)
	{
		g_trap->Print(S_COLOR_RED "Route file exceeds maximum length\n");
		return 0;
	}
	...
	g_trap->FS_Read(fileString, len, f);
...
void FlagObjects(void)
{
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			if (tlen < bestdist)
			{
				g_trap->Trace(&tr, flag_red->s.pos.trBase, mins, maxs, gWPArray[i]->origin, flag_red->s.number, MASK_SOLID, qfalse, 0, 0);
	...
	while (i < gWPNum)
	{
		if (gWPArray[i] && gWPArray[i]->inuse)
		{
			...
			if (tlen < bestdist)
			{
				g_trap->Trace(&tr, flag_blue->s.pos.trBase, mins, maxs, gWPArray[i]->origin, flag_blue->s.number, MASK_SOLID, qfalse, 0, 0);
...
int SavePathData(const char *filename)
{
	...
	g_trap->FS_Open(routePath, &f, FS_WRITE);

	B_TempFree(1024); //routePath

	if (!f)
	{
		g_trap->Print(S_COLOR_RED "ERROR: Could not open file to write path data\n");
		return 0;
	}
	...
	g_trap->FS_Write(fileString, strlen(fileString), f);
	...
	g_trap->Print("Path data has been saved and updated. You may need to restart the level for some things to be properly calculated.\n");
...
int G_RecursiveConnection(int start, int end, int weight, qboolean traceCheck, float baseHeight)
{
	...
	while (i < 4)
	{
		...
		{ //if we care about trace visibility between nodes, perform the check and mark as not valid if the trace isn't clear.
			g_trap->Trace(&tr, nodetable[start].origin, NULL, NULL, nodetable[indexDirections[i]].origin, ENTITYNUM_NONE, CONTENTS_SOLID, qfalse, 0, 0);
...
void G_DebugNodeFile()
{
	...
	g_trap->FS_Open("ROUTEDEBUG.txt", &f, FS_WRITE);
	g_trap->FS_Write(fileString, strlen(fileString), f);
	g_trap->FS_Close(f);
}
#endif
...
{ //Draw a text grid of the entire waypoint array (useful for debugging final waypoint placement)
	...
	g_trap->FS_Open("ROUTEDRAWN.txt", &f, FS_WRITE);
	g_trap->FS_Write(fileString, strlen(fileString), f);
	g_trap->FS_Close(f);
}
...
{ //Generate waypoint information on-the-fly for the random mission.
	...
	{
		g_trap->Print("Error: RMG with no terrain!\n");
		return;
	}

#ifdef PATH_TIME_DEBUG
	startTime = g_trap->Milliseconds();
#endif
	...
	while (placeY < terrain->r.absmax[1])
	{
		...
		while (placeX < terrain->r.absmax[0])
		{
			...
			downVec[2] -= 3000;
			g_trap->Trace(&tr, nodetable[nodenum].origin, trMins, trMaxs, downVec, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
...
#ifdef PATH_TIME_DEBUG
	endTime = g_trap->Milliseconds();

	g_trap->Print("Total routing time taken: %ims\n", (endTime - startTime));
#endif
...
{ //Called for RMG levels.
	...
	g_trap->BotUpdateWaypoints(gWPNum, gWPArray);
	g_trap->BotCalculatePaths(RMG.integer);
...
void LoadPath_ThisLevel(void)
{
	...
	{ //If RMG, generate the path on-the-fly
		g_trap->Cvar_Register(&bot_normgpath, "bot_normgpath", "1", CVAR_CHEAT);
	...
	g_trap->Cvar_Update(&bot_wp_edit);
...
int AcceptBotCommand(char *cmd, gentity_t *pl)
{
	...
	{
		g_trap->Print(S_COLOR_YELLOW "bot_wp_add" S_COLOR_WHITE " - Add a waypoint (optional int parameter will insert the point after the specified waypoint index in a trail)\n\n");
		g_trap->Print(S_COLOR_YELLOW "bot_wp_rem" S_COLOR_WHITE " - Remove a waypoint (removes last unless waypoint index is specified as a parameter)\n\n");
		g_trap->Print(S_COLOR_YELLOW "bot_wp_addflagged" S_COLOR_WHITE " - Same as wp_add, but adds a flagged point (type bot_wp_addflagged for help)\n\n");
		g_trap->Print(S_COLOR_YELLOW "bot_wp_switchflags" S_COLOR_WHITE " - Switches flags on an existing waypoint (type bot_wp_switchflags for help)\n\n");
		g_trap->Print(S_COLOR_YELLOW "bot_wp_tele" S_COLOR_WHITE " - Teleport yourself to the specified waypoint's location\n");
		g_trap->Print(S_COLOR_YELLOW "bot_wp_killoneways" S_COLOR_WHITE " - Removes oneway (backward and forward) flags on all waypoints in the level\n\n");
		g_trap->Print(S_COLOR_YELLOW "bot_wp_save" S_COLOR_WHITE " - Saves all waypoint data into a file for later use\n");
	...
	if (Q_stricmp (cmd, "bot_wp_tele") == 0)
	{
		...
		{
			g_trap->Print(S_COLOR_YELLOW "You didn't specify an index. Assuming last.\n");
	...
	if (Q_stricmp (cmd, "bot_wp_switchflags") == 0)
	{
		...
		{
			g_trap->Print(S_COLOR_YELLOW "Waypoint number (to modify) needed for bot_wp_switchflags\nSyntax: bot_wp_switchflags <flags> <n>\n");
		}
		return 1;
	}
