...
static int G_FindConfigstringIndex( const char *name, int start, int max, qboolean create ) {
	...
	for ( i=1 ; i<max ; i++ ) {
		g_trap->GetConfigstring( start + i, s, sizeof( s ) );
		if ( !s[0] ) {
			break;
		}
		if ( !strcmp( s, name ) ) {
	...
	if ( i == max ) {
		g_trap->Error( ERR_DROP, "G_FindConfigstringIndex: overflow" );
	}

	g_trap->SetConfigstring( start + i, name );
...
int G_ModelIndex( const char *name ) {
	...
	g_trap->FS_Open(name, &fh, FS_READ);
	if (!fh)
	{ //try models/ then, this is assumed for registering models
		g_trap->FS_Open(va("models/%s", name), &fh, FS_READ);
		if (!fh)
		{
	...
	for ( i = 0 ; i < level.maxclients ; i++ ) {
		if ( level.clients[i].pers.connected == CON_CONNECTED ) {
			if ( level.clients[i].sess.sessionTeam == team ) {
				g_trap->SendServerCommand( i, va("%s", cmd ));
			}
		}
	}
}
...
{
	//g_trap->TrueMalloc((void **)cl, sizeof(gclient_t));
	if (!gClPtrs[entNum])
	{
		gClPtrs[entNum] = (gclient_t *) BG_Alloc< moduleType::game >(sizeof(gclient_t));
	}
	*cl = gClPtrs[entNum];
}
...
void G_SetAnim(gentity_t *ent, usercmd_t *ucmd, int setAnimParts, int anim, int setAnimFlags, int blendTime)
{
	...
	pmv.trace = g_trap->Trace;
	pmv.pointcontents = g_trap->PointContents;
	...
	assert(ent->client);
    BG_SetAnim< moduleType::game >(&ent->client->ps, bgAllAnims[ent->localAnimIndex].anims, setAnimParts, anim, setAnimFlags);
#endif
}
...
gentity_t *G_PickTarget (char *targetname)
{
	...
	if (!targetname)
	{
		g_trap->Print("G_PickTarget called with NULL targetname\n");
		return NULL;
	}
	...
	if (!num_choices)
	{
		g_trap->Print("G_PickTarget: target %s not found\n", targetname);
		return NULL;
	}
...
void G_UseTargets2( gentity_t *ent, gentity_t *activator, const char *string ) {
	...
	if (ent->targetShaderName && ent->targetShaderNewName) {
		...
		g_trap->SetConfigstring(CS_SHADERSTATE, BuildShaderStateConfig());
	}
	...
	while ( (t = G_Find (t, FOFS(targetname), string)) != NULL ) {
		if ( t == ent ) {
			g_trap->Print ("WARNING: Entity used itself.\n");
		} else {
			if ( t->use ) {
		...
		if ( !ent->inuse ) {
			g_trap->Print("entity was removed while using targets\n");
			return;
		}
	}
}
...
void G_InitGentity( gentity_t *e ) {
	...
	g_trap->ICARUS_FreeEnt( (sharedEntity_t *)e );	//ICARUS information must be added after this point
}
...
static void G_SpewEntList(void)
{
	...
	fileHandle_t fh;
	g_trap->FS_Open("entspew.txt", &fh, FS_WRITE);
#endif
	...
	while (i < ENTITYNUM_MAX_NORMAL)
	{
		...
		if (ent->inuse)
		{
			...
			else if (ent->freeAfterEvent)
			{
				...
				if (fh)
				{
					g_trap->FS_Write(str, strlen(str), fh);
				}
#endif
			}
...
gentity_t *G_Spawn( void ) {
	...
	if ( i == ENTITYNUM_MAX_NORMAL ) {
		...
		*/
		G_SpewEntList();
		g_trap->Error( ERR_DROP, "G_Spawn: no free entities" );
	}
...
void G_SendG2KillQueue(void)
{
	...
	g_trap->SendServerCommand(-1, g2KillString);
...
void G_KillG2Queue(int entNum)
{
	...
	{ //This would be considered a Bad Thing.
		...
		g_trap->SendServerCommand(-1, va("kg2 %i", entNum));
		return;
	}
...
void G_FreeEntity( gentity_t *ed ) {
	...
	g_trap->UnlinkEntity ((sharedEntity_t *)ed);		// unlink from world

	g_trap->ICARUS_FreeEnt( (sharedEntity_t *)ed );	//ICARUS information must be added after this point
	...
	if (ed->ghoul2)
	{
		re->G2API_CleanGhoul2Models(&(ed->ghoul2));
	}
	...
	{ //this "client" structure is one of our dynamically allocated ones, so free the memory
		...
		while (i < MAX_SABERS)
		{
			if (ed->client->weaponGhoul2[i] && g_trap->G2API_HaveWeGhoul2Models(ed->client->weaponGhoul2[i]))
			{
				g_trap->G2API_CleanGhoul2Models(&ed->client->weaponGhoul2[i]);
			}
			i++;
		}
	...
	if (ed->s.eFlags & EF_SOUNDTRACKER)
	{
		...
		g_trap->SendServerCommand(-1, va("kls %i %i", ed->s.trickedentindex, ed->s.number));
	}
...
gentity_t *G_TempEntity( vec3_t origin, int event ) {
	...
	g_trap->LinkEntity( (sharedEntity_t *)e );
...
void G_AddPredictableEvent( gentity_t *ent, int event, int eventParm ) {
	...
	BG_AddPredictableEventToPlayerstate< moduleType::game >( event, eventParm, &ent->client->ps );
}
...
void G_AddEvent( gentity_t *ent, int event, int eventParm ) {
	...
	if ( !event ) {
		g_trap->Print( "G_AddEvent: zero event added for entity %i\n", ent->s.number );
		return;
	}
...
void TryUse( gentity_t *ent )
{
	...
	g_trap->Trace( &trace, src, vec3_origin, vec3_origin, dest, ent->s.number, MASK_OPAQUE|CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_ITEM|CONTENTS_CORPSE, qfalse, 0, 0 );
...
tryJetPack:
	...
	{ //if you used nothing, then try spewing out some ammo
		...
		g_trap->Trace(&trToss, ent->client->ps.origin, playerMins, playerMaxs, fwd, ent->s.number, ent->clipmask, qfalse, 0, 0);
...
qboolean G_ClearTrace( vec3_t start, vec3_t mins, vec3_t maxs, vec3_t end, int ignore, int clipmask )
{
	...
	g_trap->Trace( &tr, start, mins, maxs, end, ignore, clipmask, qfalse, 0, 0 );
...
qboolean G_CheckInSolid (gentity_t *self, qboolean fix)
{
	...
	g_trap->Trace(&trace, self->r.currentOrigin, mins, self->r.maxs, end, self->s.number, self->clipmask, qfalse, 0, 0);
...
int DebugLine(vec3_t start, vec3_t end, int color) {
	...
	return g_trap->DebugPolygonCreate(color, 4, points);
}
...
void G_ROFF_NotetrackCallback( gentity_t *cent, const char *notetrack)
{
	...
	if (strcmp(type, "loop") == 0)
	{
		...
		g_trap->ROFF_Play(cent->s.number, cent->roffid, qfalse);
	}
}
...
qboolean G_ExpandPointToBBox( vec3_t point, const vec3_t mins, const vec3_t maxs, int ignore, int clipmask )
{
	...
	for ( i = 0; i < 3; i++ )
	{
		...
		g_trap->Trace( &tr, start, vec3_origin, vec3_origin, end, ignore, clipmask, qfalse, 0, 0 );
	...
	g_trap->Trace( &tr, start, mins, maxs, start, ignore, clipmask, qfalse, 0, 0 );
	if ( tr.allsolid || tr.startsolid )
	{
		return qfalse;
	}
