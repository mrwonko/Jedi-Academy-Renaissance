...
{
	return BG_TempAlloc< moduleType::game >(size);
}

void B_TempFree(int size)
{
	BG_TempFree< moduleType::game >(size);
}
...
void *B_Alloc(int size)
{
	...
	g_trap->Print("Allocations used: %i\nFree allocation slots: %i\n", used, free);

	i = 0;
#endif

	ptr = g_trap->BotGetMemoryGame(size);
...
#ifdef DEBUG
		g_trap->Print("WARNING: MAXIMUM B_ALLOC ALLOCATIONS EXCEEDED\n");
#endif
	}
	...
	return BG_Alloc< moduleType::game >(size);
...
#ifdef DEBUG
		g_trap->Print("WARNING: Freeing allocation which is not in the allocation structure\n");
#endif
	}

	g_trap->BotFreeMemoryGame(ptr);
#endif
}
...
void B_CleanupAlloc(void)
{
	...
	while (i < MAX_BALLOC)
	{
		if (BAllocList[i])
		{
			g_trap->BotFreeMemoryGame(BAllocList[i]);
			BAllocList[i] = NULL;
		}
...
int BotDoChat(bot_state_t *bs, char *section, int always)
{
	...
	if (g_trap->Cvar_VariableIntegerValue("se_language"))
	...
	//g_trap->EA_Say(bs->client, chatgroup);
	inc_1 = 0;
	inc_2 = 0;
...
int ReadChatGroups(bot_state_t *bs, char *buf)
{
	...
	{
		g_trap->Print(S_COLOR_RED "Error: Personality chat section exceeds max size\n");
		return 0;
	}
...
void BotUtilizePersonality(bot_state_t *bs)
{
	...
	len = g_trap->FS_Open(bs->settings.personalityfile, &f, FS_READ);

	failed = 0;

	if (!f)
	{
		g_trap->Print(S_COLOR_RED "Error: Specified personality not found\n");
	...
	if (len >= 131072)
	{
		g_trap->Print(S_COLOR_RED "Personality file exceeds maximum length\n");
		B_TempFree(131072); //buf
		return;
	}
	...
	{
		g_trap->Print(S_COLOR_RED "Personality file contains no GeneralBotInfo group\n");
