...
void NPC_SetMiscDefaultData( gentity_t *ent )
{
	...
	if (ent->s.NPC_class == CLASS_VEHICLE && ent->m_pVehicle)
	{
		...
		g_trap->G2API_SetSurfaceOnOff( ent->ghoul2, "head_hatchcover", 0/*TURN_ON*/ );
	}
...
gNPC_t *New_NPC_t(int entNum)
{
	...
	if (!gNPCPtrs[entNum])
	{
		gNPCPtrs[entNum] = (gNPC_t *)BG_Alloc< moduleType::game > (sizeof(gNPC_t));
	}
...
qboolean NPC_StasisSpawn_Go( gentity_t *ent )
{
	...
	trace_t	tr;
	g_trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ent->s.number, MASK_NPCSOLID );
...
*/
	...
	if ( ent->spawnflags & NSF_DROP_TO_FLOOR )
	{
		...
		g_trap->Trace( &tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, bottom, ent->s.number, MASK_NPCSOLID, qfalse, 0, 0 );
...
//==NPC_Connect( newent, net_name );===================================
	...
	if ( Q_stricmp( ent->classname, "NPC_Vehicle" ) == 0 )
	{
		...
		int iVehIndex = BG_VehicleGetIndex< moduleType::game >( ent->NPC_type );
		...
		switch( g_vehicleInfo[iVehIndex].type )
		{
			case VH_ANIMAL:
				...
				G_CreateAnimalNPC< moduleType::game >( &newent->m_pVehicle, ent->NPC_type );
				break;
			case VH_SPEEDER:
				// Create the speeder (making sure all it's data is initialized).
				G_CreateSpeederNPC< moduleType::game >( &newent->m_pVehicle, ent->NPC_type );
				break;
			case VH_FIGHTER:
				// Create the fighter (making sure all it's data is initialized).
				G_CreateFighterNPC< moduleType::game >( &newent->m_pVehicle, ent->NPC_type );
				break;
			case VH_WALKER:
				// Create the walker (making sure all it's data is initialized).
				G_CreateWalkerNPC< moduleType::game >( &newent->m_pVehicle, ent->NPC_type );
				break;
		...
		else if ( !Q_stricmp( ent->NPC_type, "test" ) )
		{
			...
			for ( n = 0; n < 1 ; n++)
			{
				if ( g_entities[n].s.eType != ET_NPC && g_entities[n].client)
				{
					...
					newent->s.teamowner = newent->client->playerTeam = g_entities[n].client->playerTeam;
					break;
				}
			}
...
//=====================================================================
	...
	g_trap->UnlinkEntity((sharedEntity_t *)newent);
	...
	g_trap->LinkEntity((sharedEntity_t *)newent);
...
//==New stuff=====================================================================
	...
	{//specified team directly?
		newent->client->sess.sessionTeam = static_cast< team_t >( atoi(ent->team) );
	}
	else if ( newent->s.teamowner != TEAM_FREE )
	{
		newent->client->sess.sessionTeam = static_cast< team_t >( newent->s.teamowner );
	}
	else if ( newent->alliedTeam != TEAM_FREE )
	{
		newent->client->sess.sessionTeam = static_cast< team_t >( newent->alliedTeam );
	}
	else if ( newent->teamnodmg != TEAM_FREE )
	{
		newent->client->sess.sessionTeam = static_cast< team_t >( newent->teamnodmg );
	}
	else
	{
...
qboolean NPC_VehiclePrecache( gentity_t *spawner )
{
	...
	int iVehIndex = BG_VehicleGetIndex< moduleType::game >( spawner->NPC_type );
 	if ( iVehIndex == VEHICLE_NONE )
	{//fixme: error msg?
		return qfalse;
	}
	...
	if (pVehInfo->model && pVehInfo->model[0])
	{
		...
		{
			skin = g_trap->R_RegisterSkin(va("models/players/%s/model_%s.skin", pVehInfo->model, pVehInfo->skin));
		}
		g_trap->G2API_InitGhoul2Model(&tempG2, va("models/players/%s/model.glm", pVehInfo->model), 0, skin, 0, 0, 0);
		if (tempG2)
		{ //now, cache the anim config.
			char GLAName[1024];

			GLAName[0] = 0;
			g_trap->G2API_GetGLAName(tempG2, 0, GLAName);
			...
			if (GLAName[0])
			{
				...
				if ( slash )
				{
					...
					BG_ParseAnimationFile< moduleType::game >(GLAName, NULL, qfalse);
				}
			}
			g_trap->G2API_CleanGhoul2Models(&tempG2);
		}
	}
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle )
{
	...
	g_trap->Trace(&trace, ent->r.currentOrigin, NULL, NULL, end, 0, MASK_SOLID, qfalse, 0, 0);
	VectorCopy(trace.endpos, end);
	end[2] -= 24;
	g_trap->Trace(&trace, trace.endpos, NULL, NULL, end, 0, MASK_SOLID, qfalse, 0, 0);
	VectorCopy(trace.endpos, end);
	end[2] += 24;
	...
	g_trap->LinkEntity((sharedEntity_t *)NPCspawner);
...
void NPC_Spawn_f( gentity_t *ent )
{
	...
	g_trap->Argv(2, npc_type, 1024);
	if ( Q_stricmp( "vehicle", npc_type ) == 0 )
	{
		isVehicle = qtrue;
		g_trap->Argv(3, npc_type, 1024);
		g_trap->Argv(4, targetname, 1024);
	}
	else
	{
		g_trap->Argv(3, targetname, 1024);
	}
...
void NPC_Kill_f( void )
{
	...
	if ( Q_stricmp( "team", name ) == 0 )
	{
		...
		{
			killTeam = static_cast< npcteam_t >( GetIDForString( TeamTable, name ) );
...
void Cmd_NPC_f( gentity_t *ent )
{
	...
	else if ( Q_stricmp ( cmd, "score" ) == 0 )
	{
		...
		g_trap->Argv( 2, cmd2, sizeof( cmd2 ) );
