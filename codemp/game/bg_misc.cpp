...
int BG_GetTime( void ) {
	return mod == moduleType::game ? level.time : cg.time;
}
template int BG_GetTime< moduleType::game >( void );
template int BG_GetTime< moduleType::cgame >( void );
...
qboolean	BG_PlayerTouchesItem( playerState_t *ps, entityState_t *item, int atTime ) {
	...
	BG_EvaluateTrajectory< mod >( &item->pos, atTime, origin );
...
template qboolean BG_PlayerTouchesItem< moduleType::game >( playerState_t *ps, entityState_t *item, int atTime );
template qboolean BG_PlayerTouchesItem< moduleType::cgame >( playerState_t *ps, entityState_t *item, int atTime );
...
template void BG_EvaluateTrajectory<moduleType::game>( const trajectory_t *, int, vec3_t );
template void BG_EvaluateTrajectory<moduleType::cgame>( const trajectory_t *, int, vec3_t );
...
template void BG_EvaluateTrajectoryDelta<moduleType::game>( const trajectory_t *, int, vec3_t );
template void BG_EvaluateTrajectoryDelta<moduleType::cgame>( const trajectory_t *, int, vec3_t );
...
template void BG_AddPredictableEventToPlayerstate < moduleType::game >( int, int, playerState_t * );
template void BG_AddPredictableEventToPlayerstate < moduleType::cgame >( int, int, playerState_t * );
...
{
	if( mod == moduleType::game ) {
		CGhoul2Info_v *g2 = NULL;

		if( VALIDSTRING( skinName ) )
			re->RegisterSkin( skinName );

		//I could hook up a precache ghoul2 function, but oh well, this works
		re->G2API_InitGhoul2Model( &g2, modelName, 0, 0, 0, 0, 0 );
		//now get rid of it
		if( g2 )
...
template int BG_ModelCache< moduleType::game >( const char *modelName, const char *skinName );
template int BG_ModelCache< moduleType::cgame >( const char *modelName, const char *skinName );

template< moduleType mod > struct MAX_POOL_SIZE; // MSVC has no variable templates yet :(
template<> struct MAX_POOL_SIZE<moduleType::game> {
	static constexpr int value = 3000000; //1024000
};
template<> struct MAX_POOL_SIZE<moduleType::cgame> {
	static constexpr int value = 2048000; //don't need as much for cgame stuff. 2mb will be fine.
};
template<> struct MAX_POOL_SIZE<moduleType::ui> {
	static constexpr int value = 512000; //And for the ui the only thing we'll be using this for anyway is allocating anim data for g2 menu models
};
...
struct bg_pool {
	...
	static char data[ MAX_POOL_SIZE<mod>::value ];
	static int size;
	static int tail;
};
template< moduleType mod >
char bg_pool< mod >::data[ MAX_POOL_SIZE< mod >::value ];

template< moduleType mod >
int bg_pool< mod >::size = 0;

template< moduleType mod >
int bg_pool< mod >::tail = MAX_POOL_SIZE<mod>::value;

template< moduleType mod >
void *BG_Alloc ( int size )
{
	bg_pool< mod >::size = ((bg_pool< mod >::size + 0x00000003) & 0xfffffffc);

	if (bg_pool< mod >::size + size > bg_pool< mod >::tail)
	{
		Com_Error( ERR_DROP, "BG_Alloc: buffer exceeded tail (%d > %d)", bg_pool< mod >::size + size, bg_pool< mod >::tail);
		return 0;
	}

	bg_pool< mod >::size += size;

	return &bg_pool< mod >::data[bg_pool< mod >::size - size];
}
template void *BG_Alloc<moduleType::game>( int );
template void *BG_Alloc<moduleType::cgame>( int );
template void *BG_Alloc<moduleType::ui>( int );
...
void *BG_AllocUnaligned ( int size )
{
	if (bg_pool< mod >::size + size > bg_pool< mod >::tail)
	{
		Com_Error( ERR_DROP, "BG_AllocUnaligned: buffer exceeded tail (%d > %d)", bg_pool< mod >::size + size, bg_pool< mod >::tail);
		return 0;
	}
...
template void *BG_AllocUnaligned<moduleType::game>( int );
template void *BG_AllocUnaligned<moduleType::cgame>( int );
template void *BG_AllocUnaligned<moduleType::ui>( int );
...
void *BG_TempAlloc( int size )
{
	...
	if (bg_pool< mod >::tail - size < bg_pool< mod >::size)
	{
		Com_Error( ERR_DROP, "BG_TempAlloc: buffer exceeded head (%d > %d)", bg_pool< mod >::tail - size, bg_pool< mod >::size);
		return 0;
	}

	bg_pool< mod >::tail -= size;

	return &bg_pool< mod >::data[bg_pool< mod >::tail];
}
template void *BG_TempAlloc<moduleType::game>( int );
template void *BG_TempAlloc<moduleType::cgame>( int );
template void *BG_TempAlloc<moduleType::ui>( int );
...
void BG_TempFree( int size )
{
	...
	if (bg_pool< mod >::tail+size > MAX_POOL_SIZE< mod >::value)
	{
		Com_Error( ERR_DROP, "BG_TempFree: tail greater than size (%d > %d)", bg_pool< mod >::tail+size, MAX_POOL_SIZE< mod >::value );
	}

	bg_pool< mod >::tail += size;
}
template void BG_TempFree<moduleType::game>( int );
template void BG_TempFree<moduleType::cgame>( int );
template void BG_TempFree<moduleType::ui>( int );

template< moduleType mod >
char *BG_StringAlloc ( const char *source )
{
	char *dest = (char*)BG_Alloc< mod >( strlen ( source ) + 1 );
	strcpy( dest, source );
	return dest;
}
template char *BG_StringAlloc< moduleType::game >( const char * );
template char *BG_StringAlloc< moduleType::cgame >( const char * );
template char *BG_StringAlloc< moduleType::ui >( const char * );

template< moduleType mod >
qboolean BG_OutOfMemory ( void )
{
	return bg_pool< mod >::size >= MAX_POOL_SIZE< mod >::value;
}
template qboolean BG_OutOfMemory< moduleType::game >( void );
template qboolean BG_OutOfMemory< moduleType::cgame >( void );
template qboolean BG_OutOfMemory< moduleType::ui >( void );
