...
void SpewDebugStuffToFile(animation_t *anims)
{
	...
	g_trap->FS_Open("file_of_debug_stuff_SP.txt", &f, FS_WRITE);
	...
	g_trap->FS_Write(BGPAFtext, strlen(BGPAFtext), f);
	g_trap->FS_Close(f);
}
#endif
...
{
	*animFileIndex = BG_ParseAnimationFile< moduleType::game >(filename, NULL, qfalse);
...
void NPC_PrecacheAnimationCFG( const char *NPC_type )
{
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( token, "playerModel" ) )
		{
			...
			int handle = g_trap->G2API_PrecacheGhoul2Model( va( "models/players/%s/model.glm", value ) );
			if ( handle > 0 )//FIXME: isn't 0 a valid handle?
			{
				GLAName = g_trap->G2API_GetAnimFileNameIndex( handle );
				if ( GLAName )
				{
...
void NPC_PrecacheWeapons( team_t playerTeam, int spawnflags, char *NPCtype )
{
	...
	for ( int curWeap = WP_SABER; curWeap < WP_NUM_WEAPONS; curWeap++ )
	{
		if ( (weapons & ( 1 << curWeap )) )
		{
			...
			g_trap->G2API_PrecacheGhoul2Model( weaponModel ); // correct way is item->world_model
		}
	}
#endif
}
...
void NPC_Precache ( gentity_t *spawner )
{
	...
	while ( 1 )
	{
		...
		if ( !Q_stricmp( token, "playerTeam" ) )
		{
			...
			playerTeam = static_cast< npcteam_t >( GetIDForString( TeamTable, tk ) );
			continue;
		}
	...
	//===AI STATS=====================================================================
		...
		//		}
			...
			if ( !Q_stricmp( token, "playerTeam" ) )
			{
				...
				Com_sprintf(tk, sizeof(tk), "NPC%s", token);
				NPC->s.teamowner = NPC->client->playerTeam = static_cast< npcteam_t >( GetIDForString( TeamTable, tk ) );//TranslateTeamName(value);
				continue;
			}
			...
			if ( !Q_stricmp( token, "enemyTeam" ) )
			{
				...
				Com_sprintf(tk, sizeof(tk), "NPC%s", token);
				NPC->client->enemyTeam = static_cast< npcteam_t >( GetIDForString( TeamTable, tk ) );//TranslateTeamName(value);
				continue;
			}
	...
	//===MOVEMENT STATS============================================================
			...
			if ( !Q_stricmp( token, "height" ) )
			{
				...
				{//a flying vehicle's origin must be centered in bbox and it should spawn on the ground
					...
					g_trap->LinkEntity((sharedEntity_t *)NPC);
					//now trace down
					/*
					VectorCopy( NPC->s.origin, bottom );
					bottom[2] -= adjust;
					g_trap->Trace( &tr, NPC->s.origin, NPC->r.mins, NPC->r.maxs, bottom, NPC->s.number, MASK_NPCSOLID );
...
//===MISC===============================================================================
			...
			if ( !Q_stricmp( token, "saber" ) )
			{
				...
				saberName = (char *)BG_TempAlloc< moduleType::game >(4096);//G_NewString( value );
				strcpy(saberName, value);

				WP_SaberParseParms< moduleType::game >( saberName, &NPC->client->saber[0] );
				npcSaber1 = G_ModelIndex(va("@%s", saberName));

				BG_TempFree< moduleType::game >(4096);
				continue;
			}
			...
			if ( !Q_stricmp( token, "saber2" ) )
			{
				...
				{//can't use a second saber if first one is a two-handed saber...?
					char *saberName = (char *)BG_TempAlloc< moduleType::game >(4096);//G_NewString( value );
					strcpy(saberName, value);

					WP_SaberParseParms< moduleType::game >( saberName, &NPC->client->saber[1] );
					if ( (NPC->client->saber[1].saberFlags&SFL_TWO_HANDED) )
					{//tsk tsk, can't use a twoHanded saber as second saber
						WP_RemoveSaber< moduleType::game >( NPC->client->saber, 1 );
					}
					else
					{
...
*/
	if ( !md3Model )
	{
		...
		{ //use "kyle" for a default then
			...
			WP_SaberParseParms< moduleType::game >( DEFAULT_SABER, &NPC->client->saber[0] );
		}
...
//	const char	*filename = "ext_data/NPC2.cfg";
	...
	*marker = 0;
	...
	fileCnt = g_trap->FS_GetFileList("ext_data/NPCs", ".npc", npcExtensionListBuf, sizeof(npcExtensionListBuf) );
...
//		Com_Printf( "Parsing %s\n", holdChar );
		...
		len = g_trap->FS_Open(va( "ext_data/NPCs/%s", holdChar), &f, FS_READ);
		...
		else
		{
			if ( totallen + len >= MAX_NPC_DATA_SIZE ) {
				g_trap->Error( ERR_DROP, "NPC extensions (*.npc) are too large" );
			}
			g_trap->FS_Read(npcParseBuffer, len, f);
