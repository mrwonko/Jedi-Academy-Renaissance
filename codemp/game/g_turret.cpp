...
//----------------------------------------------------------------
{
	...
	if ( (g_trap->PointContents( start, ent->s.number )&MASK_SHOT) )
	{
		return;
	}
...
//-----------------------------------------------------
{
	...
	if ( self->enemy && self->setTime < level.time && self->attackDebounceTime < level.time )
	{
		...
		g_trap->G2API_GetBoltMatrix( self->ghoul2, self->playerModel,
					self->torsoBolt,
		...
		g_trap->G2API_GiveMeVectorFromMatrix( boltMatrix, POSITIVE_Y, fwd );
		*/
...
//-----------------------------------------------------
{
	...
	BG_EvaluateTrajectory< moduleType::game >( &top->s.apos, level.time, top->r.currentAngles );
	...
	else if ( self->enemy )
	{
		...
		g_trap->G2API_GiveMeVectorFromMatrix( boltMatrix, ORIGIN, org2 );
		*/
...
//-----------------------------------------------------
{
	...
	{//FIXME: remain single-minded or look for a new enemy every now and then?
		if ( self->enemy->health > 0 )
		{
			...
			{
				// was in valid radius
				if ( g_trap->InPVS( self->r.currentOrigin, self->enemy->r.currentOrigin ) )
				{
...
//-----------------------------------------------------
{
	...
	//base->playerModel = g_trap->G2API_InitGhoul2Model( base->ghoul2, "models/map_objects/imp_mine/turret_canon.glm", base->s.modelindex );
	//base->s.radius = 80.0f;

	//g_trap->G2API_SetBoneAngles( &base->ghoul2[base->playerModel], "Bone_body", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL );
	//base->torsoBolt = g_trap->G2API_AddBolt( &base->ghoul2[base->playerModel], "*flash03" );
	...
	g_trap->LinkEntity( (sharedEntity_t *)base );
...
qboolean turret_base_spawn_top( gentity_t *base )
{
	...
	g_trap->LinkEntity( (sharedEntity_t *)top );
	return qtrue;
}
