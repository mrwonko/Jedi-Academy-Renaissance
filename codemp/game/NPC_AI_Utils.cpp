...
int	AI_GetGroupSize( vec3_t origin, int radius, team_t playerTeam, gentity_t *avoid )
{
	...
	numEnts = g_trap->EntitiesInBox( mins, maxs, radiusEnts, MAX_RADIUS_ENTS );
...
int AI_ClosestGroupEntityNumToPoint( AIGroupInfo_t *group, vec3_t point )
{
	...
	{
		cost = g_trap->Nav_GetPathCost( group->member[i].waypoint, markerWP );
		if ( cost < bestCost )
		{
			bestCost = cost;
...
void AI_SortGroupByPathCostToEnemy( AIGroupInfo_t *group )
{
	...
	for ( i = 0; i < group->numGroup; i++ )
	{
		...
		{//FIXME: just use member->waypoint?
			...
			{
				group->member[i].pathCostToEnemy = g_trap->Nav_GetPathCost( group->member[i].waypoint, group->enemyWP );
...
qboolean AI_ValidateNoEnemyGroupMember( AIGroupInfo_t *group, gentity_t *member )
{
	...
	if ( !g_trap->InPVS( member->r.currentOrigin, center ) )
...
//		member->client->ps.weapon == WP_BOT_LASER ||		// Probe droid	- Laser blast
	...
	if ( member->enemy != group->enemy )
	{
		...
		if ( !g_trap->InPVS( member->r.currentOrigin, group->enemy->r.currentOrigin ) )
		{//not within PVS of the group enemy
			return qfalse;
		}
	}
...
//	for ( i = 0, member = &g_entities[0]; i < globals.num_entities ; i++, member++)
	...
	for ( i = 0; i < numWaiters; i++ )
	{
		...
		for ( j = 0; j < self->NPC->group->numGroup; j++ )
		{
			...
			if ( g_trap->InPVS( waiter->r.currentOrigin, member->r.currentOrigin ) )
...
qboolean AI_RefreshGroup( AIGroupInfo_t *group )
{
	...
	for ( i = 0; i < group->numGroup; i++ )
	{
		...
		if ( j < group->numGroup )
		{//found a dupe!
			g_trap->Printf( S_COLOR_RED"ERROR: member %s(%d) a duplicate group member!!!\n", g_entities[group->member[i].number].targetname, group->member[i].number );
