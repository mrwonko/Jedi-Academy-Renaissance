...
qboolean NPC_CheckEnemyStealth( gentity_t *target )
{
	...
	if ( clearLOS )
	{
		...
		contents = g_trap->PointContents( targ_org, target->s.number );
		if ( contents&CONTENTS_WATER )
		{
			int myContents = g_trap->PointContents( NPCS.NPC->client->renderInfo.eyePoint, NPCS.NPC->s.number );
	...
	*/
	...
	if ( level.alertEvents[eventID].level > AEL_MINOR && NPCS.NPCInfo->investigateCount > 1 && (NPCS.NPCInfo->scriptFlags&SCF_CHASE_ENEMIES) )
	{
		...
		{//we were able to move the investigateGoal to a point in which our bbox would fit
			...
			g_trap->Trace( &trace, NPCS.NPCInfo->investigateGoal, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, end, ENTITYNUM_NONE, ((NPCS.NPC->clipmask&~CONTENTS_BODY)|CONTENTS_BOTCLIP), qfalse, 0, 0 );
...
static void ST_CheckMoveState( void )
{
	...
	if ( ( NPCS.NPCInfo->goalEntity != NPCS.NPC->enemy ) && ( NPCS.NPCInfo->goalEntity != NULL ) )
	{
		...
		if ( NAV_HitNavGoal( NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPCInfo->goalEntity->r.currentOrigin, 16, FlyingCreature( NPCS.NPC ) ) ||
			( !g_trap->ICARUS_TaskIDPending( (sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV ) && NPCS.NPCInfo->squadState == SQUAD_SCOUT && enemyLOS && enemyDist <= 10000 ) )
...
void ST_TransferMoveGoal( gentity_t *self, gentity_t *other )
{
	if ( g_trap->ICARUS_TaskIDPending( (sharedEntity_t *)self, TID_MOVE_NAV ) )
...
//	qboolean	scouting = qfalse;
	...
	{//dissolve the group
		...
		for ( i = 0; i < group->numGroup; i++ )
		{
			...
			else if ( group->enemy->waypoint == WAYPOINT_NONE || (g_trap->Nav_GetPathCost( NPCS.NPC->waypoint, group->enemy->waypoint ) >= Q3_INFINITE) )
			{
	...
	*/
	...
	//	scouting = qfalse;
		...
		{//not already retreating
			...
			if ( TIMER_Done( NPCS.NPC, "roamTime" ) && TIMER_Done( NPCS.NPC, "hideTime" ) && NPCS.NPC->health > 10 && !g_trap->InPVS( group->enemy->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
			{//cant even see enemy
				//better go after him
			...
			{//not hit, see if there are other reasons we should run
				if ( g_trap->InPVS( NPCS.NPC->r.currentOrigin, group->enemy->r.currentOrigin ) )
...
void NPC_BSST_Attack( void )
{
	...
	{//I belong to a squad of guys - we should *always* have a group
		...
		{//I'm the first ent in my group, I'll make the command decisions
			...
			if ( commTime > 20 )
			{
				g_trap->Printf( S_COLOR_RED"ERROR: Commander time: %d\n", commTime );
			}
			else if ( commTime > 10 )
			{
				g_trap->Printf( S_COLOR_YELLOW"WARNING: Commander time: %d\n", commTime );
			}
			else if ( commTime > 2 )
			{
				g_trap->Printf( S_COLOR_GREEN"Commander time: %d\n", commTime );
			}
#endif//	AI_TIMERS
		}
	}
