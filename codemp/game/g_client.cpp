...
void ThrowSaberToAttacker(gentity_t *self, gentity_t *attacker)
{
	...
	g_trap->SetConfigstring ( CS_CLIENT_JEDIMASTER, "-1" );
	...
	if (!attacker || !attacker->client)
	{
		...
		ent->pos2[0] = 0;
		g_trap->LinkEntity((sharedEntity_t *)ent);
		return;
	}
...
//	gentity_t *te;
	...
	g_trap->SetConfigstring ( CS_CLIENT_JEDIMASTER, va("%i", other->s.number ) );
...
qboolean SpotWouldTelefrag( gentity_t *spot ) {
	...
	num = g_trap->EntitiesInBox( mins, maxs, touch, MAX_GENTITIES );
...
gentity_t *SelectRandomFurthestSpawnPoint ( vec3_t avoidPoint, vec3_t origin, vec3_t angles, team_t team, qboolean isbot ) {
	...
	{//couldn't find any of the above
		...
		if (!numSpots) {
			...
			if (!spot)
				g_trap->Error( ERR_DROP, "Couldn't find a spawn point" );
...
void BodySink( gentity_t *ent ) {
	if ( level.time - ent->timestamp > BODY_SINK_TIME + 2500 ) {
		...
		g_trap->UnlinkEntity( (sharedEntity_t *)ent );
...
static qboolean CopyToBodyQue( gentity_t *ent ) {
	...
	contents = g_trap->PointContents( ent->s.origin, -1 );
	...
	g_trap->UnlinkEntity ((sharedEntity_t *)body);
	body->s = ent->s;
	...
	g_trap->SendServerCommand(-1, va("ircg %i %i %i %i", ent->s.number, body->s.number, body->s.weapon, islight));
	...
	g_trap->LinkEntity ((sharedEntity_t *)body);
...
{ //do whatever should be done taking ragdoll and dismemberment states into account.
	...
	{ //bodyque func didn't manage to call ircg so call this to assure our limbs and ragdoll states are proper on the client.
		g_trap->SendServerCommand(-1, va("rcg %i", ent->s.clientNum));
	}
}
...
void G_DebugWrite(const char *path, const char *text)
{
	...
	g_trap->FS_Open( path, &f, FS_APPEND );
	g_trap->FS_Write(text, strlen(text), f);
	g_trap->FS_Close(f);
}
#endif
...
qboolean G_SaberModelSetup(gentity_t *ent)
{
	...
	while (i < MAX_SABERS)
	{
		if (ent->client->saber[i].model[0])
		{
			...
			{
				g_trap->G2API_CleanGhoul2Models(&(ent->client->weaponGhoul2[i]));
			}
			g_trap->G2API_InitGhoul2Model(&ent->client->weaponGhoul2[i], ent->client->saber[i].model, 0, 0, -20, 0, 0);
			...
			if (ent->client->weaponGhoul2[i])
			{
				...
				{
					g_trap->G2API_SetSkin(ent->client->weaponGhoul2[i], 0, ent->client->saber[i].skin, ent->client->saber[i].skin);
				}

				if (ent->client->saber[i].saberFlags & SFL_BOLT_TO_WRIST)
				{
					g_trap->G2API_SetBoltInfo(ent->client->weaponGhoul2[i], 0, 3+i);
				}
				else
				{ // bolt to right hand for 0, or left hand for 1
					g_trap->G2API_SetBoltInfo(ent->client->weaponGhoul2[i], 0, i);
				}
				...
				while (j < ent->client->saber[i].numBlades)
				{
					...
					tagBolt = g_trap->G2API_AddBolt(ent->client->weaponGhoul2[i], 0, tagName);

					if (tagBolt == -1)
					{
						if (j == 0)
						{ //guess this is an 0ldsk3wl saber
							tagBolt = g_trap->G2API_AddBolt(ent->client->weaponGhoul2[i], 0, "*flash");
				...
				g_trap->G2API_CopySpecificGhoul2Model(ent->client->weaponGhoul2[i], 0, ent->ghoul2, i+1);
			}
		}
		else
		{
...
void SetupGameGhoul2Model(gentity_t *ent, char *modelname, char *skinName)
{
	...
	if (ent->ghoul2 && re->G2API_HaveWeGhoul2Models(*ent->ghoul2))
	{
		re->G2API_CleanGhoul2Models(&(ent->ghoul2));
	}
	...
	if (!precachedKyle)
	{
		...
		handle = re->G2API_InitGhoul2Model(&precachedKyle, afilename, 0, 0, -20, 0, 0);
		...
		defSkin = g_trap->R_RegisterSkin("models/players/kyle/model_default.skin");
		re->G2API_SetSkin(*precachedKyle, 0, defSkin, defSkin);
	}

	if (precachedKyle && re->G2API_HaveWeGhoul2Models(*precachedKyle))
	{
		...
		{ //rww - allow option for perplayer models on server for collision and bolt stuff.
			...
			if ( ent->client->NPC_class == CLASS_VEHICLE )
			{
				...
				BG_GetVehicleModelName< moduleType::game >(realModelName, modelname, sizeof( realModelName ));
				...
				{
					skinHandle = g_trap->R_RegisterSkin(va("models/players/%s/model_%s.skin", realModelName, ent->m_pVehicle->m_pVehicleInfo->skin));
				}
				else
				{
					skinHandle = g_trap->R_RegisterSkin(va("models/players/%s/model_default.skin", realModelName));
				}
			}
			else
			{
			...
			if (skin[0])
			{
				...
				skinHandle = g_trap->R_RegisterSkin(useSkinName);
			}

			strcpy(modelFullPath, va("models/players/%s/model.glm", truncModelName));
			handle = re->G2API_InitGhoul2Model(&ent->ghoul2, modelFullPath, 0, skinHandle, -20, 0, 0);

			if (handle<0)
			{ //Huh. Guess we don't have this model. Use the default.

				if (ent->ghoul2 && g_trap->G2API_HaveWeGhoul2Models(ent->ghoul2))
				{
					re->G2API_CleanGhoul2Models(&(ent->ghoul2));
				}
				ent->ghoul2 = NULL;
				re->G2API_DuplicateGhoul2Instance(*precachedKyle, &ent->ghoul2);
			}
			else
			{
				g_trap->G2API_SetSkin(ent->ghoul2, 0, skinHandle, skinHandle);

				GLAName[0] = 0;
				g_trap->G2API_GetGLAName( ent->ghoul2, 0, GLAName);
	...
	re->G2API_AttachInstanceToEntNum(*ent->ghoul2, ent->s.number, qtrue);
	...
	if (!BGPAFtextLoaded)
	{
		if (BG_ParseAnimationFile< moduleType::game >("models/players/_humanoid/animation.cfg", bgHumanoidAnimations, qtrue) == -1)
		{
	...
	if (ent->s.number >= MAX_CLIENTS || G_PlayerHasCustomSkeleton(ent))
	{
		...
		{ //it doesn't use humanoid anims.
			...
			if ( slash )
			{
				...
				ent->localAnimIndex = BG_ParseAnimationFile< moduleType::game >(GLAName, NULL, qfalse);
			}
		}
		else
		{ //humanoid index.
	...
	{ //do special vehicle stuff
		...
		i = g_trap->G2API_AddBolt( ent->ghoul2, 0, "model_root" );

		// Setup the droid unit.
		ent->m_pVehicle->m_iDroidUnitTag = g_trap->G2API_AddBolt( ent->ghoul2, 0, "*droidunit" );

		// Setup the Exhausts.
		for ( i = 0; i < MAX_VEHICLE_EXHAUSTS; i++ )
		{
			Com_sprintf( strTemp, 128, "*exhaust%i", i + 1 );
			ent->m_pVehicle->m_iExhaustTag[i] = g_trap->G2API_AddBolt( ent->ghoul2, 0, strTemp );
		}
		...
		for ( i = 0; i < MAX_VEHICLE_MUZZLES; i++ )
		{
			...
			ent->m_pVehicle->m_iMuzzleTag[i] = g_trap->G2API_AddBolt( ent->ghoul2, 0, strTemp );
		...
		for ( i = 0; i < MAX_VEHICLE_TURRET_MUZZLES; i++ )
		{
			...
			{
				ent->m_pVehicle->m_iGunnerViewTag[i] = g_trap->G2API_AddBolt( ent->ghoul2, 0, ent->m_pVehicle->m_pVehicleInfo->turret[i].gunnerViewTag );
			}
			else
			{
	...
	{ //a player or NPC saber user
		g_trap->G2API_AddBolt(ent->ghoul2, 0, "*r_hand");
		g_trap->G2API_AddBolt(ent->ghoul2, 0, "*l_hand");

		//rhand must always be first bolt. lhand always second. Whichever you want the
		//jetpack bolted to must always be third.
		g_trap->G2API_AddBolt(ent->ghoul2, 0, "*chestg");

		//claw bolts
		g_trap->G2API_AddBolt(ent->ghoul2, 0, "*r_hand_cap_r_arm");
		g_trap->G2API_AddBolt(ent->ghoul2, 0, "*l_hand_cap_l_arm");

		g_trap->G2API_SetBoneAnim(ent->ghoul2, 0, "model_root", 0, 12, BONE_ANIM_OVERRIDE_LOOP, 1.0f, level.time, -1, -1);
		g_trap->G2API_SetBoneAngles(ent->ghoul2, 0, "upper_lumbar", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, level.time);
		g_trap->G2API_SetBoneAngles(ent->ghoul2, 0, "cranium", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, NULL, 0, level.time);

		if (!g2SaberInstance)
		{
			g_trap->G2API_InitGhoul2Model(&g2SaberInstance, "models/weapons2/saber/saber_w.glm", 0, 0, -20, 0, 0);

			if (g2SaberInstance)
			{
				// indicate we will be bolted to model 0 (ie the player) on bolt 0 (always the right hand) when we get copied
				g_trap->G2API_SetBoltInfo(g2SaberInstance, 0, 0);
				// now set up the gun bolt on it
				g_trap->G2API_AddBolt(g2SaberInstance, 0, "*blade1");
			}
		}
		...
		if (G_SaberModelSetup(ent))
		{
			if (g2SaberInstance)
			{
				g_trap->G2API_CopySpecificGhoul2Model(g2SaberInstance, 0, ent->ghoul2, 1);
			}
		}
	}
	...
	{ //some extra NPC stuff
		if (g_trap->G2API_AddBolt(ent->ghoul2, 0, "lower_lumbar") == -1)
...
The game can override any of the settings and call g_trap->SetUserinfo
if desired.
============
*/
...
void Svcmd_ToggleUserinfoValidation_f( void ) {
	if ( g_trap->Argc() == 1 ) {
		...
		for ( i=0; i<numUserinfoFields; i++ ) {
			if ( (g_userinfoValidate.integer & (1<<i)) )	g_trap->Print( "%2d [X] %s\n", i, userinfoFields[i].fieldClean );
			else											g_trap->Print( "%2d [ ] %s\n", i, userinfoFields[i].fieldClean );
		}
		...
		g_trap->Cvar_Set( "g_userinfoValidate", va( "%i", (1 << index) ^ (g_userinfoValidate.integer & ((1 << (numUserinfoFields + USERINFO_VALIDATION_MAX)) - 1)) ) );
		g_trap->Cvar_Update( &g_userinfoValidate );
...
qboolean ClientUserinfoChanged( int clientNum ) {
	...
	g_trap->GetUserinfo( clientNum, userinfo, sizeof( userinfo ) );
	...
	if ( s && *s ) {
		...
		g_trap->DropClient( clientNum, va( "Failed userinfo validation: %s", s ) );
...
//	teamTask = atoi(Info_ValueForKey(userinfo, "teamtask"));
	...
	if ( atoi( s ) < game::sv_fps.integer )
		g_trap->SendServerCommand( clientNum, va( "print \"" S_COLOR_YELLOW "Recommend setting /snaps %d or higher to match this server's sv_fps\n\"", game::sv_fps.integer ) );
	...
	g_trap->GetConfigstring( CS_PLAYERS+clientNum, oldClientinfo, sizeof( oldClientinfo ) );
	g_trap->SetConfigstring( CS_PLAYERS+clientNum, buf );
...
char *ClientConnect( int clientNum, qboolean firstTime, qboolean isBot ) {
	...
	{ //if this is the first time then auto-assign a desired siege team and show briefing for that team
		...
		/*
		g_trap->SendServerCommand(ent->s.number, va("sb %i", client->sess.siegeDesiredTeam));
		*/
...
//	if ( !client->areabits )
//		client->areabits = G_Alloc( (g_trap->AAS_PointReachabilityAreaIndex( NULL ) + 7) / 8 );
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	if ((ent->r.svFlags & SVF_BOT) && level.gametype >= GT_TEAM)
	{
		if (allowTeamReset)
		{
			...
			g_trap->GetUserinfo(clientNum, userinfo, MAX_INFO_STRING);
			...
			ent->client->sess.sessionTeam = static_cast< team_t >( preSess );
	...
	while (i < NUM_FORCE_POWERS)
	{
		...
		{
			WP_ForcePowerStop(ent, static_cast< forcePowers_t >( i ) );
		}
		i++;
	}
...
void G_UpdateClientAnims(gentity_t *self, float animSpeedScale)
{
	...
	{
		g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "model_root", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame+1, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
		g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame+1, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
		g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "Motion", self->client->ps.saberLockFrame, self->client->ps.saberLockFrame+1, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND, animSpeedScale, level.time, -1, 150);
		return;
	}
	...
	if (self->client->legsAnimExecute != legsAnim || self->client->legsLastFlip != self->client->ps.legsFlip)
	{
		...
		g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "model_root", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
...
tryTorso:
		...
		!self->noLumbar)
	{
		...
		BG_SaberStartTransAnim< moduleType::game >(self->s.number, self->client->ps.fd.saberAnimLevel, self->client->ps.weapon, f, &animSpeedScale, self->client->ps.brokenLimbs);
		...
		g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "lower_lumbar", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, /*firstFrame why was it this before?*/-1, 150);
	...
	{ //only set the motion bone for humanoids.
		g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "Motion", firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
	}
		...
		setTorso)
	{
		...
		{ //broken left arm
			...
			g_trap->G2API_SetBoneAnim(self->ghoul2, 0, brokenBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, level.time, -1, 150);
		}
		...
		{ //broken right arm
				...
				(!self->client->saber[1].model[0] || self->client->ps.weapon != WP_SABER))
			{
				...
				{ //we want to keep the broken bone updated for some cases
					g_trap->G2API_SetBoneAnim(self->ghoul2, 0, brokenBone, firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
				}
					...
					(self->client->ps.torsoAnim == TORSO_WEAPONREADY2 || self->client->ps.torsoAnim == BOTH_ATTACK2 || self->client->ps.weapon < WP_BRYAR_PISTOL))
				{
					...
					g_trap->G2API_SetBoneAnim(self->ghoul2, 0, supportBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, level.time, -1, 150);
				}
				else
				{ //we want to keep the support bone updated for some cases
					g_trap->G2API_SetBoneAnim(self->ghoul2, 0, supportBone, firstFrame, lastFrame, aFlags, lAnimSpeedScale, level.time, -1, 150);
				}
			}
			else
		...
		{ //remove the bone now so it can be set again
			...
			{ //can only have one arm broken at once.
				...
				g_trap->G2API_SetBoneAnim(self->ghoul2, 0, "lhumerus", 0, 1, 0, 0, level.time, -1, 0);
				g_trap->G2API_RemoveBone(self->ghoul2, "lhumerus", 0);
			}

			assert(brokenBone);

			//Set the flags and stuff to 0, so that the remove will succeed
			g_trap->G2API_SetBoneAnim(self->ghoul2, 0, brokenBone, 0, 1, 0, 0, level.time, -1, 0);

			//Now remove it
			g_trap->G2API_RemoveBone(self->ghoul2, brokenBone, 0);
...
void ClientSpawn(gentity_t *ent) {
	...
	g_trap->GetUserinfo( index, userinfo, sizeof( userinfo ) );
	...
	{ //make sure our new info is sent out to all the other clients, and give us a valid stance
		...
		for ( i=0; i<MAX_SABERS; i++ )
		{
			...
			{// they don't match up, force the user info
				...
				g_trap->SetUserinfo( ent->s.number, userinfo );
			}
		}
...
//	client->areabits = savedAreaBits;
	...
	{//jediVmerc is incompatible with this gametype, turn it off!
		g_trap->Cvar_Set( "g_jediVmerc", "0" );
		g_trap->Cvar_Update( &g_jediVmerc );
		if (level.gametype == GT_HOLOCRON)
		{
...
*/
	...
	g_trap->GetUsercmd( client - level.clients, &ent->client->pers.cmd );
	...
	{ //Imperial (team1) team is allied with "enemy" NPCs in this mode
		...
		{
			client->playerTeam = static_cast< npcteam_t >( ent->s.teamowner = NPCTEAM_ENEMY );
			client->enemyTeam = NPCTEAM_PLAYER;
		}
		else
		{
			client->playerTeam = static_cast< npcteam_t >( ent->s.teamowner = NPCTEAM_PLAYER );
	...
	*/
	...
	g_trap->ICARUS_FreeEnt( (sharedEntity_t *)ent );
	g_trap->ICARUS_InitEnt( (sharedEntity_t *)ent );
}
...
call g_trap->DropClient(), which will call this and do
	...
	if ( level.voteTime ) {
		if ( ent->client->mGameFlags & PSG_VOTED ) {
			if ( ent->client->pers.vote == 1 ) {
				level.voteYes--;
				g_trap->SetConfigstring( CS_VOTE_YES, va( "%i", level.voteYes ) );
			}
			else if ( ent->client->pers.vote == 2 ) {
				level.voteNo--;
				g_trap->SetConfigstring( CS_VOTE_NO, va( "%i", level.voteNo ) );
			}
		}
	...
	if ( level.teamVoteTime[voteteam] ) {
		if ( ent->client->mGameFlags & PSG_TEAMVOTED ) {
			if ( ent->client->pers.teamvote == 1 ) {
				...
				g_trap->SetConfigstring( CS_TEAMVOTE_YES, va( "%i", level.teamVoteYes[voteteam] ) );
			}
			else if ( ent->client->pers.teamvote == 2 ) {
				level.teamVoteNo[voteteam]--;
				g_trap->SetConfigstring( CS_TEAMVOTE_NO, va( "%i", level.teamVoteNo[voteteam] ) );
			}
		}
...
void ClientDisconnect( int clientNum ) {
	...
	if ( level.gametype == GT_DUEL && ent->client->sess.sessionTeam == TEAM_FREE && level.intermissiontime ) {
		g_trap->SendConsoleCommand( EXEC_APPEND, "map_restart 0\n" );
	...
	while (i < MAX_SABERS)
	{
		if (ent->client->weaponGhoul2[i] && g_trap->G2API_HaveWeGhoul2Models(ent->client->weaponGhoul2[i]))
		{
			g_trap->G2API_CleanGhoul2Models(&ent->client->weaponGhoul2[i]);
		}
		i++;
	...
	g_trap->SetConfigstring( CS_PLAYERS + clientNum, "");
