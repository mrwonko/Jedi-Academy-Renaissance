...
void Use_Target_Give( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	while ( (t = G_Find (t, FOFS(targetname), ent->target)) != NULL ) {
		...
		t->nextthink = 0;
		g_trap->UnlinkEntity( (sharedEntity_t *)t );
	}
}
...
void Use_Target_Print (gentity_t *ent, gentity_t *other, gentity_t *activator)
{
	...
	{//private, to one client only
		...
		{//make sure there's a valid client ent to send it to
			...
			{
				g_trap->SendServerCommand( activator-g_entities, va("cps \"%s\"", ent->message ));
			}
			else
			{
				g_trap->SendServerCommand( activator-g_entities, va("cp \"%s\"", ent->message ));
			}
		}
	...
	if (ent->message[0] == '@' && ent->message[1] != '@')
	{
		g_trap->SendServerCommand( -1, va("cps \"%s\"", ent->message ));
	}
	else
	{
		g_trap->SendServerCommand( -1, va("cp \"%s\"", ent->message ));
	}
}
...
void SP_target_speaker( gentity_t *ent ) {
	...
	if ( !G_SpawnString( "noise", "NOSOUND", &s ) ) {
		g_trap->Error( ERR_DROP, "target_speaker without a noise key at %s", vtos( ent->s.origin ) );
	}
...
void target_laser_think (gentity_t *self) {
	...
	g_trap->Trace( &tr, self->s.origin, NULL, NULL, end, self->s.number, CONTENTS_SOLID|CONTENTS_BODY|CONTENTS_CORPSE, qfalse, 0, 0);
...
{
	g_trap->UnlinkEntity( (sharedEntity_t *)self );
	self->nextthink = 0;
}
...
void target_laser_start (gentity_t *self)
{
	...
	if (self->target) {
		...
		if (!ent) {
			g_trap->Print ("%s at %s: %s is a bad target\n", self->classname, vtos(self->s.origin), self->target);
		}
		self->enemy = ent;
	} else {
...
void target_teleporter_use( gentity_t *self, gentity_t *other, gentity_t *activator ) {
	...
	if (!dest) {
		g_trap->Print ("Couldn't find teleporter destination\n");
		return;
	}
...
void SP_target_teleporter( gentity_t *self ) {
	if (!self->targetname)
		g_trap->Print("untargeted %s at %s\n", self->classname, vtos(self->s.origin));
...
void SP_target_location( gentity_t *self ) {
	if ( self->targetname && self->targetname[0] ) {
		...
		if ( !self->message ) {
			g_trap->Print( "target_location with no message at %s\n", vtos( self->s.origin ) );
			G_FreeEntity( self );
			return;
		}
		...
		if ( level.locations.num >= MAX_LOCATIONS ) {
			if ( !didwarn ) {
				g_trap->Print( "Maximum target_locations hit (%d)! Remaining locations will be removed.\n", MAX_LOCATIONS );
				didwarn = qtrue;
			}
			G_FreeEntity( self );
			return;
...
void target_counter_use( gentity_t *self, gentity_t *other, gentity_t *activator )
{
	...
	if ( self->count )
	{
		if ( self->target2 )
		{
			//g_trap->Printf("target_counter %s firing target2 from %s, entnum %d\n", self->targetname, activator->targetname, activator->s.number );
...
void target_random_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	//g_trap->Printf("target_random %s used by %s (entnum %d)\n", self->targetname, activator->targetname, activator->s.number );
	G_ActivateBehavior(self,BSET_USE);
	...
	while ( (t = G_Find (t, FOFS(targetname), self->target)) != NULL )
	{
		...
		if (t == self)
		{
//				g_trap->Printf ("WARNING: Entity used itself.\n");
		}
	...
	*/
	...
	if (self->behaviorSet[BSET_USE])
	{
		if ( self->spawnflags & 1 )
		{
			...
			if (!g_trap->ICARUS_IsInitialized(self->s.number))
			{//Need to be initialized through ICARUS
				...
				if ( g_trap->ICARUS_ValidEnt( (sharedEntity_t *)self->activator ) )
				{
					g_trap->ICARUS_InitEnt( (sharedEntity_t *)self->activator );
				}
				else
				{
					if (developer.integer)
			...
			g_trap->ICARUS_RunScript( (sharedEntity_t *)self->activator, va( "%s/%s", Q3_SCRIPT_DIR, self->behaviorSet[BSET_USE] ) );
		}
		else
		{
...
void target_level_change_use(gentity_t *self, gentity_t *other, gentity_t *activator)
{
	...
	g_trap->SendConsoleCommand(EXEC_NOW, va("map %s", self->message));
}
...
void SP_target_level_change( gentity_t *self )
{
	...
	{
		g_trap->Error( ERR_DROP, "target_level_change with no mapname!\n");
		return;
	}
...
{
	G_ActivateBehavior(self,BSET_USE);
	g_trap->SetConfigstring( CS_MUSIC, self->message );
}
...
void SP_target_play_music( gentity_t *self )
{
	...
	{
		g_trap->Error( ERR_DROP, "target_play_music without a music key at %s", vtos( self->s.origin ) );
	}
