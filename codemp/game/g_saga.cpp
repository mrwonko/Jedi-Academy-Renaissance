			...
			if (scl)
			{
				...
				while (j < WP_NUM_WEAPONS)
				{
					...
					{ //we use this weapon so register it.
						RegisterItem(BG_FindItemForWeapon(static_cast< weapon_t >( j )));
					}
					j++;
				}
				j = 0;
				while (j < HI_NUM_HOLDABLE)
				{
					...
					{ //we use this item so register it.
						RegisterItem(BG_FindItemForHoldable(static_cast< holdable_t >( j )));
					}
					j++;
				}
			}
...
{
	g_trap->SetConfigstring(CS_SIEGE_WINTEAM, va("%i", team));
}
...
//	int				j = 0;
	...
	{
		g_trap->SiegePersGet(&g_siegePersistant);
		if (g_siegePersistant.beatingTime)
		{
			g_trap->SetConfigstring(CS_SIEGE_TIMEOVERRIDE, va("%i", g_siegePersistant.lastTime));
		}
		else
		{
			g_trap->SetConfigstring(CS_SIEGE_TIMEOVERRIDE, "0");
		}
	}
	else
	{ //hmm, ok, nothing.
	...
	len = g_trap->FS_Open(levelname, &f, FS_READ);
	...
	g_trap->FS_Read(siege_info, len, f);
	...
	{
		g_trap->Error( ERR_DROP, "Siege teams not defined" );
	}
	...
	if (BG_SiegeGetValueGroup(siege_info, team2, gParseObjectives))
	{
		...
		{
			g_trap->Cvar_Set( "team2_icon", teamIcon);
		}
	...
	if (BG_SiegeGetValueGroup(siege_info, team1, gParseObjectives))
	{
		...
		if (BG_SiegeGetPairedValue(gParseObjectives, "TeamIcon", teamIcon))
		{
			g_trap->Cvar_Set( "team1_icon", teamIcon);
		}
	...
	BG_SiegeLoadClasses< moduleType::game >(NULL);

	if (!bgNumSiegeClasses)
	{ //We didn't find any?!
		g_trap->Error( ERR_DROP, "Couldn't find any player classes for Siege" );
	}
	...
	*/
	...
	BG_SiegeLoadTeams< moduleType::game >();

	if (!bgNumSiegeTeams)
	{ //React same as with classes.
		g_trap->Error( ERR_DROP, "Couldn't find any player teams for Siege" );
	}
	...
	g_trap->SetConfigstring(CS_SIEGE_OBJECTIVES, gObjectiveCfgStr);

	//precache saber data for classes that use sabers on both teams
	BG_PrecacheSabersForSiegeTeam< moduleType::game >(SIEGETEAM_TEAM1);
	BG_PrecacheSabersForSiegeTeam< moduleType::game >(SIEGETEAM_TEAM2);
...
void BroadcastObjectiveCompletion(int team, int objective, int final, int client)
{
	...
	//g_trap->Print("Broadcast goal completion team %i objective %i final %i\n", team, objective, final);
}
...
void SiegeClearSwitchData(void)
{
	...
	g_trap->SiegePersSet(&g_siegePersistant);
}
...
void SiegeRoundComplete(int winningteam, int winningclient)
{
	...
	g_trap->SetConfigstring(CS_SIEGE_STATE, va("3|%i", level.time)); //ended
...
void SetTeamQuick(gentity_t *ent, int team, qboolean doBegin)
{
	...
	g_trap->GetUserinfo( ent->s.number, userinfo, sizeof( userinfo ) );
	...
	ent->client->sess.sessionTeam = static_cast< team_t >( team );
...
{ //entNum is just used as something to fire targets from.
	...
	g_trap->SetConfigstring(CS_SIEGE_STATE, va("0|%i", level.time)); //we're ready to g0g0g0
}
...
void SiegeCheckTimers(void)
{
	...
	if (!gSiegeRoundBegun)
	{
		...
		{ //don't have people on both teams yet.
			...
			g_trap->SetConfigstring(CS_SIEGE_STATE, "1"); //"waiting for players on both teams"
		}
		...
		{
			g_trap->SetConfigstring(CS_SIEGE_STATE, va("2|%i", gSiegeBeginTime - SIEGE_ROUND_BEGIN_TIME)); //getting ready to begin
		}
	}
}
...
void SP_info_siege_objective (gentity_t *ent)
{
	...
	{ //j00 fux0red something up
		G_FreeEntity(ent);
		g_trap->Print("ERROR: info_siege_objective without an objective or side value\n");
		return;
	}
...
void SP_info_siege_decomplete (gentity_t *ent)
{
	...
	{ //j00 fux0red something up
		G_FreeEntity(ent);
		g_trap->Print("ERROR: info_siege_objective_decomplete without an objective or side value\n");
		return;
	}
}
...
void SiegeItemThink(gentity_t *ent)
{
	...
	if (carrier) {
		...
		{ //The carrier died so pop out where he is (unless in nodrop).
			...
			if ( g_trap->PointContents(carrier->client->ps.origin, carrier->s.number) & CONTENTS_NODROP )
			...
			else
			{
				...
				trace_t tr;
				g_trap->Trace(&tr, carrier->client->ps.origin, ent->r.mins, ent->r.maxs, carrier->client->ps.origin, ent->s.number, ent->clipmask, qfalse, 0, 0);
				...
				{//bad spawning area, try again with the trace up a bit.
					...
					TracePoint[2] += 30;
					g_trap->Trace(&tr, TracePoint, ent->r.mins, ent->r.maxs, TracePoint, ent->s.number, ent->clipmask, qfalse, 0, 0);
...
{ //once used, become active
	...
	{ //want to be on this guy's origin now then
		...
		if (targ && targ->inuse)
		{
			...
			g_trap->Trace(&tr, targ->r.currentOrigin, ent->r.mins, ent->r.maxs, targ->r.currentOrigin, targ->s.number, ent->clipmask, qfalse, 0, 0);
...
void SP_misc_siege_item (gentity_t *ent)
{
	...
	{
		g_trap->Error( ERR_DROP, "You must specify a model for misc_siege_item types." );
	}
...
void G_SiegeClientExData(gentity_t *msgTarg)
{
	...
	while (i < level.num_entities && count < MAX_EXDATA_ENTS_TO_SEND)
	{
		...
		if (ent->inuse && ent->client && msgTarg->s.number != ent->s.number &&
			...
			g_trap->InPVS(msgTarg->client->ps.origin, ent->client->ps.origin))
	...
	g_trap->SendServerCommand(msgTarg-g_entities, str);
}
