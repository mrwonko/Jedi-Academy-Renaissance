...
//			continue;
		...
		{//only do this if they're not already mad at someone
			...
			if ( distSq > 16384 /*128 squared*/ && !g_trap->InPVS( victim->r.currentOrigin, check->r.currentOrigin ) )
...
void player_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	if (self->client->ps.weapon == WP_SABER && self->client->saberKnockedTime)
	{
		...
		//g_trap->Print("DEBUG: Running saber cleanup for %s\n", self->client->pers.netname);
	...
	{ //I'm riding a vehicle
		...
		{
		case CLASS_R2D2:
			if ( !g_trap->G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "head" ) )
			{
				vec3_t	up;
...
void G_GetDismemberBolt(gentity_t *self, vec3_t boltPoint, int limbType)
{
	...
	useBolt = g_trap->G2API_AddBolt(self->ghoul2, 0, rotateBone);
	...
	g_trap->G2API_GetBoltMatrix(self->ghoul2, 0, useBolt, &boltMatrix, properAngles, properOrigin, level.time, NULL, self->modelScale);
...
extern qboolean BG_GetRootSurfNameWithVariant( CGhoul2Info_v&ghoul2, const char *rootSurfName, char *returnSurfName, int returnSize );
...
void G_Dismember( gentity_t *ent, gentity_t *enemy, vec3_t point, int limbType, float limbRollBase, float limbPitchBase, int deathAnim, qboolean postDeath )
{
	...
	if (ent->ghoul2 && limbName[0] && g_trap->G2API_GetSurfaceRenderStatus(ent->ghoul2, 0, limbName))
	...
	*/
	...
	{ //if it's an npc remove these surfs on the server too. For players we don't even care cause there's no further dismemberment after death.
		g_trap->G2API_SetSurfaceOnOff(ent->ghoul2, limbName, 0x00000100);
		g_trap->G2API_SetSurfaceOnOff(ent->ghoul2, stubCapName, 0);
	}
	...
	g_trap->LinkEntity( (sharedEntity_t *)limb );
}
...
qboolean G_GetHitLocFromSurfName( gentity_t *ent, const char *surfName, int *hitLoc, vec3_t point, vec3_t dir, vec3_t bladeDir, int mod )
{
	...
	*hitLoc = HL_NONE;
	...
	{ //humanoid
		handLBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*l_hand");
		handRBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*r_hand");
		kneeLBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*hips_l_knee");
		kneeRBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*hips_r_knee");
		footLBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*l_leg_foot");
		footRBolt = g_trap->G2API_AddBolt(ent->ghoul2, 0, "*r_leg_foot");
	}
	...
	{//FIXME: test properly for legs
		*hitLoc = HL_WAIST;
		if ( ent->client != NULL && ent->ghoul2 )
		{
			...
			if (kneeLBolt>=0)
			{
				g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, kneeLBolt,
			...
			if (*hitLoc == HL_WAIST)
			{
				if (kneeRBolt>=0)
				{
					g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, kneeRBolt,
									&boltMatrix, angles, ent->r.currentOrigin,
									actualTime, NULL, ent->modelScale );
					BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, tagOrg );
					if ( DistanceSquared( point, tagOrg ) < 100 )
	...
	else if ( !Q_strncmp( "r_arm", surfName, 5 ) )
	{
		*hitLoc = HL_ARM_RT;
		if ( ent->client != NULL && ent->ghoul2 )
		{
			...
			if (handRBolt>=0)
			{
				g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, handRBolt,
								&boltMatrix, angles, ent->r.currentOrigin,
								actualTime, NULL, ent->modelScale );
				BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, tagOrg );
	...
	else if ( !Q_strncmp( "l_arm", surfName, 5 ) )
	{
		*hitLoc = HL_ARM_LT;
		if ( ent->client != NULL && ent->ghoul2 )
		{
			...
			if (handLBolt>=0)
			{
				g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, handLBolt,
								&boltMatrix, angles, ent->r.currentOrigin,
								actualTime, NULL, ent->modelScale );
				BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, tagOrg );
				if ( DistanceSquared( point, tagOrg ) < 256 )
	...
	else if ( !Q_strncmp( "r_leg", surfName, 5 ) )
	{
		*hitLoc = HL_LEG_RT;
		if ( ent->client != NULL && ent->ghoul2 )
		{
			...
			if (footRBolt>=0)
			{
				g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, footRBolt,
								&boltMatrix, angles, ent->r.currentOrigin,
								actualTime, NULL, ent->modelScale );
				BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, tagOrg );
				if ( DistanceSquared( point, tagOrg ) < 100 )
	...
	else if ( !Q_strncmp( "l_leg", surfName, 5 ) )
	{
		*hitLoc = HL_LEG_LT;
		if ( ent->client != NULL && ent->ghoul2 )
		{
			...
			if (footLBolt>=0)
			{
				g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, footLBolt,
								&boltMatrix, angles, ent->r.currentOrigin,
								actualTime, NULL, ent->modelScale );
				BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, tagOrg );
				if ( DistanceSquared( point, tagOrg ) < 100 )
	...
	{ //full probability...
		...
		{//we care about direction (presumably for dismemberment)
			...
			{//either we don't care about probabilties or the probability let us continue
				...
				if ( tagName )
				{
					int tagBolt = g_trap->G2API_AddBolt( ent->ghoul2, 0, tagName );
					if ( tagBolt != -1 )
					{
						mdxaBone_t	boltMatrix;
						...
						VectorSet( angles, 0, ent->r.currentAngles[YAW], 0 );
						g_trap->G2API_GetBoltMatrix( ent->ghoul2, 0, tagBolt,
										&boltMatrix, angles, ent->r.currentOrigin,
										actualTime, NULL, ent->modelScale );
						BG_GiveMeVectorFromMatrix( &boltMatrix, ORIGIN, tagOrg );
...
void G_CheckForDismemberment(gentity_t *ent, gentity_t *enemy, vec3_t point, int damage, int deathAnim, qboolean postDeath)
{
	...
	else
	{
		if (d_saberGhoul2Collision.integer && ent->client && ent->client->g2LastSurfaceTime == level.time)
		{
			...
			g_trap->G2API_GetSurfaceName(ent->ghoul2, ent->client->g2LastSurfaceHit, 0, hitSurface);
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod ) {
		...
		{//FIXME: should be in its own function in g_vehicles.c now, too big to be here
			...
			{//get the last surf that was hit
				if ( targ->client && targ->client->g2LastSurfaceTime == level.time)
				{
					...
					g_trap->G2API_GetSurfaceName(targ->ghoul2, targ->client->g2LastSurfaceHit, 0, hitSurface);
	...
	if ( g_debugDamage.integer ) {
		g_trap->Print( "%i: client:%i health:%i damage:%i armor:%i\n", level.time, targ->s.number,
...
qboolean CanDamage (gentity_t *targ, vec3_t origin) {
	...
	g_trap->Trace ( &tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID, qfalse, 0, 0);
