...
static qboolean Update( Vehicle_t *pVeh, const usercmd_t *pUcmd )
{
	...
	if (!BG_FighterUpdate< mod >(pVeh, pUcmd, ((gentity_t *)pVeh->m_pParentEntity)->r.mins,
...
qboolean FighterIsLanding( Vehicle_t *pVeh, playerState_t *parentPS )
{
	...
	if ( FighterOverValidLandingSurface( pVeh )
		//only do this check on game side, because if it's cgame, it's being predicted, and it's only predicted if the local client is the driver
		&& ( mod != moduleType::game || pVeh->m_pVehicleInfo->Inhabited( pVeh ) ) //has to have a driver in order to be capable of landing
...
//a predicted event or only doing it game-side. -rich
	...
	isLandingOrLaunching = (FighterIsLanding< mod >( pVeh, parentPS )||FighterIsLaunching< mod >( pVeh, parentPS ));
	...
	{//FIXME: if start to move forward and fly over something low while still going relatively slow, you may try to land even though you don't mean to...
		...
		if ( pVeh->m_ucmd.upmove > 0 )
		{
			...
			{//taking off for the first time
				if( mod == moduleType::game ) { //MP GAME-side
	...
	if ( (pVeh->m_ucmd.upmove > 0) && pVeh->m_pVehicleInfo->turboSpeed )
	{
		if ((curTime - pVeh->m_iTurboTime)>pVeh->m_pVehicleInfo->turboRecharge)
		{
			...
			if ( mod == moduleType::game && pVeh->m_pVehicleInfo->soundTurbo )
			{
	...
	*/
	...
	else if ( mod == moduleType::game && FighterSuspended< mod >( pVeh, parentPS ) )
	{
		parentPS->speed = 0;
		pVeh->m_ucmd.forwardmove = 0;
	}
	else if ( mod == moduleType::game && !pVeh->m_pVehicleInfo->Inhabited( pVeh )
		&& parentPS->speed > 0 )
...
//STRAFING==============================================================================
	...
	if( mod == moduleType::game ) { //FIXME: get working in game and cgame
		...
		} else if( FighterSuspended< mod >( pVeh, parentPS ) ) {
			parentPS->gravity = 0;
...
static void FighterDamageRoutine( Vehicle_t *pVeh, bgEntity_t *parent, playerState_t *parentPS, playerState_t *riderPS, qboolean isDead )
{
	...
	{
		if ( !FighterSuspended< mod >( pVeh, parentPS ) )
		{
			//pVeh->m_ucmd.forwardmove = 0;
	...
	if ( mod == moduleType::game && pVeh->m_LandTrace.fraction < 1.0f )
...
static void ProcessOrientCommands( Vehicle_t *pVeh )
{
	...
	int curTime = mod == moduleType::game ? level.time : pm->cmd.serverTime;
	...
	{ //do some special stuff for when all the wings are torn off
		FighterDamageRoutine< mod >(pVeh, parent, parentPS, riderPS, isDead);
	...
	isLandingOrLanded = (FighterIsLanding< mod >( pVeh, parentPS )||FighterIsLanded( pVeh, parentPS ));
...
*/
	...
	//never gonna happen on client anyway, we can't be getting predicted unless the predicting client is boarded
 	if ( mod == moduleType::game
		&& !pVeh->m_pVehicleInfo->Inhabited( pVeh )
		&& pVeh->m_LandTrace.fraction >= groundFraction
		&& !FighterIsInSpace( (gentity_t *)parent )
		&& !FighterSuspended< mod >( pVeh, parentPS ) )
	{
		pVeh->m_ucmd.upmove = 0;
		//pVeh->m_ucmd.forwardmove = 0;
		pVeh->m_vOrientation[PITCH] += pVeh->m_fTimeModifier;
...
static void AnimateVehicle( Vehicle_t *pVeh )
{
	...
	{
		isLanding = FighterIsLanding< mod >( pVeh, parentPS );
	...
	if ( Anim != -1 )
	{
		BG_SetAnim< mod >(pVeh->m_pParentEntity->playerState, bgAllAnims[pVeh->m_pParentEntity->localAnimIndex].anims, SETANIM_BOTH, Anim, iFlags);
	}
}
...
{
	if( mod == moduleType::game ) {//ONLY in SP or on server, not cgame
		pVehInfo->AnimateVehicle = AnimateVehicle< mod >;
		...
		pVehInfo->Update = Update< mod >;
		//	pVehInfo->UpdateRider				=		UpdateRider;
	} else { //cgame prediction attachment func
		pVehInfo->AttachRiders = AttachRidersGeneric< mod >;
	}
	pVehInfo->ProcessMoveCommands = ProcessMoveCommands< mod >;
	pVehInfo->ProcessOrientCommands = ProcessOrientCommands< mod >;
//	pVehInfo->AttachRiders				=		AttachRiders;
//	pVehInfo->Ghost						=		Ghost;
//	pVehInfo->UnGhost					=		UnGhost;
//	pVehInfo->Inhabited					=		Inhabited;
}
template void G_SetFighterVehicleFunctions< moduleType::game >( vehicleInfo_t *pVehInfo );
template void G_SetFighterVehicleFunctions< moduleType::cgame >( vehicleInfo_t *pVehInfo );
...
void G_CreateFighterNPC( Vehicle_t **pVeh, const char *strType )
{
	...
	if( mod == moduleType::game ) {
		...
		if( !*pVeh ) { //only allocate a new one if we really have to
			( *pVeh ) = (Vehicle_t *)BG_Alloc< mod >( sizeof( Vehicle_t ) );
		}
	}
	memset(*pVeh, 0, sizeof(Vehicle_t));
	(*pVeh)->m_pVehicleInfo = &g_vehicleInfo[BG_VehicleGetIndex< mod >( strType )];
}
template void G_CreateFighterNPC< moduleType::game >( Vehicle_t **pVeh, const char *strType );
template void G_CreateFighterNPC< moduleType::cgame >( Vehicle_t **pVeh, const char *strType );
