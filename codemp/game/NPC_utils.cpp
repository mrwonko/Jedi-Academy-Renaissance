...
void CalcEntitySpot ( const gentity_t *ent, const spot_t spot, vec3_t point )
{
	...
	case SPOT_GROUND:
		...
		g_trap->Trace( &tr, start, ent->r.mins, ent->r.maxs, end, ent->s.number, MASK_PLAYERSOLID, qfalse, 0, 0 );
		if ( tr.fraction < 1.0 )
		{
...
qboolean NPC_UpdateAngles ( qboolean doPitch, qboolean doYaw )
{
	...
	if ( exact && g_trap->ICARUS_TaskIDPending( (sharedEntity_t *)NPCS.NPC, TID_ANGLE_FACE ) )
	{
		g_trap->ICARUS_TaskIDComplete( (sharedEntity_t *)NPCS.NPC, TID_ANGLE_FACE );
	}
	return exact;
...
qboolean G_ActivateBehavior (gentity_t *self, int bset )
{
	...
	else
	{
		...
		*/
		...
		g_trap->ICARUS_RunScript( (sharedEntity_t *)self, va( "%s/%s", Q3_SCRIPT_DIR, bs_name ) );
	}
	return qtrue;
}
...
void NPC_SetBoneAngles(gentity_t *ent, char *bone, vec3_t angles)
{
	...
	g_trap->G2API_SetBoneAngles(ent->ghoul2, 0, bone, angles, flags, up, right, forward, NULL, 100, level.time);
}
...
void NPC_SetSurfaceOnOff(gentity_t *ent, const char *surfaceName, int surfaceFlags)
{
	...
	g_trap->G2API_SetSurfaceOnOff(ent->ghoul2, surfaceName, surfaceFlags);
}
...
qboolean NPC_SomeoneLookingAtMe(gentity_t *ent)
{
	...
	while (i < MAX_CLIENTS)
	{
		...
		{
			if (g_trap->InPVS(ent->r.currentOrigin, pEnt->r.currentOrigin))
			{
...
int NPC_FindNearestEnemy( gentity_t *ent )
{
	...
	numEnts = g_trap->EntitiesInBox( mins, maxs, iradiusEnts, MAX_RADIUS_ENTS );
...
void NPC_CheckCharmed( void )
{
	...
	{//we were charmed, set us back!
		NPCS.NPC->client->playerTeam	= static_cast< npcteam_t >( NPCS.NPC->genericValue1 );
		NPCS.NPC->client->enemyTeam		= static_cast< npcteam_t >( NPCS.NPC->genericValue2 );
		NPCS.NPC->s.teamowner			= static_cast< npcteam_t >( NPCS.NPC->genericValue3 );
...
void G_GetBoltPosition( gentity_t *self, int boltIndex, vec3_t pos, int modelIndex )
{
	...
	g_trap->G2API_GetBoltMatrix( self->ghoul2, modelIndex,
				boltIndex,
...
int NPC_GetEntsNearBolt( int *radiusEnts, float radius, int boltIndex, vec3_t boltOrg )
{
	...
	return (g_trap->EntitiesInBox( mins, maxs, radiusEnts, 128 ));
}
