...
void BG_AttachToRancor( CGhoul2Info_v& ghoul2, float rancYaw, vec3_t rancOrigin, int time, qhandle_t *modelList, vec3_t modelScale, qboolean inMouth, vec3_t out_origin, vec3_t out_angles, matrix3_t out_axis )
{
	...
	if ( inMouth )
	{//in mouth
		boltIndex = re->G2API_AddBolt( ghoul2, 0, "jaw_bone" );
	}
	else
	{//in right hand
			boltIndex = re->G2API_AddBolt( ghoul2, 0, "*r_hand" );
	}
	VectorSet( rancAngles, 0, rancYaw, 0 );
	re->G2API_GetBoltMatrix( ghoul2, 0, boltIndex, &boltMatrix, rancAngles, rancOrigin, time, modelList, modelScale );
...
{
	if (  !re->G2API_GetSurfaceRenderStatus( ghoul2, 0, rootSurfName ) )
	...
	{//check variants
		...
		for ( i = 0; i < MAX_VARIANTS; i++ )
		{
			...
			if ( !re->G2API_GetSurfaceRenderStatus( ghoul2, 0, returnSurfName ) )
			{
				return qtrue;
			}
		}
