...
//-----------------------------------------------------------------------------
{
	...
	g_trap->Trace( &tr, eyePoint, mins, maxs, start, ent->s.number, MASK_SOLID|CONTENTS_SHOTCLIP, qfalse, 0, 0 );
...
//---------------------------------------------------------
{
	...
	{//need to loop this in case we hit a Jedi who dodges the shot
		...
		{
			g_trap->Trace( &tr, start, NULL, NULL, end, ignore, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
		}
		else
		{
			g_trap->Trace( &tr, start, NULL, NULL, end, ignore, MASK_SHOT, qfalse, 0, 0 );
		}
...
//---------------------------------------------------------
{
	...
	for (i = 0; i < traces; i++ )
	{
		...
		if (d_projectileGhoul2Collision.integer)
		{
			g_trap->Trace( &tr, start, NULL, NULL, end, skip, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
		}
		else
		{
			g_trap->Trace( &tr, start, NULL, NULL, end, skip, MASK_SHOT, qfalse, 0, 0 );
		}
		...
		if ( tr.entityNum == ent->s.number )
		{
			...
			skip = tr.entityNum;
#ifdef _DEBUG
			g_trap->Print( "BAD! Disruptor gun shot somehow traced back and hit the owner!\n" );
#endif
			continue;
		}
...
void DEMP2_AltRadiusDamage( gentity_t *ent )
{
	...
	numListedEntities = g_trap->EntitiesInBox( mins, maxs, iEntityList, MAX_GENTITIES );
...
//---------------------------------------------------------
{
	...
	g_trap->Trace( &tr, start, NULL, NULL, end, ent->s.number, MASK_SHOT, qfalse, 0, 0);
...
//-----------------------------------------------------------------------------
{
	...
	g_trap->Trace( &tr, ent->client->ps.origin, mins, maxs, start, ent->s.number, MASK_SOLID|CONTENTS_SHOTCLIP, qfalse, 0, 0 );
	...
	while ( hitCount < maxHits )
	{
		...
		{//do a rough trace of the path
			...
			for ( elapsedTime = timeStep; elapsedTime < floor(travelTime)+timeStep; elapsedTime += timeStep )
			{
				...
				g_trap->Trace( &trace, lastPos, mins, maxs, testPos, ignoreEntNum, clipmask, qfalse, 0, 0 );
...
void WP_PlaceLaserTrap( gentity_t *ent, qboolean alt_fire )
{
	...
	g_trap->LinkEntity((sharedEntity_t *)laserTrap);
}
...
void drop_charge (gentity_t *self, vec3_t start, vec3_t dir)
{
	...
	g_trap->LinkEntity((sharedEntity_t *)bolt);
}
...
//	}
	...
	for ( i = 0; i < traces; i++ )
	{
		...
		//g_trap->trace( &tr, start, NULL, NULL, end, skip, MASK_SHOT, G2_COLLIDE, 10 );//G2_RETURNONHIT, 0 );
		if (d_projectileGhoul2Collision.integer)
		{
			g_trap->Trace( &tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
		}
		else
		{
			g_trap->Trace( &tr, start, shot_mins, shot_maxs, end, skip, MASK_SHOT, qfalse, 0, 0 );
		}
...
void WP_FireStunBaton( gentity_t *ent, qboolean alt_fire )
{
	...
	g_trap->Trace ( &tr, muzzleStun, mins, maxs, end, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void WP_FireMelee( gentity_t *ent, qboolean alt_fire )
{
	...
	g_trap->Trace ( &tr, muzzlePunch, mins, maxs, end, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void WP_CalcVehMuzzle(gentity_t *ent, int muzzleNum)
{
	...
	g_trap->G2API_GetBoltMatrix_NoRecNoRot(ent->ghoul2, 0, pVeh->m_iMuzzleTag[muzzleNum], &boltMatrix, vehAngles,
					  ...
					  int ignoreEntNum, vec3_t camPos )
{
	...
	g_trap->Trace( &trace, cameraFocusLoc, cameramins, cameramaxs, cameraCurTarget, ignoreEntNum, MASK_CAMERACLIP, qfalse, 0, 0 );
	...
	g_trap->Trace(&trace, cameraCurTarget, cameramins, cameramaxs, cameraCurLoc, ignoreEntNum, MASK_CAMERACLIP, qfalse, 0, 0);
	if (trace.fraction < 1.0)
	{
...
qboolean WP_VehCheckTraceFromCamPos( gentity_t *ent, const vec3_t shotStart, vec3_t shotDir )
{
		...
		|| ent->m_pVehicle->m_pVehicleInfo->type == VH_WALKER)
	{
		...
		g_trap->Trace( &trace, start, vec3_origin, vec3_origin, end, ent->s.number, CONTENTS_SOLID|CONTENTS_BODY, qfalse, 0, 0 );
		...
		{//NOW do the trace from the camPos and compare with above trace
			trace_t	extraTrace;
			vec3_t	newEnd;
			int camTraceEntNum = BG_VehTraceFromCamPos< moduleType::game >( &extraTrace, (bgEntity_t *)ent, ent->r.currentOrigin, shotStart, end, newEnd, shotDir, (trace.fraction*g_cullDistance) );
			if ( camTraceEntNum )
			{
				...
				if ( pVeh->m_iMuzzleTag[i] != -1 && pVeh->m_iMuzzleWait[i] < level.time )
				{
					...
					{//have enough ammo to shoot
						...
						{//auto-aim the missile at the crosshair if there's anything there
							...
							g_trap->Trace( &trace, ent->r.currentOrigin, vec3_origin, vec3_origin, end, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void SP_emplaced_gun( gentity_t *ent )
{
	...
	g_trap->Trace(&tr, ent->s.origin, ent->r.mins, ent->r.maxs, down, ent->s.number, MASK_SOLID, qfalse, 0, 0);
