...
void WP_InitForcePowers( gentity_t *ent ) {
	...
	if ( g_maxForceRank.integer <= 0 || g_maxForceRank.integer >= NUM_FORCE_MASTERY_LEVELS ) {
		...
		g_trap->Cvar_Set( "g_maxForceRank", va( "%i", FORCE_MASTERY_JEDI_MASTER ) );
		g_trap->Cvar_Update( &g_maxForceRank );
	}
	...
	if ( level.gametype == GT_SIEGE && ent->client->siegeClass != -1 ) {
		...
		if ( !ent->client->sess.setForce )
			g_trap->SendServerCommand( ent-g_entities, "scl" );
	...
	if ( strlen( forcePowers ) != DEFAULT_FORCEPOWERS_LEN ) {
		...
		g_trap->SendServerCommand( ent-g_entities, "print \"" S_COLOR_RED "Invalid forcepowers string, setting default\n\"" );
	}
	...
	if ( ent->s.eType == ET_NPC )
			...
			if ( level.gametype != GT_HOLOCRON && level.gametype != GT_JEDIMASTER ) {
				...
				if ( !(ent->r.svFlags & SVF_BOT) && ent->s.eType != ET_NPC ) {
					if ( !g_teamAutoJoin.integer ) {
						...
						g_trap->SendServerCommand( ent-g_entities, "spc" ); // Fire up the profile menu
					}
				}
				...
				g_trap->SendServerCommand( ent->s.number, va( "nfr %i %i %i", g_maxForceRank.integer, 1, ent->client->sess.sessionTeam ) );
		...
		if ( !didEvent )
			g_trap->SendServerCommand( ent->s.number, va( "nfr %i %i %i", g_maxForceRank.integer, 0, ent->client->sess.sessionTeam ) );

		// the server has one or more force powers disabled and the client is using them in his config
		if ( warnClientLimit )
			g_trap->SendServerCommand( ent-g_entities, va( "print \"The server has one or more force powers that you have chosen disabled.\nYou will not be able to use the disable force power(s) while playing on this server.\n\"" ) );
	}
...
void ForceTeamHeal( gentity_t *self )
{
	...
	while (i < MAX_CLIENTS)
	{
		...
		if (ent && ent->client && self != ent && OnSameTeam(self, ent) && ent->client->ps.stats[STAT_HEALTH] < ent->client->ps.stats[STAT_MAX_HEALTH] && ent->client->ps.stats[STAT_HEALTH] > 0 && ForcePowerUsableOn(self, ent, FP_TEAM_HEAL) &&
			g_trap->InPVS(self->client->ps.origin, ent->client->ps.origin))
		{
...
void ForceGrip( gentity_t *self )
{
	...
	g_trap->Trace(&tr, tfrom, NULL, NULL, tto, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
...
void ForceShootLightning( gentity_t *self )
{
	...
	{//arc
		...
		for ( e = 0 ; e < numListedEntities ; e++ )
		{
			...
			//in PVS?
			if ( !traceEnt->r.bmodel && !g_trap->InPVS( ent_org, self->client->ps.origin ) )
			{//must be in PVS
				continue;
			}

			//Now check and see if we can actually hit it
			g_trap->Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	{//trace-line
		...
		g_trap->Trace( &tr, self->client->ps.origin, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
			...
			{//haha!  Jedi aren't easily confused!
				...
				{//turn them to our side
					...
					{//don't charm people who aren't capable of fighting... like ugnaughts and droids
						npcteam_t newPlayerTeam, newEnemyTeam;
...
{
	forcePowers_t powerUse = FP_FIRST;
...
void ForceThrow( gentity_t *self, qboolean pull )
{
	...
	forcePowers_t powerUse = pull ? FP_PULL : FP_PUSH;
	...
	for ( e = 0 ; e < numListedEntities ; e++ )
	{
		...
		//in PVS?
		if ( !ent->r.bmodel && !g_trap->InPVS( ent_org, self->client->ps.origin ) )
		{//must be in PVS
			continue;
		}
		...
		{//must have clear LOS
			...
			g_trap->Trace( &tr, eyePoint, vec3_origin, vec3_origin, ent_org, self->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	if ( ent_count )
	{
		...
		for ( x = 0; x < ent_count; x++ )
		{
			...
			{//push/pull the door
				...
				g_trap->Trace( &tr, trFrom, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower)
{
	...
	g_trap->Trace(&tr, self->client->ps.origin, NULL, NULL, gripEnt->client->ps.origin, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
	...
	while (i < MAX_CLIENTS)
	{
		if (G_IsMindTricked(&self->client->ps.fd, i))
		{
			...
			{ //Untrick this entity if the tricker (self) fires while in his fov
				if (g_trap->InPVS(ent->client->ps.origin, self->client->ps.origin) &&
...
void SeekerDroneUpdate(gentity_t *self)
{
	...
	if (self->client->ps.genericEnemyIndex != ENTITYNUM_NONE && self->client->ps.genericEnemyIndex != -1)
	{
		...
		if (self->client->ps.droneFireTime < level.time)
		{
			g_trap->Trace(&tr, org, NULL, NULL, en->client->ps.origin, -1, MASK_SOLID, qfalse, 0, 0);
...
{ //keep holocron status updated in holocron mode
	...
	g_trap->Cvar_Update(&g_maxHolocronCarry);
...
{ //keep jedi master status updated for JM gametype
	...
	while (i < NUM_FORCE_POWERS)
	{
		...
		i = static_cast< forcePowers_t >( static_cast< std::underlying_type_t< forcePowers_t > >( i ) + 1 );
	}
}
...
void WP_ForcePowersUpdate( gentity_t *self, usercmd_t *ucmd )
{
	...
	*/
	...
	{ //otherwise just do a check through them all to see if they need to be stopped for any reason.
		...
		{
			if ((self->client->ps.fd.forcePowersActive & (1 << i)) && i != FP_LEVITATION &&
				!BG_CanUseFPNow(level.gametype, &self->client->ps, level.time, static_cast< forcePowers_t >( i )))
			{
				WP_ForcePowerStop(self, static_cast< forcePowers_t >( i ));
			}
	...
	if ( (ucmd->buttons & BUTTON_FORCEPOWER) &&
		BG_CanUseFPNow(level.gametype, &self->client->ps, level.time, static_cast< forcePowers_t >( self->client->ps.fd.forcePowerSelected )))
	{
		if (self->client->ps.fd.forcePowerSelected == FP_LEVITATION)
			ForceJumpCharge( self, ucmd );
		else
			WP_DoSpecificPower( self, ucmd, static_cast< forcePowers_t >( self->client->ps.fd.forcePowerSelected ) );
	}
	else
	{
