...
void CG_RegisterItemVisuals( int itemNum ) {
	...
	if ( itemNum < 0 || itemNum >= bg_numItems ) {
		cg_trap->Error( ERR_DROP, "CG_RegisterItemVisuals: itemNum %d out of range [0-%d]", itemNum, bg_numItems-1 );
	}
	...
	{ //in CTY the flag model is different
		itemInfo->models[0] = cg_trap->R_RegisterModel( item->world_model[1] );
	}
	...
	{
		itemInfo->models[0] = cg_trap->R_RegisterModel( item->world_model[0] );
	}
/*
Ghoul2 Insert Start
*/
	...
	{
		handle = cg_trap->G2API_InitGhoul2Model(&itemInfo->g2Models[0], item->world_model[0], 0 , 0, 0, 0, 0);
		if (handle<0)
		{
...
*/
	if (item->icon)
	{
		...
		{ //medpack gets nomip'd by the ui or something I guess.
			itemInfo->icon = cg_trap->R_RegisterShaderNoMip( item->icon );
		}
		else
		{
			itemInfo->icon = cg_trap->R_RegisterShader( item->icon );
		}
	}
	else
	{
	...
	if ( item->giType == IT_POWERUP || item->giType == IT_HEALTH ||
		...
		if ( item->world_model[1] ) {
			itemInfo->models[1] = cg_trap->R_RegisterModel( item->world_model[1] );
		}
	}
}
...
//		VectorCopy(cg.refdef.vieworg, muzzlePoint );
	...
	cg_trap->R_AddRefEntityToScene( &beam );
*/
...
static void CG_AddWeaponWithPowerups( refEntity_t *gun, int powerups ) {
	// add powerup effects
	cg_trap->R_AddRefEntityToScene( gun );
	...
	if (!thirdPerson)
	{
		...
		if ( !ps ) {
			...
			if ( ( cent->currentState.eFlags & EF_FIRING ) && weapon->firingSound ) {
				...
				cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->firingSound );
				cent->pe.lightningFiring = qtrue;
			} else if ( weapon->readySound ) {
				cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, weapon->readySound );
			}
		}
			...
			cg.snap->ps.clientNum))
		{
			...
			if ( weaponNum == WP_STUN_BATON )
			{
				...
				gun.customShader = cg_trap->R_RegisterShader( "gfx/effects/stunPass" );
		...
		if (weaponNum == WP_STUN_BATON)
		{
			...
			while (i < 3)
			{
				...
				if (i == 0)
				{
					barrel.hModel = cg_trap->R_RegisterModel("models/weapons2/stun_baton/baton_barrel.md3");
				}
				else if (i == 1)
				{
					barrel.hModel = cg_trap->R_RegisterModel("models/weapons2/stun_baton/baton_barrel2.md3");
				}
				else
				{
					barrel.hModel = cg_trap->R_RegisterModel("models/weapons2/stun_baton/baton_barrel3.md3");
				}
				angles[YAW] = 0;
				angles[PITCH] = 0;
				angles[ROLL] = 0;
...
*/
		  ...
		  ( cent->currentState.weapon == WP_DEMP2 && cent->currentState.modelindex2 == WEAPON_CHARGING_ALT) ) )
	{
		...
		else
		{
			...
			if (!cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1))
			...
			// go away and get me the bolt position for this frame please
 			if (!(cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, newAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale)))
		...
		else if ( val > 1.0f )
		{
			...
			{
				cgame::CGCam_Shake( /*0.1f*/0.2f, 100 );
			}
		}
		else
		{
			if (ps && cent->currentState.number == ps->clientNum)
			{
				cgame::CGCam_Shake( val * val * /*0.3f*/0.6f, 100 );
			}
		}
	...
	{	// Make sure we don't do the thirdperson model effects for the local player if we're in first person
		...
		{	// Handle muzzle flashes
			...
			{	// Check the alt firing first.
				if (weapon->altMuzzleEffect)
				{
					if (!thirdPerson)
					{
						cg_trap->FX_PlayEntityEffectID(weapon->altMuzzleEffect, flashorigin, flash.axis, -1, -1, -1, -1  );
					}
					else
					{
						cg_trap->FX_PlayEffectID(weapon->altMuzzleEffect, flashorigin, flashdir, -1, -1, qfalse);
					}
				}
			}
			else
			{	// Regular firing
				if (weapon->muzzleEffect)
				{
					if (!thirdPerson)
					{
						cg_trap->FX_PlayEntityEffectID(weapon->muzzleEffect, flashorigin, flash.axis, -1, -1, -1, -1  );
					}
					else
					{
						cg_trap->FX_PlayEffectID(weapon->muzzleEffect, flashorigin, flashdir, -1, -1, qfalse);
					}
				}
			}
		}
		...
		if ( weapon->flashDlightColor[0] || weapon->flashDlightColor[1] || weapon->flashDlightColor[2] ) {
			cg_trap->R_AddLightToScene( flashorigin, 300 + (rand()&31), weapon->flashDlightColor[0],
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	if ( cg_debugGun.integer ) {
		...
		cg_trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time, &currentFrame, cgs.gameModels, 0);
...
void CG_NextWeapon_f( void ) {
	...
	{
		cg_trap->S_MuteSound(cg.snap->ps.clientNum, CHAN_WEAPON);
	}
}
...
void CG_Weapon_f( void ) {
	...
	//	if (cg.snap->ps.weaponTime < 1)
		{
			cg_trap->SendConsoleCommand("sv_saberswitch\n");
		}
		return;
	}
...
void CG_GetClientWeaponMuzzleBoltPoint(int clIndex, vec3_t to)
{
	...
	if (!cent || !cent->ghoul2 || !cg_trap->G2_HaveWeGhoul2Models(cent->ghoul2) ||
		!cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1))
	{
		return;
	}

	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
...
void CG_FireWeapon( centity_t *cent, qboolean altFire ) {
	...
	if ( ent->weapon >= WP_NUM_WEAPONS ) {
		cg_trap->Error( ERR_DROP, "CG_FireWeapon: ent->weapon >= WP_NUM_WEAPONS" );
		return;
	}
	...
	if (cg.predictedPlayerState.clientNum == cent->currentState.number)
	{
			...
			(ent->weapon == WP_CONCUSSION && !altFire))
		{
			...
			{
				cgame::CGCam_Shake(flrand(2, 3), 350);
			}
	...
	if ( ent->weapon == WP_DEMP2 ) {
		...
		if ( cent->currentState.powerups & ( 1 << PW_QUAD ) ) {
			//cg_trap->S_StartSound (NULL, cent->currentState.number, CHAN_ITEM, cgs.media.quadSound );
		}
	#endif // BASE_COMPAT
	...
	if (altFire)
	{
		...
		if ( c > 0 ) {
			...
			if ( weap->altFlashSound[c] )
			{
				cg_trap->S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->altFlashSound[c] );
			}
		}
//		if ( weap->altFlashSnd )
//		{
//			cg_trap->S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->altFlashSnd );
//		}
	}
	else
	{
		...
		if ( c > 0 ) {
			...
			if ( weap->flashSound[c] )
			{
				cg_trap->S_StartSound( NULL, ent->number, CHAN_WEAPON, weap->flashSound[c] );
			}
		}
	}
}
...
{//see if this is a missile entity that's owned by a vehicle and should do a special, overridden impact effect
	...
	{//missile is from a special vehWeapon
		...
		cg_trap->FX_PlayEffectID( g_vehWeaponInfo[cent->currentState.otherEntityNum2].iImpactFX, cent->lerpOrigin, normal, -1, -1, qfalse );
		return qtrue;
	}
	return qfalse;
}
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir, impactSound_t soundType, qboolean altFire, int charge)
{
	...
	case WP_DEMP2:
		if (altFire)
		{
			cg_trap->FX_PlayEffectID(cgs.effects.mAltDetonate, origin, dir, -1, -1, qfalse);
		}
		else
		{
	...
	case WP_THERMAL:
		cg_trap->FX_PlayEffectID( cgs.effects.thermalExplosionEffect, origin, dir, -1, -1, qfalse );
		cg_trap->FX_PlayEffectID( cgs.effects.thermalShockwaveEffect, origin, up, -1, -1, qfalse );
		break;
...
void CG_InitG2Weapons(void)
{
	...
	for ( item = bg_itemlist + 1 ; item->classname ; item++ )
	{
		if ( item->giType == IT_WEAPON )
		{
			...
			// initialise model
			cg_trap->G2API_InitGhoul2Model(&g2WeaponInstances[/*i*/item->giTag], item->world_model[0], 0, 0, 0, 0, 0);
//			cg_trap->G2API_InitGhoul2Model(&g2WeaponInstances[i], item->world_model[0],G_ModelIndex( item->world_model[0] ) , 0, 0, 0, 0);
			if (g2WeaponInstances[/*i*/item->giTag])
			{
				// indicate we will be bolted to model 0 (ie the player) on bolt 0 (always the right hand) when we get copied
				cg_trap->G2API_SetBoltInfo(g2WeaponInstances[/*i*/item->giTag], 0, 0);
				// now set up the gun bolt on it
				if (item->giTag == WP_SABER)
				{
					cg_trap->G2API_AddBolt(g2WeaponInstances[/*i*/item->giTag], 0, "*blade1");
				}
				else
				{
					cg_trap->G2API_AddBolt(g2WeaponInstances[/*i*/item->giTag], 0, "*flash");
				}
				i++;
			}
			if (i == MAX_WEAPONS)
...
void CG_ShutDownG2Weapons(void)
{
	...
	{
		cg_trap->G2API_CleanGhoul2Models(&g2WeaponInstances[i]);
	}
}
...
void CG_CopyG2WeaponInstance(centity_t *cent, int weaponNum, void *toGhoul2)
{
	...
	if (CG_G2WeaponInstance(cent, weaponNum/*-1*/))
	{
		if (weaponNum == WP_SABER)
		{
			...
			if (!ci)
			{
				cg_trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, weaponNum/*-1*/), 0, toGhoul2, 1);
			}
			else
			{ //Try both the left hand saber and the right hand saber
				...
				while (i < MAX_SABERS)
				{
					if (ci->saber[i].model[0] &&
						ci->ghoul2Weapons[i])
					{
						cg_trap->G2API_CopySpecificGhoul2Model(ci->ghoul2Weapons[i], 0, toGhoul2, i+1);
					}
					else if (ci->ghoul2Weapons[i])
					{ //if the second saber has been removed, then be sure to remove it and free the instance.
						qboolean g2HasSecondSaber = cg_trap->G2API_HasGhoul2ModelOnIndex(&(toGhoul2), 2);

						if (g2HasSecondSaber)
						{ //remove it now since we're switching away from sabers
							cg_trap->G2API_RemoveGhoul2Model(&(toGhoul2), 2);
						}
						re->G2API_CleanGhoul2Models(&ci->ghoul2Weapons[i]);
					}
		...
		else
		{
			...
			{ //a bit of a hack to remove gun model when using an emplaced weap
				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(toGhoul2), 1))
				{
					cg_trap->G2API_RemoveGhoul2Model(&(toGhoul2), 1);
				}
			}
...
void CG_CheckPlayerG2Weapons(playerState_t *ps, centity_t *cent)
{
		...
		ps->clientNum == cent->currentState.number) //don't want spectator mode forcing one client's weapon instance over another's
	{
		...
		{ //switching away from the saber
			...
			{
				cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, cgs.clientinfo[ps->clientNum].saber[0].soundOff);
			}
			...
			if (cgs.clientinfo[ps->clientNum].saber[1].soundOff &&
				...
				!ps->saberHolstered)
			{
				cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, cgs.clientinfo[ps->clientNum].saber[1].soundOff);
			}
		}
		...
		{ //switching to the saber
			...
			{
				cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, cgs.clientinfo[ps->clientNum].saber[0].soundOn);
			}

			if (cgs.clientinfo[ps->clientNum].saber[1].soundOn)
			{
				cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, cgs.clientinfo[ps->clientNum].saber[1].soundOn);
			}
