...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat )
{
	...
	{
		CG_DrawPic( iconx, y, iconSize, iconSize, cg_trap->R_RegisterShaderNoMip(
...
qboolean CG_DrawOldScoreboard( void ) {
	...
	{ //do nothing?
		...
		if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR )
		{
			...
			cg_trap->SE_GetStringTextString("MP_INGAME_PLACE",	sPlace,	sizeof(sPlace));
			cg_trap->SE_GetStringTextString("MP_INGAME_OF",		sOf,	sizeof(sOf));
			cg_trap->SE_GetStringTextString("MP_INGAME_WITH",	sWith,	sizeof(sWith));
	...
	CG_DrawPic ( SB_SCORELINE_X - 40, y - 5, SB_SCORELINE_WIDTH + 80, 40, cg_trap->R_RegisterShaderNoMip ( "gfx/menus/menu_buttonback.tga" ) );

	CG_Text_Paint ( SB_NAME_X, y, 1.0f, colorWhite, CG_GetStringEdString("MP_INGAME", "NAME"),0, 0, ITEM_TEXTSTYLE_OUTLINED, FONT_MEDIUM );
	if (cgs.gametype == GT_DUEL || cgs.gametype == GT_POWERDUEL)
	{
		char sWL[100];
		cg_trap->SE_GetStringTextString("MP_INGAME_W_L", sWL,	sizeof(sWL));
