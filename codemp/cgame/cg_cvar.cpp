...
static void CG_TeamOverlayChange( void ) {
	...
	if ( cg_drawTeamOverlay.integer > 0 && cgs.gametype >= GT_SINGLE_PLAYER)
		cg_trap->Cvar_Set( "teamoverlay", "1" );
	else
		cg_trap->Cvar_Set( "teamoverlay", "0" );
}
...
void CG_RegisterCvars( void ) {
	...
	for ( i=0, cv=cvarTable; i<cvarTableSize; i++, cv++ ) {
		cg_trap->Cvar_Register( cv->vmCvar, cv->cvarName, cv->defaultString, cv->cvarFlags );
		if ( cv->update )
			cv->update();
	}
}
