...
void CG_PositionEntityOnTag( refEntity_t *entity, const refEntity_t *parent,
	...
	// lerp the tag
	cg_trap->R_LerpTag( &lerped, parentModel, parent->oldframe, parent->frame,
...
//via cg_trap each frame. Such as ones generated by events.
/*
======================
...
void CG_SetEntitySoundPosition( centity_t *cent ) {
	if ( cent->currentState.solid == SOLID_BMODEL )
	{
		...
		cg_trap->S_UpdateEntityPosition( cent->currentState.number, origin );
	}
	else
	{
...
void CG_S_StopLoopingSound(int entityNum, sfxHandle_t sfx)
{
	...
	//cg_trap->S_StopLoopingSound(entityNum);
}
...
void CG_S_UpdateLoopingSounds(int entityNum)
{
	...
	while (i < cent->numLoopingSounds)
	{
		...
		//cg_trap->S_AddLoopingSound(entityNum, cSound->origin, cSound->velocity, cSound->sfx);
		//I guess just keep using lerpOrigin for now,
		cg_trap->S_AddLoopingSound(entityNum, lerpOrg, cSound->velocity, cSound->sfx);
		i++;
	}
}
...
static void CG_EntityEffects( centity_t *cent ) {
	...
	if ( cent->currentState.loopSound || ((cent->currentState.loopIsSoundset && cent->currentState.number >= MAX_CLIENTS)
		...
		{ //If this is so, then first get our soundset from the index, and loopSound actually contains which part of the set to
		  //use rather than a sound index (BMS_START [0], BMS_MID [1], or BMS_END [2]). Typically loop sounds will be BMS_MID.
			...
			{
				realSoundIndex = cg_trap->AS_GetBModelSound(soundSet, cent->currentState.loopSound);
			}
		}
		else
		{
		...
		if (realSoundIndex != -1)
		{
			if ( cent->currentState.solid == SOLID_BMODEL )
			{
				...
				v = cgs.inlineModelMidpoints[ cent->currentState.modelindex ];
				VectorAdd( cent->lerpOrigin, v, origin );
				cg_trap->S_AddLoopingSound( cent->currentState.number, origin, vec3_origin,
					realSoundIndex );
			}
			else if (cent->currentState.eType != ET_SPEAKER) {
				cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, realSoundIndex );
			} else {
				cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, realSoundIndex );
			//	cg_trap->S_AddRealLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, realSoundIndex );
			}
		}
	}
	...
	if ( cent->currentState.constantLight && cent->currentState.eType != ET_PLAYER && cent->currentState.eType != ET_BODY && cent->currentState.eType != ET_NPC && cent->currentState.eType != ET_INVISIBLE ) {
		...
		cg_trap->R_AddLightToScene( cent->lerpOrigin, i, r, g, b );
	}
...
void FX_DrawPortableShield(centity_t *cent)
{
	...
	if (team == TEAM_RED)
	{
		...
		{
			shader = cg_trap->R_RegisterShader( "gfx/misc/red_dmgshield" );
		}
		else
		{
			shader = cg_trap->R_RegisterShader( "gfx/misc/red_portashield" );
		}
	}
	else
	{
		if (cent->currentState.trickedentindex)
		{
			shader = cg_trap->R_RegisterShader( "gfx/misc/blue_dmgshield" );
		}
		else
		{
			shader = cg_trap->R_RegisterShader( "gfx/misc/blue_portashield" );
		}
	}
...
//g2r
	...
	for (i=0; i<8; i++)
	{
		...
		point[i].radius = 1;
		point[i].customShader = cg_trap->R_RegisterShader("textures/tests/circle");
		...
		cg_trap->R_AddRefEntityToScene (&point[i]);
	}
#endif
	*/
}
...
void G2_BoltToGhoul2Model(centity_t *cent, refEntity_t *ent)
{
	...
	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, modelNum, boltNum, &boltMatrix, cg_entities[entNum].currentState.angles, cg_entities[entNum].currentState.origin, cg.time, cgs.gameModels, cent->modelScale);
...
void CG_Disintegration(centity_t *cent, refEntity_t *ent)
{
	...
	if ( cg.time - ent->endTime < 1000 && (timescale.value * timescale.value * Q_flrand(0.0f, 1.0f)) > 0.05f )
	{
		vec3_t fxOrg, fxDir;
		mdxaBone_t	boltMatrix;
		int torsoBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "lower_lumbar");

		VectorSet(fxDir, 0, 1, 0);

		cg_trap->G2API_GetBoltMatrix( cent->ghoul2, 0, torsoBolt, &boltMatrix, cent->lerpAngles, cent->lerpOrigin, cg.time,
		...
		cg_trap->FX_PlayEffectID( cgs.effects.mDisruptorDeathSmoke, fxOrg, fxDir, -1, -1, qfalse );
...
static qboolean CG_RenderTimeEntBolt(centity_t *cent)
{
	...
	getBolt = cg_trap->G2API_AddBolt(cl->ghoul2, 0, "lhand");

	cg_trap->G2API_GetBoltMatrix(cl->ghoul2, 0, getBolt, &matrix, cl->turAngles, cl->lerpOrigin, cg.time, cgs.gameModels, cl->modelScale);
...
static void CG_General( centity_t *cent ) {
	...
	{ //this is a bad thing
		if (re->G2API_HaveWeGhoul2Models(*cent->ghoul2))
		{
...
*/
	...
	{ //mini-animation routine for general objects that want to play quick ghoul2 anims
		...
		{
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", cent->currentState.torsoAnim,
	...
	{ //special case for client limbs
		...
		if (!cent->ghoul2)
		{
			...
			{
				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(clEnt->ghoul2), 2))
				{ //don't want to bother dealing with a second saber on limbs and stuff, just remove the thing
					cg_trap->G2API_RemoveGhoul2Model(&(clEnt->ghoul2), 2);
				}

				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(clEnt->ghoul2), 3))
				{ //turn off jetpack also I suppose
					cg_trap->G2API_RemoveGhoul2Model(&(clEnt->ghoul2), 3);
				}

				if (clEnt->localAnimIndex <= 0)
				{ //humanoid
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "model_root", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "pelvis", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, cgs.gameModels, 100, cg.time);
				}
				else
				{
					cg_trap->G2API_SetBoneAngles(clEnt->ghoul2, 0, "model_root", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
				...
				re->G2API_DuplicateGhoul2Instance(*clEnt->ghoul2, &cent->ghoul2);
			}
			...
			newBolt = cg_trap->G2API_AddBolt( cent->ghoul2, 0, limbTagName );
			if ( newBolt != -1 )
			{
				vec3_t boltOrg, boltAng;

				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, newBolt, &matrix, cent->lerpAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);

				BG_GiveMeVectorFromMatrix(&matrix, ORIGIN, boltOrg);
				BG_GiveMeVectorFromMatrix(&matrix, NEGATIVE_Y, boltAng);

				cg_trap->FX_PlayEffectID(cgs.effects.mBlasterSmoke, boltOrg, boltAng, -1, -1, qfalse);
			}

			cent->bolt4 = newBolt;

			cg_trap->G2API_SetRootSurface(cent->ghoul2, 0, limbName);

			cg_trap->G2API_SetNewOrigin(cent->ghoul2, cg_trap->G2API_AddBolt(cent->ghoul2, 0, rotateBone));

			cg_trap->G2API_SetSurfaceOnOff(cent->ghoul2, limbCapName, 0);

			cg_trap->G2API_SetSurfaceOnOff(clEnt->ghoul2, limbName, 0x00000100);
			cg_trap->G2API_SetSurfaceOnOff(clEnt->ghoul2, stubCapName, 0);

			newBolt = cg_trap->G2API_AddBolt( clEnt->ghoul2, 0, stubTagName );
			if ( newBolt != -1 )
			{
				vec3_t boltOrg, boltAng;

				cg_trap->G2API_GetBoltMatrix(clEnt->ghoul2, 0, newBolt, &matrix, clEnt->lerpAngles, clEnt->lerpOrigin, cg.time, cgs.gameModels, clEnt->modelScale);
			...
			{ //Cut his weapon holding arm off, so remove the weapon
				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(clEnt->ghoul2), 1))
				{
					cg_trap->G2API_RemoveGhoul2Model(&(clEnt->ghoul2), 1);
				}
			}
		...
		if (cent->ghoul2 && cent->bolt4 != -1 && cent->trailTime < cg.time)
		{
				...
				(cent->currentState.pos.trDelta[0] || cent->currentState.pos.trDelta[1] || cent->currentState.pos.trDelta[2]) )
			{
				...
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt4, &matrix, cent->lerpAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
		...
		cent->currentState.NPC_class != CLASS_VEHICLE )
	{
		...
		cg_trap->G2API_SetBoneAngles( cent->ghoul2, 0, "Bone02", empAngles, BONE_ANGLES_REPLACE, NEGATIVE_Y, NEGATIVE_X, POSITIVE_Z, NULL, 0, cg.time);
	}

	s1 = &cent->currentState;

	// if set to invisible, skip
	if ((!s1->modelindex) && !(cg_trap->G2_HaveWeGhoul2Models(cent->ghoul2)))
	{
		return;
	}
...
*/
	...
	{ //If the game says this guy uses a ghoul2 model and the g2 instance handle is null, then initialize it
		if (!cent->ghoul2 && !cent->currentState.bolt1)
		{
			...
			re->G2API_InitGhoul2Model(&cent->ghoul2, modelName, 0, 0, 0, 0, 0);
			if (cent->ghoul2 && cg_trap->G2API_SkinlessModel(cent->ghoul2, 0))
			{ //well, you'd never want a skinless model, so try to get his skin...
				...
				{ //got it
					...
					skin = cg_trap->R_RegisterSkin(skinName);
				}
				cg_trap->G2API_SetSkin(cent->ghoul2, 0, skin, skin);
			}
		}
	...
	{ //bodies should have a radius as well
		...
		{ //all bodies should already have a ghoul2 instance. Use it to set the torso/head angles to 0.
			...
			cg_trap->G2API_SetBoneAngles(cent->ghoul2, 0, "pelvis", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
			cg_trap->G2API_SetBoneAngles(cent->ghoul2, 0, "thoracic", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 0, cg.time);
			cg_trap->G2API_SetBoneAngles(cent->ghoul2, 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
			cg_trap->G2API_SetBoneAngles(cent->ghoul2, 0, "lower_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, cgs.gameModels, 100, cg.time);
			cg_trap->G2API_SetBoneAngles(cent->ghoul2, 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, cgs.gameModels, 100, cg.time);
		}
	}
	...
	{ //special render, it's a holocron
		...
		ent.hModel = cg_trap->R_RegisterModel(forceHolocronModels[s1->modelindex+128]);
	...
	if ( cent->currentState.time > cg.time && cent->currentState.weapon == WP_EMPLACED_GUN )
	{
		...
		ent.customShader = cg_trap->R_RegisterShader( "gfx/effects/turretflashdie" );
	...
	{
		ent.customShader = cg_trap->R_RegisterShader( "models/map_objects/imp_mine/turret_chair_dmg.tga" );
		//cg_trap->R_AddRefEntityToScene( &ent );
	}
			...
			*/
			...
			{ //begin the disintegration effect
				...
				if (!cent->dustTrailTime)
				{
					...
					if (lightSide)
					{
						cg_trap->S_StartSound ( NULL, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound("sound/weapons/force/see.wav") );
					}
					else
					{
						cg_trap->S_StartSound ( NULL, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound("sound/weapons/force/lightning") );
					}
				}
				ent.endTime = cent->dustTrailTime;
				ent.renderfx |= RF_DISINTEGRATE2;
			...
			*/
			...
			if (curTimeDif < 3400)
			{
				if (lightSide)
				{
					if (curTimeDif < 2200)
					{ //probably temporary
						cg_trap->S_StartSound ( NULL, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/weapons/saber/saberhum1.wav" ) );
					}
				}
				else
	...
	{ //this is an in-flight saber being rendered manually
		...
		for ( i = -4; i < 10; i += 1 )
		{
			...
			//cg_trap->FX_AddSprite( org, NULL, NULL, 5.5f, 5.5f, wv, wv, 0.0f, 0.0f, 1.0f, cgs.media.yellowSaberGlowShader, 0x08000000 );
	...
	{ //if force sight is active, render the laser multiple times up to the force sight level to increase visibility
		if (cent->currentState.bolt2 == 1)
		{
			...
			cg_trap->FX_PlayEffectID( beamID, beamOrg, cent->currentState.pos.trDelta, -1, -1, qfalse );
		}
		else
		{
			int i = 0;
...
static void CG_Speaker( centity_t *cent ) {
	...
	cg_trap->S_StartSound (NULL, cent->currentState.number, CHAN_ITEM, cgs.gameSounds[cent->currentState.eventParm] );
...
static void CG_Item( centity_t *cent ) {
	...
	if ( es->modelindex >= bg_numItems ) {
		cg_trap->Error( ERR_DROP, "Bad item index %i on entity", es->modelindex );
	}
...
*/
		...
		!cg_simpleItems.integer)
	{
		...
		if (!doGrey)
		{
			cg_trap->FX_PlayEffectID(cgs.effects.itemCone, ent.origin, uNorm, -1, -1, qfalse);
		}
	}
...
*/
	...
	if ( cg_simpleItems.integer && item->giType != IT_TEAM ) {
		...
		if (CG_GreyItem(item->giType, item->giTag, cg.snap->ps.fd.forceSide))
		{
			...
			{
				ent.customShader = cg_trap->R_RegisterShader("gfx/misc/mp_light_enlight_disable");
			}
			else
			{
				ent.customShader = cg_trap->R_RegisterShader("gfx/misc/mp_dark_enlight_disable");
			}
		}
		cg_trap->R_AddRefEntityToScene(&ent);
		return;
...
*/
	...
	if ( item->giType == IT_WEAPON ) {
		...
		ent.nonNormalizedAxes = qtrue;
		//cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin, cgs.media.weaponHoverSound );
	}
	...
	if ( item->giType == IT_WEAPON && wi->barrelModel ) {
		...
		cg_trap->R_AddRefEntityToScene( &barrel );
	}
	*/
...
void CG_CreateDistortionTrailPart(centity_t *cent, float scale, vec3_t pos)
{
	...
	ent.hModel = cg_trap->R_RegisterModel("models/weapons2/merr_sonn/trailmodel.md3");
...
//	int	col;
	...
	if (s1->weapon == WP_SABER)
	{
		...
		{ //no g2, or server changed the model we are using
			...
			{
				re->G2API_InitGhoul2Model(&cent->ghoul2, saberModel, 0, 0, 0, 0, 0);
			}
			else
			{
				re->G2API_InitGhoul2Model(&cent->ghoul2, "models/weapons2/saber/saber_w.glm", 0, 0, 0, 0, 0);
			}
			return;
		}
	...
	{//using an over-ridden trail effect!
		...
		{ //a vehicle with an override for the weapon trail fx or model
			cg_trap->FX_PlayEffectID( g_vehWeaponInfo[s1->otherEntityNum2].iShotFX, cent->lerpOrigin, forward, -1, -1, qfalse );
			if ( g_vehWeaponInfo[s1->otherEntityNum2].iLoopSound )
			{
				vec3_t	velocity;
				BG_EvaluateTrajectoryDelta< moduleType::cgame >( &cent->currentState.pos, cg.time, velocity );
				cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, velocity, g_vehWeaponInfo[s1->otherEntityNum2].iLoopSound );
			}
			//add custom model
		...
		{//a regular missile
			cg_trap->FX_PlayEffectID( cgs.gameEffects[s1->otherEntityNum2], cent->lerpOrigin, forward, -1, -1, qfalse );
			if ( s1->loopSound )
			{
				vec3_t	velocity;
				BG_EvaluateTrajectoryDelta< moduleType::cgame >( &cent->currentState.pos, cg.time, velocity );
				cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, velocity, s1->loopSound );
			}
	...
	else if ( cent->currentState.eFlags & EF_ALT_FIRING )
	{
		...
		{
			cg_trap->R_AddLightToScene(cent->lerpOrigin, weapon->altMissileDlight,
		...
		if ( weapon->altMissileSound ) {
			...
			cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, velocity, weapon->altMissileSound );
		}
	...
	else
	{
		...
		{
			cg_trap->R_AddLightToScene(cent->lerpOrigin, weapon->missileDlight,
		...
		if ( weapon->missileSound )
		{
			...
			cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, velocity, weapon->missileSound );
		}
...
*/
	...
	{//special code for adding the beam to the attached tripwire mine
		...
		cg_trap->FX_PlayEffectID( cgs.effects.mTripMineLaser, beamOrg, ent.axis[0], -1, -1, qfalse );
	}
}
...
void CG_PlayDoorLoopSound( centity_t *cent )
{
	...
	sfx = cg_trap->AS_GetBModelSound( soundSet, CG_BMS_MID );
...
void CG_PlayDoorSound( centity_t *cent, int type )
{
	...
	sfx = cg_trap->AS_GetBModelSound( soundSet, type );
	...
	cg_trap->S_StartSound( NULL, cent->currentState.number, CHAN_AUTO, sfx );
}
...
void CG_AdjustPositionForMover( const vec3_t in, int moverNum, int fromTime, int toTime, vec3_t out ) {
	...
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, fromTime, oldOrigin );
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.apos, fromTime, oldAngles );

	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, toTime, origin );
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.apos, toTime, angles );
...
static void CG_InterpolateEntityPosition( centity_t *cent ) {
	...
	if ( cg.nextSnap == NULL ) {
		cg_trap->Error( ERR_DROP, "CG_InterpoateEntityPosition: cg.nextSnap == NULL" );
		return;
	}
	...
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, cg.snap->serverTime, current );
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->nextState.pos, cg.nextSnap->serverTime, next );

	cent->lerpOrigin[0] = current[0] + f * ( next[0] - current[0] );
	cent->lerpOrigin[1] = current[1] + f * ( next[1] - current[1] );
	cent->lerpOrigin[2] = current[2] + f * ( next[2] - current[2] );

	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.apos, cg.snap->serverTime, current );
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->nextState.apos, cg.nextSnap->serverTime, next );
...
static void CG_FX( centity_t *cent )
{
	...
	if (efxIndex)
	{
		if (s1->isPortalEnt)
		{
			cg_trap->FX_PlayEffectID(efxIndex, cent->lerpOrigin, fxDir, -1, -1, qtrue );
		}
		else
		{
			cg_trap->FX_PlayEffectID(efxIndex, cent->lerpOrigin, fxDir, -1, -1, qfalse );
		}
	}
...
*/
	...
	if ( cent->currentState.soundSetIndex && cent->currentState.eType != ET_MOVER )
	{
		...
		if (soundSet && soundSet[0])
		{
			cg_trap->S_AddLocalSet(soundSet, cg.refdef.vieworg, cent->lerpOrigin, cent->currentState.number, cg.time);
		}
	}
/*
Ghoul2 Insert End
*/
	...
	default:
		cg_trap->Error( ERR_DROP, "Bad entity type: %i\n", cent->currentState.eType );
		break;
...
defaultoffsetposition:
		...
		objectID = cg_trap->FX_RegisterEffect(argument);
		...
		if (objectID)
		{
			...
			cg_trap->FX_PlayEffectID(objectID, useOrigin, useAngles, -1, -1, qfalse);
		}
	}
	else if (strcmp(type, "sound") == 0)
	{
		objectID = cg_trap->S_RegisterSound(argument);
		cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_BODY, objectID);
	}
