...
const char	*CG_PlaceString( int rank ) {
	...
	cg_trap->SE_GetStringTextString("MP_INGAME_NUMBER_ST",sST, sizeof(sST) );
	cg_trap->SE_GetStringTextString("MP_INGAME_NUMBER_ND",sND, sizeof(sND) );
	cg_trap->SE_GetStringTextString("MP_INGAME_NUMBER_RD",sRD, sizeof(sRD) );
	cg_trap->SE_GetStringTextString("MP_INGAME_NUMBER_TH",sTH, sizeof(sTH) );
	cg_trap->SE_GetStringTextString("MP_INGAME_TIED_FOR" ,sTiedFor,sizeof(sTiedFor) );
...
static void CG_Obituary( entityState_t *ent ) {
	...
	if ( target < 0 || target >= MAX_CLIENTS ) {
		cg_trap->Error( ERR_DROP, "CG_Obituary: target out of range" );
	}
	...
	if ( attacker == cg.snap->ps.clientNum ) {
		...
		if ( cgs.gametype < GT_TEAM && cgs.gametype != GT_DUEL && cgs.gametype != GT_POWERDUEL ) {
				...
				CG_ThereIsAMaster())
			{
				char part1[512];
				char part2[512];
				cg_trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", part1, sizeof(part1));
				cg_trap->SE_GetStringTextString("MP_INGAME_JMKILLED_NOTJM", part2, sizeof(part2));
			...
			else
			{
				char sPlaceWith[256];
				char sKilledStr[256];
				cg_trap->SE_GetStringTextString("MP_INGAME_PLACE_WITH",     sPlaceWith, sizeof(sPlaceWith));
				cg_trap->SE_GetStringTextString("MP_INGAME_KILLED_MESSAGE", sKilledStr, sizeof(sKilledStr));
...
void CG_ToggleBinoculars(centity_t *cent, int forceZoom)
{
	...
	*/
	...
	{
		cg_trap->S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.zoomStart );
	}
	else if (cg.snap->ps.zoomMode == 2)
	{
		cg_trap->S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.zoomEnd );
	}
}
...
static void CG_UseItem( centity_t *cent ) {
	...
	default:
	case HI_NONE:
		//cg_trap->S_StartSound (NULL, es->number, CHAN_BODY, cgs.media.useNothingSound );
		break;
	...
	case HI_SEEKER:
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.deploySeeker );
		break;
...
//	case HI_MEDKIT:
	...
	case HI_MEDPAC_BIG:
		...
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.medkitSound );
		break;
	case HI_JETPACK:
		break; //Do something?
	case HI_HEALTHDISP:
...
static void CG_ItemPickup( int itemNum ) {
	...
	{ //don't print messages for flags, they have their own pickup event broadcasts
		...
		if ( cg_trap->SE_GetStringTextString( va("SP_INGAME_%s",Q_strupr(upperKey)), text, sizeof( text )))
		{
...
void CG_PainEvent( centity_t *cent, int health ) {
	...
	cg_trap->S_StartSound( NULL, cent->currentState.number, CHAN_VOICE,
...
qboolean BG_GetRootSurfNameWithVariant( CGhoul2Info_v &ghoul2, const char *rootSurfName, char *returnSurfName, int returnSize );
void CG_ReattachLimb(centity_t *source)
{
	...
	{//re-apply the skin
		if ( ci->torsoSkin > 0 )
		{
			cg_trap->G2API_SetSkin(source->ghoul2,0,ci->torsoSkin,ci->torsoSkin);
		}
	}
	...
	while (i <= G2_MODELPART_RLEG)
	{
		if (source->torsoBolt & (1 << (i-10)))
		{
			...
			cg_trap->G2API_SetSurfaceOnOff(source->ghoul2, limbName, 0);
			cg_trap->G2API_SetSurfaceOnOff(source->ghoul2, stubCapName, 0x00000100);
		}
		i++;
	}
	*/
...
void DoFall(centity_t *cent, entityState_t *es, int clientNum)
{
	...
	{ //corpses crack into the ground ^_^
		if (delta > 25)
		{
			cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.fallSound );
		}
		else
		{
			cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/movers/objects/objectHit.wav" ) );
		}
	}
	...
	{
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.landSound );
	}
...
void CG_TryPlayCustomSound( vec3_t origin, int entityNum, int channel, const char *soundName )
{
	...
	cg_trap->S_StartSound(origin, entityNum, channel, cSound);
}
...
void CG_CalcVehMuzzle(Vehicle_t *pVeh, centity_t *ent, int muzzleNum)
{
	...
	cg_trap->G2API_GetBoltMatrix_NoRecNoRot(ent->ghoul2, 0, pVeh->m_iMuzzleTag[muzzleNum], &boltMatrix, vehAngles,
	...
	{//go through all muzzles and
		...
		{//this muzzle fired
			...
			if ( muzFX )
			{
				...
				//cg_trap->FX_PlayEffectID(muzFX, pVeh->m_vMuzzlePos[curMuz], pVeh->m_vMuzzleDir[curMuz], -1, -1, qfalse);
				cg_trap->FX_PlayBoltedEffectID(muzFX, veh->currentState.origin, veh->ghoul2, pVeh->m_iMuzzleTag[curMuz], veh->currentState.number, 0, 0, qtrue);
			}
		}
	}
}
...
==============
*/
#define	DEBUGNAME(x) if(cg_debugEvents.integer){cg_trap->Print(x"\n");}
...
void CG_EntityEvent( centity_t *cent, vec3_t position ) {
	...
	if ( cg_debugEvents.integer ) {
		cg_trap->Print( "ent:%3i  event:%3i ", es->number, event );
	}
	...
	case EV_FOOTSTEP:
		...
		if (cg_footsteps.integer) {
			...
			cg_trap->S_StartSound (NULL, es->number, CHAN_BODY, cgs.media.footsteps[ soundType ][rand()&3] );
		}
		break;
	case EV_FOOTSTEP_METAL:
		DEBUGNAME("EV_FOOTSTEP_METAL");
		if (cg_footsteps.integer) {
			cg_trap->S_StartSound (NULL, es->number, CHAN_BODY,
	...
	case EV_GLOBAL_DUEL:
			...
			es->groundEntityNum == cg.predictedPlayerState.clientNum)
		{
			...
			cg_trap->S_StartLocalSound( cgs.media.countFightSound, CHAN_ANNOUNCER );
		}
		break;
	...
	case EV_JUMP:
		...
		{
			cg_trap->S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*jump1.wav" ) );
		}
		break;
	case EV_ROLL:
		DEBUGNAME("EV_ROLL");
		...
		cg_trap->S_StartSound (NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*jump1.wav" ) );
		cg_trap->S_StartSound( NULL, es->number, CHAN_BODY, cgs.media.rollSound  );
	...
	case EV_TAUNT:
		DEBUGNAME("EV_TAUNT");
		{
			...
			if ( soundIndex )
			{
				cg_trap->S_StartSound (NULL, es->number, CHAN_VOICE, soundIndex );
			}
		}
		break;
	...
	case EV_WATER_TOUCH:
		...
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.watrInSound );
		break;
	case EV_WATER_LEAVE:
		DEBUGNAME("EV_WATER_LEAVE");
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.watrOutSound );
		break;
	case EV_WATER_UNDER:
		DEBUGNAME("EV_WATER_UNDER");
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.watrUnSound );
		break;
	case EV_WATER_CLEAR:
		DEBUGNAME("EV_WATER_CLEAR");
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, CG_CustomSound( es->number, "*gasp.wav" ) );
		break;
	...
	case EV_ITEM_PICKUP:
		DEBUGNAME("EV_ITEM_PICKUP");
		{
			...
			{ //a holocron most likely
				...
				cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO,	cgs.media.holocronPickup );
			...
			if ( /*item->giType != IT_POWERUP && */item->giType != IT_TEAM) {
				...
				{
					cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO,	cg_trap->S_RegisterSound( item->pickup_sound ) );
				}
			}
	...
	case EV_GLOBAL_ITEM_PICKUP:
		DEBUGNAME("EV_GLOBAL_ITEM_PICKUP");
		{
			...
			if( item->pickup_sound && item->pickup_sound[0] ) {
				cg_trap->S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, cg_trap->S_RegisterSound( item->pickup_sound) );
			}
	...
	case EV_NOAMMO:
		DEBUGNAME("EV_NOAMMO");
//		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.noAmmoSound );
		if ( es->number == cg.snap->ps.clientNum )
		{
			...
			{//just letting us know our vehicle is out of ammo
				...
				{//play the "no Ammo" sound for this weapon
					cg_trap->S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, localCent->m_pVehicle->m_pVehicleInfo->weapon[es->eventParm].soundNoAmmo );
				}
				else
				{//play the default "no ammo" sound
					cg_trap->S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, cgs.media.noAmmoSound );
				}
	...
	case EV_CHANGE_WEAPON:
		DEBUGNAME("EV_CHANGE_WEAPON");
		{
			...
			{
				cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, weaponInfo->selectSound );
			}
			...
			{ //not sure what SP is doing for this but I don't want a select sound for saber (it has the saber-turn-on)
				cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.selectSound );
			}
		}
		break;
	case EV_FIRE_WEAPON:
		...
		{ //special case for turret firing
			...
			if (cent->ghoul2)
			{
				if (!cent->bolt1)
				{
					cent->bolt1 = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*flash01");
				}
				if (!cent->bolt2)
				{
					cent->bolt2 = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*flash02");
				}
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Bone02", 1, 4, BONE_ANIM_OVERRIDE_FREEZE|BONE_ANIM_BLEND,
					1.0f, cg.time, -1, 300);
			}
			else
			{
			...
			{
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt2, &matrix, cent->currentState.angles, cent->currentState.origin, cg.time, cgs.gameModels, cent->modelScale);
			}
			else
			{
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, cent->bolt1, &matrix, cent->currentState.angles, cent->currentState.origin, cg.time, cgs.gameModels, cent->modelScale);
			}
			...
			cg_trap->FX_PlayEffectID(cgs.effects.mEmplacedMuzzleFlash, gunpoint, gunangle, -1, -1, qfalse);
		}
	...
	case EV_SABER_ATTACK:
		...
		{
			qhandle_t swingSound = cg_trap->S_RegisterSound(va("sound/weapons/saber/saberhup%i.wav", Q_irand(1, 8)));
            ...
            cg_trap->S_StartSound(es->pos.trBase, es->number, CHAN_WEAPON, swingSound );
		}
		break;
	...
	case EV_SABER_HIT:
		DEBUGNAME("EV_SABER_HIT");
		{
			...
			int hitSound = cg_trap->S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3)));
			...
			{ //Make lots of sparks, something special happened
				...
				cg_trap->S_StartSound(es->origin, es->number, CHAN_AUTO, hitSound );
				cg_trap->FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1, qfalse );
				cg_trap->FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1, qfalse );
				cg_trap->FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1, qfalse );
				cg_trap->FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1, qfalse );
				cg_trap->FX_PlayEffectID( hitPersonFxID, es->origin, fxDir, -1, -1, qfalse );
			...
			{ //hit a person
				...
				{	// moderate or big hits.
					cg_trap->FX_PlayEffectID( hitPersonSmallFxID, es->origin, fxDir, -1, -1, qfalse );
				}
				else if ( es->eventParm == 2 )
				{	// this is for really big hits.
					cg_trap->FX_PlayEffectID( hitPersonMidFxID, es->origin, fxDir, -1, -1, qfalse );
				}
				else
			...
			{ //hit something else
				...
				//old jk2mp method
				/*
				cg_trap->S_StartSound(es->origin, es->number, CHAN_AUTO, cg_trap->S_RegisterSound("sound/weapons/saber/saberhit.wav"));
				cg_trap->FX_PlayEffectID( cg_trap->FX_RegisterEffect("saber/spark.efx"), es->origin, fxDir, -1, -1, qfalse );
				*/

				cg_trap->FX_PlayEffectID( hitOtherFxID, es->origin, fxDir, -1, -1, qfalse );
			}
	...
	case EV_SABER_BLOCK:
		DEBUGNAME("EV_SABER_BLOCK");
		{
			...
			{ //saber block
				...
				qhandle_t	blockSound = cg_trap->S_RegisterSound(va( "sound/weapons/saber/saberblock%d.wav", Q_irand(1, 9) ));
...

				{
					...
					cg_trap->S_StartSound(es->origin, es->number, CHAN_AUTO, blockSound );
					cg_trap->FX_PlayEffectID( blockFXID, es->origin, fxDir, -1, -1, qfalse );
			...
			{ //projectile block
				...
				cg_trap->FX_PlayEffectID(cgs.effects.mBlasterDeflect, es->origin, fxDir, -1, -1, qfalse);
			}
		}
		break;
	...
	case EV_SABER_CLASHFLARE:
		DEBUGNAME("EV_SABER_CLASHFLARE");
		{
			cg_saberFlashTime = cg.time-50;
			VectorCopy( es->origin, cg_saberFlashPos );
			cg_trap->S_StartSound ( es->origin, -1, CHAN_WEAPON, cg_trap->S_RegisterSound( va("sound/weapons/saber/saberhitwall%i", Q_irand(1, 3)) ) );
		}
		break;
	...
	case EV_SABER_UNHOLSTER:
		DEBUGNAME("EV_SABER_UNHOLSTER");
		{
			...
			if (ci)
			{
				...
				{
					cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, ci->saber[0].soundOn );
				}
				if (ci->saber[1].soundOn)
				{
					cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, ci->saber[1].soundOn );
				}
			}
		}
		break;
	...
	case EV_BECOME_JEDIMASTER:
		DEBUGNAME("EV_SABER_UNHOLSTER");
		{
			...
			cg_trap->FX_PlayEffectID(cgs.effects.mJediSpawn, pos, ang, -1, -1, qfalse);

			cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/weapons/saber/saberon.wav" ) );

			if (cg.snap->ps.clientNum == es->number)
			{
				cg_trap->S_StartLocalSound(cgs.media.happyMusic, CHAN_LOCAL);
	...
	case EV_DISRUPTOR_ZOOMSOUND:
		...
		if (es->number == cg.snap->ps.clientNum)
		{
			...
			{
				cg_trap->S_StartLocalSound(cg_trap->S_RegisterSound("sound/weapons/disruptor/zoomstart.wav"), CHAN_AUTO);
			}
			else
			{
				cg_trap->S_StartLocalSound(cg_trap->S_RegisterSound("sound/weapons/disruptor/zoomend.wav"), CHAN_AUTO);
			}
		}
		break;
	case EV_PREDEFSOUND:
		DEBUGNAME("EV_PREDEFSOUND");
		{
			...
			case PDSOUND_PROTECTHIT:
				sID = cg_trap->S_RegisterSound("sound/weapons/force/protecthit.mp3");
				break;
			case PDSOUND_PROTECT:
				sID = cg_trap->S_RegisterSound("sound/weapons/force/protect.mp3");
				break;
			case PDSOUND_ABSORBHIT:
				sID = cg_trap->S_RegisterSound("sound/weapons/force/absorbhit.mp3");
				...
				break;
			case PDSOUND_ABSORB:
				sID = cg_trap->S_RegisterSound("sound/weapons/force/absorb.mp3");
				break;
			case PDSOUND_FORCEJUMP:
				sID = cg_trap->S_RegisterSound("sound/weapons/force/jump.mp3");
				break;
			case PDSOUND_FORCEGRIP:
				sID = cg_trap->S_RegisterSound("sound/weapons/force/grip.mp3");
				break;
			default:
				break;
			}
			...
			if (sID != 1)
			{
				cg_trap->S_StartSound(es->origin, es->number, CHAN_AUTO, sID);
			}
		}
		break;
		...
		DEBUGNAME("EV_TEAM_POWER");
		{
			...
			while (clnum < MAX_CLIENTS)
			{
				if (CG_InClientBitflags(es, clnum))
				{
					if (es->eventParm == 1)
					{ //eventParm 1 is heal
						cg_trap->S_StartSound (NULL, clnum, CHAN_AUTO, cgs.media.teamHealSound );
						cg_entities[clnum].teamPowerEffectTime = cg.time + 1000;
						cg_entities[clnum].teamPowerType = 1;
					}
					else
					{ //eventParm 2 is force regen
						cg_trap->S_StartSound (NULL, clnum, CHAN_AUTO, cgs.media.teamRegenSound );
						cg_entities[clnum].teamPowerEffectTime = cg.time + 1000;
	...
	case EV_SCREENSHAKE:
		...
		{
			cgame::CGCam_Shake(es->angles[0], es->time);
		}
		break;
	case EV_LOCALTIMER:
	...
	case EV_PLAYER_TELEPORT_IN:
		DEBUGNAME("EV_PLAYER_TELEPORT_IN");
		{
			...
			cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.teleInSound );

			if (tr.fraction == 1)
			{
				break;
			}
			cg_trap->FX_PlayEffectID(cgs.effects.mSpawn, pos, ang, -1, -1, qfalse);
		}
		break;
	...
	case EV_PLAYER_TELEPORT_OUT:
		DEBUGNAME("EV_PLAYER_TELEPORT_OUT");
		{
			...
			cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.teleOutSound );
	...
	case EV_ITEM_POP:
		...
		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.respawnSound );
		break;
	...
	case EV_BODYFADE:
		...
		if (cent->ghoul2 && cg_trap->G2_HaveWeGhoul2Models(cent->ghoul2))
		{
			//turn the inside of the face off, to avoid showing the mouth when we start alpha fading the corpse
			cg_trap->G2API_SetSurfaceOnOff( cent->ghoul2, "head_eyes_mouth", 0x00000002/*G2SURFACEFLAG_OFF*/ );
		}
	...
	case EV_DESTROY_GHOUL2_INSTANCE:
		...
		if (cg_entities[es->eventParm].ghoul2 && cg_trap->G2_HaveWeGhoul2Models(cg_entities[es->eventParm].ghoul2))
		{
			...
			re->G2API_CleanGhoul2Models(&(cg_entities[es->eventParm].ghoul2));
		}
		break;
	...
	case EV_DESTROY_WEAPON_MODEL:
		...
		if (cg_entities[es->eventParm].ghoul2 && cg_trap->G2_HaveWeGhoul2Models(cg_entities[es->eventParm].ghoul2) &&
			cg_trap->G2API_HasGhoul2ModelOnIndex(&(cg_entities[es->eventParm].ghoul2), 1))
		{
			cg_trap->G2API_RemoveGhoul2Model(&(cg_entities[es->eventParm].ghoul2), 1);
		}
		break;
	...
	case EV_GIVE_NEW_RANK:
		...
		{
			cg_trap->Cvar_Set("ui_rankChange", va("%i", es->eventParm));

			cg_trap->Cvar_Set("ui_myteam", va("%i", es->bolt2));

			if (!( cg_trap->Key_GetCatcher() & KEYCATCH_UI ) && !es->bolt1)
			{
				cg_trap->OpenUIMenu(UIMENU_PLAYERCONFIG);
			}
		}
		break;
	...
	case EV_SET_FREE_SABER:
		...
		cg_trap->Cvar_Set("ui_freeSaber", va("%i", es->eventParm));
		break;

	case EV_SET_FORCE_DISABLE:
		DEBUGNAME("EV_SET_FORCE_DISABLE");

		cg_trap->Cvar_Set("ui_forcePowerDisable", va("%i", es->eventParm));
		break;
	...
	case EV_CONC_ALT_IMPACT:
		DEBUGNAME("EV_CONC_ALT_IMPACT");
		{
			...
			{ //one effect would be.. a whole lot better
                ...
                cg_trap->FX_PlayEffectID(cgs.effects.mConcussionAltRing, spot, es->angles2, -1, -1, qfalse);
			}
...
//		cg_trap->S_StartSound (NULL, es->number, CHAN_AUTO, cgs.media.missileStick );
		break;
	...
	case EV_MISSILE_HIT:
		...
		{//hack: this is an index to a custom effect to use
			cg_trap->FX_PlayEffectID(cgs.gameEffects[es->emplacedOwner], position, dir, -1, -1, qfalse);
		}
	...
	case EV_PLAY_EFFECT:
		...
		if (eID != -1)
		{
			...
			cg_trap->FX_PlayEffectID(eID, es->origin, fxDir, -1, -1, qfalse);
		}
		break;
	...
	case EV_PLAY_PORTAL_EFFECT_ID:
		DEBUGNAME("EV_PLAY_EFFECT_ID");
		{
			...
			else
			{
				...
				if (s && s[0])
				{
					efxIndex = cg_trap->FX_RegisterEffect(s);
				}
			}
			...
			if (efxIndex)
			{
				if (portalEffect)
				{
					cg_trap->FX_PlayEffectID(efxIndex, position, fxDir, -1, -1, qtrue );
				}
				else
				{
					cg_trap->FX_PlayEffectID(efxIndex, position, fxDir, -1, -1, qfalse );
				}
			}
		}
		break;
	...
	case EV_BMODEL_SOUND:
		DEBUGNAME("EV_BMODEL_SOUND");
		{
			...
			sfx = cg_trap->AS_GetBModelSound(soundSet, es->eventParm);
			...
			cg_trap->S_StartSound( NULL, es->number, CHAN_AUTO, sfx );
		}
		break;
	...
	case EV_MUTE_SOUND:
		...
		cg_trap->S_MuteSound(es->trickedentindex2, es->trickedentindex);
			...
			if (sfx)
			{
				...
				{ //play on the head as well to simulate hearing in radio and in world
					...
					{ //don't hear it if this person is on the other team, but they can still
						...
						cg_trap->S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_MENU1, sfx);
					}
				}
				if (ci->team == cg.predictedPlayerState.persistant[PERS_TEAM])
				{ //add to the chat box
					...
					cg_trap->Print("*%s", vchatstr);
				}

				//and play in world for everyone
				cg_trap->S_StartSound (NULL, clientNum, CHAN_VOICE, sfx);
	...
	case EV_GENERAL_SOUND:
		...
		{
			if ( cgs.gameSounds[ es->eventParm ] ) {
				cg_trap->S_StartSound (NULL, es->number, es->saberEntityNum, cgs.gameSounds[ es->eventParm ] );
			} else {
				s = CG_ConfigString( CS_SOUNDS + es->eventParm );
				cg_trap->S_StartSound (NULL, es->number, es->saberEntityNum, CG_CustomSound( es->number, s ) );
			}
		}
		break;
	...
	case EV_GLOBAL_SOUND:	// play from the player's head so it never diminishes
		...
		if ( cgs.gameSounds[ es->eventParm ] ) {
			cg_trap->S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_MENU1, cgs.gameSounds[ es->eventParm ] );
		} else {
			s = CG_ConfigString( CS_SOUNDS + es->eventParm );
			cg_trap->S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_MENU1, CG_CustomSound( es->number, s ) );
		}
		break;
	...
	case EV_ENTITY_SOUND:
		...
		if ( cgs.gameSounds[ es->eventParm ] ) {
			cg_trap->S_StartSound (NULL, es->clientNum, es->trickedentindex, cgs.gameSounds[ es->eventParm ] );
		} else {
			s = CG_ConfigString( CS_SOUNDS + es->eventParm );
			cg_trap->S_StartSound (NULL, es->clientNum, es->trickedentindex, CG_CustomSound( es->clientNum, s ) );
		}
		break;

	case EV_PLAY_ROFF:
		DEBUGNAME("EV_PLAY_ROFF");
		cg_trap->ROFF_Play(es->weapon, es->eventParm, es->trickedentindex);
		break;
	...
	case EV_DEATH3:
		DEBUGNAME("EV_DEATHx");
		cg_trap->S_StartSound( NULL, es->number, CHAN_VOICE,
				CG_CustomSound( es->number, va("*death%i.wav", event - EV_DEATH1 + 1) ) );
		if (es->eventParm && es->number == cg.snap->ps.clientNum)
		{
			cg_trap->S_StartLocalSound(cgs.media.dramaticFailure, CHAN_LOCAL);
			CGCam_SetMusicMult(0.3f, 5000);
		}
		break;
	...
	case EV_POWERUP_QUAD:
		...
		//cg_trap->S_StartSound (NULL, es->number, CHAN_ITEM, cgs.media.quadSound );
		break;
	case EV_POWERUP_BATTLESUIT:
		...
		//cg_trap->S_StartSound (NULL, es->number, CHAN_ITEM, cgs.media.protectSound );
		break;
#endif // BASE_COMPAT
	...
	case EV_FORCE_DRAINED:
		...
		cg_trap->S_StartSound (NULL, es->owner, CHAN_AUTO, cgs.media.drainSound );
	...
	case EV_GIB_PLAYER:
		...
		//cg_trap->S_StartSound( NULL, es->number, CHAN_BODY, cgs.media.gibSound );
	...
	case EV_WEAPON_CHARGE:
		...
		{
			cg_trap->S_StartSound(NULL, es->number, CHAN_WEAPON, cg_weapons[es->eventParm].chargeSound);
		}
		else if (es->eventParm == WP_DISRUPTOR)
		{
			cg_trap->S_StartSound(NULL, es->number, CHAN_WEAPON, cgs.media.disruptorZoomLoop);
		}
		break;
	...
	case EV_WEAPON_CHARGE_ALT:
		...
		{
			cg_trap->S_StartSound(NULL, es->number, CHAN_WEAPON, cg_weapons[es->eventParm].altChargeSound);
		}
		break;
	...
	default:
		DEBUGNAME("UNKNOWN");
		cg_trap->Error( ERR_DROP, "Unknown event: %i", event );
		break;
	}
...
void CG_CheckEvents( centity_t *cent ) {
	...
	BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, cg.snap->serverTime, cent->lerpOrigin );
