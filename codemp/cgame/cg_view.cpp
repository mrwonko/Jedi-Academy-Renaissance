...
void CG_TestModel_f (void) {
	...
	if ( cg_trap->Cmd_Argc() < 2 ) {
		return;
	}

	Q_strncpyz (cg.testModelName, CG_Argv( 1 ), MAX_QPATH );
	cg.testModelEntity.hModel = cg_trap->R_RegisterModel( cg.testModelName );
	...
	if (! cg.testModelEntity.hModel ) {
		cg_trap->Print( "Can't register model\n" );
		return;
	}
...
static void CG_AddTestModel (void) {
	...
	cg_trap->R_AddRefEntityToScene( &cg.testModelEntity );
}
...
static void CG_CalcVrect (void) {
	...
	if ( cg.snap->ps.pm_type == PM_INTERMISSION ) {
		...
		if ( cg_viewsize.integer < 30 ) {
			cg_trap->Cvar_Set( "cg_viewsize", "30" );
			cg_trap->Cvar_Update( &cg_viewsize );
			size = 30;
		} else if ( cg_viewsize.integer > 100 ) {
			cg_trap->Cvar_Set( "cg_viewsize", "100" );
			cg_trap->Cvar_Update( &cg_viewsize );
			size = 100;
		} else {
		...
		else if (cg.predictedPlayerState.zoomMode)
		{
			if (!cg.predictedPlayerState.zoomLocked)
			{
				...
				{	// Still zooming
					...
					{
						cg_trap->S_StartSound(cg.refdef.vieworg, ENTITYNUM_WORLD, CHAN_LOCAL, cgs.media.disruptorZoomLoop);
				...
				{// valid turret
					...
					{//I control this turret
						...
						{//crap... guess?
							...
							{
								boltIndex = cg_trap->G2API_AddBolt( vehCent->ghoul2, 0, vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum].yawBone );
							}
							else if ( vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum].pitchBone )
							{
								boltIndex = cg_trap->G2API_AddBolt( vehCent->ghoul2, 0, vehCent->m_pVehicle->m_pVehicleInfo->turret[turretNum].pitchBone );
							}
							else
						...
						if ( boltIndex != -1 )
						{
							...
							vec3_t fwd, up;
							cg_trap->G2API_GetBoltMatrix_NoRecNoRot(vehCent->ghoul2, 0, boltIndex, &boltMatrix, vehCent->lerpAngles,
...
/*
	if (cg.cameraMode) {
		...
		if (cg_trap->getCameraInfo(cg.time, &origin, &angles)) {
...
static void CG_PowerupTimerSounds( void ) {
	...
	for ( i = 0 ; i < MAX_POWERUPS ; i++ ) {
		...
		if ( ( t - cg.time ) / POWERUP_BLINK_TIME != ( t - cg.oldTime ) / POWERUP_BLINK_TIME ) {
			//cg_trap->S_StartSound( NULL, cg.snap->ps.clientNum, CHAN_ITEM, cgs.media.wearOffSound );
		}
	}
}
...
void CG_DrawSkyBoxPortal(const char *cstr)
{
	...
	if (!token || !token[0])
	{
		cg_trap->Error( ERR_DROP, "CG_DrawSkyBoxPortal: error parsing skybox configstring\n");
	}
	...
	if (!token || !token[0])
	{
		cg_trap->Error( ERR_DROP, "CG_DrawSkyBoxPortal: error parsing skybox configstring.  No fog state\n");
	}
	...
	{ //rww - also had to add this to add effects being rendered in portal sky areas properly.
		cg_trap->FX_AddScheduledEffects(qtrue);
	}
...
static void CG_PlayBufferedSounds( void ) {
	if ( cg.soundTime < cg.time ) {
		if (cg.soundBufferOut != cg.soundBufferIn && cg.soundBuffer[cg.soundBufferOut]) {
			cg_trap->S_StartLocalSound(cg.soundBuffer[cg.soundBufferOut], CHAN_ANNOUNCER);
	...
	for ( num = 0 ; num < ENTITYNUM_NONE ; num++ )
	{
		...
		{ //keep sound for this entity updated in accordance with its attached entity at all times
			...
			{ //this is actually the player, so center the sound origin right on top of us
				...
				cg_trap->S_UpdateEntityPosition( cent->currentState.number, cent->lerpOrigin );
			}
			else
			{
				cg_trap->S_UpdateEntityPosition( cent->currentState.number, cg_entities[cent->currentState.trickedentindex].lerpOrigin );
			}
		}
...
void CG_SE_UpdateMusic(void)
{
	...
	if (cgScreenEffects.music_volume_time < cg.time)
	{
		if (cgScreenEffects.music_volume_multiplier != 1.0f || cgScreenEffects.music_volume_set)
		{
			...
			cg_trap->Cvar_Set("s_musicMult", musMultStr);
...
void CG_DoCameraShake( vec3_t origin, float intensity, int radius, int time )
{
	...
	cgame::CGCam_Shake( realIntensity, time );
}
...
void CG_EmplacedView(vec3_t angles)
{
	...
	if (override)
	{
		...
		if (override == 2)
		{
			cg_trap->SetClientForceAngle(cg.time + 5000, cg.refdef.viewangles);
		}
	}
...
void CG_DrawAutoMap(void)
{
	...
	cg_trap->R_GetRealRes(&vWidth, &vHeight);
	...
	{ //constantly adjust to current height
		cg_trap->R_AutomapElevationAdjustment(cg.predictedPlayerState.origin[2]);
	}
	else
	{
		...
		{
			cg_trap->R_AutomapElevationAdjustment(tr.endpos[2]);
		}
	}
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	{
		cg_trap->Cvar_Set ( "ui_myteam", va("%i", cg.snap->ps.persistant[PERS_TEAM]) );
	}
		...
		cg_siegeClassIndex != cgs.clientinfo[cg.snap->ps.clientNum].siegeIndex)
	{
		...
		{
			cg_trap->Cvar_Set("ui_mySiegeClass", "<none>");
		}
		else
		{
			cg_trap->Cvar_Set("ui_mySiegeClass", bgSiegeClasses[cg_siegeClassIndex].name);
		}
	}
	...
	if ( !cg.snap || ( cg.snap->snapFlags & SNAPFLAG_NOT_ACTIVE ) )
	{
		...
		{
			cg.snapshotTimeoutTime = cg_trap->Milliseconds ( );
		}

		// If we have been waiting too long then just error out
		if ( cg.snapshotTimeoutTime > 0 && (cg_trap->Milliseconds ( ) - cg.snapshotTimeoutTime > cg_snapshotTimeout.integer * 1000) )
		{
		...
		&& !cg.predictedPlayerState.generic1 )//not as a passenger
	{
			...
			&& cent->m_pVehicle->m_pVehicleInfo->type == VH_FIGHTER )
		{
			...
			cg_trap->SetUserCmdValue( cg.weaponSelect, mSensitivity, mPitchOverride, mYawOverride, mSensitivityOverride, cg.forceSelect, cg.itemSelect, bUseFighterPitch );
			isFighter = qtrue;
		}
	}
...
#endif //VEH_CONTROL_SCHEME_4
	{
		...
		{
			cg_trap->SetUserCmdValue( cg.weaponSelect, mSensitivity, mPitchOverride, mYawOverride, 0.0f, cg.forceSelect, cg.itemSelect, qtrue );
			veh = NULL; //this is done because I don't want an extra assign each frame because I am so perfect and super efficient.
		}
		else
		{
			cg_trap->SetUserCmdValue( cg.weaponSelect, mSensitivity, mPitchOverride, mYawOverride, 0.0f, cg.forceSelect, cg.itemSelect, qfalse );
		}
	}
	...
	{
		cg_trap->R_SetRangedFog(-cg_linearFogOverride);
	}
	else if (cg.predictedPlayerState.zoomMode)
	{ //zooming with binoculars or sniper, set the fog range based on the zoom level -rww
		...
		cg_trap->R_SetRangedFog(cg.refdef.fov_x*64.0f);
	}
	else if (cg_rangedFogging)
	{ //disable it
		cg_rangedFogging = qfalse;
		cg_trap->R_SetRangedFog(0.0f);
	}
	...
	if ( !cg.hyperspace)
	{
		cg_trap->FX_AddScheduledEffects(qfalse);
	}
	...
	if (cstr && cstr[0])
	{
		cg_trap->S_UpdateAmbientSet( cstr, cg.refdef.vieworg );
	}

	// update audio positions
	cg_trap->S_Respatialize( cg.snap->ps.clientNum, cg.refdef.vieworg, cg.refdef.viewaxis, inwater );
	...
	if (timescale.value != cg_timescaleFadeEnd.value) {
		...
		if (cg_timescaleFadeSpeed.value) {
			cg_trap->Cvar_Set("timescale", va("%f", timescale.value));
		}
	}
	...
	if ( cg_stats.integer ) {
		cg_trap->Print( "cg.clientFrame:%i\n", cg.clientFrame );
	}
}
