...
int CG_Text_Width(const char *text, float scale, int iMenuFont)
{
	...
	return cg_trap->R_Font_StrLenPixels(text, iFontIndex, scale);
}
...
int CG_Text_Height(const char *text, float scale, int iMenuFont)
{
	...
	return cg_trap->R_Font_HeightPixels(iFontIndex, scale);
}
...
void CG_Text_Paint(float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style, int iMenuFont)
{
	...
	cg_trap->R_Font_DrawString(	x,		// int ox
							y,		// int oy
...
/*		val[0] = ammo % 5;
		...
		for ( int t = 0; t < 5; t++ )
		{
			...
			CG_DrawRotatePic2( cx, cy, 24, 38, 45 - t * 10, cg_trap->R_RegisterShader( va("gfx/2d/char%d",val[4-t] )));
		}
*/
		...
		if ( cg.predictedPlayerState.weaponstate == WEAPON_CHARGING_ALT )
		{
			...
			cg_trap->R_DrawStretchPic(257, 435, 134*max, 34, 0, 0, max, 1, cgs.media.disruptorChargeShader);
		}
//		cg_trap->R_SetColor( colorTable[CT_WHITE] );
//		CG_DrawPic( 0, 0, 640, 480, cgs.media.disruptorMask );
...
void CG_DrawFlagModel( float x, float y, float w, float h, int team, qboolean force2D ) {
	...
	if ( !force2D && cg_draw3DIcons.integer ) {
		...
		cg_trap->R_ModelBounds( cm, mins, maxs );
...
void CG_DrawHealth( menuDef_t *menuHUD )
{
	...
	{
		// Print health amount
		cg_trap->R_SetColor( focusItem->window.foreColor );
...
static void CG_DrawAmmo( centity_t	*cent,menuDef_t *menuHUD)
{
	...
	{ //just draw "infinite"
		...
		cg_trap->R_SetColor( colorTable[CT_YELLOW] );
		if (focusItem)
		{
...
void CG_DrawForcePower( menuDef_t *menuHUD )
{
	...
	if (cg.forceHUDTotalFlashTime > cg.time )
	{
		...
		if (cg.forceHUDNextFlashTime < cg.time)
		{
			...
			cg_trap->S_StartSound (NULL, 0, CHAN_LOCAL, cgs.media.noforceSound );
...
//		memcpy(calcColor, colorTable[CT_WHITE], sizeof(vec4_t));
	...
	if (focusItem)
	{
		...
		if ( flash )
		{
			cg_trap->R_SetColor( colorTable[CT_RED] );
		}
		else
		{
			cg_trap->R_SetColor( focusItem->window.foreColor );
...
void CG_DrawHUD(centity_t	*cent)
{
	...
	if (cg.predictedPlayerState.pm_type != PM_SPECTATOR)
	{
		...
		Menu_Paint< moduleType::cgame >( menuHUD, qtrue );
		...
		{
			//cg_trap->Error( ERR_DROP, "CG_ChatBox_ArrayInsert: unable to locate HUD menu file ");
		}
...
//	addX = (float) smallIconSize * .75;
	...
	for (iconCnt=0;iconCnt<sideLeftIconCnt;i--)
	{
		...
		if (cgs.media.invenIcons[i])
		{
			...
			cg_trap->R_SetColor(colorTable[CT_ICON_BLUE]);
...
float CG_DrawVehicleShields( const menuDef_t	*menuHUD, const centity_t *veh )
{
	...
	if (item)
	{
		cg_trap->R_SetColor( item->window.foreColor );
		CG_DrawPic(
			item->window.rect.x,
			item->window.rect.y,
			item->window.rect.w,
...
void CG_DrawVehicleTurboRecharge( const menuDef_t	*menuHUD, const centity_t *veh )
{
	...
	if (item)
	{
		...
		{
			percent = 1.0f;
			cg_trap->R_SetColor( colorTable[CT_GREEN] );
		}
		else
		{
...
*/
	...
	{//state changed, play sound
		...
		cg_trap->S_StartSound (NULL, cg.predictedPlayerState.clientNum, CHAN_LOCAL, cg_trap->S_RegisterSound( "sound/vehicles/common/linkweaps.wav" ) );
	}
	...
	if ( drawLink )
	{
		...
		if (item)
		{
			cg_trap->R_SetColor( colorTable[CT_CYAN] );
			...
			default:
				{
					...
					{ //slightly misleading to use this value, but don't want to add more to entstate.
						...
						cg_trap->Cvar_VariableStringBuffer(va("team%i_objective%i", cent->currentState.brokenLimbs, cent->currentState.frame), objState, 1024);
					...
					&& cent->currentState.speed > 0 )
				{
					if ( cent->m_pVehicle && cent->m_pVehicle->m_pVehicleInfo->radarIconHandle )
					{
							...
							&& cgs.clientinfo[ cent->currentState.m_iVehicleNum-1 ].infoValid )
						{
							...
							{
								cg_trap->R_SetColor ( teamColor );
							}
							else
							{
								cg_trap->R_SetColor ( g_color_table[ColorIndex(COLOR_RED)] );
							}
						}
						else
						{
				...
				{//a mover that's close to me and I'm in a vehicle
					...
					{//not close enough to always warn, yet, so check its direction
						...
						for ( predictTime = timeStep; predictTime < 5000; predictTime+=timeStep )
						{
							...
							BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, cg.time+predictTime, asteroidPos );
					...
					{//possible collision
						...
						if ( impactSoundDebounceTime < cg.time )
						{
							...
							cg_trap->S_StartSound( soundOrg, ENTITYNUM_WORLD, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/vehicles/common/impactalarm.wav" ) );
						}
						...
						cg_trap->R_SetColor ( asteroidColor );
						CG_DrawPic ( x - 4 + xOffset, ly - 4, arrowBaseScale, arrowBaseScale, cg_trap->R_RegisterShaderNoMip( "gfx/menus/radar/asteroid" ) );
					}
				}
				break;
						...
						if ( cent->currentState.otherEntityNum == cg.predictedPlayerState.clientNum || cent->currentState.otherEntityNum == cg.predictedPlayerState.m_iVehicleNum )
						{
							if ( radarLockSoundDebounceTime < cg.time )
							{
								...
								if ( actualDist > RADAR_MISSILE_RANGE * 0.66f )
								{
									...
									arrowBaseScale = 3.0f;
									alarmSound = cg_trap->S_RegisterSound( "sound/vehicles/common/lockalarm1.wav" );
								}
								else if ( actualDist > RADAR_MISSILE_RANGE/3.0f )
								{
									radarLockSoundDebounceTime = cg.time + 500;
									arrowBaseScale = 6.0f;
									alarmSound = cg_trap->S_RegisterSound( "sound/vehicles/common/lockalarm2.wav" );
								}
								else
								{
									radarLockSoundDebounceTime = cg.time + 250;
									arrowBaseScale = 9.0f;
									alarmSound = cg_trap->S_RegisterSound( "sound/vehicles/common/lockalarm3.wav" );
								}
								...
								cg_trap->S_StartSound( soundOrg, ENTITYNUM_WORLD, CHAN_AUTO, alarmSound );
							}
						}
					}
					zScale = 1.0f;
...
//					((lwidth/2 - len/2) * TINYCHAR_WIDTH);
			...
			for (j = 0; j <= PW_NUM_POWERUPS; j++) {
				if (ci->powerups & (1 << j)) {
					...
					item = BG_FindItemForPowerup( static_cast< powerup_t >( j ) );

					if (item) {
						CG_DrawPic( xx + xOffset, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
						cg_trap->R_RegisterShader( item->icon ) );
						if (right) {
							xx -= TINYCHAR_WIDTH;
						} else {
							xx += TINYCHAR_WIDTH;
		...
		if (cg.snap->ps.powerups[j] > cg.time)
		{
			...
			if (item)
			{
				...
				if (cgs.gametype == GT_CTY && (j == PW_REDFLAG || j == PW_BLUEFLAG))
				{
					if (j == PW_REDFLAG)
					{
						icoShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_rflag_ys" );
					}
					else
					{
						icoShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_bflag_ys" );
					}
				}
				else
				{
					icoShader = cg_trap->R_RegisterShader( item->icon );
				}
...
static void CG_DrawReward( void ) {
	...
	if ( !color ) {
		if (cg.rewardStack > 0) {
			...
			cg.rewardStack--;
			color = CG_FadeColor( cg.rewardTime, REWARD_TIME );
			cg_trap->S_StartLocalSound(cg.rewardSound[0], CHAN_ANNOUNCER);
		} else {
			return;
		}
	}
...
static void CG_DrawDisconnect( void ) {
	...
	cmdNum = cg_trap->GetCurrentCmdNumber() - CMD_BACKUP + 1;
	cg_trap->GetUserCmd( cmdNum, &cmd );
	...
	CG_DrawPic( x, y, 48, 48, cg_trap->R_RegisterShader("gfx/2d/net.tga" ) );
}
...
static void CG_DrawLagometer( void ) {
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			if ( color != 1 ) {
				color = 1;
				cg_trap->R_SetColor( g_color_table[ColorIndex(COLOR_YELLOW)] );
			}
			if ( v > range ) {
				v = range;
			}
			cg_trap->R_DrawStretchPic ( ax + aw - a, mid - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 2 ) {
				color = 2;
				cg_trap->R_SetColor( g_color_table[ColorIndex(COLOR_BLUE)] );
			}
			v = -v;
			if ( v > range ) {
				v = range;
			}
			cg_trap->R_DrawStretchPic( ax + aw - a, mid, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			if ( lagometer.snapshotFlags[i] & SNAPFLAG_RATE_DELAYED ) {
				...
				if ( color != 3 ) {
					color = 3;
					cg_trap->R_SetColor( g_color_table[ColorIndex(COLOR_GREEN)] );
				}
			}
			v = v * vscale;
			if ( v > range ) {
			...
			cg_trap->R_DrawStretchPic( ax + aw - a, ay + ah - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 4 ) {
				color = 4;		// RED for dropped snapshots
				cg_trap->R_SetColor( g_color_table[ColorIndex(COLOR_RED)] );
			}
			cg_trap->R_DrawStretchPic( ax + aw - a, ay + ah - range, 1, range, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
...
{
//	if (!( cg_trap->Key_GetCatcher() & KEYCATCH_UI ))
	{
		cg_trap->OpenUIMenu(UIMENU_CLOSEALL);
		cg_trap->Cvar_Set("cg_siegeMessage", str);
		if (objectiveScreen)
		{
			cg_trap->OpenUIMenu(UIMENU_SIEGEOBJECTIVES);
		}
		else
		{
			cg_trap->OpenUIMenu(UIMENU_SIEGEMESSAGE);
		}
	}
}
...
void CG_DrawSiegeMessageNonMenu( const char *str )
{
	...
	if (str[0]=='@')
	{
		cg_trap->SE_GetStringTextString(str+1, text, sizeof(text));
		str = text;
	}
...
static void CG_DrawCrosshair( vec3_t worldPoint, int chEntValid ) {
	...
	cg_trap->R_DrawStretchPic( chX, chY, w, h, 0, 0, 1, 1, hShader );
	...
	if ( corona ) // drawing extra bits
	{
		...
		cg_trap->R_DrawStretchPic( x + cg.refdef.x + 0.5 * (640 - w),
					...
					{//I'm in a vehicle
							...
							&& veh->m_pVehicle->m_pVehicleInfo->weapon[0].ID > VEH_WEAPON_BASE )//valid vehicle weapon
						{
							...
							{//our primary weapon's projectile has a speed
								...
								cg_trap->R_SetColor ( g_color_table[ColorIndex(COLOR_RED)] );
								CG_DrawPic( leadX-8, leadY-8, 16, 16, cg_trap->R_RegisterShader( "gfx/menus/radar/lead" ) );
							}
						}
					}
				}
	...
	if ( CG_WorldCoordToScreenCoord( org, &cx, &cy ))
	{
		...
		if ( oldDif != dif )
		{
			if ( dif == 8 )
			{
				if ( cg.snap->ps.m_iVehicleNum )
				{
					cg_trap->S_StartSound( org, 0, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/vehicles/weapons/common/lock.wav" ));
				}
				else
				{
					cg_trap->S_StartSound( org, 0, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/weapons/rocket/lock.wav" ));
				}
			}
			else
			{
				if ( cg.snap->ps.m_iVehicleNum )
				{
					cg_trap->S_StartSound( org, 0, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/vehicles/weapons/common/tick.wav" ));
				}
				else
				{
					cg_trap->S_StartSound( org, 0, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/weapons/rocket/tick.wav" ));
				}
			}
		}
		...
		for ( i = 0; i < dif; i++ )
		{
			...
			CG_DrawRotatePic( cx - sz, cy - sz, sz, sz, i * 45.0f, cg_trap->R_RegisterShaderNoMip( "gfx/2d/wedge" ));
		}
		...
		if ( dif == 8 )
		{
			...
			CG_DrawPic( cx - sz, cy - sz * 2, sz * 2, sz * 2, cg_trap->R_RegisterShaderNoMip( "gfx/2d/lock" ));
		}
	}
}
...
qboolean CG_CalcVehicleMuzzlePoint( int entityNum, vec3_t start, vec3_t d_f, vec3_t d_rt, vec3_t d_up)
{
	...
	{//draw from barrels
		...
		bolt = cg_trap->G2API_AddBolt( vehCent->ghoul2, 0, "*flash1");
		cg_trap->G2API_GetBoltMatrix( vehCent->ghoul2, 0, bolt, &boltMatrix,
...
{
	int bolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*cannonflash");
	...
	if (bolt != -1)
	{
		...
		cg_trap->G2API_GetBoltMatrix_NoRecNoRot(cent->ghoul2, 0, bolt, &boltMatrix, cent->lerpAngles, cent->lerpOrigin, cg.time, NULL, cent->modelScale);
		BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, start);
...
static void CG_DrawVote(void) {
	...
	if ( cgs.voteModified ) {
		...
		cg_trap->S_StartLocalSound( cgs.media.talkSound, CHAN_LOCAL_SOUND );
	}
	...
	if ( !Q_strncmp( cgs.voteString, "map_restart", 11 ) )
		cg_trap->SE_GetStringTextString( "MENUS_RESTART_MAP", sCmd, sizeof( sCmd ) );
	else if ( !Q_strncmp( cgs.voteString, "vstr nextmap", 12 ) )
		cg_trap->SE_GetStringTextString( "MENUS_NEXT_MAP", sCmd, sizeof( sCmd ) );
	else if ( !Q_strncmp( cgs.voteString, "g_doWarmup", 10 ) )
		cg_trap->SE_GetStringTextString( "MENUS_WARMUP", sCmd, sizeof( sCmd ) );
	else if ( !Q_strncmp( cgs.voteString, "g_gametype", 10 ) ) {
		cg_trap->SE_GetStringTextString( "MENUS_GAME_TYPE", sCmd, sizeof( sCmd ) );
		...
		cg_trap->SE_GetStringTextString( "MENUS_NEW_MAP", sCmd, sizeof( sCmd ) );
		sParm = cgs.voteString+4;
	}
	else if ( !Q_strncmp( cgs.voteString, "kick", 4 ) ) {
		cg_trap->SE_GetStringTextString( "MENUS_KICK_PLAYER", sCmd, sizeof( sCmd ) );
	...
	cg_trap->SE_GetStringTextString( "MENUS_VOTE", sVote, sizeof( sVote ) );
...
//	s = va( "Starts in: %i", sec + 1 );
	...
	if ( sec != cg.warmupCount ) {
		...
		if (cgs.gametype != GT_SIEGE)
		{
			switch ( sec ) {
			case 0:
				cg_trap->S_StartLocalSound( cgs.media.count1Sound, CHAN_ANNOUNCER );
				break;
			case 1:
				cg_trap->S_StartLocalSound( cgs.media.count2Sound, CHAN_ANNOUNCER );
				break;
			case 2:
				cg_trap->S_StartLocalSound( cgs.media.count3Sound, CHAN_ANNOUNCER );
				break;
			default:
				break;
			}
...
void CG_DrawTimedMenus() {
	if (cg.voiceTime) {
		...
		if ( t > 2500 ) {
			...
			cg_trap->Cvar_Set("cl_conXOffset", "0");
			cg.voiceTime = 0;
		}
	}
}
...
void CG_DrawFlagStatus()
{
	...
	if (cgs.gametype == GT_CTY)
	{
		if (team == TEAM_RED)
		{
			myFlagTakenShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_rflag_x" );
			theirFlagShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_bflag_ys" );
		}
		else
		{
			myFlagTakenShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_bflag_x" );
			theirFlagShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_rflag_ys" );
		}
	}
	else
	{
		...
		{
			myFlagTakenShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_rflag_x" );
			theirFlagShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_bflag" );
		}
		else
		{
			myFlagTakenShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_bflag_x" );
			theirFlagShader = cg_trap->R_RegisterShaderNoMip( "gfx/hud/mpi_rflag" );
		}
	}
...
//	cg_trap->Cvar_Set("ui_siegeTimer", timeStr);
...
static void CG_DrawSiegeHUDItem(void)
{
	...
	if (handle)
	{
		cg_trap->R_ModelBounds( handle, mins, maxs );
	}
	else
	{
...
void CG_ChatBox_ArrayInsert(chatBoxItem_t **array, int insPoint, int maxNum, chatBoxItem_t *item)
{
	...
	{ //recursively call, to move everything up to the top
		...
		{
			cg_trap->Error( ERR_DROP, "CG_ChatBox_ArrayInsert: Exceeded array size");
		}
...
void CG_DrawMiscStaticModels( void ) {
	...
	for( i = 0; i < cgs.numMiscStaticModels; i++ ) {
		...
		if( !cg_trap->R_InPVS( cg.refdef.vieworg, cullorg, cg.refdef.areamask ) ) {
			continue;
		}
...
void CG_DrawActive( stereoFrame_t stereoView ) {
	...
	default:
		separation = 0;
		cg_trap->Error( ERR_DROP, "CG_DrawActive: Undefined stereoView" );
	}
	...
	// draw 3D view
	cg_trap->R_RenderScene( &cg.refdef );
