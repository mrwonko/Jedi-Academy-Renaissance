...
void FX_DEMP2_ProjectileThink( centity_t *cent, const struct weaponInfo_s *weapon )
{
	...
	cg_trap->FX_PlayEffectID( cgs.effects.demp2ProjectileEffect, cent->lerpOrigin, forward, -1, -1, qfalse );
}
...
{
	cg_trap->FX_PlayEffectID( cgs.effects.demp2WallImpactEffect, origin, normal, -1, -1, qfalse );
}
...
{
	cg_trap->FX_PlayEffectID( cgs.effects.demp2FleshImpactEffect, origin, normal, -1, -1, qfalse );
}
...
{
//NOTENOTE Fix this after cg_trap calls for all primitives are created.
/*
	vec3_t	dir, chaos,
			c1, c2,
			v1, v2;
	...
	FX_AddBezier( start, targ1,
						...
						1.0f, 0.0f, 0.0f,
						chaos, chaos, 0.0f,
						1.0f, cg_trap->R_RegisterShader( "gfx/misc/electric2" ), FX_ALPHA_LINEAR );
	...
	FX_AddSprite( c1, NULL, NULL, 12.0f + Q_flrand(-1.0f, 1.0f) * 4, 0.0f, 1.0f, 1.0f, Q_flrand(0.0f, 1.0f) * 360, 0.0f, 1.0f,
						cg_trap->R_RegisterShader( "gfx/misc/lightningFlash" ));
