...
//		if (ent->number > MAX_CLIENTS && cg.snap && ent->genericenemyindex && (ent->genericenemyindex-MAX_GENTITIES) == cg.snap->ps.clientNum)
		...
		if ( ent->solid == SOLID_BMODEL ) {
			...
			cmodel = cg_trap->CM_InlineModel( ent->modelindex );
			VectorCopy( cent->lerpAngles, angles );
			BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, cg.physicsTime, origin );
		} else {
			// encoded bbox
			x = (ent->solid & 255);
			zd = ((ent->solid>>8) & 255);
			...
			cmodel = cg_trap->CM_TempModel( bmins, bmaxs, 0 );
		...
		cg_trap->CM_TransformedTrace ( &trace, start, end, mins, maxs, cmodel,  mask, origin, angles, 0);
...
void	CG_Trace( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
	...
	cg_trap->CM_Trace ( &t, start, end, mins, maxs, 0, mask, 0);
...
int		CG_PointContents( const vec3_t point, int passEntityNum ) {
	...
	contents = cg_trap->CM_PointContents (point, 0);
	...
	for ( i = 0 ; i < cg_numSolidEntities ; i++ ) {
		...
		contents |= cg_trap->CM_TransformedPointContents( point, cmodel, cent->lerpOrigin, cent->lerpAngles );
	}
...
static void CG_InterpolatePlayerState( qboolean grabAngles ) {
	...
	*out = cg.snap->ps;
	...
	if ( grabAngles ) {
		...
		cmdNum = cg_trap->GetCurrentCmdNumber();
...
static void CG_TouchItem( centity_t *cent ) {
	...
	if ( !BG_PlayerTouchesItem< moduleType::cgame >( &cg.predictedPlayerState, &cent->currentState, cg.time ) ) {
		return;
	}
...
*/
	...
	// grab it
	BG_AddPredictableEventToPlayerstate< moduleType::cgame >( EV_ITEM_PICKUP, cent->currentState.number , &cg.predictedPlayerState);
...
static void CG_TouchTriggerPrediction( void ) {
	...
	for ( i = 0 ; i < cg_numTriggerEntities ; i++ ) {
		...
		cg_trap->CM_Trace( &trace, cg.predictedPlayerState.origin, cg.predictedPlayerState.origin, cg_pmove.mins, cg_pmove.maxs, cmodel, -1, 0 );
...
void CG_PredictPlayerState( void ) {
	...
	if (cg_pmove.ghoul2 != pEnt->ghoul2) //only update it if the g2 instance has changed
	{
			...
			cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR)
		{
			...
			cg_pmove.g2Bolts_LFoot = cg_trap->G2API_AddBolt(pEnt->ghoul2, 0, "*l_leg_foot");
			cg_pmove.g2Bolts_RFoot = cg_trap->G2API_AddBolt(pEnt->ghoul2, 0, "*r_leg_foot");
		}
		else
		{
			cg_pmove.ghoul2 = NULL;
	...
	cg_trap->GetUserCmd( cmdNum, &oldestCmd );
	if ( oldestCmd.serverTime > cg.snap->ps.commandTime
		&& oldestCmd.serverTime < cg.time ) {	// special check for map_restart
		if ( cg_showMiss.integer ) {
			cg_trap->Print ("exceeded PACKET_BACKUP on commands\n");
		}
		return;
	}

	// get the latest command so we can know which commands are from previous map_restarts
	cg_trap->GetUserCmd( current, &latestCmd );
	...
	if ( pmove_msec.integer < 8 ) {
		cg_trap->Cvar_Set("pmove_msec", "8");
	}
	else if (pmove_msec.integer > 33) {
		cg_trap->Cvar_Set("pmove_msec", "33");
	}
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		// get the command
		cg_trap->GetUserCmd( cmdNum, &cg_pmove.cmd );
			...
			cg.predictedVehicleState.commandTime == oldVehicleState.commandTime )
		{
			...
			if ( cg.thisFrameTeleport ) {
				...
				if ( cg_showVehMiss.integer ) {
					cg_trap->Print( "VEH PredictionTeleport\n" );
				}
				...
				if ( cg_showVehMiss.integer ) {
					if (!VectorCompare( oldVehicleState.origin, adjusted )) {
						cg_trap->Print("VEH prediction error\n");
					}
				}
				...
				if ( len > 0.1 ) {
					if ( cg_showVehMiss.integer ) {
						cg_trap->Print("VEH Prediction miss: %f\n", len);
					}
					if ( cg_errorDecay.integer ) {
						...
						if ( f > 0 && cg_showVehMiss.integer ) {
							cg_trap->Print("VEH Double prediction decay: %f\n", f);
						}
			...
			cg.predictedPlayerState.commandTime == oldPlayerState.commandTime )
		{
			...
			if ( cg.thisFrameTeleport ) {
				...
				VectorClear( cg.predictedError );
				if ( cg_showMiss.integer ) {
					cg_trap->Print( "PredictionTeleport\n" );
				}
				cg.thisFrameTeleport = qfalse;
			} else {
				vec3_t	adjusted;
				...
				if ( cg_showMiss.integer ) {
					if (!VectorCompare( oldPlayerState.origin, adjusted )) {
						cg_trap->Print("prediction error\n");
					}
				}
				...
				if ( len > 0.1 ) {
					if ( cg_showMiss.integer ) {
						cg_trap->Print("Prediction miss: %f\n", len);
					}
					if ( cg_errorDecay.integer ) {
						int		t;
						float	f;
						...
						if ( f > 0 && cg_showMiss.integer ) {
							cg_trap->Print("Double prediction decay: %f\n", f);
						}
						VectorScale( cg.predictedError, f, cg.predictedError );
					} else {
						VectorClear( cg.predictedError );
		...
		Pmove< moduleType::cgame >(&cg_pmove);
		...
		{ //we're riding a vehicle, let's predict it
			...
			{ //make sure pointer is set up to go to our predicted state
				...
				cg_trap->GetUserCmd( cmdNum, &veh->m_pVehicle->m_ucmd );
				...
				*/
				...
				Pmove< moduleType::cgame >(&cg_vehPmove);
				/*
	...
	if ( cg_showMiss.integer > 1 ) {
		cg_trap->Print( "[%i : %i] ", cg_pmove.cmd.serverTime, cg.time );
	}

	if ( !moved ) {
		if ( cg_showMiss.integer ) {
			cg_trap->Print( "not moved\n" );
		}
		goto revertES;
	}
	...
	if ( cg_showMiss.integer ) {
		if (cg.predictedPlayerState.eventSequence > oldPlayerState.eventSequence + MAX_PS_EVENTS) {
			cg_trap->Print("WARNING: dropped event\n");
		}
	}
	...
	if ( cg_showMiss.integer ) {
		if (cg.eventSequence > cg.predictedPlayerState.eventSequence) {
			cg_trap->Print("WARNING: double event\n");
