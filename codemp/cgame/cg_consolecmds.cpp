...
void CG_TargetCommand_f( void ) {
	...
	cg_trap->Cmd_Argv( 1, test, 4 );
	cg_trap->SendClientCommand( va( "gc %i %i", targetNum, atoi( test ) ) );
}
...
static void CG_SizeUp_f (void) {
	cg_trap->Cvar_Set( "cg_viewsize", va( "%i", Q_min( cg_viewsize.integer + 10, 100 ) ) );
}
...
static void CG_SizeDown_f (void) {
	cg_trap->Cvar_Set( "cg_viewsize", va( "%i", Q_max( cg_viewsize.integer - 10, 30 ) ) );
}
...
static void CG_ScoresDown_f( void ) {
	...
	if ( cg.scoresRequestTime + 2000 < cg.time ) {
		...
		cg_trap->SendClientCommand( "score" );
...
static void CG_TellTarget_f( void ) {
	...
	cg_trap->Cmd_Args( message, sizeof(message) );
	Com_sprintf( command, sizeof(command), "tell %i %s", clientNum, message );
	cg_trap->SendClientCommand( command );
}
...
static void CG_StartOrbit_f( void ) {
	...
	cg_trap->Cvar_VariableStringBuffer( "developer", var, sizeof( var ) );
	if ( !atoi(var) ) {
		return;
	}
	if (cg_cameraOrbit.value != 0) {
		cg_trap->Cvar_Set ("cg_cameraOrbit", "0");
		cg_trap->Cvar_Set("cg_thirdPerson", "0");
	} else {
		cg_trap->Cvar_Set("cg_cameraOrbit", "5");
		cg_trap->Cvar_Set("cg_thirdPerson", "1");
		cg_trap->Cvar_Set("cg_thirdPersonAngle", "0");
		cg_trap->Cvar_Set("cg_thirdPersonRange", "100");
	}
}
...
qboolean CG_ConsoleCommand( void ) {
	cgame::consoleCommand_t	*command = NULL;

	command = (cgame::consoleCommand_t *)Q_LinearSearch( CG_Argv( 0 ), cgame::commands, cgame::numCommands, sizeof( cgame::commands[0] ), cgame::cmdcmp );
...
void CG_InitConsoleCommands( void ) {
	...
	for ( i = 0; i < cgame::numCommands; i++ )
		cg_trap->AddCommand( cgame::commands[i].cmd );
