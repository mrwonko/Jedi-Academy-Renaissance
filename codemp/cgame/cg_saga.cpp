...
void CG_PrecacheSiegeObjectiveAssetsForTeam(int myTeam)
{
	...
	if (!siege_valid)
	{
		cg_trap->Error( ERR_DROP, "Siege data does not exist on client!\n");
		return;
	}
	...
	if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives))
	{
		...
		{ //eh, just try 32 I guess
			...
			if (BG_SiegeGetValueGroup(cgParseObjectives, objstr, foundobjective))
			{
				...
				{
					cg_trap->R_RegisterShaderNoMip(str);
				}
...
void CG_PrecachePlayersForSiegeTeam(int team)
{
	...
	while (i < stm->numClasses)
	{
		...
		if (scl->forcedModel[0])
		{
			...
			cg_trap->R_RegisterModel(va("models/players/%s/model.glm", scl->forcedModel));
			if (scl->forcedSkin[0])
			{
				cg_trap->R_RegisterSkin(va("models/players/%s/model_%s.skin", scl->forcedModel, scl->forcedSkin));
...
void CG_InitSiegeMode(void)
{
	...
	len = cg_trap->FS_Open(levelname, &f, FS_READ);
	...
	cg_trap->FS_Read(siege_info, len, f);
	...
	if (BG_SiegeGetValueGroup(siege_info, "Teams", teams))
	{
		...
		cg_trap->Cvar_VariableStringBuffer("cg_siegeTeam1", buf, 1024);
		...
		{ //it's a damn stringed reference.
			char b[256];
			cg_trap->SE_GetStringTextString(team1+1, b, 256);
			cg_trap->Cvar_Set("cg_siegeTeam1Name", b);
		}
		else
		{
			cg_trap->Cvar_Set("cg_siegeTeam1Name", team1);
		}

		cg_trap->Cvar_VariableStringBuffer("cg_siegeTeam2", buf, 1024);
		if (buf[0] && Q_stricmp(buf, "none"))
		{
		...
		{ //it's a damn stringed reference.
			char b[256];
			cg_trap->SE_GetStringTextString(team2+1, b, 256);
			cg_trap->Cvar_Set("cg_siegeTeam2Name", b);
		}
		else
		{
			cg_trap->Cvar_Set("cg_siegeTeam2Name", team2);
		}
	}
	else
	{
		cg_trap->Error( ERR_DROP, "Siege teams not defined");
	}
	...
	if (BG_SiegeGetValueGroup(siege_info, team1, teamInfo))
	{
		...
		{
			cg_trap->Cvar_Set( "team1_icon", teamIcon);
		}
	...
	{
		cg_trap->Error( ERR_DROP, "No team entry for '%s'\n", team1);
	}

	if (BG_SiegeGetPairedValue(siege_info, "mapgraphic", teamInfo))
	{
		cg_trap->Cvar_Set("siege_mapgraphic", teamInfo);
	}
	else
	{
		cg_trap->Cvar_Set("siege_mapgraphic", "gfx/mplevels/siege1_hoth");
	}

	if (BG_SiegeGetPairedValue(siege_info, "missionname", teamInfo))
	{
		cg_trap->Cvar_Set("siege_missionname", teamInfo);
	}
	else
	{
		cg_trap->Cvar_Set("siege_missionname", " ");
	}
	...
	if (BG_SiegeGetValueGroup(siege_info, team2, teamInfo))
	{
		if (BG_SiegeGetPairedValue(teamInfo, "TeamIcon", teamIcon))
		{
			cg_trap->Cvar_Set( "team2_icon", teamIcon);
		}
	...
	BG_SiegeLoadClasses< moduleType::cgame >(NULL);

	if (!bgNumSiegeClasses)
	{ //We didn't find any?!
		cg_trap->Error( ERR_DROP, "Couldn't find any player classes for Siege");
	}

	//Now load the teams since we have class data.
	BG_SiegeLoadTeams< moduleType::cgame >();

	if (!bgNumSiegeTeams)
	{ //React same as with classes.
		cg_trap->Error( ERR_DROP, "Couldn't find any player teams for Siege");
	}
	...
	if (BG_SiegeGetValueGroup(siege_info, team1, teamInfo))
	{
		...
		{
			cgSiegeTeam1PlShader = cg_trap->R_RegisterShaderNoMip(btime);
		}
		else
		{
	...
	if (BG_SiegeGetValueGroup(siege_info, team2, teamInfo))
	{
		...
		if (BG_SiegeGetPairedValue(teamInfo, "FriendlyShader", btime))
		{
			cgSiegeTeam2PlShader = cg_trap->R_RegisterShaderNoMip(btime);
		}
		else
		{
	...
	while (i <= SIEGETEAM_TEAM2)
	{
		...
		while (j < sTeam->numClasses)
		{
			...
			{ //This class has a forced model, so precache it.
				cg_trap->R_RegisterModel(va("models/players/%s/model.glm", cl->forcedModel));
				...
				{ //also has a forced skin, precache it.
					...
					cg_trap->R_RegisterSkin(useSkinName);
				}
			}
	...
	BG_PrecacheSabersForSiegeTeam< moduleType::cgame >(SIEGETEAM_TEAM1);
	BG_PrecacheSabersForSiegeTeam< moduleType::cgame >(SIEGETEAM_TEAM2);
...
void CG_ParseSiegeObjectiveStatus(const char *str)
{
	...
	while (str[i])
	{
		...
		else if (str[i] == '-')
		{
			...
			if (str[i] == '1')
			{ //it's completed
				cg_trap->Cvar_Set(cvarName, "1");
			}
			else
			{ //otherwise assume it is not
				cg_trap->Cvar_Set(cvarName, "0");
			}
			...
			{ //now set the description and graphic cvars to by read by the menu
				...
				{
					cg_trap->Cvar_Set(cvarName, buffer);
				}
				else
				{
					cg_trap->Cvar_Set(cvarName, "UNSPECIFIED");
				}
				...
				{
					cg_trap->Cvar_Set(cvarName, "0 0 32 32");
				}
			}
		}
		i++;
...
void CG_SiegeRoundOver(centity_t *ent, int won)
{
	...
	if (!siege_valid)
	{
		cg_trap->Error( ERR_DROP, "ERROR: Siege data does not exist on client!\n");
		return;
	}
	...
	if (BG_SiegeGetValueGroup(siege_info, teamstr, cgParseObjectives))
	{
		...
		*/
		...
		if (soundstr[0])
		{
			cg_trap->S_StartLocalSound(cg_trap->S_RegisterSound(soundstr), CHAN_ANNOUNCER);
		}
	}
}
...
void CG_SiegeBriefingDisplay(int team, int dontshow)
{
	...
	cg_trap->Cvar_Set(va("siege_primobj_inuse"), "0");
	...
	{ //do up to 16 objectives I suppose
		...
		properValue[0] = 0;
		cg_trap->Cvar_VariableStringBuffer(va("team%i_objective%i", useTeam, i), properValue, 1024);
		if (primary)
		{
			cg_trap->Cvar_Set(va("siege_primobj"), properValue);
		}
		else
		{
			cg_trap->Cvar_Set(va("siege_objective%i", i), properValue);
		}

		//Now set the long desc cvar for the menu to display.
		properValue[0] = 0;
		cg_trap->Cvar_VariableStringBuffer(va("team%i_objective%i_longdesc", useTeam, i), properValue, 1024);
		if (primary)
		{
			cg_trap->Cvar_Set(va("siege_primobj_longdesc"), properValue);
		}
		else
		{
			cg_trap->Cvar_Set(va("siege_objective%i_longdesc", i), properValue);
		}

		//Now set the gfx cvar for the menu to display.
		properValue[0] = 0;
		cg_trap->Cvar_VariableStringBuffer(va("team%i_objective%i_gfx", useTeam, i), properValue, 1024);
		if (primary)
		{
			cg_trap->Cvar_Set(va("siege_primobj_gfx"), properValue);
		}
		else
		{
			cg_trap->Cvar_Set(va("siege_objective%i_gfx", i), properValue);
		}

		//Now set the mapicon cvar for the menu to display.
		properValue[0] = 0;
		cg_trap->Cvar_VariableStringBuffer(va("team%i_objective%i_mapicon", useTeam, i), properValue, 1024);
		if (primary)
		{
			cg_trap->Cvar_Set(va("siege_primobj_mapicon"), properValue);
		}
		else
		{
			cg_trap->Cvar_Set(va("siege_objective%i_mapicon", i), properValue);
		}

		//Now set the mappos cvar for the menu to display.
		properValue[0] = 0;
		cg_trap->Cvar_VariableStringBuffer(va("team%i_objective%i_mappos", useTeam, i), properValue, 1024);
		if (primary)
		{
			cg_trap->Cvar_Set(va("siege_primobj_mappos"), properValue);
		}
		else
		{
			cg_trap->Cvar_Set(va("siege_objective%i_mappos", i), properValue);
		}
		...
		{ //found a valid objective description
			if ( primary )
			{
				cg_trap->Cvar_Set(va("siege_primobj_desc"), objectiveDesc);
				//this one is marked not in use because it gets primobj
				cg_trap->Cvar_Set(va("siege_objective%i_inuse", i), "0");
				cg_trap->Cvar_Set(va("siege_primobj_inuse"), "1");

				cg_trap->Cvar_Set(va("team%i_objective%i_inuse", useTeam, i), "1");

			}
			else
			{
				cg_trap->Cvar_Set(va("siege_objective%i_desc", i), objectiveDesc);
				cg_trap->Cvar_Set(va("siege_objective%i_inuse", i), "2");
				cg_trap->Cvar_Set(va("team%i_objective%i_inuse", useTeam, i), "2");
		...
		{ //didn't find one, so set the "inuse" cvar to 0 for the objective and mark it non-complete.
			cg_trap->Cvar_Set(va("siege_objective%i_inuse", i), "0");
			cg_trap->Cvar_Set(va("siege_objective%i", i), "0");
			cg_trap->Cvar_Set(va("team%i_objective%i_inuse", useTeam, i), "0");
			cg_trap->Cvar_Set(va("team%i_objective%i", useTeam, i), "0");

			cg_trap->Cvar_Set(va("siege_objective%i_mappos", i), "");
			cg_trap->Cvar_Set(va("team%i_objective%i_mappos", useTeam, i), "");
			cg_trap->Cvar_Set(va("siege_objective%i_gfx", i), "");
			cg_trap->Cvar_Set(va("team%i_objective%i_gfx", useTeam, i), "");
			cg_trap->Cvar_Set(va("siege_objective%i_mapicon", i), "");
			cg_trap->Cvar_Set(va("team%i_objective%i_mapicon", useTeam, i), "");
		}
...
{
	int numEntries = cg_trap->Cmd_Argc();
	int i = 0;
...
void CG_SetSiegeTimerCvar ( int msec )
{
	...
	cg_trap->Cvar_Set("ui_siegeTimer", va( "%i:%i%i", mins, tens, seconds ) );
}
