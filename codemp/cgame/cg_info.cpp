...
/*
	...
	if ( loadingPlayerIconCount < MAX_LOADING_PLAYER_ICONS ) {
		...
		loadingPlayerIcons[loadingPlayerIconCount] = cg_trap->R_RegisterShaderNoMip( iconName );
...
//	Q_CleanStr( personality );
	...
	if( cgs.gametype == GT_SINGLE_PLAYER ) {
		cg_trap->S_RegisterSound( va( "sound/player/announce/%s.wav", personality ));
	}
	*/
...
void CG_DrawInformation( void ) {
	...
	levelshot = cg_trap->R_RegisterShaderNoMip( va( "levelshots/%s", s ) );
	if ( !levelshot ) {
		levelshot = cg_trap->R_RegisterShaderNoMip( "menu/art/unknownmap_mp" );
	}
...
//	CG_DrawLoadingIcons();
	...
	cg_trap->Cvar_VariableStringBuffer( "sv_running", buf, sizeof( buf ) );
	if ( !atoi( buf ) ) {
		// server hostname
		...
		{	// display global MOTD at bottom (mirrors ui_main UI_DrawConnectScreen
			...
			cg_trap->Cvar_VariableStringBuffer( "cl_motdString", motdString, sizeof( motdString ) );
    ...
    if (cgs.gametype != GT_SIEGE)
	{
		...
		if ( value && !valueNOFP && (value < NUM_FORCE_MASTERY_LEVELS) ) {
			...
			cg_trap->SE_GetStringTextString("MP_INGAME_MAXFORCERANK",fmStr, sizeof(fmStr));
