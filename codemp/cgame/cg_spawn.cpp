...
qboolean CG_SpawnString( const char *key, const char *defaultString, char **out ) {
	...
	if( !cg.spawning ) {
		*out = (char*)defaultString;
		// cg_trap->Error( ERR_DROP, "CG_SpawnString() called while not spawning" );
	}
...
qboolean CG_SpawnVector( const char *key, const char *defaultString, float *out ) {
	...
	if ( sscanf( s, "%f %f %f", &out[0], &out[1], &out[2] ) != 3 ) {
		cg_trap->Print( "CG_SpawnVector: Failed sscanf on %s (default: %s)\n", key, defaultString );
		VectorClear( out );
		return qfalse;
	}
	return present;
...
void SP_misc_model_static( void ) {
	...
	if( cgs.numMiscStaticModels >= MAX_STATIC_MODELS ) {
		cg_trap->Error( ERR_DROP, "MAX_STATIC_MODELS(%i) hit", MAX_STATIC_MODELS );
	}

	CG_SpawnString( "model", "", &model );

	if( !model || !model[0] ) {
		cg_trap->Error( ERR_DROP, "misc_model_static with no model." );
	}
	...
	modelIndex = cg_trap->R_RegisterModel( model );
	if( modelIndex == 0 ) {
		cg_trap->Error( ERR_DROP, "misc_model_static failed to load model '%s'", model );
		return;
	}
...
void SP_misc_skyportal_orient( void ) {
	if( cg_skyOri )
		cg_trap->Print( S_COLOR_YELLOW "WARNING: multiple misc_skyportal_orients found.\n" );
...
void SP_misc_weather_zone( void ) {
	...
	if( !model || !model[0] ) {
		cg_trap->Error( ERR_DROP, "misc_weather_zone with invalid brush model data." );
		return;
	}

	cg_trap->R_ModelBounds( cg_trap->R_RegisterModel( model ), mins, maxs );

	cg_trap->WE_AddWeatherZone( mins, maxs );
}
...
void CG_ParseEntityFromSpawnVars( void ) {
	...
	if( CG_SpawnString( "classname", "", &classname ) ) {
		s = (cgame::spawn_t *)Q_LinearSearch( classname, cgame::spawns, ARRAY_LEN( cgame::spawns ), sizeof( cgame::spawn_t ), cgame::spawncmp );
		if ( s )
			s->spawn();
	}
}
...
char *CG_AddSpawnVarToken( const char *string ) {
	...
	if( cg.numSpawnVarChars + l + 1 > MAX_SPAWN_VARS_CHARS ) {
		cg_trap->Error( ERR_DROP, "CG_AddSpawnVarToken: MAX_SPAWN_VARS_CHARS" );
	}
...
qboolean CG_ParseSpawnVars( void ) {
	...
	if( !cg_trap->R_GetEntityToken( com_token, sizeof( com_token ) ) ) {
		// end of spawn string
		return qfalse;
	}

	if( com_token[0] != '{' ) {
		cg_trap->Error( ERR_DROP, "CG_ParseSpawnVars: found %s when expecting {", com_token );
	}

	// go through all the key / value pairs
	while( 1 ) {
		// parse key
		if( !cg_trap->R_GetEntityToken( keyname, sizeof( keyname ) ) ) {
			cg_trap->Error( ERR_DROP, "CG_ParseSpawnVars: EOF without closing brace" );
		}
		...
		if( com_token[0] == '}' ) {
			cg_trap->Error( ERR_DROP, "CG_ParseSpawnVars: closing brace without data" );
		}

		if( cg.numSpawnVars == MAX_SPAWN_VARS ) {
			cg_trap->Error( ERR_DROP, "CG_ParseSpawnVars: MAX_SPAWN_VARS" );
		}
...
namespace cgame {
	void SP_worldspawn( void ) {
		...
		if( Q_stricmp( s, "worldspawn" ) ) {
			cg_trap->Error( ERR_DROP, "SP_worldspawn: The first entity isn't 'worldspawn'" );
		}
...
void CG_ParseEntitiesFromString( void ) {
	...
	cg_trap->R_GetEntityToken( NULL, -1 );
	...
	if( !CG_ParseSpawnVars() ) {
		cg_trap->Error( ERR_DROP, "ParseEntities: no entities" );
	}
