...
void CG_FreeLocalEntity( localEntity_t *le ) {
	if ( !le->prev ) {
		cg_trap->Error( ERR_DROP, "CG_FreeLocalEntity: not active" );
		return;
	}
...
void CG_BloodTrail( localEntity_t *le ) {
	...
	for ( ; t <= t2; t += step ) {
		BG_EvaluateTrajectory< moduleType::cgame >( &le->pos, t, newOrigin );
...
void CG_FragmentBounceSound( localEntity_t *le, trace_t *trace ) {
	...
	if ( rand() & 1 )
	{
		...
		if ( s )
		{
			cg_trap->S_StartSound( trace->endpos, ENTITYNUM_WORLD, CHAN_AUTO, s );
		}
...
void CG_ReflectVelocity( localEntity_t *le, trace_t *trace ) {
	...
	BG_EvaluateTrajectoryDelta< moduleType::cgame >( &le->pos, hitTime, velocity );
...
void CG_AddFragment( localEntity_t *le ) {
	...
	if ( le->pos.trType == TR_STATIONARY ) {
		...
		if ( t < (SINK_TIME*2) ) {
			...
			cg_trap->R_AddRefEntityToScene( &le->refEntity );
		} else {
			cg_trap->R_AddRefEntityToScene( &le->refEntity );
		}
	...
	BG_EvaluateTrajectory< moduleType::cgame >( &le->pos, cg.time, newOrigin );
	...
	if ( trace.fraction == 1.0 ) {
		...
		if ( le->leFlags & LEF_TUMBLE ) {
			...
			BG_EvaluateTrajectory< moduleType::cgame >( &le->angles, cg.time, angles );
	...
	if (!trace.startsolid)
	{
		...
		{ //specified bounce sound (debris)
			cg_trap->S_StartSound(le->pos.trBase, ENTITYNUM_WORLD, CHAN_AUTO, le->bounceSound);
		}
...
void CG_AddFadeRGB( localEntity_t *le ) {
	...
	cg_trap->R_AddRefEntityToScene( re );
}
...
static void CG_AddMoveScaleFade( localEntity_t *le ) {
	...
	BG_EvaluateTrajectory< moduleType::cgame >( &le->pos, cg.time, re->origin );
...
static void CG_AddExplosion( localEntity_t *ex ) {
	...
	if ( ex->light ) {
		...
		cg_trap->R_AddLightToScene(ent->origin, light, ex->lightColor[0], ex->lightColor[1], ex->lightColor[2] );
	}
}
...
static void CG_AddSpriteExplosion( localEntity_t *le ) {
	...
	if ( le->light ) {
		...
		cg_trap->R_AddLightToScene(re.origin, light, le->lightColor[0], le->lightColor[1], le->lightColor[2] );
	}
}
...
void CG_AddLocalEntities( void ) {
	...
	for ( ; le != &cg_activeLocalEntities ; le = next ) {
		...
		switch ( le->leType ) {
		default:
			cg_trap->Error( ERR_DROP, "Bad leType: %i", le->leType );
			break;
