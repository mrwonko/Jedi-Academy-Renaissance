...
void CG_SetInitialSnapshot( snapshot_t *snap ) {
	...
	if ((cg_entities[snap->ps.clientNum].ghoul2 == NULL) && cg_trap->G2_HaveWeGhoul2Models(cgs.clientinfo[snap->ps.clientNum].ghoul2Model))
	{
		re->G2API_DuplicateGhoul2Instance(*cgs.clientinfo[snap->ps.clientNum].ghoul2Model, &cg_entities[snap->ps.clientNum].ghoul2);
		CG_CopyG2WeaponInstance(&cg_entities[snap->ps.clientNum], FIRST_WEAPON, cg_entities[snap->ps.clientNum].ghoul2);

		if (cg_trap->G2API_AddBolt(cg_entities[snap->ps.clientNum].ghoul2, 0, "face") == -1)
...
static void CG_TransitionSnapshot( void ) {
	...
	if ( !cg.snap ) {
		cg_trap->Error( ERR_DROP, "CG_TransitionSnapshot: NULL cg.snap" );
	}
	if ( !cg.nextSnap ) {
		cg_trap->Error( ERR_DROP, "CG_TransitionSnapshot: NULL cg.nextSnap" );
	}
...
static snapshot_t *CG_ReadNextSnapshot( void ) {
	...
	if ( cg.latestSnapshotNum > cgs.processedSnapshotNum + 1000 ) {
		cg_trap->Print( "WARNING: CG_ReadNextSnapshot: way out of range, %i > %i\n",
	...
	while ( cgs.processedSnapshotNum < cg.latestSnapshotNum ) {
		...
		r = cg_trap->GetSnapshot( cgs.processedSnapshotNum, dest );

		// FIXME: why would cg_trap->GetSnapshot return a snapshot with the same server time
...
void CG_ProcessSnapshots( void ) {
	...
	cg_trap->GetCurrentSnapshotNumber( &n, &cg.latestSnapshotTime );
	if ( n != cg.latestSnapshotNum ) {
		if ( n < cg.latestSnapshotNum ) {
			// this should never happen
			cg_trap->Error( ERR_DROP, "CG_ProcessSnapshots: n < cg.latestSnapshotNum" );
		}
	...
	do {
		...
		if ( !cg.nextSnap ) {
			...
			if ( cg.nextSnap->serverTime < cg.snap->serverTime ) {
				cg_trap->Error( ERR_DROP, "CG_ProcessSnapshots: Server time went backwards" );
			}
		}
	...
	if ( cg.snap == NULL ) {
		cg_trap->Error( ERR_DROP, "CG_ProcessSnapshots: cg.snap == NULL" );
	}
	...
	if ( cg.nextSnap != NULL && cg.nextSnap->serverTime <= cg.time ) {
		cg_trap->Error( ERR_DROP, "CG_ProcessSnapshots: cg.nextSnap->serverTime <= cg.time" );
	}
