...
void CG_TestLine( vec3_t start, vec3_t end, int time, unsigned int color, int radius) {
	...
	re->customShader = cgs.media.whiteShader; //cg_trap->R_RegisterShaderNoMip("textures/colombia/canvas_doublesided");
...
void CG_DoGlass( vec3_t verts[4], vec3_t normal, vec3_t dmgPt, vec3_t dmgDir, float dmgRadius, int maxShards )
{
	...
	cg_trap->S_StartSound( dmgPt, -1, CHAN_AUTO, cg_trap->S_RegisterSound("sound/effects/glassbreak1.wav"));
...
void CG_GlassShatter(int entnum, vec3_t dmgPt, vec3_t dmgDir, float dmgRadius, int maxShards)
{
	...
	{
		cg_trap->R_GetBModelVerts(cgs.inlineDrawModel[cg_entities[entnum].currentState.modelindex], verts, normal);
...
void CG_MiscModelExplosion( vec3_t mins, vec3_t maxs, int size, material_t chunkType )
{
	...
	eID1 = cg_trap->FX_RegisterEffect( effect );

	if ( effect2 && effect2[0] )
	{
		// FIXME: real precache
		eID2 = cg_trap->FX_RegisterEffect( effect2 );
	}
	...
	for ( i = 0; i < ct; i++ )
	{
		...
		{
			cg_trap->FX_PlayEffectID( eID2, org, dir, -1, -1, qfalse );
		}
		else
		{
			cg_trap->FX_PlayEffectID( eID1, org, dir, -1, -1, qfalse );
		}
	}
}
						...
						float speed, int numChunks, material_t chunkType, int customChunk, float baseScale )
{
	...
	default:
		break;
	case MAT_GLASS:
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgs.media.glassChunkSound );
		return;
		break;
	case MAT_GRATE1:
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgs.media.grateSound );
		return;
		break;
	case MAT_ELECTRICAL:// (sparks)
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cg_trap->S_RegisterSound (va("sound/ambience/spark%d.wav", Q_irand(1, 6))) );
		return;
		break;
	case MAT_DRK_STONE:
	case MAT_LT_STONE:
	...
	case MAT_SNOWY_ROCK:
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgs.media.rockBreakSound );
		bounce = LEBS_ROCK;
		speedMod = 0.5f; // rock blows up less
		break;
	case MAT_GLASS_METAL:
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgs.media.glassChunkSound ); // FIXME: should probably have a custom sound
		bounce = LEBS_METAL;
		break;
	case MAT_CRATE1:
	case MAT_CRATE2:
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgs.media.crateBreakSound[Q_irand(0,1)] );
		break;
	case MAT_METAL:
	case MAT_METAL2:
	case MAT_METAL3:
	case MAT_ELEC_METAL:// FIXME: maybe have its own sound?
		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgs.media.chunkSound );
		bounce = LEBS_METAL;
		speedMod = 0.8f; // metal blows up a bit more
		break;
	case MAT_ROPE:
//		cg_trap->S_StartSound( NULL, owner, CHAN_BODY, cgi_S_RegisterSound( "" ));  FIXME:  needs a sound
		return;
		break;
	}
...
void CG_ScorePlum( int client, vec3_t org, int score ) {
	...
	//cg_trap->Print( "Plum origin %i %i %i -- %i\n", (int)org[0], (int)org[1], (int)org[2], (int)Distance(org, lastPos));
								...
								int msec, qboolean isSprite, float scale, int flags )
{
	...
	if ( msec <= 0 ) {
		cg_trap->Error( ERR_DROP, "CG_MakeExplosion: msec = %i", msec );
	}
