...
void CG_CheckAmmo( void ) {
	...
	if ( cg.lowAmmoWarning != previous ) {
		cg_trap->S_StartLocalSound( cgs.media.noAmmoSound, CHAN_LOCAL_SOUND );
	}
#endif
		...
		if (i > cg.eventSequence - MAX_PREDICTED_EVENTS) {
			...
			if ( ps->events[i & (MAX_PS_EVENTS-1)] != cg.predictableEvents[i & (MAX_PREDICTED_EVENTS-1) ] ) {
				...
				if ( cg_showMiss.integer ) {
					cg_trap->Print("WARNING: changed predicted event\n");
				}
			}
		}
	}
...
//			cg_trap->S_StartLocalSound( cgs.media.shieldPierceSound, CHAN_LOCAL_SOUND );
		}
		else
		{	// Shields didn't really stand in our way.
//			cg_trap->S_StartLocalSound( cgs.media.hitSound, CHAN_LOCAL_SOUND );
		}

		//FIXME: Hit sounds?
		/*
		if (armor > 50 ) {
			cg_trap->S_StartLocalSound( cgs.media.hitSoundHighArmor, CHAN_LOCAL_SOUND );
		} else if (armor || health > 100) {
			cg_trap->S_StartLocalSound( cgs.media.hitSoundLowArmor, CHAN_LOCAL_SOUND );
		} else {
			cg_trap->S_StartLocalSound( cgs.media.hitSound, CHAN_LOCAL_SOUND );
		}
		*/
	} else if ( ps->persistant[PERS_HITS] < ops->persistant[PERS_HITS] ) {
		//cg_trap->S_StartLocalSound( cgs.media.hitTeamSound, CHAN_LOCAL_SOUND );
	}
	...
	if (ps->persistant[PERS_PLAYEREVENTS] != ops->persistant[PERS_PLAYEREVENTS]) {
		if ((ps->persistant[PERS_PLAYEREVENTS] & PLAYEREVENT_DENIEDREWARD) !=
			...
			cg_trap->S_StartLocalSound( cgs.media.deniedSound, CHAN_ANNOUNCER );
		}
		else if ((ps->persistant[PERS_PLAYEREVENTS] & PLAYEREVENT_GAUNTLETREWARD) !=
				(ops->persistant[PERS_PLAYEREVENTS] & PLAYEREVENT_GAUNTLETREWARD)) {
			cg_trap->S_StartLocalSound( cgs.media.humiliationSound, CHAN_ANNOUNCER );
		}
		reward = qtrue;
	}
#else
	...
	if ( cgs.timelimit > 0 && cgAnnouncerTime < cg.time ) {
		...
		if ( !( cg.timelimitWarnings & 4 ) && msec > ( cgs.timelimit * 60 + 2 ) * 1000 ) {
			...
			//cg_trap->S_StartLocalSound( cgs.media.suddenDeathSound, CHAN_ANNOUNCER );
		}
		else if ( !( cg.timelimitWarnings & 2 ) && msec > (cgs.timelimit - 1) * 60 * 1000 ) {
			cg.timelimitWarnings |= 1 | 2;
			cg_trap->S_StartLocalSound( cgs.media.oneMinuteSound, CHAN_ANNOUNCER );
			cgAnnouncerTime = cg.time + 3000;
		}
		else if ( cgs.timelimit > 5 && !( cg.timelimitWarnings & 1 ) && msec > (cgs.timelimit - 5) * 60 * 1000 ) {
			cg.timelimitWarnings |= 1;
			cg_trap->S_StartLocalSound( cgs.media.fiveMinuteSound, CHAN_ANNOUNCER );
			cgAnnouncerTime = cg.time + 3000;
		}
	}
