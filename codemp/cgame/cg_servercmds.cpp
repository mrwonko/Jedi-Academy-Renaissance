...
static void CG_ParseTeamInfo( void ) {
	...
	if ( numSortedTeamPlayers < 0 || numSortedTeamPlayers > TEAM_MAXOVERLAY ) {
		cg_trap->Error( ERR_DROP, "CG_ParseTeamInfo: numSortedTeamPlayers out of range (%d)", numSortedTeamPlayers );
		return;
	}
	...
	for ( i=0; i<numSortedTeamPlayers; i++ ) {
		...
		if ( client < 0 || client >= MAX_CLIENTS ) {
			cg_trap->Error( ERR_DROP, "CG_ParseTeamInfo: bad client number: %d", client );
			return;
		}
...
void CG_ParseServerinfo( void ) {
	...
	cgs.debugMelee = atoi( Info_ValueForKey( info, "g_debugMelee" ) ); //cg_trap->Cvar_GetHiddenVarValue("g_iknowkungfu");
	...
	cg_trap->Cvar_Set("g_gametype", va("%i", cgs.gametype));
	...
	cg_trap->Cvar_Set ( "ui_about_mapname", mapname );

	Com_sprintf( cgs.mapname, sizeof( cgs.mapname ), "maps/%s.bsp", mapname );
	Com_sprintf( cgs.rawmapname, sizeof( cgs.rawmapname ), "maps/%s", mapname );
//	Q_strncpyz( cgs.redTeam, Info_ValueForKey( info, "g_redTeam" ), sizeof(cgs.redTeam) );
//	cg_trap->Cvar_Set("g_redTeam", cgs.redTeam);
//	Q_strncpyz( cgs.blueTeam, Info_ValueForKey( info, "g_blueTeam" ), sizeof(cgs.blueTeam) );
//	cg_trap->Cvar_Set("g_blueTeam", cgs.blueTeam);

	cg_trap->Cvar_Set ( "ui_about_gametype", va("%i", cgs.gametype ) );
	cg_trap->Cvar_Set ( "ui_about_fraglimit", va("%i", cgs.fraglimit ) );
	cg_trap->Cvar_Set ( "ui_about_duellimit", va("%i", cgs.duel_fraglimit ) );
	cg_trap->Cvar_Set ( "ui_about_capturelimit", va("%i", cgs.capturelimit ) );
	cg_trap->Cvar_Set ( "ui_about_timelimit", va("%i", cgs.timelimit ) );
	cg_trap->Cvar_Set ( "ui_about_maxclients", va("%i", cgs.maxclients ) );
	cg_trap->Cvar_Set ( "ui_about_dmflags", va("%i", cgs.dmflags ) );
	cg_trap->Cvar_Set ( "ui_about_hostname", Info_ValueForKey( info, "sv_hostname" ) );
	cg_trap->Cvar_Set ( "ui_about_needpass", Info_ValueForKey( info, "g_needpass" ) );
	cg_trap->Cvar_Set ( "ui_about_botminplayers", Info_ValueForKey ( info, "bot_minplayers" ) );

	//Set the siege teams based on what the server has for overrides.
	cg_trap->Cvar_Set("cg_siegeTeam1", Info_ValueForKey(info, "g_siegeTeam1"));
	cg_trap->Cvar_Set("cg_siegeTeam2", Info_ValueForKey(info, "g_siegeTeam2"));
	...
	while (o && *o) {
		...
		if (n && *n) {
			...
			if (o) {
				...
				timeOffset[o-t] = 0;
				o++;
				cg_trap->R_RemapShader( originalShader, newShader, timeOffset );
			}
		} else {
			break;
		}
...
static void CG_RegisterCustomSounds(clientInfo_t *ci, int setType, const char *psDir)
{
	...
	for ( i = 0 ; i<iTableEntries; i++ )
	{
		...
		s++;
		hSFX = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", psDir, s) );
			...
			if (p)
			{
				...
				if (atoi(testNumber))
				{
					*p = 0;
					...
					hSFX = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", psDir, modifiedSound) );
				}
			}
		}
	  ...
	  //But that would require extra configstring indexing and that is a bad thing.
		...
		while (j < MAX_CUSTOM_SOUNDS)
		{
			...
			{ //whatever it is, try registering it under this folder.
				...
				cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", pEnd, sEnd) );
...
static void CG_ConfigStringModified( void ) {
	...
	else if ( num == CS_CLIENT_DUELISTS )
	{
		...
		{ //don't register vehicle names and saber names as models.
			cgs.gameModels[ num-CS_MODELS ] = cg_trap->R_RegisterModel( modelName );
		}
		else
		{
...
// GHOUL2 Insert start
		...
		cgs.skins[ num-CS_CHARSKINS ] = cg_trap->R_RegisterSkin( str );
		*/
		//rww - removed and replaced with CS_G2BONES
// Ghoul2 Insert end
	} else if ( num >= CS_SOUNDS && num < CS_SOUNDS+MAX_SOUNDS ) {
		if ( str[0] != '*' ) {	// player specific sounds don't register here
			cgs.gameSounds[ num-CS_SOUNDS] = cg_trap->S_RegisterSound( str );
		}
		else if (str[1] == '$')
		{ //an NPC soundset
		...
		{
			cgs.gameEffects[ num-CS_EFFECTS] = cg_trap->FX_RegisterEffect( str );
		}
	}
...
void CG_KillCEntityG2(int entNum)
{
	...
	if (ci)
	{
		...
		else if (ci->ghoul2Model && cg_trap->G2_HaveWeGhoul2Models(ci->ghoul2Model))
		{
		...
		while (j < MAX_SABERS)
		{
			if (ci->ghoul2Weapons[j] && cg_trap->G2_HaveWeGhoul2Models(ci->ghoul2Weapons[j]))
			{
				re->G2API_CleanGhoul2Models(&ci->ghoul2Weapons[j]);
	...
	if (cent->ghoul2 && re->G2API_HaveWeGhoul2Models(*cent->ghoul2))
	{
		re->G2API_CleanGhoul2Models(&cent->ghoul2);
		cent->ghoul2 = NULL;
	}

	if (cent->grip_arm && re->G2API_HaveWeGhoul2Models(*cent->grip_arm))
	{
		re->G2API_CleanGhoul2Models(&cent->grip_arm);
		cent->grip_arm = NULL;
	}

	if (cent->frame_hold && re->G2API_HaveWeGhoul2Models(*cent->frame_hold))
	{
...
static void CG_MapRestart( void ) {
	if ( cg_showMiss.integer ) {
		cg_trap->Print( "CG_MapRestart\n" );
	}

	cg_trap->R_ClearDecals ( );
	//FIXME: cg_trap->FX_Reset?
	...
	if (cg_singlePlayerActive.integer) {
		cg_trap->Cvar_Set("ui_matchStartTime", va("%i", cg.time));
		if (cg_recordSPDemo.integer && cg_recordSPDemoName.string && *cg_recordSPDemoName.string) {
			cg_trap->SendConsoleCommand(va("set g_synchronousclients 1 ; record %s \n", cg_recordSPDemoName.string));
		}
	}
	*/
//	cg_trap->Cvar_Set("cg_thirdPerson", "0");
}
...
static void CG_BodyQueueCopy(centity_t *cent, int clientNum, int knownWeapon)
{
	...
	re->G2API_DuplicateGhoul2Instance(*source->ghoul2, &cent->ghoul2);

	if (source->isRagging)
	{ //just reset it now.
		source->isRagging = qfalse;
		cg_trap->G2API_SetRagDoll(source->ghoul2, NULL); //calling with null parms resets to no ragdoll.
	}

	//either force the weapon from when we died or remove it if it was a dropped weapon
	if (knownWeapon > WP_BRYAR_PISTOL && cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1))
	{
		cg_trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 1);
	}
	else if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1))
	{
		cg_trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, knownWeapon), 0, cent->ghoul2, 1);
	}
	...
	if (!cent->ownerRagging)
	{
		...
		cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "upper_lumbar", aNum, eFrame, flags, animSpeed, cg.time, -1, 150);
		cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", aNum, eFrame, flags, animSpeed, cg.time, -1, 150);
		cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", aNum, eFrame, flags, animSpeed, cg.time, -1, 150);
	}
...
static void CG_SiegeClassSelect_f( void ) {
	...
	cg_trap->OpenUIMenu( UIMENU_CLASSSEL ); //UIMENU_CLASSSEL
}

static void CG_SiegeProfileMenu_f( void ) {
	if ( !cg.demoPlayback ) {
		cg_trap->Cvar_Set( "ui_myteam", "3" );
		cg_trap->OpenUIMenu( UIMENU_PLAYERCONFIG ); //UIMENU_CLASSSEL
	}
}
...
static void CG_NewForceRank_f( void ) {
	...
	if ( cg_trap->Cmd_Argc() < 3 ) {
#ifdef _DEBUG
		cg_trap->Print("WARNING: Invalid newForceRank string\n");
#endif
		return;
	}
	...
	cg_trap->Cvar_Set( "ui_rankChange", va( "%i", newRank ) );

	cg_trap->Cvar_Set( "ui_myteam", va( "%i", setTeam ) );

	if ( !( cg_trap->Key_GetCatcher() & KEYCATCH_UI ) && doMenu && !cg.demoPlayback )
...
static void CG_KillGhoul2_f( void ) {
	...
	for ( i=1; i<argNum; i++ ) {
		...
		if ( cg_entities[indexNum].ghoul2 && cg_trap->G2_HaveWeGhoul2Models( cg_entities[indexNum].ghoul2 ) ) {
...
static void CG_RestoreClientGhoul_f( void ) {
...
#ifdef _DEBUG
	if ( !cg_trap->G2_HaveWeGhoul2Models( clent->ghoul2 ) )
	...
	if ( clent->isRagging ) {
		...
		cg_trap->G2API_SetRagDoll( clent->ghoul2, NULL ); //calling with null parms resets to no ragdoll.
	}

	//clear all the decals as well
	cg_trap->G2API_ClearSkinGore( clent->ghoul2 );
...
static void CG_CenterPrintSE_f( void ) {
	...
	cg_trap->SE_GetStringTextString( x, strEd, MAX_STRINGED_SV_STRING );
	CG_CenterPrint( strEd, SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH );
}
...
static void CG_Chat_f( void ) {
	...
	if ( !strcmp( cmd, "chat" ) ) {
		...
		if ( !cg_teamChatsOnly.integer ) {
			...
			if ( cg_trap->Cmd_Argc() < 4 )
				return;

			cg_trap->Cmd_Argv( 1, name, sizeof( name ) );
			cg_trap->Cmd_Argv( 2, loc, sizeof( loc ) );
			cg_trap->Cmd_Argv( 3, color, sizeof( color ) );
			cg_trap->Cmd_Argv( 4, message, sizeof( message ) );

			//get localized text
			if ( loc[0] == '@' )
				cg_trap->SE_GetStringTextString( loc+1, loc, sizeof( loc ) );
...
static void CG_RemapShader_f( void ) {
	if ( cg_trap->Cmd_Argc() == 4 ) {
		...
		cg_trap->Cmd_Argv( 1, shader1, sizeof( shader1 ) );
		cg_trap->Cmd_Argv( 2, shader2, sizeof( shader2 ) );
		cg_trap->R_RemapShader( shader1, shader2, CG_Argv( 3 ) );
	}
}
...
static void CG_ServerCommand( void ) {
	...
	cgame::serverCommand_t	*command = NULL;
	...
	command = (cgame::serverCommand_t *)Q_LinearSearch( cmd, cgame::commands, cgame::numCommands, sizeof( cgame::commands[0] ), cgame::svcmdcmp );
	...
	cg_trap->Print( "Unknown client game command: %s\n", cmd );
}
...
void CG_ExecuteNewServerCommands( int latestSequence ) {
	while ( cgs.serverCommandSequence < latestSequence ) {
		if ( cg_trap->GetServerCommand( ++cgs.serverCommandSequence ) ) {
			CG_ServerCommand();
		}
	}
}
