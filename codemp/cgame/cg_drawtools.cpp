...
void CG_DrawSides(float x, float y, float w, float h, float size) {
	...
	cg_trap->R_DrawStretchPic( x, y, size, h, 0, 0, 0, 0, cgs.media.whiteShader );
	cg_trap->R_DrawStretchPic( x + w - size, y, size, h, 0, 0, 0, 0, cgs.media.whiteShader );
}

void CG_DrawTopBottom(float x, float y, float w, float h, float size) {
	size *= cgs.screenYScale;
	cg_trap->R_DrawStretchPic( x, y, w, size, 0, 0, 0, 0, cgs.media.whiteShader );
	cg_trap->R_DrawStretchPic( x, y + h - size, w, size, 0, 0, 0, 0, cgs.media.whiteShader );
}
...
void CG_FillRect2( float x, float y, float width, float height, const float *color ) {
	cg_trap->R_SetColor( color );
	cg_trap->R_DrawStretchPic( x, y, width, height, 0, 0, 0, 0, cgs.media.whiteShader);
	cg_trap->R_SetColor( NULL );
}
...
void CG_DrawPic( float x, float y, float width, float height, qhandle_t hShader ) {
	cg_trap->R_DrawStretchPic( x, y, width, height, 0, 0, 1, 1, hShader );
}
...
void CG_DrawRotatePic( float x, float y, float width, float height,float angle, qhandle_t hShader ) {
	cg_trap->R_DrawRotatePic( x, y, width, height, 0, 0, 1, 1, angle, hShader );
}
...
void CG_DrawRotatePic2( float x, float y, float width, float height,float angle, qhandle_t hShader ) {
	cg_trap->R_DrawRotatePic2( x, y, width, height, 0, 0, 1, 1, angle, hShader );
}
...
void CG_DrawChar( int x, int y, int width, int height, int ch ) {
	...
	cg_trap->R_DrawStretchPic( ax, ay, aw, ah, fcol, frow, fcol + size, frow + size2, cgs.media.charsetShader );
...
static void CG_TileClearBox( int x, int y, int w, int h, qhandle_t hShader ) {
	...
	s2 = (x+w)/64.0;
	t2 = (y+h)/64.0;
	cg_trap->R_DrawStretchPic( x, y, w, h, s1, t1, s2, t2, hShader );
}
