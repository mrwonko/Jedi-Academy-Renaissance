...
sfxHandle_t	CG_CustomSound( int clientNum, const char *soundName ) {
	...
	if ( soundName[0] != '*' ) {
		return cg_trap->S_RegisterSound( soundName );
	}
	...
	//cg_trap->Error( ERR_DROP, "Unknown custom sound: %s", lSoundName );
#ifndef FINAL_BUILD
...
qboolean CG_ParseSurfsFile( const char *modelName, const char *skinName, char *surfOff, char *surfOn )
{
	...
	// load the file
	len = cg_trap->FS_Open( sfilename, &f, FS_READ );
	if ( len <= 0 )
	{//no file
		return qfalse;
	}
...
retryModel:
	...
	if (ci->ghoul2Model && re->G2API_HaveWeGhoul2Models(*ci->ghoul2Model))
	{
		re->G2API_CleanGhoul2Models(&(ci->ghoul2Model));
	}
	...
	checkSkin = cg_trap->R_RegisterSkin(useSkinName);
	...
	{ //fallback to the default skin
		ci->torsoSkin = cg_trap->R_RegisterSkin(va("models/players/%s/model_default.skin", modelName, skinName));
	}
	Com_sprintf( afilename, sizeof( afilename ), "models/players/%s/model.glm", modelName );
	handle = re->G2API_InitGhoul2Model(&ci->ghoul2Model, afilename, 0, ci->torsoSkin, 0, 0, 0);
	...
	cg_trap->G2API_SetSkin(ci->ghoul2Model, 0, ci->torsoSkin, ci->torsoSkin);

	GLAName[0] = 0;

	cg_trap->G2API_GetGLAName( ci->ghoul2Model, 0, GLAName);
	if (GLAName[0] != 0)
	{
	...
	if (!BGPAFtextLoaded)
	{
		...
		else if (BG_ParseAnimationFile< moduleType::cgame >("models/players/_humanoid/animation.cfg", bgHumanoidAnimations, qtrue) == -1)
		{
			Com_Printf( "Failed to load animation file models/players/_humanoid/animation.cfg\n" );
			return qfalse;
		}

		BG_ParseAnimationEvtFile< moduleType::cgame >( "models/players/_humanoid/", 0, -1 ); //get the sounds for the humanoid anims
...
//		}
	...
	{ //make sure the player anim sounds are loaded even if the anims already are
		BG_ParseAnimationEvtFile< moduleType::cgame >( "models/players/_humanoid/", 0, -1 );
//		if (cgs.gametype == GT_SIEGE)
//		{
...
//		}
	...
	{//turn on/off any surfs
		...
		if ( surfOff[0] )
		{
			...
			while ( 1 )
			{
				...
				//turn off this surf
				cg_trap->G2API_SetSurfaceOnOff( ci->ghoul2Model, token, 0x00000002/*G2SURFACEFLAG_OFF*/ );
			}
		}
		if ( surfOn[0] )
		{
			...
			while ( 1 )
			{
				...
				//turn on this surf
				cg_trap->G2API_SetSurfaceOnOff( ci->ghoul2Model, token, 0 );
			}
		}
	}


	ci->bolt_rhand = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*r_hand");

	if (!cg_trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", 0, 12, BONE_ANIM_OVERRIDE_LOOP, 1.0f, cg.time, -1, -1))
	{
		badModel = qtrue;
	}

	if (!cg_trap->G2API_SetBoneAngles(ci->ghoul2Model, 0, "upper_lumbar", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, cg.time))
	{
		badModel = qtrue;
	}

	if (!cg_trap->G2API_SetBoneAngles(ci->ghoul2Model, 0, "cranium", tempVec, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, NULL, 0, cg.time))
	{
		badModel = qtrue;
	}

	ci->bolt_lhand = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*l_hand");

	//rhand must always be first bolt. lhand always second. Whichever you want the
	//jetpack bolted to must always be third.
	cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*chestg");

	//claw bolts
	cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*r_hand_cap_r_arm");
	cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*l_hand_cap_l_arm");

	ci->bolt_head = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*head_top");
	if (ci->bolt_head == -1)
	{
		ci->bolt_head = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "ceyebrow");
	}

	ci->bolt_motion = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "Motion");

	//We need a lower lumbar bolt for footsteps
	ci->bolt_llumbar = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "lower_lumbar");
	...
	{ //special case, turn off the jetpack surfs
		cg_trap->G2API_SetSurfaceOnOff(ci->ghoul2Model, "torso_rjet", TURN_OFF);
		cg_trap->G2API_SetSurfaceOnOff(ci->ghoul2Model, "torso_cjet", TURN_OFF);
		cg_trap->G2API_SetSurfaceOnOff(ci->ghoul2Model, "torso_ljet", TURN_OFF);
	}
...
//	ent->s.radius = 90;
	...
	{
		/*
		if (cg_entities[clientNum].ghoul2 && cg_trap->G2_HaveWeGhoul2Models(cg_entities[clientNum].ghoul2))
		{
			cg_trap->G2API_CleanGhoul2Models(&(cg_entities[clientNum].ghoul2));
		}
		cg_trap->G2API_DuplicateGhoul2Instance(ci->ghoul2Model, &cg_entities[clientNum].ghoul2);
		*/
	...
	ci->modelIcon = cg_trap->R_RegisterShaderNoMip ( va ( "models/players/%s/icon_%s", modelName, skinName ) );
	if (!ci->modelIcon)
	{
        int i = 0;
		int j;
		...
		{ //looks like it actually may be a custom model skin, let's try getting the icon...
			ci->modelIcon = cg_trap->R_RegisterShaderNoMip ( va ( "models/players/%s/%s", modelName, iconName ) );
		}
	}
	return qtrue;
}
...
//load anim info
int CG_G2SkelForModel( CGhoul2Info_v *g2)
{
	int animIndex = -1;
	char GLAName[MAX_QPATH];
	char *slash;
	...
	GLAName[0] = 0;
	char* temp = re->G2API_GetGLAName(*g2, 0);
	if( temp ) {
		strcpy( GLAName, temp );
	}
	...
	if ( slash )
	{
		...
		animIndex = BG_ParseAnimationFile< moduleType::cgame >(GLAName, NULL, qfalse);
	}
...
int CG_G2EvIndexForModel( CGhoul2Info_v *g2, int animIndex)
{
	int evtIndex = -1;
	char GLAName[MAX_QPATH];
	char *slash;
	...
	if ( slash )
	{
		...
		*slash = 0;
		...
		evtIndex = BG_ParseAnimationEvtFile< moduleType::cgame >(GLAName, animIndex, bgNumAnimEvents);
	}
...
void CG_LoadCISounds(clientInfo_t *ci, qboolean modelloaded)
{
	...
	{//try default sounds.cfg first
		fLen = cg_trap->FS_Open(va("models/players/%s/sounds.cfg", dir), &f, FS_READ);
		if ( !f )
		{//no?  Look for _default sounds.cfg
			fLen = cg_trap->FS_Open(va("models/players/%s/sounds_default.cfg", dir), &f, FS_READ);
		}
	}
	else
	{//use the .skin associated with this skin
		fLen = cg_trap->FS_Open(va("models/players/%s/sounds_%s.cfg", dir, ci->skinName), &f, FS_READ);
		if ( !f )
	...
	if (f)
	{
		cg_trap->FS_Read(soundpath, fLen, f);
		soundpath[fLen] = 0;
	...
	for ( i = 0 ; i < MAX_CUSTOM_SOUNDS ; i++ )
	{
		...
		if (soundpath[0])
		{
			ci->sounds[i] = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", soundpath, soundName) );
		}
		else
		{
			if (modelloaded)
			{
				ci->sounds[i] = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", dir, soundName) );
			}
		}
		...
		{ //failed the load, try one out of the generic path
			if (isFemale)
			{
				ci->sounds[i] = cg_trap->S_RegisterSound( va("sound/%s/%s", DEFAULT_FEMALE_SOUNDPATH, soundName) );
			}
			else
			{
				ci->sounds[i] = cg_trap->S_RegisterSound( va("sound/%s/%s", DEFAULT_MALE_SOUNDPATH, soundName) );
			}
		}
	}
	...
	{ //load the siege sounds then
		for ( i = 0 ; i < MAX_CUSTOM_SIEGE_SOUNDS; i++ )
		{
			...
			if (soundpath[0])
			{
				ci->siegeSounds[i] = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", soundpath, soundName) );
				if ( !ci->siegeSounds[i] )
					ci->siegeSounds[i] = cg_trap->S_RegisterSound( va( "sound/%s/%s", soundpath, soundName ) );
			}
			else
			{
				if (modelloaded)
				{
					ci->siegeSounds[i] = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", dir, soundName) );
				}
			}
			...
			{ //failed the load, try one out of the generic path
				if (isFemale)
				{
					ci->siegeSounds[i] = cg_trap->S_RegisterSound( va("sound/%s/%s", DEFAULT_FEMALE_SOUNDPATH, soundName) );
				}
				else
				{
					ci->siegeSounds[i] = cg_trap->S_RegisterSound( va("sound/%s/%s", DEFAULT_MALE_SOUNDPATH, soundName) );
				}
			}
		}
	}
	...
	{ //load the Duel sounds then
		for ( i = 0 ; i < MAX_CUSTOM_DUEL_SOUNDS; i++ )
		{
			...
			if (soundpath[0])
			{
				ci->duelSounds[i] = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", soundpath, soundName) );
			}
			else
			{
				if (modelloaded)
				{
					ci->duelSounds[i] = cg_trap->S_RegisterSound( va("sound/chars/%s/misc/%s", dir, soundName) );
				}
			}
			...
			{ //failed the load, try one out of the generic path
				if (isFemale)
				{
					ci->duelSounds[i] = cg_trap->S_RegisterSound( va("sound/%s/%s", DEFAULT_FEMALE_SOUNDPATH, soundName) );
				}
				else
				{
					ci->duelSounds[i] = cg_trap->S_RegisterSound( va("sound/%s/%s", DEFAULT_MALE_SOUNDPATH, soundName) );
				}
			}
		}
	}

	cg_trap->S_Shutup(qfalse);
}
...
void CG_LoadClientInfo( clientInfo_t *ci ) {
	...
	if (ci->team == TEAM_SPECTATOR)
	{
		...
		if (ci->ghoul2Model && cg_trap->G2_HaveWeGhoul2Models(ci->ghoul2Model))
		{
			cg_trap->G2API_CleanGhoul2Models(&ci->ghoul2Model);
		}
	...
	*/
	...
	{ //yeah.. kind of a hack I guess. Don't care until they are actually ingame with a valid class.
		...
		{
			cg_trap->Error( ERR_DROP, "DEFAULT_MODEL (%s) failed to register", fallbackModel );
		}
	}
	else
	{
		if ( !CG_RegisterClientModelname( ci, ci->modelName, ci->skinName, teamname, clientNum ) ) {
			//cg_trap->Error( ERR_DROP, "CG_RegisterClientModelname( %s, %s, %s, %s %s ) failed", ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName, teamname );
			...
			if( cgs.gametype >= GT_TEAM) {
				...
				if ( !CG_RegisterClientModelname( ci, fallbackModel, ci->skinName, teamname, -1 ) ) {
					cg_trap->Error( ERR_DROP, "DEFAULT_MODEL / skin (%s/%s) failed to register", fallbackModel, ci->skinName );
				}
			} else {
				if ( !CG_RegisterClientModelname( ci, fallbackModel, "default", teamname, -1 ) ) {
					cg_trap->Error( ERR_DROP, "DEFAULT_MODEL (%s) failed to register", fallbackModel );
	...
	if (clientNum != -1)
	{
		cg_trap->G2API_ClearAttachedInstance(clientNum);
	}

	if (clientNum != -1 && ci->ghoul2Model && re->G2API_HaveWeGhoul2Models(*ci->ghoul2Model))
	{
		if (cg_entities[clientNum].ghoul2 && re->G2API_HaveWeGhoul2Models(*cg_entities[clientNum].ghoul2))
		{
			re->G2API_CleanGhoul2Models(&cg_entities[clientNum].ghoul2);
		}
		re->G2API_DuplicateGhoul2Instance(*ci->ghoul2Model, &cg_entities[clientNum].ghoul2);

		//Attach the instance to this entity num so we can make use of client-server
		//shared operations if possible.
		re->G2API_AttachInstanceToEntNum(*cg_entities[clientNum].ghoul2, clientNum, qfalse);


		if (re->G2API_AddBolt(*cg_entities[clientNum].ghoul2, 0, "face") == -1)
	...
	if ( ci->torsoModel ) {
		...
		if ( cg_trap->R_LerpTag( &tag, ci->torsoModel, 0, 0, 1, "tag_flag" ) ) {
			ci->newAnims = qtrue;
		}
	}
...
{
	re->G2API_InitGhoul2Model(&ci->ghoul2Weapons[saberNum], ci->saber[saberNum].model, 0, ci->saber[saberNum].skin, 0, 0, 0);
	...
	if (ci->ghoul2Weapons[saberNum])
	{
		...
		{
			cg_trap->G2API_SetSkin(ci->ghoul2Weapons[saberNum], 0, ci->saber[saberNum].skin, ci->saber[saberNum].skin);
		}

		if (ci->saber[saberNum].saberFlags & SFL_BOLT_TO_WRIST)
		{
			cg_trap->G2API_SetBoltInfo(ci->ghoul2Weapons[saberNum], 0, 3+saberNum);
		}
		else
		{
			cg_trap->G2API_SetBoltInfo(ci->ghoul2Weapons[saberNum], 0, saberNum);
		}

		while (k < ci->saber[saberNum].numBlades)
		{
			tagName = va("*blade%i", k+1);
			tagBolt = cg_trap->G2API_AddBolt(ci->ghoul2Weapons[saberNum], 0, tagName);

			if (tagBolt == -1)
			{
				if (k == 0)
				{ //guess this is an 0ldsk3wl saber
					tagBolt = cg_trap->G2API_AddBolt(ci->ghoul2Weapons[saberNum], 0, "*flash");
...
//	to->footsteps = from->footsteps;
	...
	if (to->ghoul2Model && re->G2API_HaveWeGhoul2Models(*to->ghoul2Model))
	{
		re->G2API_CleanGhoul2Models(&to->ghoul2Model);
	}
	if (from->ghoul2Model && re->G2API_HaveWeGhoul2Models(*from->ghoul2Model))
	{
		re->G2API_DuplicateGhoul2Instance(*from->ghoul2Model, &to->ghoul2Model);
	}
	...
	while (i < MAX_SABERS)
	{
		if (to->ghoul2Weapons[i] && cg_trap->G2_HaveWeGhoul2Models(to->ghoul2Weapons[i]))
		{
			cg_trap->G2API_CleanGhoul2Models(&to->ghoul2Weapons[i]);
		}
...
//			&& !Q_stricmp( ci->redTeam, match->redTeam )
			...
			&& match->bolt_head) //if the bolts haven't been initialized, this "match" is useless to us
		{
			...
			if (clientNum == i)
			{
				if (match->ghoul2Model && re->G2API_HaveWeGhoul2Models(*match->ghoul2Model))
...
//					ci->footsteps = match->footsteps;
					...
					while (k < MAX_SABERS)
					{
						if (match->ghoul2Weapons[k] && match->ghoul2Weapons[k] != ci->ghoul2Weapons[k])
						{
							...
							{
								cg_trap->G2API_CleanGhoul2Models(&ci->ghoul2Weapons[k]);
							}
...
//			 Q_stricmp( ci->headSkinName, match->headSkinName ) ||
	...
	//cg_trap->Print( "CG_SetDeferredClientInfo: no valid clients!\n" );
...
void CG_NewClientInfo( int clientNum, qboolean entitiesInitialized ) {
	...
	char		*v;
	char		*slash;
	CGhoul2Info_v *oldGhoul2;
	CGhoul2Info_v *oldG2Weapons[MAX_SABERS];
	int i = 0;
	int k = 0;
	...
	if ( !configstring[0] ) {
		...
		while (k < MAX_SABERS)
		{
			if (ci->ghoul2Weapons[k] && re->G2API_HaveWeGhoul2Models(*ci->ghoul2Weapons[k]))
			{
				re->G2API_CleanGhoul2Models(&ci->ghoul2Weapons[k]);
			}
			k++;
		}
...
//	Q_strncpyz(newInfo.blueTeam, v, MAX_TEAMNAME);
	...
	if ( cg_forceModel.integer ) {
		...
		cg_trap->Cvar_VariableStringBuffer( "model", modelStr, sizeof( modelStr ) );
	...
	if (v && Q_stricmp(v, ci->saberName))
	{
		...
		WP_SetSaber< moduleType::cgame >(clientNum, newInfo.saber, 0, newInfo.saberName);
		saberUpdate[0] = qtrue;
	}
	else
	{
	...
	if (v && Q_stricmp(v, ci->saber2Name))
	{
		...
		WP_SetSaber< moduleType::cgame >(clientNum, newInfo.saber, 1, newInfo.saber2Name);
		saberUpdate[1] = qtrue;
	}
	else
	{
			...
			if (saberUpdate[j])
			{
				if (newInfo.saber[j].model[0])
				{
					...
					{ //free the old instance(s)
						re->G2API_CleanGhoul2Models(&oldG2Weapons[j]);
						oldG2Weapons[j] = 0;
					}
	...
	if (ci->ghoul2Model &&
		...
		re->G2API_HaveWeGhoul2Models(*ci->ghoul2Model))
	...
	*ci = newInfo;
	...
	{	// Copy the new ghoul2 model to the centity.
		...
		if (anim)
		{
			...
			cg_trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", firstFrame, anim->firstFrame + anim->numFrames, flags, animSpeed, cg.time, setFrame, 150);
		...
		if (anim)
		{
			...
			//rww - Set the animation again because it just got reset due to the model change
			cg_trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "lower_lumbar", firstFrame, anim->firstFrame + anim->numFrames, flags, animSpeed, cg.time, setFrame, 150);
		...
		if (clientNum != -1)
		{
			//Attach the instance to this entity num so we can make use of client-server
			//shared operations if possible.
			cg_trap->G2API_AttachInstanceToEntNum(cg_entities[clientNum].ghoul2, clientNum, qfalse);
		}

		if (cg_trap->G2API_AddBolt(cg_entities[clientNum].ghoul2, 0, "face") == -1)
		{ //check now to see if we have this bone for setting anims and such
			cg_entities[clientNum].noFace = qtrue;
		}
								...
								centity_t *const cent, footstepType_t footStepType )
{
	...
	cg_trap->CM_Trace( &trace, origin, end, mins, maxs, 0, MASK_PLAYERSOLID, 0);
	...
	{
	 	cg_trap->S_StartSound( NULL, cent->currentState.clientNum, CHAN_BODY, cgs.media.footsteps[soundType][rand()&3] );
	}
	...
	if ( effectID != -1 )
	{
		cg_trap->FX_PlayEffectID( effectID, trace.endpos, trace.plane.normal, -1, -1, qfalse );
	}
...
static void CG_PlayerFootsteps( centity_t *cent, footstepType_t footStepType )
{
		...
		&& cent->currentState.NPC_class != CLASS_SWAMP )
	{
		...
		case FOOTSTEP_R:
		case FOOTSTEP_HEAVY_R:
			footBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_leg_foot");//cent->gent->footRBolt;
			break;
		case FOOTSTEP_L:
		case FOOTSTEP_HEAVY_L:
		default:
			footBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_leg_foot");//cent->gent->footLBolt;
			break;
		}


		//FIXME: get yaw orientation of the foot and use on decal
		cg_trap->G2API_GetBoltMatrix( cent->ghoul2, 0, footBolt, &boltMatrix, tempAngles, cent->lerpOrigin,
...
void CG_PlayerAnimEventDo( centity_t *cent, animevent_t *animEvent )
{
	...
	case AEV_SOUND:
		{	// are there variations on the sound?
			...
			if ( holdSnd > 0 )
			{
				cg_trap->S_StartSound( NULL, cent->currentState.number, channel, holdSnd );
			}
		}
		break;
	case AEV_SABER_SWING:
		...
		else
		{
			...
			swingSound = cg_trap->S_RegisterSound(va("sound/weapons/saber/saberhup%i.wav", randomSwing));
		}
		cg_trap->S_StartSound(cent->currentState.pos.trBase, cent->currentState.number, CHAN_AUTO, swingSound );
		break;
	case AEV_SABER_SPIN:
		if (cent->currentState.eType == ET_NPC)
		{
		...
		else
		{
			...
			{
			case 0://saberspinoff
				spinSound = cg_trap->S_RegisterSound( "sound/weapons/saber/saberspinoff.wav" );
				break;
			case 1://saberspin
				spinSound = cg_trap->S_RegisterSound( "sound/weapons/saber/saberspin.wav" );
				break;
			case 2://saberspin1
				spinSound = cg_trap->S_RegisterSound( "sound/weapons/saber/saberspin1.wav" );
				break;
			case 3://saberspin2
				spinSound = cg_trap->S_RegisterSound( "sound/weapons/saber/saberspin2.wav" );
				break;
			case 4://saberspin3
				spinSound = cg_trap->S_RegisterSound( "sound/weapons/saber/saberspin3.wav" );
				break;
			default://random saberspin1-3
				spinSound = cg_trap->S_RegisterSound( va( "sound/weapons/saber/saberspin%d.wav", Q_irand(1,3)) );
				break;
			}
		}
		if ( spinSound )
		{
			cg_trap->S_StartSound( NULL, cent->currentState.clientNum, CHAN_AUTO, spinSound );
		}
		break;
	case AEV_FOOTSTEP:
	...
	case AEV_EFFECT:
		...
		if (animEvent->stringData && animEvent->stringData[0] && cent && cent->ghoul2)
		{
			...
			{//must be a weapon, try weapon 0?
				animEvent->eventData[AED_BOLTINDEX] = cg_trap->G2API_AddBolt( cent->ghoul2, 1, animEvent->stringData );
				if ( animEvent->eventData[AED_BOLTINDEX] != -1 )
				{//found it!
				...
				{//hmm, just try on the player model, then?
					animEvent->eventData[AED_BOLTINDEX] = cg_trap->G2API_AddBolt( cent->ghoul2, 0, animEvent->stringData );
				}
			}
			else
			{
		...
		if ( animEvent->eventData[AED_BOLTINDEX] != -1 )
		{
			...
			cg_trap->G2API_GetBoltMatrix(cent->ghoul2, animEvent->eventData[AED_MODELINDEX], animEvent->eventData[AED_BOLTINDEX], &matrix, lAngles,
			...
			cg_trap->FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], bPoint, bAngle, -1, -1, qfalse);
		}
		else
		{
			vec3_t bAngle;
			...
			VectorSet(bAngle, 0, 1, 0);
			cg_trap->FX_PlayEffectID(animEvent->eventData[AED_EFFECTINDEX], cent->lerpOrigin, bAngle, -1, -1, qfalse);
		}
#endif
		break;
...
{ //this also sets the lerp frames, so I suggest you keep calling it regardless of if you want anim sounds.
	...
	if (cg_trap->G2API_GetBoneFrame(cent->ghoul2, "model_root", cg.time, &currentFrame, cgs.gameModels, 0))
	{
	...
	if (cg_trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time, &currentFrame, cgs.gameModels, 0))
	{
		curFrame = floor( currentFrame );
	}
...
static void CG_SetLerpFrameAnimation( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float animSpeedMult, qboolean torsoOnly, qboolean flipState) {
	...
	if ( newAnimation < 0 || newAnimation >= MAX_TOTALANIMATIONS ) {
		cg_trap->Error( ERR_DROP, "Bad animation number: %i", newAnimation );
	}
	...
	if (cent->ghoul2)
	{
		...
		BG_SaberStartTransAnim< moduleType::cgame >(cent->currentState.number, cent->currentState.fireflag, cent->currentState.weapon, newAnimation, &animSpeed, cent->currentState.brokenLimbs);
		...
		{
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", firstFrame, lastFrame, flags, animSpeed,cg.time, beginFrame, blendTime);
			return;
		}
		...
		{ //rww - The guesswork based on the lerp frame figures is usually BS, so I've resorted to a call to get the frame of the bone directly.
			...
			{ //we already checked, and this is the same anim, same flip state, but different speed, so we want to resume with the new speed off of the same frame.
				cg_trap->G2API_GetBoneFrame(cent->ghoul2, "lower_lumbar", cg.time, &GBAcFrame, NULL, 0);
			...
			cg_trap->G2API_GetBoneFrame(cent->ghoul2, "model_root", cg.time, &GBAcFrame, NULL, 0);
			...
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", firstFrame, lastFrame, flags, animSpeed,cg.time, beginFrame, blendTime);
		...
		{ //make sure we're humanoid before we access the motion bone
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
		}
		...
		{ //broken left arm
			...
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, brokenBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, cg.time, -1, blendTime);
		}
		...
		{ //broken right arm
				...
				(!ci->saber[1].model[0] || cent->currentState.weapon != WP_SABER))
			{
				...
				{ //don't affect this arm if holding a gun, just make the other arm support it
					...
					*/
					...
					cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, brokenBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, cg.time, -1, 0);
				}
				else
				{ //we want to keep the broken bone updated for some cases
					cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, brokenBone, firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
				}
					...
					(newAnimation == TORSO_WEAPONREADY2 || newAnimation == BOTH_ATTACK2 || cent->currentState.weapon < WP_BRYAR_PISTOL))
				{
					...
					cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, supportBone, armFirstFrame, armLastFrame, armFlags, armAnimSpeed, cg.time, -1, 150);
				}
				else
				{ //we want to keep the support bone updated for some cases
					cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, supportBone, firstFrame, lastFrame, flags, animSpeed, cg.time, beginFrame, blendTime);
				}
			}
			...
			(ci->brokenLimbs ||
			cg_trap->G2API_GetBoneFrame(cent->ghoul2, "lhumerus", cg.time, &unused, cgs.gameModels, 0) ||
			cg_trap->G2API_GetBoneFrame(cent->ghoul2, "rhumerus", cg.time, &unused, cgs.gameModels, 0)))
		...
		{ //remove the bone now so it can be set again
			...
			{ //can only have one arm broken at once.
				...
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lhumerus", 0, 1, 0, 0, cg.time, -1, 0);
				if (!cg_trap->G2API_RemoveBone(cent->ghoul2, "lhumerus", 0))
				{
					assert(0);
			...
			{
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lhumerus", 0, 1, 0, 0, cg.time, -1, 0);
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "rhumerus", 0, 1, 0, 0, cg.time, -1, 0);
				cg_trap->G2API_RemoveBone(cent->ghoul2, "lhumerus", 0);
				cg_trap->G2API_RemoveBone(cent->ghoul2, "rhumerus", 0);
				ci->brokenLimbs = 0;
			}
			else
			{
				//Set the flags and stuff to 0, so that the remove will succeed
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, brokenBone, 0, 1, 0, 0, cg.time, -1, 0);

				//Now remove it
				if (!cg_trap->G2API_RemoveBone(cent->ghoul2, brokenBone, 0))
				{
					assert(0);
...
static void CG_RunLerpFrame( centity_t *cent, clientInfo_t *ci, lerpFrame_t *lf, qboolean flipState, int newAnimation, float speedScale, qboolean torsoOnly)
{
	...
	if (cent->currentState.forceFrame)
	{
		if (lf->lastForcedFrame != cent->currentState.forceFrame)
		{
			...
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", cent->currentState.forceFrame, cent->currentState.forceFrame+1, flags, animSpeed, cg.time, -1, 150);
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", cent->currentState.forceFrame, cent->currentState.forceFrame+1, flags, animSpeed, cg.time, -1, 150);
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", cent->currentState.forceFrame, cent->currentState.forceFrame+1, flags, animSpeed, cg.time, -1, 150);
		}
...
{ //we want to hold off on setting the bone angles until the end of the frame, because every time we set
  //them the entire skeleton has to be reconstructed.
	...
	cg_trap->G2API_SetBoneAngles(ghoul2, modelIndex, boneName, angles, flags, up, right, forward, modelList,
...
void	CG_Rag_Trace( trace_t *result, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
	...
	cg_trap->CM_Trace ( result, start, end, mins, maxs, 0, mask, 0);
...
void CG_TempTestFunction(centity_t *cent, vec3_t forcedAngles)
{
	...
	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 1, 0, &boltMatrix, tAngles, cent->lerpOrigin,
		cg.time, cgs.gameModels, cent->modelScale);
...
static int CG_RagAnimForPositioning(centity_t *cent)
{
	...
	assert(cent->ghoul2);
	bolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "pelvis");
	assert(bolt > -1);

	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &matrix, cent->turAngles, cent->lerpOrigin,
		cg.time, cgs.gameModels, cent->modelScale);
...
qboolean CG_RagDoll(centity_t *cent, vec3_t forcedAngles)
{
	...
	{ //If we're not in a ragdoll state, perform the checks.
		...
		if (!inSomething)
		{
			...
			{ //only trace from the hands if the death anim is already done.
				boltChecks[0] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "rhand");
				boltChecks[1] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "lhand");
			}
			else
			{ //otherwise start the trace loop at the cranium.
				i = 2;
			}
			boltChecks[2] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "cranium");
			//boltChecks[3] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "rtarsal");
			//boltChecks[4] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "ltarsal");
			boltChecks[3] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "rtalus");
			boltChecks[4] = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "ltalus");
			...
			cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, boltChecks[2], &boltMatrix, tAng, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			...
			while (i < 5)
			{
				...
				{ //when doing hands, trace to the head instead of origin
					cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, boltChecks[i], &boltMatrix, tAng, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
	...
	{ //We're in a ragdoll state, so make the call to keep our positions updated and whatnot.
		...
		{ //ik must be reset before ragdoll is started, or you'll get some interesting results.
			cg_trap->G2API_SetBoneIKState(cent->ghoul2, cg.time, NULL, IKS_NONE, NULL);
...
#if 0
		{
			...
			*/
			...
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", tParms.startFrame, tParms.endFrame, flags, animSpeed,cg.time, -1, blendTime);
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", tParms.startFrame, tParms.endFrame, flags, animSpeed, cg.time, -1, blendTime);
			cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", tParms.startFrame, tParms.endFrame, flags, animSpeed, cg.time, -1, blendTime);
		}
#elif 1 //with my new method of doing things I want it to continue the anim
		{
			...
			if (cg_trap->G2API_GetBoneAnim(cent->ghoul2, "model_root", cg.time, &currentFrame, &startFrame, &endFrame, &flags, &animSpeed, cgs.gameModels, 0))
			{ //lock the anim on the current frame.
				...
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "lower_lumbar", currentFrame, currentFrame+1, flags, animSpeed,cg.time, currentFrame, blendTime);
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "model_root", currentFrame, currentFrame+1, flags, animSpeed, cg.time, currentFrame, blendTime);
				cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "Motion", currentFrame, currentFrame+1, flags, animSpeed, cg.time, currentFrame, blendTime);
			}
		}
#endif
		...
		cg_trap->G2API_SetRagDoll(cent->ghoul2, &tParms);
		...
		cg_trap->G2API_AnimateG2Models(cent->ghoul2, cg.time, &tuParms);
			...
			(cent->currentState.eType != ET_NPC || cent->currentState.NPC_class != CLASS_VEHICLE))
		{
			...
			if (grabEnt->ghoul2)
			{
				...
				cg_trap->G2API_GetBoltMatrix(grabEnt->ghoul2, 0, 0, &matrix, grabEnt->turAngles, grabEnt->lerpOrigin,
					cg.time, cgs.gameModels, grabEnt->modelScale);
				BG_GiveMeVectorFromMatrix(&matrix, ORIGIN, bOrg);

				//Get our hand's location
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, 0, &matrix, cent->turAngles, cent->lerpOrigin,
					cg.time, cgs.gameModels, cent->modelScale);
				BG_GiveMeVectorFromMatrix(&matrix, ORIGIN, thisHand);

				//Get the position of the thoracic bone for hinting its velocity later on
				thorBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "thoracic");
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, thorBolt, &matrix, cent->turAngles, cent->lerpOrigin,
					cg.time, cgs.gameModels, cent->modelScale);
				...
				{
					cg_trap->G2API_RagForceSolve(cent->ghoul2, qfalse);
				}
				else
				{
					cg_trap->G2API_RagForceSolve(cent->ghoul2, qtrue);
				}

				//got the hand pos of him, now we want to make our hand go to it
				cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rhand", bOrg);
				cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rradius", bOrg);
				cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rradiusX", bOrg);
				cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rhumerusX", bOrg);
				cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rhumerus", bOrg);

				//Make these two solve quickly so we can update decently
				cg_trap->G2API_RagPCJGradientSpeed(cent->ghoul2, "rhumerus", 1.5f);
				cg_trap->G2API_RagPCJGradientSpeed(cent->ghoul2, "rradius", 1.5f);

				//Break the constraints on them I suppose
				VectorSet(pcjMin, -999, -999, -999);
				VectorSet(pcjMax, 999, 999, 999);
				cg_trap->G2API_RagPCJConstraint(cent->ghoul2, "rhumerus", pcjMin, pcjMax);
				cg_trap->G2API_RagPCJConstraint(cent->ghoul2, "rradius", pcjMin, pcjMax);
				...
				cg_trap->G2API_RagEffectorKick(cent->ghoul2, "thoracic", hands);
				cg_trap->G2API_RagEffectorKick(cent->ghoul2, "ceyebrow", hands);
				...
				{ //make sure it's reasonably updated
					...
					{ //if we're being dragged, then kick all the bones around a bit
						...
						while (cg_effectorStringTable[i])
						{
							...
							cg_trap->G2API_RagEffectorKick(cent->ghoul2, cg_effectorStringTable[i], dVel);
...
#if 0
							{
								...
								vec3_t borg;
								vec3_t vorg;
								int b = cg_trap->G2API_AddBolt(cent->ghoul2, 0, cg_effectorStringTable[i]);

								cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, b, &bm, cent->turAngles, cent->lerpOrigin, cg.time,
		...
		{ //reset things to their normal rag state
			...
			//got the hand pos of him, now we want to make our hand go to it
			cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rhand", NULL);
			cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rradius", NULL);
			cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rradiusX", NULL);
			cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rhumerusX", NULL);
			cg_trap->G2API_RagEffectorGoal(cent->ghoul2, "rhumerus", NULL);

			VectorSet(dVel, 0.0f, 0.0f, -64.0f);
			cg_trap->G2API_RagEffectorKick(cent->ghoul2, "rhand", dVel);

			cg_trap->G2API_RagPCJGradientSpeed(cent->ghoul2, "rhumerus", 0.0f);
			cg_trap->G2API_RagPCJGradientSpeed(cent->ghoul2, "rradius", 0.0f);
	...
	{ //cycle through the 4 bone index values on the entstate
		...
		{ //if it's non-0 then it could have something in it.
			...
			{ //got the bone, now set the angles from the corresponding entitystate boneangles value.
				...
				cg_trap->G2API_SetBoneAngles(cent->ghoul2, 0, boneName, boneAngles, flags, up, right, forward, cgs.gameModels, 100, cg.time);
			}
		}
...
static void CG_G2SetHeadBlink( centity_t *cent, qboolean bStart )
{
	...
	const int hReye = cg_trap->G2API_AddBolt( cent->ghoul2, 0, "reye" );
	const int hLeye = cg_trap->G2API_AddBolt( cent->ghoul2, 0, "leye" );
	...
	cg_trap->G2API_SetBoneAngles( cent->ghoul2, 0, "leye", desiredAngles,
	...
	if (!bWink)
	{
		cg_trap->G2API_SetBoneAngles( cent->ghoul2, 0, "reye", desiredAngles,
			BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL, blendTime, cg.time );
	}
}
...
//	if (!animatingHead || ( animations[anim].firstFrame != startFrame ) )// only set the anim if we aren't going to do the same animation again
	...
	//		firstFrame, lastFrame, animFlags, animSpeed, cg.time, -1, blendTime);
		cg_trap->G2API_SetBoneAnim(cent->ghoul2, 0, "face", firstFrame, lastFrame, animFlags, animSpeed,
			cg.time, -1, blendTime);
	}
}
...
qboolean CG_G2PlayerHeadAnims( centity_t *cent )
{
	...
	else
	{
		...
		voiceVolume = cg_trap->S_GetVoiceVolume(cent->currentState.number);
...
static void CG_G2PlayerAngles( centity_t *cent, matrix3_t legs, vec3_t legsAngles)
{
	...
	else if (cent->isRagging)
	{
		...
		cg_trap->G2API_SetRagDoll(cent->ghoul2, NULL); //calling with null parms resets to no ragdoll.
	}
	...
	{ //don't do these things on non-humanoids
		...
		{ //then put our arm in this client's hand
			...
			if (other && other->ghoul2 && ci->bolt_lhand)
			{
				...
				cg_trap->G2API_GetBoltMatrix(other->ghoul2, 0, ci->bolt_lhand, &boltMatrix, other->turAngles, other->lerpOrigin, cg.time, cgs.gameModels, other->modelScale);
...
static void CG_TrailItem( centity_t *cent, qhandle_t hModel ) {
	...
	ent.hModel = hModel;
	cg_trap->R_AddRefEntityToScene( &ent );
}
#endif
...
static void CG_PlayerFlag( centity_t *cent, qhandle_t hModel ) {
	...
	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_llumbar, &boltMatrix, tAng, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
...
static void CG_PlayerPowerups( centity_t *cent, refEntity_t *torso ) {
	...
	if ( !powerups ) {
		...
		if ( powerups & ( 1 << PW_QUAD ) )
			cg_trap->R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 0.2f, 0.2f, 1 );
	#endif // BASE_COMPAT
	...
	if ( powerups & ( 1 << PW_REDFLAG ) ) {
		...
		cg_trap->R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 1.0, 0.2f, 0.2f );
	}

	// blueflag
	if ( powerups & ( 1 << PW_BLUEFLAG ) ) {
		CG_PlayerFlag( cent, cgs.media.blueFlagModel );
		cg_trap->R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 0.2f, 0.2f, 1.0 );
	}

	// neutralflag
	if ( powerups & ( 1 << PW_NEUTRALFLAG ) ) {
		cg_trap->R_AddLightToScene( cent->lerpOrigin, 200 + (rand()&31), 1.0, 1.0, 1.0 );
	}
...
static qboolean CG_PlayerShadow( centity_t *cent, float *shadowPlane ) {
	...
	*shadowPlane = 0;
	...
	{ //stencil
		...
		cg_trap->CM_Trace( &trace, cent->lerpOrigin, end, mins, maxs, 0, MASK_PLAYERSOLID, 0 );
...
static void CG_PlayerSplash( centity_t *cent ) {
	...
	cg_trap->CM_Trace( &trace, start, end, NULL, NULL, 0, ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ), 0 );
	...
	cg_trap->R_AddPolysToScene( cgs.media.wakeMarkShader, 4, verts, 1 );
}
...
static void CG_ForcePushBlur( vec3_t org, centity_t *cent )
{
	if (!cent || !cg_renderToTextureFX.integer)
	{
		...
		ex->color[1] = 32;
		ex->color[2] = 40;
		ex->refEntity.customShader = cg_trap->R_RegisterShader( "gfx/effects/forcePush" );
...
static void CG_ForcePushBodyBlur( centity_t *cent )
{
	...
	{ //go through all the bones we want to put a blur effect on
		bolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, cg_pushBoneNames[i]);
		...
		cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, bolt, &boltMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
...
static void CG_ForceGripEffect( vec3_t org )
{
	...
	*/
	...
	ex->color[1] = 255;
	ex->color[2] = 255;
	ex->refEntity.customShader = cgs.media.redSaberGlowShader;//cg_trap->R_RegisterShader( "gfx/effects/forcePush" );
}
...
int CG_LightVerts( vec3_t normal, int numVerts, polyVert_t *verts )
{
	...
	cg_trap->R_LightForPoint( verts[0].xyz, ambientLight, directedLight, lightDir );
...
static void CG_DoSaberLight( saberInfo_t *saber )
{
	...
	{//actually have something to do
		...
		cg_trap->R_AddLightToScene( mid, diameter + (Q_flrand(0.0f, 1.0f)*8.0f), rgb[0], rgb[1], rgb[2] );
	}
}
...
void CG_DoSaber( vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color, int rfx, qboolean doLight )
{
	...
	{	// always add a light because sabers cast a nice glow before they slice you in half!!  or something...
		...
		cg_trap->R_AddLightToScene( mid, (length*1.4f) + (Q_flrand(0.0f, 1.0f)*3.0f), rgb[0], rgb[1], rgb[2] );
	}
	...
	cg_trap->R_AddRefEntityToScene( &saber );
...
void CG_GetTagWorldPosition( refEntity_t *model, char *tag, vec3_t pos, matrix3_t axis )
{
	...
	cg_trap->R_LerpTag( &orientation, model->hModel, model->oldframe, model->frame,
...
//	byte			colors[4];
	...
	// get the fragments
	numFragments = cg_trap->R_MarkFragments( 4, (const float (*)[3])originalPoints,
...
qboolean CG_G2TraceCollide(trace_t *tr, vec3_t const mins, vec3_t const maxs, const vec3_t lastValidStart, const vec3_t lastValidEnd)
{
	...
	if (g2Hit && g2Hit->ghoul2)
	{
		...
		if (com_optvehtrace->integer &&
			...
			g2Hit->m_pVehicle)
		{
			cg_trap->G2API_CollisionDetectCache ( G2Trace, g2Hit->ghoul2, angles, g2Hit->lerpOrigin, cg.time, g2Hit->currentState.number, (float *)lastValidStart, (float *)lastValidEnd, g2Hit->modelScale, 0, cg_g2TraceLod.integer, fRadius );
		}
		else
		{
			cg_trap->G2API_CollisionDetect ( G2Trace, g2Hit->ghoul2, angles, g2Hit->lerpOrigin, cg.time, g2Hit->currentState.number, (float *)lastValidStart, (float *)lastValidEnd, g2Hit->modelScale, 0, cg_g2TraceLod.integer, fRadius );
		}
	...
	while (!doneWithTraces)
	{
		...
		{ //hit a client..
			...
			{ //it succeeded with the ghoul2 trace
				cg_trap->FX_PlayEffectID( cgs.effects.mSaberBloodSparks, trace.endpos, trace.plane.normal, -1, -1, qfalse );
				cg_trap->S_StartSound(trace.endpos, trace.entityNum, CHAN_AUTO, cg_trap->S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3))));
			}
		}
					  ...
					  vec3_t entposition, float entangle, void *ghoul2, vec3_t scale, int lifeTime)
{
	...
	if (cg_trap->G2API_GetNumGoreMarks(ghoul2, 0) >= cg_ghoul2Marks.integer)
	...
	*/
	...
	cg_trap->G2API_AddSkinGore(ghoul2, &goreSkin);
}
		...
		{ //this is the guy the server says we last hit, so continue.
			...
			if (doEffect)
			{
				...
				{ //do standard player/live ent hit sparks
					cg_trap->FX_PlayEffectID( hitPersonFxID, trace.endpos, trace.plane.normal, -1, -1, qfalse );
					//cg_trap->S_StartSound(trace.endpos, trace.entityNum, CHAN_AUTO, cg_trap->S_RegisterSound(va("sound/weapons/saber/saberhit%i.wav", Q_irand(1, 3))));
				}
				else
				{ //do the cut effect
					cg_trap->FX_PlayEffectID( hitOtherFxID, trace.endpos, trace.plane.normal, -1, -1, qfalse );
				}
				doEffect = qfalse;
			}
		}
...
void CG_AddSaberBlade( centity_t *cent, centity_t *scent, refEntity_t *saber, int renderfx, int modelIndex, int saberNum, int bladeNum, vec3_t origin, vec3_t angles, qboolean fromSaber, qboolean dontDraw)
{
	...
	{
		cg_trap->FX_PlayBoltedEffectID(client->saber[saberNum].bladeEffect, scent->lerpOrigin,
	...
	{
		cg_trap->FX_PlayBoltedEffectID(client->saber[saberNum].bladeEffect2, scent->lerpOrigin,
			scent->ghoul2, bladeNum, scent->currentState.number, useModelIndex, -1, qfalse);
	}
	//get the boltMatrix
	cg_trap->G2API_GetBoltMatrix(scent->ghoul2, useModelIndex, bladeNum, &boltMatrix, futureAngles, origin, cg.time, cgs.gameModels, scent->modelScale);
		...
		for ( i = 0; i < 1; i++ )//was 2 because it would go through architecture and leave saber trails on either side of the brush - but still looks bad if we hit a corner, blade is still 8 longer than hit
		{
			...
			if ( trace.fraction < 1.0f )
			{
				...
				else
				{
					...
					{
						cg_trap->FX_PlayEffectID( cgs.effects.mSparks, trace.endpos, trDir, -1, -1, qfalse );
					}
				}
					...
					if ( client->saber[saberNum].blade[bladeNum].trail.haveOldPos[i] )
					{
						...
						{//only put marks on architecture
							...
							{//ugh, need to have a real sound debouncer... or do this game-side
								...
								cg_trap->S_StartSound ( trace.endpos, -1, CHAN_WEAPON, cg_trap->S_RegisterSound( va("sound/weapons/saber/saberhitwall%i", Q_irand(1, 3)) ) );
							}
						}
					}
					else
		...
		if (!dontDraw)
		{
			if ( (BG_SuperBreakWinAnim(cent->currentState.torsoAnim) || saberMoveData[cent->currentState.saberMove].trailLength > 0 || ((cent->currentState.powerups & (1 << PW_SPEED) && cg_speedTrail.integer)) || (cent->currentState.saberInFlight && saberNum == 0)) && cg.time < saberTrail->lastTime + 2000 ) // if we have a stale segment, don't draw until we have a fresh one
			{
				...
				if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4)
				{
					...
					cg_trap->R_SetRefractProp(1.0f, 0.0f, qtrue, qtrue);

					//shader 2 is always the crazy refractive shader.
					cg_trap->R_AddPolyToScene( 2, 4, verts );
				}
				else
	#endif
				{
					...
					//Here we will use the happy process of filling a struct in with arguments and passing it to a cg_trap function
					...
					{ //don't draw it if the last time is way out of date
						...
						if (cg_saberTrail.integer == 2 && cg_shadows.integer != 2 && cgs.glconfig.stencilBits >= 4)
						{
							cg_trap->R_SetRefractionProperties(1.0f, 0.0f, qtrue, qtrue); //don't need to do this every frame.. but..
...
JustDoIt:
	...
	CG_DoSaber( org_, axis_[0], saberLen, client->saber[saberNum].blade[bladeNum].lengthMax, client->saber[saberNum].blade[bladeNum].radius,
		static_cast< saber_colors_t >( scolor ), renderfx, (qboolean)(client->saber[saberNum].numBlades < 3 && !(client->saber[saberNum].saberFlags2&SFL2_NO_DLIGHT)) );
}
...
void CG_DrawPlayerSphere(centity_t *cent, vec3_t origin, float scale, int shader)
{
	...
	ent.customShader = cg_trap->R_RegisterShader("effects/refract_2");
	cg_trap->R_AddRefEntityToScene( &ent );
}
...
void CG_AddLightningBeam(vec3_t start, vec3_t end)
{
	...
	*/
	...
	b.rgbParm = 0.0f;
	b.killTime = 50;
	b.shader = cg_trap->R_RegisterShader( "gfx/misc/electric2" );
	b.flags = 0x00000001; //FX_ALPHA_LINEAR

	cg_trap->FX_AddBezier(&b);
}
		...
		{ //got it, register the skin under the model path.
			...
			{ //got it.. terminate at the slash and register.
				...
				*p = 0;
				...
				skinID = cg_trap->R_RegisterSkin(useSkinName);
			}
		}
	}
...
void CG_CacheG2AnimInfo(char *modelName)
{
	...
	{ //it's a vehicle name actually, let's precache the whole vehicle
		BG_GetVehicleModelName< moduleType::cgame >(useModel, useModel, sizeof( useModel ) );
		BG_GetVehicleSkinName< moduleType::cgame >(useSkin, sizeof( useSkin ) );
		if ( useSkin[0] )
		{ //use a custom skin
			cg_trap->R_RegisterSkin(va("models/players/%s/model_%s.skin", useModel, useSkin));
		}
		else
		{
			cg_trap->R_RegisterSkin(va("models/players/%s/model_default.skin", useModel));
		}
		Q_strncpyz(useModel, va("models/players/%s/model.glm", useModel), sizeof( useModel ) );
	}

	cg_trap->G2API_InitGhoul2Model(&g2, useModel, 0, 0, 0, 0, 0);
	...
	if (g2)
	{
		...
		GLAName[0] = 0;
		cg_trap->G2API_GetGLAName(g2, 0, GLAName);
		...
		if (animIndex != -1)
		{
			...
			BG_ParseAnimationEvtFile< moduleType::cgame >(originalModelName, animIndex, bgNumAnimEvents);
		}

		//Now free the temp instance
		cg_trap->G2API_CleanGhoul2Models(&g2);
	}
}
...
static void CG_RegisterVehicleAssets( Vehicle_t *pVeh )
{
	...
	{
		pVeh->m_pVehicleInfo->iExhaustFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->exhaustFX );
	}
	if ( pVeh->m_pVehicleInfo->trailFX )
	{
		pVeh->m_pVehicleInfo->iTrailFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->trailFX );
	}
	if ( pVeh->m_pVehicleInfo->impactFX )
	{
		pVeh->m_pVehicleInfo->iImpactFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->impactFX );
	}
	if ( pVeh->m_pVehicleInfo->explodeFX )
	{
		pVeh->m_pVehicleInfo->iExplodeFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->explodeFX );
	}
	if ( pVeh->m_pVehicleInfo->wakeFX )
	{
		pVeh->m_pVehicleInfo->iWakeFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->wakeFX );
	}

	if ( pVeh->m_pVehicleInfo->dmgFX )
	{
		pVeh->m_pVehicleInfo->iDmgFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->dmgFX );
	}
	if ( pVeh->m_pVehicleInfo->wpn1FX )
	{
		pVeh->m_pVehicleInfo->iWpn1FX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->wpn1FX );
	}
	if ( pVeh->m_pVehicleInfo->wpn2FX )
	{
		pVeh->m_pVehicleInfo->iWpn2FX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->wpn2FX );
	}
	if ( pVeh->m_pVehicleInfo->wpn1FireFX )
	{
		pVeh->m_pVehicleInfo->iWpn1FireFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->wpn1FireFX );
	}
	if ( pVeh->m_pVehicleInfo->wpn2FireFX )
	{
		pVeh->m_pVehicleInfo->iWpn2FireFX = cg_trap->FX_RegisterEffect( pVeh->m_pVehicleInfo->wpn2FireFX );
	}
	*/
}
...
void CG_G2AnimEntModelLoad(centity_t *cent)
{
	...
	if (cModelName && cModelName[0])
	{
		...
		{ //vehicles pass their veh names over as model names, then we get the model name from the veh type
			...
			int iVehIndex = BG_VehicleGetIndex< moduleType::cgame >( vehType );

			switch( g_vehicleInfo[iVehIndex].type )
			{
				case VH_ANIMAL:
					// Create the animal (making sure all it's data is initialized).
					G_CreateAnimalNPC< moduleType::cgame >( &cent->m_pVehicle, vehType );
					break;
				case VH_SPEEDER:
					// Create the speeder (making sure all it's data is initialized).
					G_CreateSpeederNPC< moduleType::cgame >( &cent->m_pVehicle, vehType );
					break;
				case VH_FIGHTER:
					// Create the fighter (making sure all it's data is initialized).
					G_CreateFighterNPC< moduleType::cgame >( &cent->m_pVehicle, vehType );
					break;
				case VH_WALKER:
					// Create the walker (making sure all it's data is initialized).
					G_CreateWalkerNPC< moduleType::cgame >( &cent->m_pVehicle, vehType );
					break;
			...
			BG_GetVehicleModelName< moduleType::cgame >(modelName, modelName, sizeof( modelName ) );
			if (cent->m_pVehicle->m_pVehicleInfo->skin &&
				cent->m_pVehicle->m_pVehicleInfo->skin[0])
			{ //use a custom skin
				skinID = cg_trap->R_RegisterSkin(va("models/players/%s/model_%s.skin", modelName, cent->m_pVehicle->m_pVehicleInfo->skin));
			}
			else
			{
				skinID = cg_trap->R_RegisterSkin(va("models/players/%s/model_default.skin", modelName));
			}
			strcpy(modelName, va("models/players/%s/model.glm", modelName));

			//this sound is *only* used for vehicles now
			cgs.media.noAmmoSound = cg_trap->S_RegisterSound( "sound/weapons/noammo.wav" );
		}
		else
		{
		...
		if (cent->ghoul2)
		{ //clean it first!
            re->G2API_CleanGhoul2Models(&cent->ghoul2);
		}

		re->G2API_InitGhoul2Model(&cent->ghoul2, modelName, 0, skinID, 0, 0, 0);
		...
		if (cent->ghoul2)
		{
			...
			{ //do special vehicle stuff
				...
				i = cg_trap->G2API_AddBolt( cent->ghoul2, 0, "model_root" );

				// Setup the droid unit.
				cent->m_pVehicle->m_iDroidUnitTag = cg_trap->G2API_AddBolt( cent->ghoul2, 0, "*droidunit" );

				// Setup the Exhausts.
				for ( i = 0; i < MAX_VEHICLE_EXHAUSTS; i++ )
				{
					Com_sprintf( strTemp, 128, "*exhaust%i", i + 1 );
					cent->m_pVehicle->m_iExhaustTag[i] = cg_trap->G2API_AddBolt( cent->ghoul2, 0, strTemp );
				}
				...
				for ( i = 0; i < MAX_VEHICLE_MUZZLES; i++ )
				{
					...
					cent->m_pVehicle->m_iMuzzleTag[i] = cg_trap->G2API_AddBolt( cent->ghoul2, 0, strTemp );
				...
				for ( i = 0; i < MAX_VEHICLE_TURRETS; i++ )
				{
					...
					{
						cent->m_pVehicle->m_iGunnerViewTag[i] = cg_trap->G2API_AddBolt( cent->ghoul2, 0, cent->m_pVehicle->m_pVehicleInfo->turret[i].gunnerViewTag );
					}
					else
					{
			...
			if (cent->currentState.npcSaber1)
			{
				...
				{
					saber++; //skip over the @
					WP_SetSaber< moduleType::cgame >(cent->currentState.number, cent->npcClient->saber, 0, saber);
				}
			}
			if (cent->currentState.npcSaber2)
			{
				...
				{
					saber++; //skip over the @
					WP_SetSaber< moduleType::cgame >(cent->currentState.number, cent->npcClient->saber, 1, saber);
				}
			}
			...
			if ( cent->currentState.NPC_class != CLASS_VEHICLE )
			{
				while (j < MAX_SABERS)
				{
					if (cent->npcClient->saber[j].model[0])
					{
						...
						{ //free the old instance(s)
							re->G2API_CleanGhoul2Models(&cent->npcClient->ghoul2Weapons[j]);
			...
			cg_trap->G2API_SetSkin(cent->ghoul2, 0, skinID, skinID);

			cent->localAnimIndex = -1;

			GLAName[0] = 0;
			cg_trap->G2API_GetGLAName(cent->ghoul2, 0, GLAName);
			...
			{ //it doesn't use humanoid anims.
				...
				if ( slash )
				{
					...
					cent->localAnimIndex = BG_ParseAnimationFile< moduleType::cgame >(GLAName, NULL, qfalse);
				}
			}
			else
			{ //humanoid index.
				cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_hand");
				cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_hand");

				//rhand must always be first bolt. lhand always second. Whichever you want the
				//jetpack bolted to must always be third.
				cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*chestg");

				//claw bolts
				cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_hand_cap_r_arm");
				cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_hand_cap_l_arm");
				...
				if (cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*head_top") == -1)
				{
					cg_trap->G2API_AddBolt(cent->ghoul2, 0, "ceyebrow");
				}
				cg_trap->G2API_AddBolt(cent->ghoul2, 0, "Motion");
			}

			// If this is a not vehicle...
			if ( cent->currentState.NPC_class != CLASS_VEHICLE )
			{
				if (cg_trap->G2API_AddBolt(cent->ghoul2, 0, "lower_lumbar") == -1)
				{ //check now to see if we have this bone for setting anims and such
					cent->noLumbar = qtrue;
				}

				if (cg_trap->G2API_AddBolt(cent->ghoul2, 0, "face") == -1)
				{ //check now to see if we have this bone for setting anims and such
					cent->noFace = qtrue;
				}
			}
			...
			if (cent->localAnimIndex != -1)
			{
				...
				cent->eventAnimIndex = BG_ParseAnimationEvtFile< moduleType::cgame >(originalModelName, cent->localAnimIndex, bgNumAnimEvents);
			}
		}
	}
...
static void CG_CreateSurfaceDebris(centity_t *cent, int surfNum, int fxID, qboolean throwPart)
{
	...
	{ //right wing flame
		b = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_wingdamage");
		if ( throwPart
			&& cent->m_pVehicle
	...
	{ //left wing flame
		b = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_wingdamage");
		if ( throwPart
			&& cent->m_pVehicle
			&& cent->m_pVehicle->m_pVehicleInfo )
		{
	...
	{ //nose flame
		b = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*nosedamage");
		if ( cent->m_pVehicle
			&& cent->m_pVehicle->m_pVehicleInfo )
		{
	...
	{
		b = cg_trap->G2API_AddBolt(cent->ghoul2, 0, surfName);
	}
	...
	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, b, &boltMatrix, cent->lerpAngles, cent->lerpOrigin, cg.time,
		cgs.gameModels, cent->modelScale);
	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, v);
	BG_GiveMeVectorFromMatrix(&boltMatrix, POSITIVE_Z, d);

	cg_trap->FX_PlayEffectID(fxID, v, d, -1, -1, qfalse);
	if ( throwPart && lostPartFX )
	{//throw off a ship part, too
		vec3_t	fxFwd;
		AngleVectors( cent->lerpAngles, fxFwd, NULL, NULL );
		cg_trap->FX_PlayEffectID(lostPartFX, v, fxFwd, -1, -1, qfalse);
	}
}
...
qboolean CG_VehicleAttachDroidUnit( centity_t *droidCent, refEntity_t *legs )
{
	...
	{//the only NPCs that can ride a vehicle are droids...???
			...
			&& vehCent->m_pVehicle->m_iDroidUnitTag != -1 )
		{
			...
			cg_trap->G2API_GetBoltMatrix(vehCent->ghoul2, 0, vehCent->m_pVehicle->m_iDroidUnitTag, &boltMatrix, vehCent->lerpAngles, vehCent->lerpOrigin, cg.time,
...
void CG_G2Animated( centity_t *cent )
{
	...
	{ //looks like it's time for an update.
		...
		while (i < BG_NUM_TOGGLEABLE_SURFACES && bgToggleableSurfaces[i])
		{
			...
			{ //it wasn't off before but it's off now, so reflect this change in the g2 instance.
				...
				cg_trap->G2API_SetSurfaceOnOff(cent->ghoul2, bgToggleableSurfaces[i], TURN_OFF);
			}

			if (!(cent->npcLocalSurfOn & (1 << i)) &&
				(cent->currentState.surfacesOn & (1 << i)))
			{ //same as above, but on instead of off.
				cg_trap->G2API_SetSurfaceOnOff(cent->ghoul2, bgToggleableSurfaces[i], TURN_ON);
			}
	...
	if (cent->currentState.weapon &&
		!cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1) &&
		!(cent->currentState.eFlags & EF_DEAD))
	{ //if the server says we have a weapon and we haven't copied one onto ourselves yet, then do so.
		cg_trap->G2API_CopySpecificGhoul2Model(g2WeaponInstances[cent->currentState.weapon], 0, cent->ghoul2, 1);

		if (cent->currentState.weapon == WP_SABER)
		{
			cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/weapons/saber/saberon.wav" ));
		}
	}
	*/
...
void CG_ForceFPLSPlayerModel(centity_t *cent, clientInfo_t *ci)
{
	...
	if (cg_fpls.integer && !cg.renderingThirdPerson)
	{
		...
		skinHandle = cg_trap->R_RegisterSkin("models/players/kyle/model_fpls2.skin");

		cg_trap->G2API_CleanGhoul2Models(&(ci->ghoul2Model));

		ci->torsoSkin = skinHandle;
		cg_trap->G2API_InitGhoul2Model(&ci->ghoul2Model, "models/players/kyle/model.glm", 0, ci->torsoSkin, 0, 0, 0);

		ci->bolt_rhand = cg_trap->G2API_AddBolt(ci->ghoul2Model, 0, "*r_hand");

		cg_trap->G2API_SetBoneAnim(ci->ghoul2Model, 0, "model_root", 0, 12, BONE_ANIM_OVERRIDE_LOOP, 1.0f, cg.time, -1, -1);
		cg_trap->G2API_SetBoneAngles(ci->ghoul2Model, 0, "upper_lumbar", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL, 0, cg.time);
		cg_trap->G2API_SetBoneAngles(ci->ghoul2Model, 0, "cranium", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Z, NEGATIVE_Y, POSITIVE_X, NULL, 0, cg.time);
	...
	cg_trap->G2API_CleanGhoul2Models(&(cent->ghoul2));
	cg_trap->G2API_DuplicateGhoul2Instance(ci->ghoul2Model, &cent->ghoul2);

	//Attach the instance to this entity num so we can make use of client-server
	//shared operations if possible.
	cg_trap->G2API_AttachInstanceToEntNum(cent->ghoul2, cent->currentState.number, qfalse);
}
#endif
...
{
	//cg_trap->TrueMalloc((void **)ci, sizeof(clientInfo_t));
	*ci = (clientInfo_t *) BG_Alloc< moduleType::cgame >(sizeof(clientInfo_t));
}

void CG_DestroyNPCClient(clientInfo_t **ci)
{
	memset(*ci, 0, sizeof(clientInfo_t));
	//cg_trap->TrueFree((void **)ci);
}
...
static void CG_ForceElectrocution( centity_t *cent, const vec3_t origin, vec3_t tempAngles, qhandle_t shader, qboolean alwaysDo )
{
	...
	if (cent->localAnimIndex <= 1)
	{ //humanoid
		torsoBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "lower_lumbar");
		elbowLBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_arm_elbow");
		elbowRBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_arm_elbow");
		handLBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_hand");
		handRBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_hand");
		footLBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*l_leg_foot");
		footRBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*r_leg_foot");
	}
	else if (cent->currentState.NPC_class == CLASS_PROTOCOL)
	{ //any others that can use these bolts too?
		torsoBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "lower_lumbar");
		elbowLBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*bicep_lg");
		elbowRBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*bicep_rg");
		handLBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*hand_l");
		handRBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*weapon");
		footLBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*foot_lg");
		footRBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, "*foot_rg");
	}
	...
	if (bolt>=0)
	{
		found = cg_trap->G2API_GetBoltMatrix( cent->ghoul2, 0, bolt,
...
void CG_InitJetpackGhoul2(void)
{
	...
	cg_trap->G2API_InitGhoul2Model(&cg_g2JetpackInstance, JETPACK_MODEL, 0, 0, 0, 0, 0);
	...
	cg_trap->G2API_SetBoltInfo(cg_g2JetpackInstance, 0, 2);

	//Add the bolts jet effects will be played from
	cg_trap->G2API_AddBolt(cg_g2JetpackInstance, 0, "torso_ljet");
	cg_trap->G2API_AddBolt(cg_g2JetpackInstance, 0, "torso_rjet");
}
...
void CG_CleanJetpackGhoul2(void)
{
	if (cg_g2JetpackInstance)
	{
		cg_trap->G2API_CleanGhoul2Models(&cg_g2JetpackInstance);
...
static QINLINE void CG_VehicleEffects(centity_t *cent)
{
	...
	{//in hyperspace!
			...
			&& (cg.time-cg.predictedVehicleState.hyperSpaceTime) < HYPERSPACE_TIME )
		{
			...
			{//can't be from the last time we were in hyperspace, so play the effect!
				cg_trap->FX_PlayBoltedEffectID( cgs.effects.mHyperspaceStars, cent->lerpOrigin, cent->ghoul2, 0,
			...
			{//close
				...
				{//okay to do a flyby sound on this vehicle
					if ( DotProduct( myFwd, theirFwd ) < 500 )
					{
						...
						cg_trap->S_StartSound(NULL, cent->currentState.clientNum, CHAN_LESS_ATTEN, flyBySound );
	...
	{//engines rev up for the first time
		cg_trap->S_StartSound(NULL, cent->currentState.clientNum, CHAN_LESS_ATTEN, cent->m_pVehicle->m_pVehicleInfo->soundEngineStart );
	}
	...
	if ( pVehNPC->m_pVehicleInfo->type != VH_ANIMAL )
	{
		...
		{ //see if anything has been blown off
			...
			while (i < BG_NUM_TOGGLEABLE_SURFACES)
			{
				...
				{ //this is decidedly a destroyable surface, let's check its status
					int surfTest = cg_trap->G2API_GetSurfaceRenderStatus(cent->ghoul2, 0, bgToggleableSurfaces[i]);
		...
		{//until we attach it, we need to debounce this
			...
			{//FIXME: only do this when accelerator is being pressed! (must have a driver?)
				...
				if (0)
				{//hurt
					cg_trap->FX_PlayEffectID( cgs.effects.mBlackSmoke, org, fwd, -1, -1, qfalse );
				}
				else if ( pVehNPC->m_pVehicleInfo->iTrailFX )
				{//okay, do normal trail
					cg_trap->FX_PlayEffectID( pVehNPC->m_pVehicleInfo->iTrailFX, org, fwd, -1, -1, qfalse );
				}
				//=====================================================================
				//EXHAUST FX
				//=====================================================================
				...
				if ( doExhaust && cent->ghoul2 )
				{
					...
					for ( i = 0; i < MAX_VEHICLE_EXHAUSTS; i++ )
					{
						...
						{
							cg_trap->FX_PlayBoltedEffectID(fx, cent->lerpOrigin, cent->ghoul2, pVehNPC->m_iExhaustTag[i],
														cent->currentState.number, 0, 0, qtrue);
						}
						else
						{ //fixme: bolt these too
							...
							cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, pVehNPC->m_iExhaustTag[i], &boltMatrix, flat,
								cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);

							BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
							VectorCopy(fwd, boltDir); //fixme?

							cg_trap->FX_PlayEffectID( fx, boltOrg, boltDir, -1, -1, qfalse );
						}
					}
				}
				//=====================================================================
				...
				if ( pVehNPC->m_pVehicleInfo->iTrailFX != 0 && cent->ghoul2 )
				{
					...
					for ( i = 1; i < 5; i++ )
					{
						int trailBolt = cg_trap->G2API_AddBolt(cent->ghoul2, 0, va("*trail%d",i) );
						// We hit an invalid tag, we quit (they should be created in order so tough luck if not).
						if ( trailBolt == -1 )
						{
							break;
						...
						cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, trailBolt, &boltMatrix, getBoltAngles,
							cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);

						BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
						VectorCopy(fwd, boltDir); //fixme?

						cg_trap->FX_PlayEffectID( pVehNPC->m_pVehicleInfo->iTrailFX, boltOrg, boltDir, -1, -1, qfalse );
					}
				}
			}
			//FIXME armor needs to be sent over network
			{
				...
				{//just plain dead, use flames
					...
					//	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, pVehNPC->m_iDriverTag, &boltMatrix, getBoltAngles,
					//		cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);

					//	BG_GiveMeVectorFromMatrix(&boltMatrix, ORIGIN, boltOrg);
					//}
					cg_trap->FX_PlayEffectID( cgs.effects.mShipDestBurning, boltOrg, up, -1, -1, qfalse );
				}
			}
			...
			{//FIXME: should use something attached.. but want it to build up over time, so...
				...
				{//flaming!
					...
					VectorScale( fwd, -1, fwd );
					cg_trap->FX_PlayEffectID( cg_trap->FX_RegisterEffect("ships/fire"), org, fwd, -1, -1, qfalse );
					nextFXDelay = 50;
				}
			}
			*/
...
void CG_Player( centity_t *cent ) {
	...
	if (cent->currentState.eType != ET_NPC)
	{
		clientNum = cent->currentState.clientNum;
		if ( clientNum < 0 || clientNum >= MAX_CLIENTS ) {
			cg_trap->Error( ERR_DROP, "Bad clientNum on player entity");
		}
		ci = &cgs.clientinfo[ clientNum ];
	}
	else
	{
		...
		if (cent->npcClient->ghoul2Model != cent->ghoul2 && cent->ghoul2)
		{
			...
			if (cent->localAnimIndex <= 1)
			{
				cent->npcClient->bolt_rhand = cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*r_hand");
				cent->npcClient->bolt_lhand = cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*l_hand");

				//rhand must always be first bolt. lhand always second. Whichever you want the
				//jetpack bolted to must always be third.
				cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*chestg");

				//claw bolts
				cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*r_hand_cap_r_arm");
				cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*l_hand_cap_l_arm");

				cent->npcClient->bolt_head = cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "*head_top");
				if (cent->npcClient->bolt_head == -1)
				{
					cent->npcClient->bolt_head = cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "ceyebrow");
				}
				cent->npcClient->bolt_motion = cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "Motion");
				cent->npcClient->bolt_llumbar = cg_trap->G2API_AddBolt(cent->npcClient->ghoul2Model, 0, "lower_lumbar");
			}
			else
			{
...
#endif
		cg_trap->G2API_ClearAttachedInstance(cent->currentState.number);
...
#endif
			re->G2API_DuplicateGhoul2Instance(*ci->ghoul2Model, &cent->ghoul2);
	...
	{ //do crazy smoothing
		...
		{ //do it
			cg_trap->G2API_AbsurdSmoothing(cent->ghoul2, qtrue);
		}
		else
		{ //turn it off
			ci->superSmoothTime = 0;
			cg_trap->G2API_AbsurdSmoothing(cent->ghoul2, qfalse);
		}
	}
	...
	{ //should have a jetpack attached
		...
		if (!cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 3))
		{
			cg_trap->G2API_CopySpecificGhoul2Model(cg_g2JetpackInstance, 0, cent->ghoul2, 3);
		}
		...
		if (cent->currentState.eFlags & EF_JETPACK_ACTIVE)
		{
			...
			while (n < 2)
			{
				...
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 3, n, &mat, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
				...
				{ //create effects
					...
					//Play the effect
					cg_trap->FX_PlayEffectID(cgs.effects.mBobaJet, flamePos, flameDir, -1, -1, qfalse);
					cg_trap->FX_PlayEffectID(cgs.effects.mBobaJet, flamePos, flameDir, -1, -1, qfalse);

					//Keep the jet fire sound looping
					cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin,
						cg_trap->S_RegisterSound( "sound/effects/fire_lp" ) );
				}
				else
				{ //just idling
			...
			cg_trap->S_AddLoopingSound( cent->currentState.number, cent->lerpOrigin, vec3_origin,
				cg_trap->S_RegisterSound( "sound/boba/JETHOVER" ) );
		}
	}
	else if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 3))
	{ //fixme: would be good if this could be done not every frame
		cg_trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 3);
	}

	g2HasWeapon = cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->ghoul2), 1);
	...
	{ //kill the weapon if the limb holding it is no longer on the model
		cg_trap->G2API_RemoveGhoul2Model(&(cent->ghoul2), 1);
		g2HasWeapon = qfalse;
	}
		...
		else
		{
			...
			if (cent->currentState.eType != ET_NPC)
			{
				...
				{ //switching away from the saber
					//cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound( "sound/weapons/saber/saberoffquick.wav" ));
					if (ci->saber[0].soundOff
						&& !cent->currentState.saberHolstered)
					{
						cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, ci->saber[0].soundOff);
					}
					...
					if (ci->saber[1].soundOff &&
						...
						!cent->currentState.saberHolstered)
					{
						cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, ci->saber[1].soundOff);
					}
				...
				{ //switching to the saber
					...
					{
						cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, ci->saber[0].soundOn);
					}

					if (ci->saber[1].soundOn)
					{
						cg_trap->S_StartSound(cent->lerpOrigin, cent->currentState.number, CHAN_AUTO, ci->saber[1].soundOn);
					}
...
// minimal_add:
	...
	if ( ((cent->currentState.eFlags & EF_SEEKERDRONE) || cent->currentState.genericenemyindex != -1) && cent->currentState.eType != ET_NPC )
	{
		...
		seeker.hModel = cg_trap->R_RegisterModel("models/items/remote.md3");
		cg_trap->R_AddRefEntityToScene( &seeker );
	}
	...
	cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_lhand, &lHandMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
	...
	else
	{
		if (cg_fpls.integer && cent->currentState.weapon == WP_SABER && cg.snap && cent->currentState.number == cg.snap->ps.clientNum)
		{
			...
			cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_head, &headMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
		...
		cent->frame_minus2_refreshed)
	{
		...
		if (cent->frame_minus1_refreshed)
		{
			...
			cg_trap->R_AddRefEntityToScene(&reframe_minus1);
		}
		...
		if (cent->frame_minus2_refreshed)
		{
			...
			cg_trap->R_AddRefEntityToScene(&reframe_minus2);
		}
	}
		...
		&& cent->currentState.NPC_class != CLASS_VEHICLE)
	{
		...
		{//alternate back and forth between left and right
			...
			cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &rHandMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
		...
		{
			//cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_lhand, &boltMatrix, tAng, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			if (!gotLHandMatrix)
			{
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_lhand, &lHandMatrix, cent->turAngles, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
				gotLHandMatrix = qtrue;
		...
		{//arc
			//cg_trap->FX_PlayEffectID( cgs.effects.forceLightningWide, efOrg, fxDir );
			//cg_trap->FX_PlayEntityEffectID(cgs.effects.forceDrainWide, efOrg, axis, cent->boltInfo, cent->currentState.number, -1, -1);
			cg_trap->FX_PlayEntityEffectID(cgs.effects.forceDrainWide, efOrg, axis, -1, -1, -1, -1);
		}
		else
		{//line
			//cg_trap->FX_PlayEffectID( cgs.effects.forceLightning, efOrg, fxDir );
			//cg_trap->FX_PlayEntityEffectID(cgs.effects.forceDrain, efOrg, axis, cent->boltInfo, cent->currentState.number, -1, -1);
			cg_trap->FX_PlayEntityEffectID(cgs.effects.forceDrain, efOrg, axis, -1, -1, -1, -1);
		}
		...
		if (cent->bolt4 < cg.time)
		{
			...
			cg_trap->S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound("sound/weapons/force/drain.wav") );
		}
		*/
	}
	...
	{//doing the electrocuting
		...
		{//arc
			//cg_trap->FX_PlayEffectID( cgs.effects.forceLightningWide, efOrg, fxDir );
			//cg_trap->FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrg, axis, cent->boltInfo, cent->currentState.number, -1, -1);
			cg_trap->FX_PlayEntityEffectID(cgs.effects.forceLightningWide, efOrg, axis, -1, -1, -1, -1);
		}
		else
		{//line
			//cg_trap->FX_PlayEffectID( cgs.effects.forceLightning, efOrg, fxDir );
			//cg_trap->FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrg, axis, cent->boltInfo, cent->currentState.number, -1, -1);
			cg_trap->FX_PlayEntityEffectID(cgs.effects.forceLightning, efOrg, axis, -1, -1, -1, -1);
		}
		...
		{
			cent->bolt4 = cg.time + 100;
			cg_trap->S_StartSound(NULL, cent->currentState.number, CHAN_AUTO, cg_trap->S_RegisterSound("sound/weapons/force/lightning.wav") );
		}
		*/
	}
	...
	if ( cent->currentState.powerups & (1 << PW_DISINT_4) )
	{
			...
			(cg.renderingThirdPerson || cent->currentState.number != cg.snap->ps.clientNum) )
		{
			...
			//Render a scaled version of the model's hand with a n337 looking shader
			{
				...
				if (cent->grip_arm && cg_trap->G2_HaveWeGhoul2Models(cent->grip_arm))
				{
					cg_trap->G2API_CleanGhoul2Models(&(cent->grip_arm));
				}
				...
				cg_trap->G2API_DuplicateGhoul2Instance(cent->ghoul2, &cent->grip_arm);

				//remove all other models
				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->grip_arm), 1))
				{ //weapon right
					cg_trap->G2API_RemoveGhoul2Model(&(cent->grip_arm), 1);
				}
				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->grip_arm), 2))
				{ //weapon left
					cg_trap->G2API_RemoveGhoul2Model(&(cent->grip_arm), 2);
				}
				if (cg_trap->G2API_HasGhoul2ModelOnIndex(&(cent->grip_arm), 3))
				{ //jetpack
					cg_trap->G2API_RemoveGhoul2Model(&(cent->grip_arm), 3);
				}

				cg_trap->G2API_SetRootSurface(cent->grip_arm, 0, limbName);
				cg_trap->G2API_SetNewOrigin(cent->grip_arm, cg_trap->G2API_AddBolt(cent->grip_arm, 0, rotateBone));
				cg_trap->G2API_SetSurfaceOnOff(cent->grip_arm, limbCapName, 0);
				...
				regrip_arm.customShader = cg_trap->R_RegisterShader( "gfx/misc/red_portashield" );
				...
				cg_trap->R_AddRefEntityToScene( &regrip_arm );
			}
			*/
		}
	...
	{
		cg_trap->S_AddLoopingSound( cent->currentState.number, cg.refdef.vieworg, vec3_origin,
	...
	if (cent->teamPowerEffectTime > cg.time)
	{
		...
		else
		{
			...
			legs.customShader = cg_trap->R_RegisterShader( "powerups/ysalimarishell" );
			cg_trap->R_AddRefEntityToScene(&legs);
		...
		cent->currentState.number))
	{
		if (cent->ghoul2)
		{
			...
			cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_head, &boltMatrix, tAng, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
			...
			//cg_trap->FX_PlayEntityEffectID(cg_trap->FX_RegisterEffect("force/confusion.efx"), efOrg, axis, cent->boltInfo, cent->currentState.number);
			cg_trap->FX_PlayEntityEffectID(cgs.effects.mForceConfustionOld, efOrg, axis, -1, -1, -1, -1);
		}
	}
	...
	if (cgs.gametype == GT_HOLOCRON && cent->currentState.time2 && (cg.renderingThirdPerson || cg.snap->ps.clientNum != cent->currentState.number))
	{
		...
		while (i < NUM_FORCE_POWERS && renderedHolos < 3)
		{
			if (cent->currentState.time2 & (1 << i))
			{
				...
				holoRef.hModel = cg_trap->R_RegisterModel(forceHolocronModels[i]);
				cg_trap->R_AddRefEntityToScene( &holoRef );
...
stillDoSaber:
		...
		&& cent->currentState.saberHolstered < 2 )
	{
			...
			&& !(cent->currentState.eFlags & EF_DEAD))//still alive
		{
				...
				&& !cent->currentState.saberInFlight)
			{
				...
				if (hasLen)
				{
					cg_trap->S_AddLoopingSound( cent->currentState.number, soundSpot, vec3_origin,
						ci->saber[0].soundLoop );
					didFirstSound = qtrue;
				}
			}
			...
			&& cent->currentState.saberEntityNum)
		{
			...
			{ //saber is in flight, do not have it as a standard weapon model
				...
				if (g2HasWeapon)
				{
					...
					cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &boltMat, cent->turAngles, cent->lerpOrigin,
						cg.time, cgs.gameModels, cent->modelScale);
				...
				else
				{
					...
					{ //clean if we already have one (because server changed model string index)
						re->G2API_CleanGhoul2Models(&(saberEnt->ghoul2));
						saberEnt->ghoul2 = 0;
					}

					if (saberModel && saberModel[0])
					{
						re->G2API_InitGhoul2Model(&saberEnt->ghoul2, saberModel, 0, 0, 0, 0, 0);
					}
					else if (ci->saber[0].model[0])
					{
						re->G2API_InitGhoul2Model(&saberEnt->ghoul2, ci->saber[0].model, 0, 0, 0, 0, 0);
					}
					else
					{
						re->G2API_InitGhoul2Model(&saberEnt->ghoul2, "models/weapons2/saber/saber_w.glm", 0, 0, 0, 0, 0);
					}
					//cg_trap->G2API_DuplicateGhoul2Instance(cent->ghoul2, &saberEnt->ghoul2);
				...
				if (addBolts)
				{
					...
					while (m < ci->saber[0].numBlades)
					{
						...
						tagBolt = cg_trap->G2API_AddBolt(saberEnt->ghoul2, 0, tagName);

						if (tagBolt == -1)
						{
							if (m == 0)
							{ //guess this is an 0ldsk3wl saber
								tagBolt = cg_trap->G2API_AddBolt(saberEnt->ghoul2, 0, "*flash");
			...
			/*else if (cent->bolt4 != 2)
			{
				if (saberEnt->ghoul2)
				{
					cg_trap->G2API_AddBolt(saberEnt->ghoul2, 0, "*flash");
					cent->bolt4 = 2;
				}
			}*/
			...
			if (saberEnt && saberEnt->ghoul2 /*&& cent->bolt4 == 2*/)
			{
				...
				cg_trap->G2API_GetBoltMatrix(cent->ghoul2, 0, ci->bolt_rhand, &boltMatrix, tAng, cent->lerpOrigin, cg.time, cgs.gameModels, cent->modelScale);
				...
				//cg_trap->FX_AddSprite( NULL, efOrg, NULL, NULL, 8.0f, 8.0f, wv, wv, 0.0f, 0.0f, 1.0f, cgs.media.yellowSaberGlowShader, 0x08000000 );
	...
	if (drawPlayerSaber)
	{
		...
		else if (!cent->currentState.saberInFlight)
		{
			...
			{
				cg_trap->G2API_CopySpecificGhoul2Model(CG_G2WeaponInstance(cent, WP_SABER), 0, cent->ghoul2, 1);
		...
		{ //adjust the glow by how far away you are from your dueling partner
			...
			if (duelEnt)
			{
...

				{
					...
					cg_trap->R_AddRefEntityToScene( &legs );	//draw the shell
	...
	if (cent->currentState.eFlags & EF_DISINTEGRATION)
	{
		...
		cg_trap->G2API_SetBoneAnim(legs.ghoul2, 0, "model_root", cent->miscTime, cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, cent->miscTime, -1);

		if (!cent->noLumbar)
		{
			cg_trap->G2API_SetBoneAnim(legs.ghoul2, 0, "lower_lumbar", cent->miscTime, cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, cent->miscTime, -1);

			if (cent->localAnimIndex <= 1)
			{
				cg_trap->G2API_SetBoneAnim(legs.ghoul2, 0, "Motion", cent->miscTime, cent->miscTime, BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, cent->miscTime, -1);
			}
		}
		...
		else
		{
			if (cg.renderingThirdPerson || cent->currentState.number != cg.predictedPlayerState.clientNum)
			{
				...
				*/
				...
				{
					cg_trap->R_SetRefractionProperties(1.0f, 0.0f, qfalse, qfalse); //don't need to do this every frame.. but..
	...
	if (cent->frame_hold_time >= cg.time)
	{
		...
		{ //We're taking the ghoul2 instance from the original refent and duplicating it onto our refent alias so that we can then freeze the frame and fade it for the effect
			if (cent->frame_hold && cg_trap->G2_HaveWeGhoul2Models(cent->frame_hold) &&
				cent->frame_hold != cent->ghoul2)
			{
				re->G2API_CleanGhoul2Models(&(cent->frame_hold));
			}
			reframe_hold = legs;
			cent->frame_hold_refreshed = 1;
			reframe_hold.ghoul2 = NULL;

			re->G2API_DuplicateGhoul2Instance(*cent->ghoul2, &cent->frame_hold);

			//Set the animation to the current frame and freeze on end
			//cg_trap->G2API_SetBoneAnim(cent->frame_hold.ghoul2, 0, "model_root", cent->frame_hold.frame, cent->frame_hold.frame, BONE_ANIM_OVERRIDE_FREEZE, 1.0f, cg.time, cent->frame_hold.frame, -1);
			cg_trap->G2API_SetBoneAnim(cent->frame_hold, 0, "model_root", legs.frame, legs.frame, 0, 1.0f, cg.time, legs.frame, -1);
		}
		else
		{
			reframe_hold = legs;
		...
		reframe_hold.ghoul2 = cent->frame_hold;
		cg_trap->R_AddRefEntityToScene(&reframe_hold);
	}
	else
	{
	...
	{ //aborb is represented by green..
		...
		*/
		...
		cg_trap->R_AddRefEntityToScene( &prot );
	}
	...
	if ( cent->currentState.emplacedOwner > cg.time )
	{
		...
		if ( dif > 0 && Q_flrand(0.0f, 1.0f) > 0.4f )
		{
			...
			if ( Q_flrand(0.0f, 1.0f) > 0.9f )
				cg_trap->S_StartSound ( NULL, cent->currentState.number, CHAN_AUTO, cgs.media.crackleSound );
		}
...
endOfCall:
	...
	{
		cg_trap->G2API_SetBoneAngles(cgBoneAnglePostSet.ghoul2, cgBoneAnglePostSet.modelIndex, cgBoneAnglePostSet.boneName,
...
//	cent->extrapolated = qfalse;
		...
		!(cent->currentState.eFlags & EF_DEAD))
	{
		...
		BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.pos, cg.time, cent->lerpOrigin );
		BG_EvaluateTrajectory< moduleType::cgame >( &cent->currentState.apos, cg.time, cent->lerpAngles );
...
//		VectorCopy( cent->lerpOrigin, cent->rawOrigin );
		...
		if ((cent->ghoul2 == NULL) && ci->ghoul2Model && re->G2API_HaveWeGhoul2Models(*ci->ghoul2Model))
		{
			re->G2API_DuplicateGhoul2Instance(*ci->ghoul2Model, &cent->ghoul2);
			cent->weapon = 0;
			cent->ghoul2weapon = NULL;
	...
	if ( cg_debugPosition.integer ) {
		cg_trap->Print("%i ResetPlayerEntity yaw=%i\n", cent->currentState.number, cent->pe.torso.yawAngle );
	}
}
