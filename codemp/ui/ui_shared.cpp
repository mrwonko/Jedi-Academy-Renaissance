...
// MSVC has no variable templates yet :(
template< moduleType mod > struct MEM_POOL_SIZE {
	static constexpr int value = 4 * 1024 * 1024;
};
template<> struct MEM_POOL_SIZE< moduleType::cgame > {
	static constexpr int value = 128 * 1024;
};

template< moduleType mod >
struct memoryPool {
	// FIXME mrwonko CRITICAL: never cleared (https://github.com/mrwonko/Jedi-Academy-Renaissance/issues/52)
	static char data[ MEM_POOL_SIZE< mod >::value ];
	static int allocPoint;
	static qboolean outOfMemory;
};
template< moduleType mod >
char memoryPool< mod >::data[ MEM_POOL_SIZE< mod >::value ];
template struct memoryPool< moduleType::ui >;
template< moduleType mod >
int memoryPool< mod >::allocPoint;
template< moduleType mod >
qboolean memoryPool< mod >::outOfMemory;
...
void *UI_Alloc( int size ) {
	...
	if ( memoryPool< mod >::allocPoint + size > MEM_POOL_SIZE< mod>::value ) {
		memoryPool< mod >::outOfMemory = qtrue;
		if (DC->Print) {
	...
	p = &memoryPool< mod >::data[memoryPool< mod >::allocPoint];

	memoryPool< mod >::allocPoint += ( size + 15 ) & ~15;
...
void UI_InitMemory( void ) {
	memoryPool< mod >::allocPoint = 0;
	memoryPool< mod >::outOfMemory = qfalse;
}

template< moduleType mod >
qboolean UI_OutOfMemory( void ) {
	return memoryPool< mod >::outOfMemory;
}
template qboolean UI_OutOfMemory< moduleType::ui >( void );
...
const char *String_Alloc(const char *p) {
	...
	if (len + strPoolIndex + 1 < STRING_POOL_SIZE) {
		...
		str  = (stringDef_t *) UI_Alloc< moduleType::ui >(sizeof(stringDef_t));
		str->next = NULL;
		str->str = &strPool[ph];
		if (last) {
			last->next = str;
...
void String_Report() {
	...
	f = memoryPool< mod >::allocPoint;
	f /= MEM_POOL_SIZE< mod >::value;
	f *= 100;
	Com_Printf("Memory Pool is %.1f%% full, %i bytes out of %i used.\n", f, memoryPool< mod >::allocPoint, MEM_POOL_SIZE< mod >::value);
}
template void String_Report< moduleType::ui >();
...
void String_Init() {
	...
	Item_SetupKeywordHash< moduleType::ui >();
	Menu_SetupKeywordHash< moduleType::ui >();
...
void Window_Paint(windowDef_t *w, float fadeAmount, float fadeClamp, float fadeCycle)
{
	...
	if ( w->style == WINDOW_STYLE_FILLED ) {
		...
		if ( mod != moduleType::cgame && w->flags & WINDOW_PLAYERCOLOR ) {
			vec4_t	color;
...
void Item_TextField_Paste( itemDef_t *item ) {
	...
	ui_trap->GetClipboardData( buff, sizeof(buff) );
	...
	for ( i = 0; i < pasteLen; i++ ) {
		Item_TextField_HandleKey< mod >( item, buff[i]|K_CHAR_FLAG );
	}
}
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
	...
	if (item->cvar) {
		...
		if( key & K_CHAR_FLAG ) {
			...
			if( mod == moduleType::ui ) {
				if( key == 'v' - 'a' + 1 ) {	// ctrl-v is paste
					Item_TextField_Paste< mod >( item );
					return qtrue;
				}
		}
	...
	if (menu) {
		...
		for (i = 0; i < menuCount; i++) {
			...
			//	Menus_Activate(&Menus[i]);
				...
				Menu_HandleKey< moduleType::ui >(&Menus[i], key, down);
			}
		}
...
void Menu_HandleKey(menuDef_t *menu, int key, qboolean down) {
	...
	{
		if (!Item_TextField_HandleKey< mod >(g_editItem, key))
		{
			g_editingField = qfalse;
			g_editItem = NULL;
			return;
...
void Item_SetTextExtents(itemDef_t *item, int *width, int *height, const char *text) {
	...
	*height = item->textRect.h;
	...
	if (*width == 0 || (item->type == ITEM_TYPE_OWNERDRAW && item->textalignment == ITEM_ALIGN_CENTER)
		|| ( mod == moduleType::cgame && item->text && item->text[0]=='@' && item->asset != se_language.modificationCount )	//string package language changed
		)
	{
		...
		*height = DC->textHeight(textPtr, item->textscale, item->iMenuFont);
		...
		if ( mod == moduleType::cgame && item->text && item->text[0]=='@' )//string package
		{//mark language
...
void Item_Text_AutoWrapped_Paint(itemDef_t *item) {
	...
	{
		if( mod == moduleType::ui ) {
			ui_trap->SE_GetStringTextString( &textPtr[ 1 ], text, sizeof( text ) );
		} else {
			cg_trap->SE_GetStringTextString( &textPtr[ 1 ], text, sizeof( text ) );
		}
		textPtr = text;
	}
	if (*textPtr == '\0') {
...
void Item_Text_Wrapped_Paint(itemDef_t *item) {
	...
	Item_TextColor(item, &color);
	Item_SetTextExtents< mod >(item, &width, &height, textPtr);
...
void Item_Text_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_WRAPPED) {
		Item_Text_Wrapped_Paint< mod >(item);
		return;
	}
	if (item->window.flags & WINDOW_AUTOWRAPPED) {
		Item_Text_AutoWrapped_Paint< mod >(item);
		return;
	}
...
void Item_TextField_Paint(itemDef_t *item) {
	...
	if (item->cvar) {
		...
		{
			if( mod == moduleType::ui ) {
				ui_trap->SE_GetStringTextString( &buff[ 1 ], buff, sizeof( buff ) );
			} else {
				cg_trap->SE_GetStringTextString( &buff[ 1 ], buff, sizeof( buff ) );
			}
		}
	}
...
void Item_YesNo_Paint(itemDef_t *item) {
	...
	if( mod == moduleType::ui ) {
		ui_trap->SE_GetStringTextString( "MENUS_YES", sYES, sizeof( sYES ) );
		ui_trap->SE_GetStringTextString( "MENUS_NO", sNO, sizeof( sNO ) );
	} else {
		cg_trap->SE_GetStringTextString( "MENUS_YES", sYES, sizeof( sYES ) );
		cg_trap->SE_GetStringTextString( "MENUS_NO", sNO, sizeof( sNO ) );
	}
...
void Item_Multi_Paint(itemDef_t *item) {
	...
	{
		if( mod == moduleType::ui ) {
			ui_trap->SE_GetStringTextString( &text[ 1 ], temp, sizeof( temp ) );
		} else {
			cg_trap->SE_GetStringTextString( &text[ 1 ], temp, sizeof( temp ) );
		}
		text = temp;
	}
	// Is is specifying a cvar to get the item name from?
...
// do NOT do this or it corrupts asian text!!!					Q_strupr(keyname[1]);
				...
				if( mod == moduleType::ui ) {
					ui_trap->SE_GetStringTextString( "MENUS_KEYBIND_OR", sOR, sizeof( sOR ) );
				} else {
					cg_trap->SE_GetStringTextString( "MENUS_KEYBIND_OR", sOR, sizeof( sOR ) );
				}
...
void Item_Model_Paint(itemDef_t *item)
{
	...
	if ( mod == moduleType::ui && uiInfo.moveAnimTime && (uiInfo.moveAnimTime < uiInfo.uiDC.realTime))
	{
		if (modelPtr)
		{
			...
			case BOTH_FORCEJUMP1:
				...
				ItemParse_asset_model_go< mod >( item, modelPath, &uiInfo.moveAnimTime );
  ...
  if (item->window.flags & WINDOW_TIMEDVISIBLE) {
	...
	//		else
			{	// Draw the desctext
				...
				{
					if( mod == moduleType::ui ) {
						ui_trap->SE_GetStringTextString( &textPtr[ 1 ], temp, sizeof( temp ) );
					} else {
						cg_trap->SE_GetStringTextString( &textPtr[ 1 ], temp, sizeof( temp ) );
					}
					textPtr = temp;
				}
	...
	Window_Paint< mod >(&item->window, parent->fadeAmount , parent->fadeClamp, parent->fadeCycle);
	...
	case ITEM_TYPE_EDITFIELD:
	case ITEM_TYPE_NUMERICFIELD:
		Item_TextField_Paint< mod >(item);
		break;
	case ITEM_TYPE_COMBO:
		break;
	case ITEM_TYPE_LISTBOX:
		Item_ListBox_Paint< mod >(item);
		break;
	case ITEM_TYPE_TEXTSCROLL:
	...
	case ITEM_TYPE_MODEL:
		Item_Model_Paint< mod >(item);
		break;
	case ITEM_TYPE_YESNO:
		Item_YesNo_Paint< mod >(item);
		break;
	case ITEM_TYPE_MULTI:
		Item_Multi_Paint< mod >(item);
		break;
	case ITEM_TYPE_BIND:
		Item_Bind_Paint< mod >(item);
		break;
	case ITEM_TYPE_SLIDER:
		Item_Slider_Paint< mod >(item);
		break;
	default:
		break;
	}
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
	...
	Window_Paint< mod >(&menu->window, menu->fadeAmount, menu->fadeClamp, menu->fadeCycle );
...
template void Menu_Paint< moduleType::cgame >( menuDef_t *menu, qboolean forcePaint );
...
void Item_ValidateTypeData(itemDef_t *item)
{
	...
	switch ( item->type )
	{
		case ITEM_TYPE_LISTBOX:
		{
			item->typeData.listbox = (listBoxDef_t *)UI_Alloc< moduleType::ui >(sizeof(listBoxDef_t));
		...
		case ITEM_TYPE_SLIDER:
		{
			item->typeData.edit = (editFieldDef_t *)UI_Alloc< moduleType::ui >(sizeof(editFieldDef_t));
		...
		case ITEM_TYPE_MULTI:
		{
			item->typeData.multi = (multiDef_t *)UI_Alloc< moduleType::ui >(sizeof(multiDef_t));
			memset(item->typeData.multi, 0, sizeof(multiDef_t));
			break;
		}
		case ITEM_TYPE_MODEL:
		{
			item->typeData.model = (modelDef_t *)UI_Alloc< moduleType::ui >(sizeof(modelDef_t));
			memset(item->typeData.model, 0, sizeof(modelDef_t));
			break;
		}
		case ITEM_TYPE_TEXTSCROLL:
		{
			item->typeData.textscroll = (textScrollDef_t *)UI_Alloc< moduleType::ui >(sizeof(textScrollDef_t));
...
void UI_InsertG2Pointer( CGhoul2Info_v *ghoul2)
{
	...
	{ //if we aren't reusing a chain then allocate space for it.
		(*nextFree) = (uiG2PtrTracker_t *)BG_Alloc< moduleType::ui >(sizeof(uiG2PtrTracker_t));
		(*nextFree)->next = NULL;
	}
...
void UI_CleanupGhoul2(void)
{
	...
	while (next)
	{
		if (next->ghoul2 && re->G2API_HaveWeGhoul2Models(*next->ghoul2)) //found a g2 instance, clean it.
			re->G2API_CleanGhoul2Models(&next->ghoul2);
...
qboolean ItemParse_asset_model_go( itemDef_t *item, const char *name,int *runTimeLength )
{
		...
		*runTimeLength = 0.0f;
		...
		if( !Q_stricmp( &name[ strlen( name ) - 4 ], ".glm" ) ) { //it's a ghoul2 model then
			if( item->ghoul2 ) {
				...
				re->G2API_CleanGhoul2Models( &item->ghoul2 );	//remove ghoul info
				item->flags &= ~ITF_G2VALID;
			}

			g2Model = re->G2API_InitGhoul2Model( &item->ghoul2, name, 0, modelPtr->g2skin, 0, 0, 0 );
			if( g2Model >= 0 ) {
	...
	//					DC->g2hilev_SetAnim(&item->ghoul2[0], "model_root", modelPtr->g2anim);
					...
					GLAName[ 0 ] = 0;
					{
						char *temp = re->G2API_GetGLAName( *item->ghoul2, 0 );
						if( temp ) {
							strcpy( GLAName, temp );
						}
					}
					...
					if( GLAName[ 0 ] ) {
						...
						if( slash ) { //If this isn't true the gla path must be messed up somehow.
							...
							if( animIndex != -1 ) { //We parsed out the animation info for whatever model this is
								...
								re->G2API_SetBoneAnim( *item->ghoul2, 0, "model_root", sFrame, eFrame, flags, animSpeed, time, -1, blendTime );
				...
				if( modelPtr->g2skin ) {
					//					DC->g2_SetSkin( &item->ghoul2[0], 0, modelPtr->g2skin );//this is going to set the surfs on/off matching the skin file
									//ui_trap->G2API_InitGhoul2Model(&item->ghoul2, name, 0, modelPtr->g2skin, 0, 0, 0);
									//ahh, what are you doing?!
					re->G2API_SetSkin( *item->ghoul2, 0, modelPtr->g2skin, modelPtr->g2skin );
				}
			}
			/*
			else
...
template qboolean ItemParse_asset_model_go< moduleType::ui >( itemDef_t *item, const char *name, int *runTimeLength );
...
qboolean ItemParse_asset_model( itemDef_t *item, int handle ) {
	...
	if( mod == moduleType::ui ) {
		if( !Q_stricmp( token.string, "ui_char_model" ) ) {
			...
			ui_trap->Cvar_VariableStringBuffer( "ui_char_model", ui_char_model, sizeof( ui_char_model ) );
			Com_sprintf( modelPath, sizeof( modelPath ), "models/players/%s/model.glm", ui_char_model );
			return ( ItemParse_asset_model_go< mod >( item, modelPath, &animRunLength ) );
		}
	}
	return (ItemParse_asset_model_go< mod >( item, token.string, &animRunLength ));
}
...
qboolean ItemParse_model_g2skin( itemDef_t *item, int handle ) {
	...
	modelPtr->g2skin = re->RegisterSkin(token.string);
...
qboolean ItemParse_model_g2skin_go( itemDef_t *item, const char *skinName )
{
	...
	{ //it was parsed correctly so still return true.
		modelPtr->g2skin = 0;
		re->G2API_SetSkin(*item->ghoul2, 0, 0, 0);
	...
	if ( item->ghoul2 )
	{
		defSkin = re->RegisterSkin(skinName);
		re->G2API_SetSkin(*item->ghoul2, 0, defSkin, defSkin);
	}
	...
	if (!Q_stricmp(token.string,"feeder") && item->special == FEEDER_LANGUAGES)
	{
		if( mod == moduleType::ui ) {
			for( ; multiPtr->count < uiInfo.languageCount; multiPtr->count++ ) {
				// The displayed text
				ui_trap->SE_GetLanguageName( (const int)multiPtr->count, (char *)currLanguage[ multiPtr->count ] );	// eg "English"
				multiPtr->cvarList[ multiPtr->count ] = languageString;
				// The cvar value that goes into se_language
				ui_trap->SE_GetLanguageName( (const int)multiPtr->count, (char *)currLanguage[ multiPtr->count ] );
...
template< moduleType mod >
struct itemParse {
	static keywordHash_t keywords[];

	static keywordHash_t *keywordHash[ KEYWORDHASH_SIZE ];
};

template< moduleType mod > keywordHash_t* itemParse< mod >::keywordHash[ KEYWORDHASH_SIZE ];

template< moduleType mod >
keywordHash_t itemParse< mod >::keywords[] = {
	...
	{ "asset_model",		ItemParse_asset_model< mod >,		NULL },
	...
	{ "cvarStrList",		ItemParse_cvarStrList< mod >,		NULL },
	...
	{ "isSaber",			ItemParse_isSaber< mod >,			NULL },
	{ "isSaber2",			ItemParse_isSaber2< mod >,			NULL },
	...
	{ "model_g2skin",		ItemParse_model_g2skin< mod >,		NULL },
...
void Item_SetupKeywordHash(void) {
	...
	memset(itemParse< mod >::keywordHash, 0, sizeof(itemParse< mod >::keywordHash));
	for (i = 0; itemParse< mod >::keywords[i].keyword; i++) {
		KeywordHash_Add(itemParse< mod >::keywordHash, &itemParse< mod >::keywords[i]);
	}
}
...
qboolean Item_Parse(int handle, itemDef_t *item) {
	...
	while ( 1 ) {
		...
		key = KeywordHash_Find(itemParse< mod >::keywordHash, token.string);
		if (!key) {
...
static void Item_TextScroll_BuildLines ( itemDef_t* item )
{
	...
	{
		ui_trap->SE_GetStringTextString( &psText[1], text, sizeof(text));
		psText = text;
	}
	...
	while (*psCurrentTextReadPos && (scrollPtr->iLineCount < MAX_TEXTSCROLL_LINES) )
	{
		...
		while ( *psCurrentTextReadPos )
		{
			...
			// read letter...
			//
			uiLetter = re->AnyLanguage_ReadCharFromString(psCurrentTextReadPos, &iAdvanceCount, &bIsTrailingPunctuation);
			...
			if ( DC->textWidth( sLineForDisplay, item->textscale, item->iMenuFont ) >= iBoxWidth )
			{
				...
				//
				if (uiLetter > 255 && bIsTrailingPunctuation && !re->Language_UsesSpaces())
				{
			...
			//
			if (bIsTrailingPunctuation || uiLetter == ' ' || (uiLetter > 255 && !re->Language_UsesSpaces()))
			{
				psBestLineBreakSrcPos = psCurrentTextReadPos;
			}
		}
...
qboolean MenuParse_itemDef( itemDef_t *item, int handle ) {
	...
	if (menu->itemCount < MAX_MENUITEMS) {
		itemDef_t *newItem = menu->items[menu->itemCount] = (itemDef_t *) UI_Alloc< mod >(sizeof(itemDef_t));
		Item_Init(newItem);
		if (!Item_Parse<mod>(handle, newItem))
		{
			return qfalse;
		}
...
template< moduleType mod >
keywordHash_t *menuParse< mod >::keywordHash[ KEYWORDHASH_SIZE ];

template< moduleType mod >
keywordHash_t menuParse< mod >::keywords[] = {
	...
	{"itemDef",				MenuParse_itemDef< mod >,	NULL	},
...
void Menu_SetupKeywordHash(void) {
	...
	memset(menuParse< mod >::keywordHash, 0, sizeof(menuParse< mod >::keywordHash));
	for (i = 0; menuParse< mod >::keywords[i].keyword; i++) {
		KeywordHash_Add(menuParse< mod >::keywordHash, &menuParse< mod >::keywords[i]);
	}
}
...
qboolean Menu_Parse(int handle, menuDef_t *menu) {
	...
	while ( 1 ) {
		...
		key = KeywordHash_Find(menuParse< mod >::keywordHash, token.string);
		if (!key) {
...
void Menu_New(int handle) {
	...
	if (menuCount < MAX_MENUS) {
		Menu_Init(menu);
		if (Menu_Parse< mod >(handle, menu)) {
			Menu_PostParse(menu);
			menuCount++;
		}
	}
}
template void Menu_New< moduleType::cgame >( int handle );
template void Menu_New< moduleType::ui>( int handle );
...
void Menu_PaintAll() {
	...
	for (i = 0; i < Menu_Count(); i++) {
		Menu_Paint< moduleType::ui >(&Menus[i], qfalse);
	}
