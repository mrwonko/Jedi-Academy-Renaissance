...
int UI_ParseInfos( char *buf, int max, char *infos[] ) {
	...
	while ( 1 ) {
		...
		infos[count] = (char *) UI_Alloc< moduleType::ui >(strlen(info) + strlen("\\num\\") + strlen(va("%d", MAX_ARENAS)) + 1);
		if (infos[count]) {
			strcpy(infos[count], info);
#ifndef FINAL_BUILD
			if (ui_trap->Cvar_VariableValue("com_buildScript"))
			{
				char *botFile = Info_ValueForKey(info, "personality");
				if (botFile && botFile[0])
				{
					int fh = 0;
					ui_trap->FS_Open(botFile, &fh, FS_READ);
					if (fh)
					{
						ui_trap->FS_Close(fh);
					}
...
static void UI_LoadArenasFromFile( char *filename ) {
	...
	if ( !f ) {
		ui_trap->Print( S_COLOR_RED "file not found: %s\n", filename );
		return;
	}
	if ( len >= MAX_ARENAS_TEXT ) {
		ui_trap->Print( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_ARENAS_TEXT );
		ui_trap->FS_Close( f );
		return;
	}
...
void UI_LoadArenas( void ) {
	...
	numdirs = ui_trap->FS_GetFileList( "scripts", ".arena", dirlist, ARRAY_LEN( dirlist ) );
	dirptr  = dirlist;
...
//	ui_trap->Print( "%i arenas parsed\n", ui_numArenas );
	if (UI_OutOfMemory< moduleType::ui >()) {
		ui_trap->Print(S_COLOR_YELLOW"WARNING: not anough memory in pool to load all arenas\n");
	}
...
static void UI_LoadBotsFromFile( char *filename ) {
	...
	if ( len >= MAX_BOTS_TEXT ) {
		ui_trap->Print( S_COLOR_RED "file too large: %s is %i, max allowed is %i", filename, len, MAX_BOTS_TEXT );
		ui_trap->FS_Close( f );
		return;
	}
...
void UI_LoadBots( void ) {
	...
	ui_trap->Cvar_Register( &botsFile, "g_botsFile", "", CVAR_INIT|CVAR_ROM );
	if( *botsFile.string ) {
	...
	numdirs = ui_trap->FS_GetFileList("scripts", ".bot", dirlist, 1024 );
	dirptr  = dirlist;
	for (i = 0; i < numdirs; i++, dirptr += dirlen+1) {
		dirlen = strlen(dirptr);
		strcpy(filename, "scripts/");
...
char *UI_GetBotInfoByNumber( int num ) {
	if( num < 0 || num >= ui_numBots ) {
		ui_trap->Print( S_COLOR_RED "Invalid bot number: %i\n", num );
		return NULL;
	}
