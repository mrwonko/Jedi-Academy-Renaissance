...
bgLoadedAnim_t uiAllAnims[MAX_ANIM_FILES];
int uiNumAllAnims = 1; //start off at 1, because 0 will always be assigned to humanoid.

animation_t *UI_AnimsetAlloc(void)
{
	assert (uiNumAllAnims < MAX_ANIM_FILES);
	uiAllAnims[uiNumAllAnims].anims = (animation_t *) BG_Alloc< moduleType::ui >(sizeof(animation_t)*MAX_TOTALANIMATIONS);

	return uiAllAnims[uiNumAllAnims].anims;
}
...
int UI_ParseAnimationFile(const char *filename, animation_t *animset, qboolean isHumanoid)
{
	...
	if (!isHumanoid)
	{
		...
		{ //see if it's been loaded already
			if (!Q_stricmp(uiAllAnims[i].filename, filename))
			{
				animset = uiAllAnims[i].anims;
	...
	{ //rww - We are always using the same animation config now. So only load it once.
		len = ui_trap->FS_Open( filename, &f, FS_READ );
		...
		ui_trap->FS_Read( UIPAFtext, len, f );
		UIPAFtext[len] = 0;
		ui_trap->FS_Close( f );
	}
	else
...
//		animset[animNum].initialLerp = ceil(1000.0f / fabs(fps));
	...
	*/
	...
	if (isHumanoid)
	{
		uiAllAnims[0].anims = animset;
		Q_strncpyz(uiAllAnims[0].filename, filename, sizeof(uiAllAnims[0].filename));
	...
	{
		uiAllAnims[nextIndex].anims = animset;
		Q_strncpyz(uiAllAnims[nextIndex].filename, filename, sizeof(uiAllAnims[nextIndex].filename));
...
// Convert ui's net source to AS_* used by ui_trap calls.
...
void AssetCache(void) {
	...
	uiInfo.uiDC.Assets.gradientBar			= ui_trap->R_RegisterShaderNoMip( ASSET_GRADIENTBAR );
	uiInfo.uiDC.Assets.fxBasePic			= ui_trap->R_RegisterShaderNoMip( ART_FX_BASE );
	uiInfo.uiDC.Assets.fxPic[0]				= ui_trap->R_RegisterShaderNoMip( ART_FX_RED );
	uiInfo.uiDC.Assets.fxPic[1]				= ui_trap->R_RegisterShaderNoMip( ART_FX_ORANGE );//ui_trap->R_RegisterShaderNoMip( ART_FX_YELLOW );
	uiInfo.uiDC.Assets.fxPic[2]				= ui_trap->R_RegisterShaderNoMip( ART_FX_YELLOW );//ui_trap->R_RegisterShaderNoMip( ART_FX_GREEN );
	uiInfo.uiDC.Assets.fxPic[3]				= ui_trap->R_RegisterShaderNoMip( ART_FX_GREEN );//ui_trap->R_RegisterShaderNoMip( ART_FX_TEAL );
	uiInfo.uiDC.Assets.fxPic[4]				= ui_trap->R_RegisterShaderNoMip( ART_FX_BLUE );
	uiInfo.uiDC.Assets.fxPic[5]				= ui_trap->R_RegisterShaderNoMip( ART_FX_PURPLE );//ui_trap->R_RegisterShaderNoMip( ART_FX_CYAN );
	uiInfo.uiDC.Assets.fxPic[6]				= ui_trap->R_RegisterShaderNoMip( ART_FX_WHITE );
	uiInfo.uiDC.Assets.scrollBar			= ui_trap->R_RegisterShaderNoMip( ASSET_SCROLLBAR );
	uiInfo.uiDC.Assets.scrollBarArrowDown	= ui_trap->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWDOWN );
	uiInfo.uiDC.Assets.scrollBarArrowUp		= ui_trap->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWUP );
	uiInfo.uiDC.Assets.scrollBarArrowLeft	= ui_trap->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWLEFT );
	uiInfo.uiDC.Assets.scrollBarArrowRight	= ui_trap->R_RegisterShaderNoMip( ASSET_SCROLLBAR_ARROWRIGHT );
	uiInfo.uiDC.Assets.scrollBarThumb		= ui_trap->R_RegisterShaderNoMip( ASSET_SCROLL_THUMB );
	uiInfo.uiDC.Assets.sliderBar			= ui_trap->R_RegisterShaderNoMip( ASSET_SLIDER_BAR );
	uiInfo.uiDC.Assets.sliderThumb			= ui_trap->R_RegisterShaderNoMip( ASSET_SLIDER_THUMB );

	// Icons for various server settings.
	uiInfo.uiDC.Assets.needPass			= ui_trap->R_RegisterShaderNoMip( "gfx/menus/needpass" );
	uiInfo.uiDC.Assets.noForce			= ui_trap->R_RegisterShaderNoMip( "gfx/menus/noforce" );
	uiInfo.uiDC.Assets.forceRestrict	= ui_trap->R_RegisterShaderNoMip( "gfx/menus/forcerestrict" );
	uiInfo.uiDC.Assets.saberOnly		= ui_trap->R_RegisterShaderNoMip( "gfx/menus/saberonly" );
	uiInfo.uiDC.Assets.trueJedi			= ui_trap->R_RegisterShaderNoMip( "gfx/menus/truejedi" );

	for( n = 0; n < NUM_CROSSHAIRS; n++ ) {
		uiInfo.uiDC.Assets.crosshairShader[n] = ui_trap->R_RegisterShaderNoMip( va("gfx/2d/crosshair%c", 'a' + n ) );
	}
}

void _UI_DrawSides(float x, float y, float w, float h, float size) {
	size *= uiInfo.uiDC.xscale;
	ui_trap->R_DrawStretchPic( x, y, size, h, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
	ui_trap->R_DrawStretchPic( x + w - size, y, size, h, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
}

void _UI_DrawTopBottom(float x, float y, float w, float h, float size) {
	size *= uiInfo.uiDC.yscale;
	ui_trap->R_DrawStretchPic( x, y, w, size, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
	ui_trap->R_DrawStretchPic( x, y + h - size, w, size, 0, 0, 0, 0, uiInfo.uiDC.whiteShader );
}
/*
================
UI_DrawRect
...
void _UI_DrawRect( float x, float y, float width, float height, float size, const float *color ) {
	ui_trap->R_SetColor( color );

	_UI_DrawTopBottom(x, y, width, height, size);
	_UI_DrawSides(x, y, width, height, size);

	ui_trap->R_SetColor( NULL );
}
...
{
	int iFontIndex = ui::MenuFontToHandle(iMenuFont);

	return ui_trap->R_Font_StrLenPixels(text, iFontIndex, scale);
}
...
int Text_Height(const char *text, float scale, int iMenuFont)
{
	...
	return ui_trap->R_Font_HeightPixels(iFontIndex, scale);
}
...
void Text_Paint(float x, float y, float scale, vec4_t color, const char *text, float adjust, int limit, int style, int iMenuFont)
{
	...
	ui_trap->R_Font_DrawString(	x,		// int ox
							y,		// int oy
...
void Text_PaintWithCursor(float x, float y, float scale, vec4_t color, const char *text, int cursorPos, char cursor, int limit, int style, int iMenuFont)
{
	...
	//
	{
		...
		int iCopyCount = limit > 0 ? Q_min( (int)strlen( text ), limit ) : (int)strlen( text );
			...
			{
				int iFontIndex = ui::MenuFontToHandle( iMenuFont );
				int iNextXpos  = ui_trap->R_Font_StrLenPixels(sTemp, iFontIndex, scale );
...
static void Text_Paint_Limit(float *maxX, float x, float y, float scale, vec4_t color, const char* text, float adjust, int limit, int iMenuFont)
{
	...
	//float fMax = *maxX;
	int iPixelLen = ui_trap->R_Font_StrLenPixels(text, iFontIndex, scale);
	if (x + iPixelLen > *maxX)
	{
		...
		while (*psText && (x + ui_trap->R_Font_StrLenPixels(sTemp, iFontIndex, scale)<=*maxX)
				...
				)
		{
			...
			uiLetter = ui_trap->R_AnyLanguage_ReadCharFromString(psText, &iAdvanceCount, NULL);
...
static void UI_BuildPlayerList() {
	...
	ui_trap->GetClientState( &cs );
	ui_trap->GetConfigString( CS_PLAYERS + cs.clientNum, info, MAX_INFO_STRING );
	uiInfo.playerNumber = cs.clientNum;
	uiInfo.teamLeader = atoi(Info_ValueForKey(info, "tl"));
	team = atoi(Info_ValueForKey(info, "t"));
	ui_trap->GetConfigString( CS_SERVERINFO, info, sizeof(info) );
	count = atoi( Info_ValueForKey( info, "sv_maxclients" ) );
	uiInfo.playerCount = 0;
	uiInfo.myTeamCount = 0;
	playerTeamNumber = 0;
	for( n = 0; n < count; n++ ) {
		ui_trap->GetConfigString( CS_PLAYERS + n, info, MAX_INFO_STRING );
	...
	if (!uiInfo.teamLeader) {
		ui_trap->Cvar_Set("cg_selectedPlayer", va("%d", playerTeamNumber));
	}

	n = ui_trap->Cvar_VariableValue("cg_selectedPlayer");
	...
	if (n < uiInfo.myTeamCount) {
		ui_trap->Cvar_Set("cg_selectedPlayerName", uiInfo.teamNames[n]);
	}
	else
	{
		ui_trap->Cvar_Set("cg_selectedPlayerName", "Everyone");
	}

	if (!team || team == TEAM_SPECTATOR || !uiInfo.teamLeader)
	{
		n = uiInfo.myTeamCount;
		ui_trap->Cvar_Set("cg_selectedPlayer", va("%d", n));
		ui_trap->Cvar_Set("cg_selectedPlayerName", "N/A");
	}
}
...
void UI_SetActiveMenu( uiMenuCommand_t menu ) {
	...
	if (Menu_Count() > 0) {
		...
		switch ( menu ) {
		case UIMENU_NONE:
			ui_trap->Key_SetCatcher( ui_trap->Key_GetCatcher() & ~KEYCATCH_UI );
			ui_trap->Key_ClearStates();
			ui_trap->Cvar_Set( "cl_paused", "0" );
			Menus_CloseAll();

			return;
		case UIMENU_MAIN:
			{
				//	ui_trap->Cvar_Set( "sv_killserver", "1" );
				ui_trap->Key_SetCatcher( KEYCATCH_UI );
				//	ui_trap->S_StartLocalSound( trap_S_RegisterSound("sound/misc/menu_background.wav", qfalse) , CHAN_LOCAL_SOUND );
				//	ui_trap->S_StartBackgroundTrack("sound/misc/menu_background.wav", NULL);
				if (uiInfo.inGameLoad)
					UI_LoadNonIngame();

				Menus_CloseAll();
				Menus_ActivateByName("main");
				ui_trap->Cvar_VariableStringBuffer("com_errorMessage", buf, sizeof(buf));
				...
				if (buf[0])
				{
					...
					{
						ui_trap->Cvar_Set("com_errorMessage", "");
					}
				}
				return;
			}
		...
		case UIMENU_VOICECHAT:
			...
			if (ui_trap->Cvar_VariableValue( "g_gametype" ) < GT_TEAM)
			{
				return;
			}
...
const char *UI_GetStringEdString(const char *refSection, const char *refName)
{
	...
	ui_trap->SE_GetStringTextString(va("%s_%s", refSection, refName), text, sizeof(text));
	return text;
}

void UI_SetColor( const float *rgba ) {
	ui_trap->R_SetColor( rgba );
}
...
char *GetMenuBuffer(const char *filename) {
	...
	len = ui_trap->FS_Open( filename, &f, FS_READ );
	if ( !f ) {
		ui_trap->Print( S_COLOR_RED "menu file not found: %s, using default\n", filename );
		return defaultMenu;
	}
	if ( len >= MAX_MENUFILE ) {
		ui_trap->Print( S_COLOR_RED "menu file too large: %s is %i, max allowed is %i\n", filename, len, MAX_MENUFILE );
		ui_trap->FS_Close( f );
		return defaultMenu;
	}

	ui_trap->FS_Read( buf, len, f );
	buf[len] = 0;
	ui_trap->FS_Close( f );
	//COM_Compress(buf);
	return buf;
...
qboolean Asset_Parse(int handle) {
	...
	if (!ui_trap->PC_ReadToken(handle, &token))
		return qfalse;
	...
	while ( 1 ) {
		...
		if (Q_stricmp(token.string, "font") == 0) {
			int pointSize;
			if (!ui_trap->PC_ReadToken(handle, &token) || !PC_Int_Parse(handle,&pointSize)) {
				return qfalse;
			}
			//ui_trap->R_RegisterFont(tempStr, pointSize, &uiInfo.uiDC.Assets.textFont);
			uiInfo.uiDC.Assets.qhMediumFont = ui_trap->R_RegisterFont(token.string);
		...
		if (Q_stricmp(token.string, "smallFont") == 0) {
			...
			//ui_trap->R_RegisterFont(token, pointSize, &uiInfo.uiDC.Assets.smallFont);
			uiInfo.uiDC.Assets.qhSmallFont = ui_trap->R_RegisterFont(token.string);
			continue;
		}
		...
		if (Q_stricmp(token.string, "small2Font") == 0) {
			...
			//ui_trap->R_RegisterFont(token, pointSize, &uiInfo.uiDC.Assets.smallFont);
			uiInfo.uiDC.Assets.qhSmall2Font = ui_trap->R_RegisterFont(token.string);
			continue;
		}
		...
		if (Q_stricmp(token.string, "bigFont") == 0) {
			...
			//ui_trap->R_RegisterFont(token, pointSize, &uiInfo.uiDC.Assets.bigFont);
			uiInfo.uiDC.Assets.qhBigFont = ui_trap->R_RegisterFont(token.string);
			continue;
		}
		...
		if (Q_stricmp(token.string, "cursor") == 0)
		{
			...
			uiInfo.uiDC.Assets.cursor = ui_trap->R_RegisterShaderNoMip( uiInfo.uiDC.Assets.cursorStr);
			continue;
		}
		...
		if (Q_stricmp(token.string, "gradientbar") == 0) {
			...
			uiInfo.uiDC.Assets.gradientBar = ui_trap->R_RegisterShaderNoMip(token.string);
			continue;
		}
		...
		if (Q_stricmp(token.string, "menuEnterSound") == 0) {
			...
			uiInfo.uiDC.Assets.menuEnterSound = ui_trap->S_RegisterSound( token.string );
			continue;
		}
		...
		if (Q_stricmp(token.string, "menuExitSound") == 0) {
			...
			uiInfo.uiDC.Assets.menuExitSound = ui_trap->S_RegisterSound( token.string );
			continue;
		}
		...
		if (Q_stricmp(token.string, "itemFocusSound") == 0) {
			...
			uiInfo.uiDC.Assets.itemFocusSound = ui_trap->S_RegisterSound( token.string );
			continue;
		}
		...
		if (Q_stricmp(token.string, "menuBuzzSound") == 0) {
			...
			uiInfo.uiDC.Assets.menuBuzzSound = ui_trap->S_RegisterSound( token.string );
			continue;
		}
		...
		if (Q_stricmp(token.string, "moveRollSound") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.moveRollSound = ui_trap->S_RegisterSound( token.string );
			}
			continue;
		}
		...
		if (Q_stricmp(token.string, "moveJumpSound") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.moveJumpSound = ui_trap->S_RegisterSound( token.string );
			}
		...
		if (Q_stricmp(token.string, "datapadmoveSaberSound1") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.datapadmoveSaberSound1 = ui_trap->S_RegisterSound( token.string );
			}
		...
		if (Q_stricmp(token.string, "datapadmoveSaberSound2") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.datapadmoveSaberSound2 = ui_trap->S_RegisterSound( token.string );
			}
		...
		if (Q_stricmp(token.string, "datapadmoveSaberSound3") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.datapadmoveSaberSound3 = ui_trap->S_RegisterSound( token.string );
			}
		...
		if (Q_stricmp(token.string, "datapadmoveSaberSound4") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.datapadmoveSaberSound4 = ui_trap->S_RegisterSound( token.string );
			}
		...
		if (Q_stricmp(token.string, "datapadmoveSaberSound5") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.datapadmoveSaberSound5 = ui_trap->S_RegisterSound( token.string );
			}
		...
		if (Q_stricmp(token.string, "datapadmoveSaberSound6") == 0)
		{
			if (ui_trap->PC_ReadToken(handle,&token))
			{
				uiInfo.uiDC.Assets.datapadmoveSaberSound6 = ui_trap->S_RegisterSound( token.string );
			}
			...
			if (PC_Script_Parse(handle, &tempStr))
			{
				...
				do
				{
					...
					if (soundFile[0] != 0 && soundFile[0] != ';') {
						ui_trap->S_RegisterSound( soundFile);
					}
				} while (soundFile[0]);
			}
			continue;
...
void UI_ParseMenu(const char *menuFile) {
	...
	handle = ui_trap->PC_LoadSource(menuFile);
	if (!handle) {
		return;
	}
	...
	while ( 1 ) {
		...
		if (Q_stricmp(token.string, "menudef") == 0) {
			// start a new menu
			Menu_New< moduleType::ui >(handle);
		}
	}
	ui_trap->PC_FreeSource(handle);
}
...
void UI_LoadMenus(const char *menuFile, qboolean reset) {
	pc_token_t token;
	int handle;
//	int start = ui_trap->Milliseconds();

	ui_trap->PC_LoadGlobalDefines ( "ui/jamp/menudef.h" );

	handle = ui_trap->PC_LoadSource( menuFile );
	if (!handle) {
		Com_Printf( S_COLOR_YELLOW "menu file not found: %s, using default\n", menuFile );
		handle = ui_trap->PC_LoadSource( "ui/jampmenus.txt" );
		if (!handle) {
			ui_trap->Error( ERR_DROP, S_COLOR_RED "default menu file not found: ui/jampmenus.txt, unable to continue!\n" );
		}
	}
...
//	Com_Printf("UI menu load time = %d milli seconds\n", ui_trap->Milliseconds() - start);

	ui_trap->PC_FreeSource( handle );

	ui_trap->PC_RemoveAllGlobalDefines ( );
}
...
static void UI_DrawHandicap(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	h = Com_Clamp(5, 100, ui_trap->Cvar_VariableValue("handicap"));
	i = 20 - h / 5;
...
static void UI_SetCapFragLimits(qboolean uiVars) {
	...
	if (uiVars) {
		ui_trap->Cvar_Set("ui_captureLimit", va("%d", cap));
		ui_trap->Cvar_Set("ui_fragLimit", va("%d", frag));
	} else {
		ui_trap->Cvar_Set("capturelimit", va("%d", cap));
		ui_trap->Cvar_Set("fraglimit", va("%d", frag));
	}
}
...
static void UI_DrawNetGameType(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	...
	{
		ui_trap->Cvar_Set("ui_netGametype", "0");
		ui_trap->Cvar_Update(&ui_netGametype);
		ui_trap->Cvar_Set("ui_actualNetGametype", "0");
		ui_trap->Cvar_Update(&ui_actualNetGametype);
	}
...
static void UI_DrawAutoSwitch(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	int switchVal = ui_trap->Cvar_VariableValue("cg_autoswitch");
...
static void UI_DrawJoinGameType(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	...
	{
		ui_trap->Cvar_Set("ui_joinGametype", "0");
		ui_trap->Cvar_Update(&ui_joinGametype);
	}
...
static void UI_DrawClanLogo(rectDef_t *rect, float scale, vec4_t color) {
	...
	if (i >= 0 && i < uiInfo.teamCount) {
		...
		if (uiInfo.teamList[i].teamIcon == -1) {
			uiInfo.teamList[i].teamIcon = ui_trap->R_RegisterShaderNoMip(uiInfo.teamList[i].imageName);
			uiInfo.teamList[i].teamIcon_Metal = ui_trap->R_RegisterShaderNoMip(va("%s_metal",uiInfo.teamList[i].imageName));
			uiInfo.teamList[i].teamIcon_Name = ui_trap->R_RegisterShaderNoMip(va("%s_name", uiInfo.teamList[i].imageName));
		}
	...
	if (i >= 0 && i < uiInfo.teamCount) {
		if (uiInfo.teamList[i].cinematic >= -2) {
			if (uiInfo.teamList[i].cinematic == -1) {
				uiInfo.teamList[i].cinematic = ui_trap->CIN_PlayCinematic(va("%s.roq", uiInfo.teamList[i].imageName), 0, 0, 0, 0, (CIN_loop | CIN_silent) );
			}
			if (uiInfo.teamList[i].cinematic >= 0) {
				ui_trap->CIN_RunCinematic(uiInfo.teamList[i].cinematic);
				ui_trap->CIN_SetExtents(uiInfo.teamList[i].cinematic, rect->x, rect->y, rect->w, rect->h);
				ui_trap->CIN_DrawCinematic(uiInfo.teamList[i].cinematic);
			} else {
				ui_trap->R_SetColor( color );
...
static void UI_DrawPreviewCinematic(rectDef_t *rect, float scale, vec4_t color) {
	if (uiInfo.previewMovie > -2) {
		uiInfo.previewMovie = ui_trap->CIN_PlayCinematic(va("%s.roq", uiInfo.movieList[uiInfo.movieIndex]), 0, 0, 0, 0, (CIN_loop | CIN_silent) );
		if (uiInfo.previewMovie >= 0) {
			ui_trap->CIN_RunCinematic(uiInfo.previewMovie);
			ui_trap->CIN_SetExtents(uiInfo.previewMovie, rect->x, rect->y, rect->w, rect->h);
			ui_trap->CIN_DrawCinematic(uiInfo.previewMovie);
		} else {
...
static void UI_DrawSkill(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	int i;
	i = ui_trap->Cvar_VariableValue( "g_spSkill" );
...
static void UI_DrawSkinColor(rectDef_t *rect, float scale, vec4_t color, int textStyle, int val, int min, int max, int iMenuFont)
{
	...
	{
	case TEAM_RED:
		ui_trap->SE_GetStringTextString("MENUS_TEAM_RED", s, sizeof(s));
//		Com_sprintf(s, sizeof(s), "Red\0");
		break;
	case TEAM_BLUE:
		ui_trap->SE_GetStringTextString("MENUS_TEAM_BLUE", s, sizeof(s));
//		Com_sprintf(s, sizeof(s), "Blue\0");
		break;
	default:
		ui_trap->SE_GetStringTextString("MENUS_DEFAULT", s, sizeof(s));
...
static void UI_DrawForceSide(rectDef_t *rect, float scale, vec4_t color, int textStyle, int val, int min, int max, int iMenuFont)
{
	...
	{
		switch((int)(ui_trap->Cvar_VariableValue("ui_myteam")))
		{
		case TEAM_RED:
	...
	{
		ui_trap->SE_GetStringTextString("MENUS_FORCEDESC_LIGHT",s, sizeof(s));
	...
	{
		ui_trap->SE_GetStringTextString("MENUS_FORCEDESC_DARK",s, sizeof(s));
		menu = Menus_FindByName("forcealloc");
		if (menu)
		{
...
static void UI_DrawJediNonJedi(rectDef_t *rect, float scale, vec4_t color, int textStyle, int val, int min, int max, int iMenuFont)
{
	...
	if ( val == FORCE_NONJEDI )
		ui_trap->SE_GetStringTextString("MENUS_NO",s, sizeof(s));
	else
		ui_trap->SE_GetStringTextString("MENUS_YES",s, sizeof(s));
...
static void UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color, qboolean blue, int num, int textStyle, int iMenuFont)
{
	...
	// 1 - Human
	// 2..NumCharacters - Bot
	int value = ui_trap->Cvar_VariableValue(va(blue ? "ui_blueteam%i" : "ui_redteam%i", num));
	const char *text;
	int maxcl = ui_trap->Cvar_VariableValue( "sv_maxClients" );
	vec4_t finalColor;
	int numval = num;
...
static void UI_DrawMapPreview(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	...
	if (map < 0 || map > uiInfo.mapCount) {
		if (net) {
			ui_trap->Cvar_Set("ui_currentNetMap", "0");
			ui_trap->Cvar_Update(&ui_currentNetMap);
		} else {
			ui_trap->Cvar_Set("ui_currentMap", "0");
			ui_trap->Cvar_Update(&ui_currentMap);
		}
		map = 0;
	}

	if (uiInfo.mapList[map].levelShot == -1) {
		uiInfo.mapList[map].levelShot = ui_trap->R_RegisterShaderNoMip(uiInfo.mapList[map].imageName);
	}

	if (uiInfo.mapList[map].levelShot > 0) {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, uiInfo.mapList[map].levelShot);
	} else {
		UI_DrawHandlePic( rect->x, rect->y, rect->w, rect->h, ui_trap->R_RegisterShaderNoMip("menu/art/unknownmap_mp"));
	}
}
...
static void UI_DrawMapCinematic(rectDef_t *rect, float scale, vec4_t color, qboolean net) {
	...
	if (uiInfo.mapList[map].cinematic >= -1) {
		if (uiInfo.mapList[map].cinematic == -1) {
			uiInfo.mapList[map].cinematic = ui_trap->CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[map].mapLoadName), 0, 0, 0, 0, (CIN_loop | CIN_silent) );
		}
		if (uiInfo.mapList[map].cinematic >= 0) {
			ui_trap->CIN_RunCinematic(uiInfo.mapList[map].cinematic);
			ui_trap->CIN_SetExtents(uiInfo.mapList[map].cinematic, rect->x, rect->y, rect->w, rect->h);
			ui_trap->CIN_DrawCinematic(uiInfo.mapList[map].cinematic);
		} else {
...
*/
	...
	if (menu)
	{
		...
		if ((int)(ui_trap->Cvar_VariableValue("ui_myteam")) != TEAM_SPECTATOR)
		{
...
static void UI_DrawNetSource(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	if (ui_netSource.integer < 0 || ui_netSource.integer >= numNetSources) {
		ui_trap->Cvar_Set("ui_netSource", "0");
		ui_trap->Cvar_Update(&ui_netSource);
	}

	ui_trap->SE_GetStringTextString("MENUS_SOURCE", holdSPString, sizeof(holdSPString) );
...
static void UI_DrawNetMapCinematic(rectDef_t *rect, float scale, vec4_t color) {
	...
	if (uiInfo.serverStatus.currentServerCinematic >= 0) {
		ui_trap->CIN_RunCinematic(uiInfo.serverStatus.currentServerCinematic);
		ui_trap->CIN_SetExtents(uiInfo.serverStatus.currentServerCinematic, rect->x, rect->y, rect->w, rect->h);
		ui_trap->CIN_DrawCinematic(uiInfo.serverStatus.currentServerCinematic);
	} else {
...
{
	ui_trap->SE_GetStringTextString("MENUS_GAME", holdSPString, sizeof(holdSPString));
...
static void UI_DrawTier(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	int i;
	i = ui_trap->Cvar_VariableValue( "ui_currentTier" );
...
static void UI_DrawTierMap(rectDef_t *rect, int index) {
	...
	if (uiInfo.tierList[i].mapHandles[index] == -1) {
		uiInfo.tierList[i].mapHandles[index] = ui_trap->R_RegisterShaderNoMip(va("levelshots/%s", uiInfo.tierList[i].maps[index]));
	}
...
static void UI_DrawTierMapName(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont) {
	...
	j = ui_trap->Cvar_VariableValue("ui_currentMap");
...
static void UI_NextOpponent() {
	...
	if (i == j) {
 	...
 	ui_trap->Cvar_Set( "ui_opponentName", uiInfo.teamList[i].teamName );
}
...
//			s = "Jedi";
    ...
    case UI_BLUETEAM1:
		...
		case UI_BLUETEAM8:
			...
			value = ui_trap->Cvar_VariableValue(va("ui_blueteam%i", iUse));
			if (value <= 1) {
				text = "Human";
			} else {
				value -= 2;
    ...
    case UI_REDTEAM1:
		...
		case UI_REDTEAM8:
			...
			value = ui_trap->Cvar_VariableValue(va("ui_redteam%i", iUse));
			if (value <= 1) {
				text = "Human";
			} else {
				value -= 2;
...
static void UI_DrawServerRefreshDate(rectDef_t *rect, float scale, vec4_t color, int textStyle, int iMenuFont)
{
	if (uiInfo.serverStatus.refreshActive)
	{
		...
		ui_trap->SE_GetStringTextString("MP_INGAME_GETTINGINFOFORSERVERS", holdSPString, sizeof(holdSPString));
		Text_Paint(rect->x, rect->y, scale, newColor, va((char *) holdSPString, ui_trap->LAN_GetServerCount(UI_SourceForLAN())), 0, 0, textStyle, iMenuFont);
	}
	else
	{
		char buff[64];
		Q_strncpyz(buff, UI_Cvar_VariableString(va("ui_lastServerRefresh_%i", ui_netSource.integer)), sizeof(buff));
		ui_trap->SE_GetStringTextString("MP_INGAME_SERVER_REFRESHTIME", holdSPString, sizeof(holdSPString));
...
static qboolean UI_OwnerDrawVisible(int flags) {
	...
	while (flags) {
		if (flags & UI_SHOW_FFA) {
			if (ui_trap->Cvar_VariableValue("g_gametype") != GT_FFA &&
				ui_trap->Cvar_VariableValue("g_gametype") != GT_HOLOCRON &&
				ui_trap->Cvar_VariableValue("g_gametype") != GT_JEDIMASTER) {
				vis = qfalse;
			}
			flags &= ~UI_SHOW_FFA;
		}
...
static qboolean UI_Handicap_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set( "handicap", va( "%i", h) );
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_GameType_HandleKey(int flags, float *special, int key, qboolean resetMap) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set("ui_gametype", va("%d", value));
		ui_trap->Cvar_Update(&ui_gametype);
...
static qboolean UI_NetGameType_HandleKey(int flags, float *special, int key)
{
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER)
	{
		...
		ui_trap->Cvar_Set( "ui_netGametype", va("%d", value));
		ui_trap->Cvar_Update(&ui_netGametype);
		ui_trap->Cvar_Set( "ui_actualNetGametype", va("%d", uiInfo.gameTypes[ui_netGametype.integer].gtEnum));
		ui_trap->Cvar_Update(&ui_actualNetGametype);
		ui_trap->Cvar_Set( "ui_currentNetMap", "0");
		ui_trap->Cvar_Update(&ui_currentNetMap);
...
static qboolean UI_AutoSwitch_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set( "cg_autoswitch", va("%i", switchVal));
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_JoinGameType_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set( "ui_joinGametype", va("%d", value));
		ui_trap->Cvar_Update(&ui_joinGametype);
...
static qboolean UI_Skill_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		int i = ui_trap->Cvar_VariableValue( "g_spSkill" );
		...
		ui_trap->Cvar_Set("g_spSkill", va("%i", i));
		ui_trap->Cvar_Update(&g_spSkill);
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_TeamName_HandleKey(int flags, float *special, int key, qboolean blue) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set( (blue) ? "ui_blueTeam" : "ui_redTeam", uiInfo.teamList[i].teamName);
...
static qboolean UI_TeamMember_HandleKey(int flags, float *special, int key, qboolean blue, int num) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		int value = ui_trap->Cvar_VariableValue(cvar);
		int maxcl = ui_trap->Cvar_VariableValue( "sv_maxClients" );
		int numval = num;
		...
		ui_trap->Cvar_Set(cvar, va("%i", value));
		return qtrue;
	}
	return qfalse;
}
	...
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		if(value >= UIAS_GLOBAL1 && value <= UIAS_GLOBAL5)
		{
			...
			while(value >= UIAS_GLOBAL1 && value <= UIAS_GLOBAL5)
			{
				...
				ui_trap->Cvar_VariableStringBuffer(cvarname, masterstr, sizeof(masterstr));
				if(*masterstr)
					break;
		...
		ui_trap->Cvar_Set( "ui_netSource", va("%d", value));
		ui_trap->Cvar_Update(&ui_netSource);
...
static qboolean UI_NetFilter_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set( "ui_serverFilterType", va("%d", value));
		ui_trap->Cvar_Update(&ui_serverFilterType);
...
static qboolean UI_BotName_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
//		int game = ui_trap->Cvar_VariableValue("g_gametype");
		int value = uiInfo.botIndex;
...
static qboolean UI_Crosshair_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		ui_trap->Cvar_Set("cg_drawCrosshair", va("%d", uiInfo.currentCrosshair));
		return qtrue;
	}
	return qfalse;
}
...
static qboolean UI_SelectedPlayer_HandleKey(int flags, float *special, int key) {
	if (key == A_MOUSE1 || key == A_MOUSE2 || key == A_ENTER || key == A_KP_ENTER) {
		...
		selected = ui_trap->Cvar_VariableValue("cg_selectedPlayer");
		...
		if (selected == uiInfo.myTeamCount) {
		 	...
		 	ui_trap->Cvar_Set( "cg_selectedPlayerName", uiInfo.teamNames[selected]);
		}
	 	ui_trap->Cvar_Set( "cg_selectedPlayer", va("%d", selected));
	}
	return qfalse;
}
...
*/
static int QDECL UI_ServersQsortCompare( const void *arg1, const void *arg2 ) {
	return ui_trap->LAN_CompareServers( UI_SourceForLAN(), uiInfo.serverStatus.sortKey, uiInfo.serverStatus.sortDir, *(int*)arg1, *(int*)arg2);
}
...
static void UI_LoadMods() {
	...
	ui_trap->SE_GetStringTextString("MENUS_ALL", sAll, sizeof(sAll));
	...
	if ( strstr( version, "2003" ) ) {
		ui_trap->SE_GetStringTextString("MENUS_JEDI_ACADEMY", sJediAcademy, sizeof(sJediAcademy));
	...
	numdirs = ui_trap->FS_GetFileList( "$modlist", "", dirlist, sizeof(dirlist) );
	dirptr  = dirlist;
	for( i = 0; i < numdirs; i++ ) {
...
static void UI_LoadMovies() {
	...
	uiInfo.movieCount = ui_trap->FS_GetFileList( "video", "roq", movielist, 4096 );
...
static void UI_LoadDemosInDirectory( loadDemoContext_t *ctx, const char *directory )
{
	char *demoname = NULL;
	char demoExt[32] = {0};
	int protocol = ui_trap->Cvar_VariableValue( "com_protocol" );
	int protocolLegacy = ui_trap->Cvar_VariableValue( "com_legacyprotocol" );
	char *dirListEnd;
	int j;
	...
	if ( !protocol )
		protocol = ui_trap->Cvar_VariableValue( "protocol" );
	if ( protocolLegacy == protocol )
		protocolLegacy = 0;

	Com_sprintf( demoExt, sizeof( demoExt ), ".%s%d", DEMO_EXTENSION, protocol);

	uiInfo.demoCount += ui_trap->FS_GetFileList( directory, demoExt, ctx->demoList, sizeof( ctx->demoList ) );
	...
	if ( ctx->dirListHead < dirListEnd )
	{
		...
		int numFiles = ui_trap->FS_GetFileList( directory, "/", ctx->dirListHead, dirListSpaceRemaining );
		char *dirList;
...
static void UI_StartSkirmish(qboolean next) {
	...
	temp = ui_trap->Cvar_VariableValue( "g_gametype" );
	ui_trap->Cvar_Set("ui_gameType", va("%i", temp));

	if (next) {
		int actual;
		int index = ui_trap->Cvar_VariableValue("ui_mapIndex");
	...
	ui_trap->Cvar_SetValue( "g_gametype", g );
	ui_trap->Cmd_ExecuteText( EXEC_APPEND, va( "wait ; wait ; map %s\n", uiInfo.mapList[ui_currentMap.integer].mapLoadName) );
	skill = ui_trap->Cvar_VariableValue( "g_spSkill" );
	ui_trap->Cvar_Set("ui_scoreMap", uiInfo.mapList[ui_currentMap.integer].mapName);

	k = UI_TeamIndexFromName(UI_Cvar_VariableString("ui_opponentName"));

	ui_trap->Cvar_Set("ui_singlePlayerActive", "1");

	// set up sp overrides, will be replaced on postgame
	temp = ui_trap->Cvar_VariableValue( "capturelimit" );	ui_trap->Cvar_Set("ui_saveCaptureLimit", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "fraglimit" );		ui_trap->Cvar_Set("ui_saveFragLimit", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "duel_fraglimit" );	ui_trap->Cvar_Set("ui_saveDuelLimit", va("%i", temp));

	UI_SetCapFragLimits(qfalse);

	temp = ui_trap->Cvar_VariableValue( "cg_drawTimer" );	ui_trap->Cvar_Set("ui_drawTimer", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "g_doWarmup" );		ui_trap->Cvar_Set("ui_doWarmup", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "g_friendlyFire" );	ui_trap->Cvar_Set("ui_friendlyFire", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "sv_maxClients" );	ui_trap->Cvar_Set("ui_maxClients", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "g_warmup" );		ui_trap->Cvar_Set("ui_Warmup", va("%i", temp));
	temp = ui_trap->Cvar_VariableValue( "sv_pure" );			ui_trap->Cvar_Set("ui_pure", va("%i", temp));

	ui_trap->Cvar_Set("cg_cameraOrbit", "0");
//	ui_trap->Cvar_Set("cg_thirdPerson", "0");
	ui_trap->Cvar_Set("cg_drawTimer", "1");
	ui_trap->Cvar_Set("g_doWarmup", "1");
	ui_trap->Cvar_Set("g_warmup", "15");
	ui_trap->Cvar_Set("sv_pure", "0");
	ui_trap->Cvar_Set("g_friendlyFire", "0");
//	ui_trap->Cvar_Set("g_redTeam", UI_Cvar_VariableString("ui_teamName"));
//	ui_trap->Cvar_Set("g_blueTeam", UI_Cvar_VariableString("ui_opponentName"));

	if (ui_trap->Cvar_VariableValue("ui_recordSPDemo")) {
		Com_sprintf(buff, MAX_STRING_CHARS, "%s_%i", uiInfo.mapList[ui_currentMap.integer].mapLoadName, g);
		ui_trap->Cvar_Set("ui_recordSPDemoName", buff);
	}
	...
	if (g == GT_DUEL || g == GT_POWERDUEL) {
		...
		ui_trap->Cvar_Set("sv_maxClients", va("%d", temp));
		Com_sprintf( buff, sizeof(buff), "wait ; addbot %s %f "", %i \n", uiInfo.mapList[ui_currentMap.integer].opponentName, skill, delay);
		ui_trap->Cmd_ExecuteText( EXEC_APPEND, buff );
	...
	if (g >= GT_TEAM ) {
		ui_trap->Cmd_ExecuteText( EXEC_APPEND, "wait 5; team Red\n" );
	}
}

static void UI_Update(const char *name) {
	int	val = ui_trap->Cvar_VariableValue(name);

	if (Q_stricmp(name, "s_khz") == 0)
	{
		ui_trap->Cmd_ExecuteText( EXEC_APPEND, "snd_restart\n" );
		return;
	}
	...
	if ( !Q_stricmp( name, "ui_SetName" ) ) {
		...
		ui_trap->Cvar_Set( "name", buf );
	}
	else if (Q_stricmp(name, "ui_setRate") == 0) {
		float rate = ui_trap->Cvar_VariableValue("rate");
		if (rate >= 5000) {
			ui_trap->Cvar_Set("cl_maxpackets", "30");
			ui_trap->Cvar_Set("cl_packetdup", "1");
		} else if (rate >= 4000) {
			ui_trap->Cvar_Set("cl_maxpackets", "15");
			ui_trap->Cvar_Set("cl_packetdup", "2");		// favor less prediction errors when there's packet loss
		} else {
			ui_trap->Cvar_Set("cl_maxpackets", "15");
			ui_trap->Cvar_Set("cl_packetdup", "1");		// favor lower bandwidth
		}
	}
	else if ( !Q_stricmp( name, "ui_GetName" ) ) {
		char buf[MAX_NETNAME] = {0};
		Q_strncpyz( buf, UI_Cvar_VariableString( "name" ), sizeof( buf ) );
		ui_trap->Cvar_Set( "ui_Name", buf );
	}
	else if (Q_stricmp(name, "ui_r_colorbits") == 0)
	{
		...
		{
			case 0:
				ui_trap->Cvar_SetValue( "ui_r_depthbits", 0 );
				break;

			case 16:
				ui_trap->Cvar_SetValue( "ui_r_depthbits", 16 );
				break;

			case 32:
				ui_trap->Cvar_SetValue( "ui_r_depthbits", 24 );
				break;
		}
	}
	else if (Q_stricmp(name, "ui_r_lodbias") == 0)
	{
		...
		{
			case 0:
				ui_trap->Cvar_SetValue( "ui_r_subdivisions", 4 );
				break;
			case 1:
				ui_trap->Cvar_SetValue( "ui_r_subdivisions", 12 );
				break;

			case 2:
				ui_trap->Cvar_SetValue( "ui_r_subdivisions", 20 );
				break;
		}
	}
	else if (Q_stricmp(name, "ui_r_glCustom") == 0)
	{
		...
		case 0:	// high quality
			...
			ui_trap->Cvar_SetValue( "ui_r_fullScreen", 1 );
			ui_trap->Cvar_SetValue( "ui_r_subdivisions", 4 );
			ui_trap->Cvar_SetValue( "ui_r_lodbias", 0 );
			ui_trap->Cvar_SetValue( "ui_r_colorbits", 32 );
			ui_trap->Cvar_SetValue( "ui_r_depthbits", 24 );
			ui_trap->Cvar_SetValue( "ui_r_picmip", 0 );
			ui_trap->Cvar_SetValue( "ui_r_mode", 4 );
			ui_trap->Cvar_SetValue( "ui_r_texturebits", 32 );
			ui_trap->Cvar_SetValue( "ui_r_fastSky", 0 );
			ui_trap->Cvar_SetValue( "ui_r_inGameVideo", 1 );
		//	ui_trap->Cvar_SetValue( "ui_cg_shadows", 2 );//stencil
			ui_trap->Cvar_Set( "ui_r_texturemode", "GL_LINEAR_MIPMAP_LINEAR" );
			break;
		...
		case 1: // normal
			...
			ui_trap->Cvar_SetValue( "ui_r_subdivisions", 4 );
			ui_trap->Cvar_SetValue( "ui_r_lodbias", 0 );
			ui_trap->Cvar_SetValue( "ui_r_colorbits", 0 );
			ui_trap->Cvar_SetValue( "ui_r_depthbits", 24 );
			ui_trap->Cvar_SetValue( "ui_r_picmip", 1 );
			ui_trap->Cvar_SetValue( "ui_r_mode", 3 );
			ui_trap->Cvar_SetValue( "ui_r_texturebits", 0 );
			ui_trap->Cvar_SetValue( "ui_r_fastSky", 0 );
			ui_trap->Cvar_SetValue( "ui_r_inGameVideo", 1 );
		//	ui_trap->Cvar_SetValue( "ui_cg_shadows", 2 );
			ui_trap->Cvar_Set( "ui_r_texturemode", "GL_LINEAR_MIPMAP_LINEAR" );
			break;
		...
		case 2: // fast
			...
			ui_trap->Cvar_SetValue( "ui_r_fullScreen", 1 );
			ui_trap->Cvar_SetValue( "ui_r_subdivisions", 12 );
			ui_trap->Cvar_SetValue( "ui_r_lodbias", 1 );
			ui_trap->Cvar_SetValue( "ui_r_colorbits", 0 );
			ui_trap->Cvar_SetValue( "ui_r_depthbits", 0 );
			ui_trap->Cvar_SetValue( "ui_r_picmip", 2 );
			ui_trap->Cvar_SetValue( "ui_r_mode", 3 );
			ui_trap->Cvar_SetValue( "ui_r_texturebits", 0 );
			ui_trap->Cvar_SetValue( "ui_r_fastSky", 1 );
			ui_trap->Cvar_SetValue( "ui_r_inGameVideo", 0 );
		//	ui_trap->Cvar_SetValue( "ui_cg_shadows", 1 );
			ui_trap->Cvar_Set( "ui_r_texturemode", "GL_LINEAR_MIPMAP_NEAREST" );
			break;
		...
		case 3: // fastest
			...
			ui_trap->Cvar_SetValue( "ui_r_fullScreen", 1 );
			ui_trap->Cvar_SetValue( "ui_r_subdivisions", 20 );
			ui_trap->Cvar_SetValue( "ui_r_lodbias", 2 );
			ui_trap->Cvar_SetValue( "ui_r_colorbits", 16 );
			ui_trap->Cvar_SetValue( "ui_r_depthbits", 16 );
			ui_trap->Cvar_SetValue( "ui_r_mode", 3 );
			ui_trap->Cvar_SetValue( "ui_r_picmip", 3 );
			ui_trap->Cvar_SetValue( "ui_r_texturebits", 16 );
			ui_trap->Cvar_SetValue( "ui_r_fastSky", 1 );
			ui_trap->Cvar_SetValue( "ui_r_inGameVideo", 0 );
		//	ui_trap->Cvar_SetValue( "ui_cg_shadows", 0 );
			ui_trap->Cvar_Set( "ui_r_texturemode", "GL_LINEAR_MIPMAP_NEAREST" );
			break;
		}
	}
	...
	{
		if (val == 0)	ui_trap->Cvar_SetValue( "m_pitch", 0.022f );
		else			ui_trap->Cvar_SetValue( "m_pitch", -0.022f );
	}
	else if (Q_stricmp(name, "ui_mousePitchVeh") == 0)
	{
		if (val == 0)	ui_trap->Cvar_SetValue( "m_pitchVeh", 0.022f );
		else 			ui_trap->Cvar_SetValue( "m_pitchVeh", -0.022f );
	}
}
...
static qboolean UI_DeferMenuScript ( char **args )
{
	...
	if ( !Q_stricmp ( name, "VideoSetup" ) )
	{
		...
		deferred = ui_trap->Cvar_VariableValue ( "ui_r_modified" ) ? qtrue : qfalse;
	...
	else if ( !Q_stricmp ( name, "RulesBackout" ) )
	{
		...
		deferred = ui_trap->Cvar_VariableValue ( "ui_rules_backout" ) ? qtrue : qfalse ;

		ui_trap->Cvar_Set ( "ui_rules_backout", "0" );
...
{
	ui_trap->Cvar_Set ( "r_mode", UI_Cvar_VariableString ( "ui_r_mode" ) );
	ui_trap->Cvar_Set ( "r_fullscreen", UI_Cvar_VariableString ( "ui_r_fullscreen" ) );
	ui_trap->Cvar_Set ( "r_colorbits", UI_Cvar_VariableString ( "ui_r_colorbits" ) );
	ui_trap->Cvar_Set ( "r_lodbias", UI_Cvar_VariableString ( "ui_r_lodbias" ) );
	ui_trap->Cvar_Set ( "r_picmip", UI_Cvar_VariableString ( "ui_r_picmip" ) );
	ui_trap->Cvar_Set ( "r_texturebits", UI_Cvar_VariableString ( "ui_r_texturebits" ) );
	ui_trap->Cvar_Set ( "r_texturemode", UI_Cvar_VariableString ( "ui_r_texturemode" ) );
	ui_trap->Cvar_Set ( "r_detailtextures", UI_Cvar_VariableString ( "ui_r_detailtextures" ) );
	ui_trap->Cvar_Set ( "r_ext_compress_textures", UI_Cvar_VariableString ( "ui_r_ext_compress_textures" ) );
	ui_trap->Cvar_Set ( "r_depthbits", UI_Cvar_VariableString ( "ui_r_depthbits" ) );
	ui_trap->Cvar_Set ( "r_subdivisions", UI_Cvar_VariableString ( "ui_r_subdivisions" ) );
	ui_trap->Cvar_Set ( "r_fastSky", UI_Cvar_VariableString ( "ui_r_fastSky" ) );
	ui_trap->Cvar_Set ( "r_inGameVideo", UI_Cvar_VariableString ( "ui_r_inGameVideo" ) );
	ui_trap->Cvar_Set ( "r_allowExtensions", UI_Cvar_VariableString ( "ui_r_allowExtensions" ) );
	ui_trap->Cvar_Set ( "cg_shadows", UI_Cvar_VariableString ( "ui_cg_shadows" ) );
	ui_trap->Cvar_Set ( "ui_r_modified", "0" );

	ui_trap->Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
}
...
{
	ui_trap->Cvar_Register ( NULL, "ui_r_glCustom",				"4", CVAR_INTERNAL|CVAR_ARCHIVE );

	// Make sure the cvars are registered as read only.
	ui_trap->Cvar_Register ( NULL, "ui_r_mode",					"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_fullscreen",			"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_colorbits",				"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_lodbias",				"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_picmip",				"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_texturebits",			"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_texturemode",			"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_detailtextures",		"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_ext_compress_textures",	"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_depthbits",				"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_subdivisions",			"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_fastSky",				"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_inGameVideo",			"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_allowExtensions",		"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_cg_shadows",				"0", CVAR_ROM|CVAR_INTERNAL );
	ui_trap->Cvar_Register ( NULL, "ui_r_modified",				"0", CVAR_ROM|CVAR_INTERNAL );

	// Copy over the real video cvars into their temporary counterparts
	ui_trap->Cvar_Set ( "ui_r_mode",						UI_Cvar_VariableString ( "r_mode" ) );
	ui_trap->Cvar_Set ( "ui_r_colorbits",				UI_Cvar_VariableString ( "r_colorbits" ) );
	ui_trap->Cvar_Set ( "ui_r_fullscreen",				UI_Cvar_VariableString ( "r_fullscreen" ) );
	ui_trap->Cvar_Set ( "ui_r_lodbias",					UI_Cvar_VariableString ( "r_lodbias" ) );
	ui_trap->Cvar_Set ( "ui_r_picmip",					UI_Cvar_VariableString ( "r_picmip" ) );
	ui_trap->Cvar_Set ( "ui_r_texturebits",				UI_Cvar_VariableString ( "r_texturebits" ) );
	ui_trap->Cvar_Set ( "ui_r_texturemode",				UI_Cvar_VariableString ( "r_texturemode" ) );
	ui_trap->Cvar_Set ( "ui_r_detailtextures",			UI_Cvar_VariableString ( "r_detailtextures" ) );
	ui_trap->Cvar_Set ( "ui_r_ext_compress_textures",	UI_Cvar_VariableString ( "r_ext_compress_textures" ) );
	ui_trap->Cvar_Set ( "ui_r_depthbits",				UI_Cvar_VariableString ( "r_depthbits" ) );
	ui_trap->Cvar_Set ( "ui_r_subdivisions",				UI_Cvar_VariableString ( "r_subdivisions" ) );
	ui_trap->Cvar_Set ( "ui_r_fastSky",					UI_Cvar_VariableString ( "r_fastSky" ) );
	ui_trap->Cvar_Set ( "ui_r_inGameVideo",				UI_Cvar_VariableString ( "r_inGameVideo" ) );
	ui_trap->Cvar_Set ( "ui_r_allowExtensions",			UI_Cvar_VariableString ( "r_allowExtensions" ) );
	ui_trap->Cvar_Set ( "ui_cg_shadows",					UI_Cvar_VariableString ( "cg_shadows" ) );
	ui_trap->Cvar_Set ( "ui_r_modified",					"0" );
}
...
{
	int gameType = ui_trap->Cvar_VariableValue( "g_gametype" );
	int server;
	menuDef_t *menu;
	itemDef_t *item;
	char *name = "addBot";

	server = ui_trap->Cvar_VariableValue( "sv_running" );
...
static void UI_UpdateCharacterCvars ( void )
{
	...
	ui_trap->Cvar_VariableStringBuffer("ui_char_model", model, sizeof(model));
	ui_trap->Cvar_VariableStringBuffer("ui_char_skin_head", head, sizeof(head));
	ui_trap->Cvar_VariableStringBuffer("ui_char_skin_torso", torso, sizeof(torso));
	ui_trap->Cvar_VariableStringBuffer("ui_char_skin_legs", legs, sizeof(legs));
	...
	ui_trap->Cvar_Set ( "model", skin );

	ui_trap->Cvar_Set ( "char_color_red", UI_Cvar_VariableString ( "ui_char_color_red" ) );
	ui_trap->Cvar_Set ( "char_color_green", UI_Cvar_VariableString ( "ui_char_color_green" ) );
	ui_trap->Cvar_Set ( "char_color_blue", UI_Cvar_VariableString ( "ui_char_color_blue" ) );
	ui_trap->Cvar_Set ( "ui_selectedModelIndex", "-1");
...
static void UI_GetCharacterCvars ( void )
{
	...
	ui_trap->Cvar_Set ( "ui_char_color_red", UI_Cvar_VariableString ( "char_color_red" ) );
	ui_trap->Cvar_Set ( "ui_char_color_green", UI_Cvar_VariableString ( "char_color_green" ) );
	ui_trap->Cvar_Set ( "ui_char_color_blue", UI_Cvar_VariableString ( "char_color_blue" ) );
	...
	if (skin && strchr(model,'|'))	//we have a multipart custom jedi
	{
		...
		*skin=0;
		...
		ui_trap->Cvar_Set("ui_char_model", model);
		ui_trap->Cvar_Set("ui_char_skin_head", skinhead);
		ui_trap->Cvar_Set("ui_char_skin_torso", skintorso);
		ui_trap->Cvar_Set("ui_char_skin_legs", skinlower);
	...
	else
	{
		...
		ui_trap->Cvar_Set("ui_char_model", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name);
		ui_trap->Cvar_Set("ui_char_skin_head", "head_a1");
		ui_trap->Cvar_Set("ui_char_skin_torso","torso_a1");
		ui_trap->Cvar_Set("ui_char_skin_legs", "lower_a1");
	}
}
...
void UI_SetSiegeObjectiveGraphicPos(menuDef_t *menu,const char *itemName,const char *cvarName)
{
	...
	{
		// get cvar data
		ui_trap->Cvar_VariableStringBuffer(cvarName, cvarBuf, sizeof(cvarBuf));
...
{
	menuDef_t *menu;
	int myTeam = (int)(ui_trap->Cvar_VariableValue("ui_myteam"));
	char *itemname;
	itemDef_t *item;
	int	baseClass;
	...
	baseClass = (int)ui_trap->Cvar_VariableValue("ui_siege_class");
...
static void UI_UpdateSaberCvars ( void )
{
	...
	ui_trap->Cvar_Set ( "saber1", UI_Cvar_VariableString ( "ui_saber" ) );
	ui_trap->Cvar_Set ( "saber2", UI_Cvar_VariableString ( "ui_saber2" ) );

	colorI = TranslateSaberColor( UI_Cvar_VariableString ( "ui_saber_color" ) );
	ui_trap->Cvar_Set ( "color1", va("%d",colorI));
	ui_trap->Cvar_Set ( "g_saber_color", UI_Cvar_VariableString ( "ui_saber_color" ));

	colorI = TranslateSaberColor( UI_Cvar_VariableString ( "ui_saber2_color" ) );
	ui_trap->Cvar_Set ( "color2", va("%d",colorI) );
	ui_trap->Cvar_Set ( "g_saber2_color", UI_Cvar_VariableString ( "ui_saber2_color" ));
}
...
static void UI_SetSaberBoxesandHilts (void)
{
	...
	ui_trap->Cvar_VariableStringBuffer( "ui_saber_type", sType, sizeof(sType) );

	if ( Q_stricmp( "dual", sType ) != 0 )
	{
//		ui_trap->Cvar_Set("ui_saber", "single_1");
//		ui_trap->Cvar_Set("ui_saber2", "single_1");
		getBig = qtrue;
	}

	else if (Q_stricmp( "staff", sType ) != 0 )
	{
//		ui_trap->Cvar_Set("ui_saber", "dual_1");
//		ui_trap->Cvar_Set("ui_saber2", "none");
		getBig = qtrue;
	}
...
static void UI_UpdateSaberHilt( qboolean secondSaber )
{
	...
	ui_trap->Cvar_VariableStringBuffer( saberCvarName, model, sizeof(model) );

	item->text = model;
	//read this from the sabers.cfg
	if ( UI_SaberModelForSaber( model, modelPath ) )
	{//successfully found a model
		ItemParse_asset_model_go< moduleType::ui >( item, modelPath, &animRunLength );//set the model
...
{
//	ui_trap->Cvar_Set ( "ui_saber_type", UI_Cvar_VariableString ( "g_saber_type" ) );
	ui_trap->Cvar_Set ( "ui_saber", UI_Cvar_VariableString ( "saber1" ) );
	ui_trap->Cvar_Set ( "ui_saber2", UI_Cvar_VariableString ( "saber2" ));

	ui_trap->Cvar_Set("g_saber_color", SaberColorToString(static_cast<saber_colors_t>(static_cast< int >( ui_trap->Cvar_VariableValue("color1")))));
	ui_trap->Cvar_Set("g_saber2_color", SaberColorToString(static_cast<saber_colors_t>(static_cast< int >( ui_trap->Cvar_VariableValue("color2")))));

	ui_trap->Cvar_Set ( "ui_saber_color", UI_Cvar_VariableString ( "g_saber_color" ) );
	ui_trap->Cvar_Set ( "ui_saber2_color", UI_Cvar_VariableString ( "g_saber2_color" ) );
}
...
static void UI_UpdateCharacter( qboolean changedModel )
{
	...
	ItemParse_asset_model_go< moduleType::ui >( item, modelPath, &animRunLength );
...
static void UI_CheckServerName( void )
{
	...
	ui_trap->Cvar_VariableStringBuffer( "sv_hostname", hostname, sizeof( hostname ) );
	...
	if( changed )
	{
		ui_trap->Cvar_Set("sv_hostname", hostname );
	}
...
static qboolean UI_CheckPassword( void )
{
	...
	ui_trap->LAN_GetServerInfo(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS);
...
//	ui_trap->Cvar_Set("cg_thirdPerson", "0");
	ui_trap->Cvar_Set("cg_cameraOrbit", "0");
	ui_trap->Cvar_Set("ui_singlePlayerActive", "0");
	if (uiInfo.serverStatus.currentServer >= 0 && uiInfo.serverStatus.currentServer < uiInfo.serverStatus.numDisplayServers)
	{
		ui_trap->LAN_GetServerAddressString(UI_SourceForLAN()/*ui_netSource.integer*/, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, sizeof( buff ) );
		ui_trap->Cmd_ExecuteText( EXEC_APPEND, va( "connect %s\n", buff ) );
	}
...
void UI_SiegeSetCvarsForClass( siegeClass_t *scl ) {
	...
	while (i < WP_NUM_WEAPONS)
	{
		ui_trap->Cvar_Set(va("ui_class_weapon%i", i), "gfx/2d/select");
		i++;
	}
	...
	{
		ui_trap->Cvar_Set(va("ui_class_item%i", i), "gfx/2d/select");
		i++;
	}
	...
	while (i < NUM_FORCE_POWERS)
	{
		ui_trap->Cvar_Set(va("ui_class_power%i", i), "gfx/2d/select");
		i++;
	}

	//now health and armor
	ui_trap->Cvar_Set("ui_class_health", "0");
	ui_trap->Cvar_Set("ui_class_armor", "0");

	ui_trap->Cvar_Set("ui_class_icon", "");
	...
	i = 0;
	ui_trap->Cvar_Set(va("ui_class_weapondesc%i", count), " ");	// Blank it out to start with
	while (i < WP_NUM_WEAPONS)
	{
		...
		if (scl->weapons & (1<<i))
		{
			...
			{ //we want to see what kind of saber they have, and set the cvar based on that
				...
				ui_trap->Cvar_Set(va("ui_class_weapon%i", count), saberType);
				ui_trap->Cvar_Set(va("ui_class_weapondesc%i", count), "@MENUS_AN_ELEGANT_WEAPON_FOR");
				count++;
				ui_trap->Cvar_Set(va("ui_class_weapondesc%i", count), " ");	// Blank it out to start with
			}
			else
			{
				gitem_t *item = BG_FindItemForWeapon( static_cast< weapon_t >( i ) );
				ui_trap->Cvar_Set(va("ui_class_weapon%i", count), item->icon);
				ui_trap->Cvar_Set(va("ui_class_weapondesc%i", count), item->description);
				count++;
				ui_trap->Cvar_Set(va("ui_class_weapondesc%i", count), " ");	// Blank it out to start with
			}
		}
	...
	while (i < HI_NUM_HOLDABLE)
	{
		...
		{
			gitem_t *item = BG_FindItemForHoldable( static_cast< holdable_t >( i ) );
			ui_trap->Cvar_Set(va("ui_class_item%i", count), item->icon);
			ui_trap->Cvar_Set(va("ui_class_itemdesc%i", count), item->description);
			count++;
		}
		else
		{
			ui_trap->Cvar_Set(va("ui_class_itemdesc%i", count), " ");
		}
		i++;
	}
	...
	while (i < NUM_FORCE_POWERS)
	{
		ui_trap->Cvar_Set(va("ui_class_powerlevel%i", i), "0");	// Zero this out to start.
		if (i<9)
		{
			ui_trap->Cvar_Set(va("ui_class_powerlevelslot%i", i), "0");	// Zero this out to start.
		}

		if (scl->forcePowerLevels[i])
		{
			ui_trap->Cvar_Set(va("ui_class_powerlevel%i", count), va("%i",scl->forcePowerLevels[i]));
			ui_trap->Cvar_Set(va("ui_class_power%i", count), HolocronIcons[i]);
			count++;
		}
	...
	//now health and armor
	ui_trap->Cvar_Set("ui_class_health", va("%i", scl->maxhealth));
	ui_trap->Cvar_Set("ui_class_armor", va("%i", scl->maxarmor));
	ui_trap->Cvar_Set("ui_class_speed", va("%3.2f", scl->speed));

	//now get the icon path based on the shader index
	if (scl->classShader)
	{
		ui_trap->R_ShaderNameFromIndex(shader, scl->classShader);
	}
	else
	{ //no shader
		shader[0] = 0;
	}
	ui_trap->Cvar_Set("ui_class_icon", shader);
}
			...
			if ((index>=0) && (index < BG_SiegeCountBaseClass( team, baseClass )))
			{
				if (!g_siegedFeederForcedSet)
				{
					...
					if (holdClass)	//clicked a valid item
					{
						...
						ui_trap->Cvar_Set("ui_classDesc", g_UIClassDescriptions[g_UIGloballySelectedSiegeClass].desc);
						...
						if (holdBuf)
						{
							ui_trap->Cvar_Set("ui_classPortrait",holdBuf);
						}
					}
				}
...
void UI_ClampMaxPlayers( void ) {
	...
	if ( uiInfo.gameTypes[ui_netGametype.integer].gtEnum == GT_DUEL ) {
		if ( (int)ui_trap->Cvar_VariableValue( "sv_maxClients" ) < 2 )
			ui_trap->Cvar_Set( "sv_maxClients", "2" );
	}

	// power duel requires 3 players
	else if ( uiInfo.gameTypes[ui_netGametype.integer].gtEnum == GT_POWERDUEL ) {
		if ( (int)ui_trap->Cvar_VariableValue( "sv_maxClients" ) < 3 )
			ui_trap->Cvar_Set( "sv_maxClients", "3" );
	}

	// can never exceed MAX_CLIENTS
	if ( (int)ui_trap->Cvar_VariableValue( "sv_maxClients" ) > MAX_CLIENTS ) {
		ui_trap->Cvar_Set( "sv_maxClients", XSTRING(MAX_CLIENTS) );
	}
}
...
static void UI_RunMenuScript(char **args)
{
	...
	if (String_Parse(args, &name))
	{
		if (Q_stricmp(name, "StartServer") == 0)
		{
			...
			if( ui_trap->Cvar_VariableValue( "ui_singlePlayerActive" ) )
			{
				ui_trap->Cvar_Set( "dedicated", "0" );
			}
			else
			{
				ui_trap->Cvar_SetValue( "dedicated", Com_Clamp( 0, 2, ui_dedicated.integer ) );
			}
			ui_trap->Cvar_SetValue( "g_gametype", Com_Clamp( 0, GT_MAX_GAME_TYPE, uiInfo.gameTypes[ui_netGametype.integer].gtEnum ) );
			//ui_trap->Cvar_Set("g_redTeam", UI_Cvar_VariableString("ui_teamName"));
			//ui_trap->Cvar_Set("g_blueTeam", UI_Cvar_VariableString("ui_opponentName"));
			...
			warmupTime = ui_trap->Cvar_VariableValue( "g_warmup" );
			doWarmup = ui_trap->Cvar_VariableValue( "g_doWarmup" );

			if (doWarmup && warmupTime < 1)
			{
				ui_trap->Cvar_Set("g_doWarmup", "0");
			}
			if (warmupTime < 5)
			{
				ui_trap->Cvar_Set("g_warmup", "5");
			}
			if (warmupTime > 120)
			{
				ui_trap->Cvar_Set("g_warmup", "120");
			}

			if (ui_trap->Cvar_VariableValue( "g_gametype" ) == GT_DUEL ||
				ui_trap->Cvar_VariableValue( "g_gametype" ) == GT_POWERDUEL)
			{ //always set fraglimit 1 when starting a duel game
				ui_trap->Cvar_Set("fraglimit", "1");
				ui_trap->Cvar_Set("timelimit", "0");
			}

			for (i = 0; i < PLAYERS_PER_TEAM; i++)
			{
				int bot = ui_trap->Cvar_VariableValue( va("ui_blueteam%i", i+1));
				int maxcl = ui_trap->Cvar_VariableValue( "sv_maxClients" );
				...
				bot = ui_trap->Cvar_VariableValue( va("ui_redteam%i", i+1));
				if (bot > 1) {
					int numval = i+1;
				...
				{ //this means the client filled up all their slots in the UI with bots. So stretch out an extra slot for them, and then stop adding bots.
					ui_trap->Cvar_Set("sv_maxClients", va("%i", added+1));
					break;
				}
			}
			...
			UI_SetCapFragLimits(qtrue);
			UI_MapCountByGameType(qtrue);
			ui_trap->Cvar_SetValue("ui_mapIndex", UI_GetIndexFromSelection(ui_currentMap.integer));
			ui_trap->Cvar_Update(&ui_mapIndex);
			Menu_SetFeederSelection(NULL, FEEDER_MAPS, ui_mapIndex.integer, "skirmish");
			UI_GameType_HandleKey(0, 0, A_MOUSE1, qfalse);
			UI_GameType_HandleKey(0, 0, A_MOUSE2, qfalse);
		} else if (Q_stricmp(name, "resetDefaults") == 0) {
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, "cvar_restart\n");
			Controls_SetDefaults();
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, "exec mpdefault.cfg\n");
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, "vid_restart\n" );
			...
			uiForceRank = ui_trap->Cvar_VariableValue("g_maxForceRank");
			...
			if (uiInfo.previewMovie >= 0) {
				ui_trap->CIN_StopCinematic(uiInfo.previewMovie);
			}
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("cinematic %s.roq 2\n", uiInfo.movieList[uiInfo.movieIndex]));
		} else if (Q_stricmp(name, "RunMod") == 0) {
			ui_trap->Cvar_Set( "fs_game", uiInfo.modList[uiInfo.modIndex].modName);
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
		} else if (Q_stricmp(name, "RunDemo") == 0) {
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("demo \"%s\"\n", uiInfo.demoList[uiInfo.demoIndex]));
		} else if (Q_stricmp(name, "Quake3") == 0) {
			ui_trap->Cvar_Set( "fs_game", "");
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, "vid_restart;" );
			...
			ui_trap->LAN_GetServerAddressString(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], uiInfo.serverStatusAddress, sizeof(uiInfo.serverStatusAddress));
		...
		else if (Q_stricmp(name, "JoinServer") == 0)
		{
			...
			ui_trap->Cvar_Set("ui_singlePlayerActive", "0");
			ui_trap->Cmd_ExecuteText( EXEC_NOW, "quit");
		...
		{
			ui_trap->Cmd_ExecuteText( EXEC_APPEND, "disconnect\n" );
			ui_trap->Key_SetCatcher( KEYCATCH_UI );
			Menus_CloseAll();
			Menus_ActivateByName("main");
		}
		...
		else if (Q_stricmp(name, "ServerSort") == 0)
		{
			...
			if (ui_currentNetMap.integer >=0 && ui_currentNetMap.integer < uiInfo.mapCount) {
				ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("callvote map %s\n",uiInfo.mapList[ui_currentNetMap.integer].mapLoadName) );
			}
		} else if (Q_stricmp(name, "voteKick") == 0) {
			if (uiInfo.playerIndex >= 0 && uiInfo.playerIndex < uiInfo.playerCount) {
				//ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("callvote kick \"%s\"\n",uiInfo.playerNames[uiInfo.playerIndex]) );
				ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("callvote clientkick \"%i\"\n",uiInfo.playerIndexes[uiInfo.playerIndex]) );
			}
		} else if (Q_stricmp(name, "voteGame") == 0) {
			if (ui_netGametype.integer >= 0 && ui_netGametype.integer < uiInfo.numGameTypes) {
				ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("callvote g_gametype %i\n",uiInfo.gameTypes[ui_netGametype.integer].gtEnum) );
			}
		} else if (Q_stricmp(name, "voteLeader") == 0) {
			if (uiInfo.teamIndex >= 0 && uiInfo.teamIndex < uiInfo.myTeamCount) {
				ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("callteamvote leader \"%s\"\n",uiInfo.teamNames[uiInfo.teamIndex]) );
			}
		} else if (Q_stricmp(name, "addBot") == 0) {
			if (ui_trap->Cvar_VariableValue("g_gametype") >= GT_TEAM) {
				ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("addbot \"%s\" %i %s\n", UI_GetBotNameByNumber(uiInfo.botIndex), uiInfo.skillIndex+1, (uiInfo.redBlue == 0) ? "Red" : "Blue") );
			} else {
				ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("addbot \"%s\" %i %s\n", UI_GetBotNameByNumber(uiInfo.botIndex), uiInfo.skillIndex+1, (uiInfo.redBlue == 0) ? "Red" : "Blue") );
			}
		} else if (Q_stricmp(name, "addFavorite") == 0)
		{
			if (ui_netSource.integer != UIAS_FAVORITES)
			{
				...
				ui_trap->LAN_GetServerInfo(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, MAX_STRING_CHARS);
				name[0] = addr[0] = '\0';
				Q_strncpyz(name, 	Info_ValueForKey(buff, "hostname"), sizeof( name ) );
				Q_strncpyz(addr, 	Info_ValueForKey(buff, "addr"), sizeof( addr ) );
				if (strlen(name) > 0 && strlen(addr) > 0)
				{
					res = ui_trap->LAN_AddServer(AS_FAVORITES, name, addr);
					if (res == 0)
					{
		...
		else if (Q_stricmp(name, "deleteFavorite") == 0)
		{
			...
			{
				char addr[MAX_ADDRESSLENGTH] = {0};
				ui_trap->LAN_GetServerInfo(AS_FAVORITES, uiInfo.serverStatus.displayServers[uiInfo.serverStatus.currentServer], buff, MAX_STRING_CHARS);
				addr[0] = '\0';
				Q_strncpyz(addr, 	Info_ValueForKey(buff, "addr"), sizeof( addr ) );
				if (strlen(addr) > 0)
				{
					ui_trap->LAN_RemoveServer(AS_FAVORITES, addr);
				}
			}
		}
		else if (Q_stricmp(name, "createFavorite") == 0)
		{
			...
			if (String_Parse(args, &orders)) {
				int selectedPlayer = ui_trap->Cvar_VariableValue("cg_selectedPlayer");
				if (selectedPlayer < uiInfo.myTeamCount) {
					Q_strncpyz( buff, orders, sizeof( buff ) );
					ui_trap->Cmd_ExecuteText( EXEC_APPEND, va(buff, uiInfo.teamClientNums[selectedPlayer]) );
					ui_trap->Cmd_ExecuteText( EXEC_APPEND, "\n" );
				} else {
					int i;
			...
			if (String_Parse(args, &orders)) {
				int selectedPlayer = ui_trap->Cvar_VariableValue("cg_selectedPlayer");
				if (selectedPlayer == uiInfo.myTeamCount) {
					ui_trap->Cmd_ExecuteText( EXEC_APPEND, orders );
					ui_trap->Cmd_ExecuteText( EXEC_APPEND, "\n" );
				}
				ui_trap->Key_SetCatcher( ui_trap->Key_GetCatcher() & ~KEYCATCH_UI );
				ui_trap->Key_ClearStates();
			...
			if (String_Parse(args, &orders)) {
				...
				if (selectedPlayer == uiInfo.myTeamCount)
				{
					selectedPlayer = -1;
					Q_strncpyz( buff, orders, sizeof( buff ) );
					ui_trap->Cmd_ExecuteText( EXEC_APPEND, va(buff, selectedPlayer) );
				}
				else
				{
					Q_strncpyz( buff, orders, sizeof( buff ) );
		...
		{
			int team = (int)ui_trap->Cvar_VariableValue("ui_holdteam");
			int oldteam = (int)ui_trap->Cvar_VariableValue("ui_startsiegeteam");
			qboolean	goTeam = qtrue;
			char	newclassString[512];
			char	startclassString[512];

			ui_trap->Cvar_VariableStringBuffer( "ui_mySiegeClass", newclassString, sizeof(newclassString) );
			ui_trap->Cvar_VariableStringBuffer( "ui_startsiegeclass", startclassString, sizeof(startclassString) );
			...
			if (goTeam)
			{
				if (team == 1)	// Team red
				{
					ui_trap->Cvar_Set("ui_team", va("%d", team));
				}
				...
				if (g_UIGloballySelectedSiegeClass != -1)
				{
					ui_trap->Cmd_ExecuteText( EXEC_APPEND, va("siegeclass \"%s\"\n", bgSiegeClasses[g_UIGloballySelectedSiegeClass].name) );
				}
			}
		}
		else if (Q_stricmp(name, "setBotButton") == 0)
		{
			...
			ui_trap->Cvar_Set("ui_r_glCustom", "4");
		}
		...
		{
			int forcePowerDisable = ui_trap->Cvar_VariableValue("g_forcePowerDisable");
			...
			{
				ui_trap->Cvar_Set("ui_dualforcepower", "0");
			}
			else if (forcePowerDisable==forceBitFlag)
			{
				ui_trap->Cvar_Set("ui_dualforcepower", "2");
			}
			else
			{
				ui_trap->Cvar_Set("ui_dualforcepower", "1");
			}
		}
		else if (Q_stricmp(name, "dualForcePowers") == 0)
		{
			int	dualforcePower,i, forcePowerDisable;
			dualforcePower = ui_trap->Cvar_VariableValue("ui_dualforcepower");
			...
			ui_trap->Cvar_Set("g_forcePowerDisable", va("%i",forcePowerDisable));
		}
		else if (Q_stricmp(name, "forcePowersDisable") == 0)
		{
			int	forcePowerDisable,i;

			forcePowerDisable = ui_trap->Cvar_VariableValue("g_forcePowerDisable");
		...
		else if (Q_stricmp(name, "weaponDisable") == 0)
		{
			...
			weaponDisable = ui_trap->Cvar_VariableValue(cvarString);
			...
			if (weaponDisable)
			{
				...
				ui_trap->Cvar_Set(cvarString, va("%i",weaponDisable));
			}
		}
#endif
		...
		else if (Q_stricmp(name, "setSiegeNoBots") == 0)
		{
			...
			if (uiInfo.gameTypes[ui_netGametype.integer].gtEnum == GT_SIEGE)
			{
				...
				ui_trap->Cvar_Set("bot_minplayers", "0");

				for (i=1;i<9;i++)
				{
					blueValue = ui_trap->Cvar_VariableValue(va("ui_blueteam%i",i ));
					if (blueValue>1)
					{
						ui_trap->Cvar_Set(va("ui_blueteam%i",i ), "1");
					}

					redValue = ui_trap->Cvar_VariableValue(va("ui_redteam%i",i ));
					if (redValue>1)
					{
						ui_trap->Cvar_Set(va("ui_redteam%i",i ), "1");
					}
		...
		else if (Q_stricmp(name, "setscreensaberhilt") == 0)
		{
			...
			if (menu)
			{
				...
				if (item)
				{
					...
					{
						ui_trap->Cvar_Set( "ui_saber", saberSingleHiltInfo[item->cursorPos] );
					}
				}
			}
		}
		...
		else if (Q_stricmp(name, "setscreensaberhilt2") == 0)
		{
			...
			if (menu)
			{
				...
				if (item)
				{
					if (saberSingleHiltInfo[item->cursorPos])
					{
						ui_trap->Cvar_Set( "ui_saber2", saberSingleHiltInfo[item->cursorPos] );
					}
				}
			}
		}
		else if (Q_stricmp(name, "setscreensaberstaff") == 0)
		{
			...
			if (menu)
			{
				...
				if (item)
				{
					if (saberSingleHiltInfo[item->cursorPos])
					{
						ui_trap->Cvar_Set( "ui_saber", saberStaffHiltInfo[item->cursorPos] );
					}
				}
			}
		}
		...
		{
			int team = (int)ui_trap->Cvar_VariableValue("ui_team");
			ui_trap->Cvar_Set("ui_holdteam", va("%d", team));
		...
		else if (Q_stricmp(name, "setsiegeobjbuttons") == 0)
		{
			...
			if (menu)
			{
				...
				if (String_Parse(args, &itemArg))
				{
					...
					ui_trap->Cvar_VariableStringBuffer( "currentObjMapIconItem", string, sizeof(string) );
					item = (itemDef_t *) Menu_FindItemByName((menuDef_t *) menu, string);
					if (item)
					{
						// A cvar holding the name of a cvar - how crazy is that?
						ui_trap->Cvar_VariableStringBuffer( "currentObjMapIconBackground", string, sizeof(string) );
						ui_trap->Cvar_VariableStringBuffer( string, string2, sizeof(string2) );
					...
					if (item)
					{	// store item name
						ui_trap->Cvar_Set("currentObjMapIconItem",	 item->window.name);
						if (String_Parse(args, &cvarNormalArg))
						{	// Store normal background
							ui_trap->Cvar_Set("currentObjMapIconBackground", cvarNormalArg);
							// Get higlight background
							if (String_Parse(args, &cvarLitArg))
							{	// set hightlight background
								ui_trap->Cvar_VariableStringBuffer( cvarLitArg, string, sizeof(string) );
		...
		else if (Q_stricmp(name, "updatesiegecvars") == 0)
		{
			...
			team = (int)ui_trap->Cvar_VariableValue("ui_holdteam");
			baseClass = (int)ui_trap->Cvar_VariableValue("ui_siege_class");
		...
		else if (Q_stricmp(name, "setteamclassicons") == 0)
		{
			...
			ui_trap->Cvar_VariableStringBuffer( "ui_mySiegeClass", classString, sizeof(classString) );

			ui_trap->Cvar_Set("ui_startsiegeteam", va("%d", team));
		 	ui_trap->Cvar_Set( "ui_startsiegeclass", classString);
		...
		else if (Q_stricmp(name, "updatesiegeweapondesc") == 0)
		{
			...
			if (menu)
			{
				...
				{
					char	info[MAX_INFO_VALUE];
					ui_trap->Cvar_VariableStringBuffer( va("ui_class_weapondesc%i", item->cursorPos), info, sizeof(info) );
					ui_trap->Cvar_Set( "ui_itemforceinvdesc", info );
				}
			}
		}
		else if (Q_stricmp(name, "updatesiegeinventorydesc") == 0)
		{
			...
			if (menu)
			{
				...
				{
					char info[MAX_INFO_VALUE];
					ui_trap->Cvar_VariableStringBuffer( va("ui_class_itemdesc%i", item->cursorPos), info, sizeof(info) );
					ui_trap->Cvar_Set( "ui_itemforceinvdesc", info );
				}
			}
		}
		else if (Q_stricmp(name, "updatesiegeforcedesc") == 0)
		{
			...
			if (menu)
			{
				...
				if (item)
				{
					...
					ui_trap->Cvar_VariableStringBuffer( va("ui_class_power%i", item->cursorPos), info, sizeof(info) );
					...
					for (i=0;i< NUM_FORCE_POWERS;i++)
					{
						...
						{
							ui_trap->Cvar_Set( "ui_itemforceinvdesc", forcepowerDesc[i] );
						}
					}
				}
			}
		...
		else if (Q_stricmp(name, "setcurrentNetMap") == 0)
		{
			...
			if (menu)
			{
				...
				if (item)
				{
					...
					if (listPtr)
					{
						ui_trap->Cvar_Set("ui_currentNetMap", va("%d",listPtr->cursorPos));
					}
				}
			}
		}
		...
		{
			ui_trap->Cvar_Set("ui_mousePitch", (ui_trap->Cvar_VariableValue("m_pitch") >= 0) ? "0" : "1");
		}
...
void UI_SetSiegeTeams(void)
{
	...
	if (ui_trap->GetConfigString( CS_SERVERINFO, info, sizeof(info) ))
	{
	...
	len = ui_trap->FS_Open(levelname, &f, FS_READ);
	...
	ui_trap->FS_Read(siege_info, len, f);
	...
	if (BG_SiegeGetValueGroup(siege_info, "Teams", teams))
	{
		...
		ui_trap->Cvar_VariableStringBuffer("cg_siegeTeam1", buf, 1024);
		...
		ui_trap->Cvar_VariableStringBuffer("cg_siegeTeam2", buf, 1024);
		if (buf[0] && Q_stricmp(buf, "none"))
		{
...
static void UI_SiegeClassCnt( const int team )
{
	...
	ui_trap->Cvar_Set("ui_infantry_cnt", va("%d", BG_SiegeCountBaseClass(team,0)));
	ui_trap->Cvar_Set("ui_vanguard_cnt", va("%d", BG_SiegeCountBaseClass(team,1)));
	ui_trap->Cvar_Set("ui_support_cnt", va("%d", BG_SiegeCountBaseClass(team,2)));
	ui_trap->Cvar_Set("ui_jedi_cnt", va("%d", BG_SiegeCountBaseClass(team,3)));
	ui_trap->Cvar_Set("ui_demo_cnt", va("%d", BG_SiegeCountBaseClass(team,4)));
	ui_trap->Cvar_Set("ui_heavy_cnt", va("%d", BG_SiegeCountBaseClass(team,5)));
}
...
qboolean UI_hasSkinForBase(const char *base, const char *team) {
	...
	ui_trap->FS_Open(test, &f, FS_READ);
	if (f != 0) {
		ui_trap->FS_Close(f);
		return qtrue;
	}
...
static void UI_InsertServerIntoDisplayList(int num, int position) {
	...
	ui_trap->LAN_GetServerInfo( UI_SourceForLAN(), num, info, sizeof(info) );
...
static void UI_BinaryServerInsertion(int num) {
	...
	while(mid > 0) {
		mid = len >> 1;
		//
		res = ui_trap->LAN_CompareServers( UI_SourceForLAN(), uiInfo.serverStatus.sortKey,
...
//	qboolean startRefresh = qtrue; TTimo: unused
	...
	ui_trap->Cvar_VariableStringBuffer( "cl_motdString", uiInfo.serverStatus.motd, sizeof(uiInfo.serverStatus.motd) );
	...
	if (force) {
		...
		ui_trap->LAN_MarkServerVisible(lanSource, -1, qtrue);
	}

	// get the server count (comes from the master)
	count = ui_trap->LAN_GetServerCount(lanSource);
	...
	ui_trap->Cvar_Update( &ui_browserFilterInvalidInfo );
	ui_trap->Cvar_Update( &ui_browserShowEmpty );
	ui_trap->Cvar_Update( &ui_browserShowFull );
	ui_trap->Cvar_Update( &ui_browserShowPasswordProtected );
	ui_trap->Cvar_Update( &ui_serverFilterType );
	ui_trap->Cvar_Update( &ui_joinGametype );
...
//	visible = qfalse;
	for (i = 0; i < count; i++) {
		...
		if (!ui_trap->LAN_ServerIsVisible(lanSource, i)) {
			continue;
		}
//		visible = qtrue;
		// get the ping for this server
		ping = ui_trap->LAN_GetServerPing(lanSource, i);
		if (ping > 0 || ui_netSource.integer == UIAS_FAVORITES) {

			ui_trap->LAN_GetServerInfo(lanSource, i, info, MAX_STRING_CHARS);

			// don't list servers with invalid info
			if ( ui_browserFilterInvalidInfo.integer != 0 && !UI_ServerInfoIsValid( info ) ) {
				ui_trap->LAN_MarkServerVisible( lanSource, i, qfalse );
				continue;
			}
...
static int UI_GetServerStatusInfo( const char *serverAddress, serverStatusInfo_t *info ) {
	...
	if (!info) {
		ui_trap->LAN_ServerStatus( serverAddress, NULL, 0);
		return qfalse;
	}
	memset(info, 0, sizeof(*info));
	if ( ui_trap->LAN_ServerStatus( serverAddress, info->text, sizeof(info->text)) ) {
...
static void UI_BuildFindPlayerList(qboolean force) {
	...
	if (!force) {
		...
		ui_trap->Cvar_VariableStringBuffer( "ui_findPlayer", uiInfo.findPlayerName, sizeof(uiInfo.findPlayerName));
		...
		ui_trap->Cvar_Set("cl_serverStatusResendTime", va("%d", resend));
		// reset all server status requests
		ui_trap->LAN_ServerStatus( NULL, NULL, 0);
		//
		uiInfo.numFoundPlayerServers = 1;

		ui_trap->SE_GetStringTextString("MENUS_SEARCHING", holdSPString, sizeof(holdSPString));
		ui_trap->Cvar_Set( "ui_playerServersFound", va(	holdSPString,uiInfo.pendingServerStatus.num, numFound));
	...
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
		...
		if (!uiInfo.pendingServerStatus.server[i].valid ||
			...
			if (uiInfo.pendingServerStatus.num < uiInfo.serverStatus.numDisplayServers) {
				...
				lanSource = UI_SourceForLAN();
				ui_trap->LAN_GetServerAddressString(lanSource, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num],
							uiInfo.pendingServerStatus.server[i].adrstr, sizeof(uiInfo.pendingServerStatus.server[i].adrstr));
				ui_trap->LAN_GetServerInfo(lanSource, uiInfo.serverStatus.displayServers[uiInfo.pendingServerStatus.num], infoString, sizeof(infoString));
	...
	if (i < MAX_SERVERSTATUSREQUESTS) {
		...
		ui_trap->SE_GetStringTextString("MENUS_SERVERS_FOUNDWITH", holdSPString, sizeof(holdSPString));
		// add a line that shows the number of servers found
		if (!uiInfo.numFoundPlayerServers)
		{
			ui_trap->Cvar_Set( "ui_playerServersFound", va(	holdSPString,
														0,
														"s",
...
static int UI_FeederCount(float feederID)
{
	...
	switch ( (int)feederID )
	{
		...
		case FEEDER_SIEGE_BASE_CLASS:
			team = (int)ui_trap->Cvar_VariableValue("ui_team");
			baseClass = (int)ui_trap->Cvar_VariableValue("ui_siege_class");
		...
		case FEEDER_SIEGE_CLASS_WEAPONS:
			...
			for (i=0;i< WP_NUM_WEAPONS;i++)
			{
				ui_trap->Cvar_VariableStringBuffer( va("ui_class_weapon%i", i), info, sizeof(info) );
		...
		case FEEDER_SIEGE_CLASS_INVENTORY:
			...
			{
				ui_trap->Cvar_VariableStringBuffer( va("ui_class_item%i", i), info, sizeof(info) );
		...
		case FEEDER_SIEGE_CLASS_FORCE:
			...
			for (i=0;i< NUM_FORCE_POWERS;i++)
			{
				ui_trap->Cvar_VariableStringBuffer( va("ui_class_power%i", i), info, sizeof(info) );
				if (Q_stricmp(info,"gfx/2d/select")!=0)
				{
					count++;
				}
...
static void UI_UpdatePendingPings() {
	ui_trap->LAN_ResetPings(UI_SourceForLAN());
...
static const char *UI_FeederItemText(float feederID, int index, int column,
	...
	*handle1 = *handle2 = *handle3 = -1;
	...
	else if (feederID == FEEDER_COLORCHOICES)
	{
		...
		{
			*handle1 = ui_trap->R_RegisterShaderNoMip( uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Color[index].shader);
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		...
		{
			*handle1 = ui_trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name, uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].name));
	...
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		...
		{
			*handle1 = ui_trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name, uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorso[index].name));
	...
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		...
		{
			*handle1 = ui_trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name, uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index].name));
...
static qhandle_t UI_FeederItemImage(float feederID, int index) {
	...
	else if (feederID == FEEDER_Q3HEADS)
	{
		...
		{ //we want it to load them as it draws them, like the TA feeder
			...
			int selModel = ui_trap->Cvar_VariableValue("ui_selectedModelIndex");
			...
			{ //this isn't the best way of doing this I guess, but I didn't want a whole seperate string array
			  ...
			  //print what's in q3HeadNames and the icon name would look funny.
				...
				uiInfo.q3HeadIcons[index] = ui_trap->R_RegisterShaderNoMip(iconNameFromSkinName);
			}
			return uiInfo.q3HeadIcons[index];
		}
    }
	...
	else if (feederID == FEEDER_ALLMAPS || feederID == FEEDER_MAPS)
	{
		...
		if (index >= 0 && index < uiInfo.mapCount) {
			if (uiInfo.mapList[index].levelShot == -1) {
				uiInfo.mapList[index].levelShot = ui_trap->R_RegisterShaderNoMip(uiInfo.mapList[index].imageName);
			}
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount)
		{
			...
			return ui_trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name, uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].name));
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount)
		{
			...
			return ui_trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name, uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorso[index].name));
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLegCount)
		{
			...
			return ui_trap->R_RegisterShaderNoMip(va("models/players/%s/icon_%s", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Name, uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index].name));
		}
	}
	else if (feederID == FEEDER_COLORCHOICES)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].ColorCount)
		{
			return ui_trap->R_RegisterShaderNoMip( uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].Color[index].shader);
		}
	}
	...
	else if ( feederID == FEEDER_SIEGE_CLASS_WEAPONS)
	{
		...
		for (i=0;i< WP_NUM_WEAPONS;i++)
		{
			...
			if (Q_stricmp(info,"gfx/2d/select")!=0)
			{
				if (validCnt == index)
				{
					return(ui_trap->R_RegisterShaderNoMip(info));
				}
				validCnt++;
			}
		}
	...
	else if ( feederID == FEEDER_SIEGE_CLASS_FORCE)
	{
		...
		for (i=0;i< NUM_FORCE_POWERS;i++)
		{
			...
			if (Q_stricmp(info,"gfx/2d/select")!=0)
			{
				if (validCnt == index)
				{
					ui_trap->Cvar_VariableStringBuffer( va("ui_class_powerlevel%i", validCnt), info2, sizeof(info2) );

					ui_trap->Cvar_Set(va("ui_class_powerlevelslot%i", index-slotI), info2);
					return(ui_trap->R_RegisterShaderNoMip(info));
				}
				validCnt++;
			}
...
qboolean UI_FeederSelection(float feederFloat, int index, itemDef_t *item)
{
	...
	if (feederID == FEEDER_Q3HEADS)
	{
		...
		ui_trap->Cvar_Set("ui_selectedModelIndex", va("%i", index));
		index = actual;
		if (index >= 0 && index < uiInfo.q3HeadCount)
		{
			ui_trap->Cvar_Set( "model", uiInfo.q3HeadNames[index]);	//standard model
			ui_trap->Cvar_Set ( "char_color_red", "255" );			//standard colors
			ui_trap->Cvar_Set ( "char_color_green", "255" );
			ui_trap->Cvar_Set ( "char_color_blue", "255" );
		}
	}
	else if (feederID == FEEDER_MOVES)
	{
			...
			if (item)
			{
				...
				if (modelPtr)
				{
					...
					if (datapadMoveData[uiInfo.movesTitleIndex][index].anim)
					{
						...
						{
							ui_trap->S_StartLocalSound( uiInfo.uiDC.Assets.moveJumpSound, CHAN_LOCAL );
						}
						else if (datapadMoveData[uiInfo.movesTitleIndex][index].sound == MDS_ROLL)
						{
							ui_trap->S_StartLocalSound( uiInfo.uiDC.Assets.moveRollSound, CHAN_LOCAL );
						}
						else if (datapadMoveData[uiInfo.movesTitleIndex][index].sound == MDS_SABER)
						{
							...
							ui_trap->S_StartLocalSound( *soundPtr, CHAN_LOCAL );
						}

						if (datapadMoveData[uiInfo.movesTitleIndex][index].desc)
						{
							ui_trap->Cvar_Set( "ui_move_desc", datapadMoveData[uiInfo.movesTitleIndex][index].desc);
						}
					}
					UI_SaberAttachToChar( item );
				}
	...
	else if (feederID == FEEDER_MAPS || feederID == FEEDER_ALLMAPS)
	{
		...
		if (uiInfo.mapList[map].cinematic >= 0) {
		  ui_trap->CIN_StopCinematic(uiInfo.mapList[map].cinematic);
		...
		ui_trap->Cvar_Set("ui_mapIndex", va("%d", index));
		gUISelectedMap = index;
		ui_mapIndex.integer = index;

		if (feederID == FEEDER_MAPS) {
			ui_trap->Cvar_Set("ui_currentMap", va("%d", actual));
			ui_trap->Cvar_Update(&ui_currentMap);
			uiInfo.mapList[ui_currentMap.integer].cinematic = ui_trap->CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[ui_currentMap.integer].mapLoadName), 0, 0, 0, 0, (CIN_loop | CIN_silent) );
			//ui_trap->Cvar_Set("ui_opponentModel", uiInfo.mapList[ui_currentMap.integer].opponentName);
			//updateOpponentModel = qtrue;
		} else {
			ui_trap->Cvar_Set("ui_currentNetMap", va("%d", actual));
			ui_trap->Cvar_Update(&ui_currentNetMap);
			uiInfo.mapList[ui_currentNetMap.integer].cinematic = ui_trap->CIN_PlayCinematic(va("%s.roq", uiInfo.mapList[ui_currentNetMap.integer].mapLoadName), 0, 0, 0, 0, (CIN_loop | CIN_silent) );
		}
		...
		ui_trap->LAN_GetServerInfo(UI_SourceForLAN(), uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS);
		uiInfo.serverStatus.currentServerPreview = ui_trap->R_RegisterShaderNoMip(va("levelshots/%s", Info_ValueForKey(info, "mapname")));
		if (uiInfo.serverStatus.currentServerCinematic >= 0) {
			ui_trap->CIN_StopCinematic(uiInfo.serverStatus.currentServerCinematic);
			uiInfo.serverStatus.currentServerCinematic = -1;
		}
		mapName = Info_ValueForKey(info, "mapname");
		if (mapName && *mapName) {
			uiInfo.serverStatus.currentServerCinematic = ui_trap->CIN_PlayCinematic(va("%s.roq", mapName), 0, 0, 0, 0, (CIN_loop | CIN_silent) );
		}
	} else if (feederID == FEEDER_SERVERSTATUS) {
		//
	} else if (feederID == FEEDER_FINDPLAYER) {
	...
	else if (feederID == FEEDER_PLAYER_SKIN_HEAD)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHeadCount)
		{
			ui_trap->Cvar_Set("ui_char_skin_head", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinHead[index].name);
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_TORSO)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorsoCount)
		{
			ui_trap->Cvar_Set("ui_char_skin_torso", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinTorso[index].name);
		}
	}
	else if (feederID == FEEDER_PLAYER_SKIN_LEGS)
	{
		if (index >= 0 && index < uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLegCount)
		{
			ui_trap->Cvar_Set("ui_char_skin_legs", uiInfo.playerSpecies[uiInfo.playerSpeciesIndex].SkinLeg[index].name);
		}
	}
	else if (feederID == FEEDER_PLAYER_SPECIES)
	{
	...
	{
//		ui_trap->Cvar_VariableStringBuffer( va("ui_class_weapondesc%i", index), info, sizeof(info) );
//		ui_trap->Cvar_Set( "ui_itemforceinvdesc", info );
	}
	else if (feederID == FEEDER_SIEGE_CLASS_INVENTORY)
	{
//		ui_trap->Cvar_VariableStringBuffer( va("ui_class_itemdesc%i", index), info, sizeof(info) );
//		ui_trap->Cvar_Set( "ui_itemforceinvdesc", info );
	}
	else if (feederID == FEEDER_SIEGE_CLASS_FORCE)
	{
...
//		int validCnt = 0;
		...
		ui_trap->Cvar_VariableStringBuffer( va("ui_class_power%i", index), info, sizeof(info) );
	...
	while ( 1 ) {
		...
		if (token[0] == '{') {
			...
			//if (uiInfo.mapCount == 0) {
  			...
  			//  uiInfo.mapList[uiInfo.mapCount].cinematic = ui_trap->CIN_PlayCinematic(va("%s.roq",uiInfo.mapList[uiInfo.mapCount].mapLoadName), qfalse, qfalse, qtrue, 0, 0, 0, 0);
			//}
  		uiInfo.mapList[uiInfo.mapCount].cinematic = -1;
			uiInfo.mapList[uiInfo.mapCount].levelShot = ui_trap->R_RegisterShaderNoMip(va("levelshots/%s_small", uiInfo.mapList[uiInfo.mapCount].mapLoadName));
...
static int UI_PlayCinematic(const char *name, float x, float y, float w, float h) {
	return ui_trap->CIN_PlayCinematic(name, x, y, w, h, (CIN_loop | CIN_silent));
}

static void UI_StopCinematic(int handle) {
	if (handle >= 0) {
		ui_trap->CIN_StopCinematic(handle);
	} else {
		handle = abs(handle);
		if (handle == UI_MAPCINEMATIC) {
			if (uiInfo.mapList[ui_currentMap.integer].cinematic >= 0) {
				ui_trap->CIN_StopCinematic(uiInfo.mapList[ui_currentMap.integer].cinematic);
			...
			if (i >= 0 && i < uiInfo.teamCount) {
				if (uiInfo.teamList[i].cinematic >= 0) {
					ui_trap->CIN_StopCinematic(uiInfo.teamList[i].cinematic);
...
static void UI_DrawCinematic(int handle, float x, float y, float w, float h) {
	ui_trap->CIN_SetExtents(handle, x, y, w, h);
	ui_trap->CIN_DrawCinematic(handle);
}

static void UI_RunCinematicFrame(int handle) {
	ui_trap->CIN_RunCinematic(handle);
}
...
nextSearch:
	...
	{ //search light side folder
		numfiles = ui_trap->FS_GetFileList("forcecfg/light", "fcf", filelist, 2048 );
		uiInfo.forceConfigLightIndexBegin = uiInfo.forceConfigCount-1;
	}
	else
	{ //search dark side folder
		numfiles = ui_trap->FS_GetFileList("forcecfg/dark", "fcf", filelist, 2048 );
...
static qboolean bIsImageFile(const char* dirptr, const char* skinname)
{
	...
	ui_trap->FS_Open(fpath, &f, FS_READ);
	if (!f)
	{ //not there, try png
...
static void UI_BuildQ3Model_List( void )
{
	...
	numdirs = ui_trap->FS_GetFileList("models/players", "/", dirlist, 2048 );
	dirptr  = dirlist;
	for (i=0; i<numdirs && uiInfo.q3HeadCount < MAX_Q3PLAYERMODELS; i++,dirptr+=dirlen+1)
	{
		...
		numfiles = ui_trap->FS_GetFileList( va("models/players/%s",dirptr), "skin", filelist, 2048 );
		fileptr  = filelist;
		for (j=0; j<numfiles && uiInfo.q3HeadCount < MAX_Q3PLAYERMODELS;j++,fileptr+=filelen+1)
		{
			...
			*/
			...
			{ //if it exists
				...
				uiInfo.q3HeadIcons[uiInfo.q3HeadCount++] = 0;//ui_trap->R_RegisterShaderNoMip(fpath);
...
void UI_SiegeInit(void)
{
	...
	BG_SiegeLoadClasses< moduleType::ui >(g_UIClassDescriptions);
	...
	BG_SiegeLoadTeams< moduleType::ui >();
...
static void UI_BuildPlayerModel_List( qboolean inGameLoad )
{
	...
	// iterate directory of all player models
	numdirs = ui_trap->FS_GetFileList("models/players", "/", dirlist, dirListSize );
	dirptr  = dirlist;
	for (i=0; i<numdirs; i++,dirptr+=dirlen+1)
	{
		...
		filelen = ui_trap->FS_Open(fpath, &f, FS_READ);
		...
		if (f)
		{
			...
			ui_trap->FS_Read(buffer, filelen, f);
			ui_trap->FS_Close(f);
			...
			numfiles = ui_trap->FS_GetFileList( va("models/players/%s",dirptr), ".skin", filelist, sizeof(filelist) );
			fileptr  = filelist;
			for (j=0; j<numfiles; j++,fileptr+=filelen+1)
			{
				if (ui_trap->Cvar_VariableValue("fs_copyfiles") > 0 )
				{
					ui_trap->FS_Open(va("models/players/%s/%s",dirptr,fileptr), &f, FS_READ);
					if (f)
						ui_trap->FS_Close(f);
				}
			...
			if (!inGameLoad && ui_PrecacheModels.integer)
			{
				...
				g2Model = ui_trap->G2API_InitGhoul2Model(&ghoul2, fpath, 0, 0, 0, 0, 0);
				if (g2Model >= 0)
				{
//					ui_trap->G2API_RemoveGhoul2Model( &ghoul2, 0 );
					ui_trap->G2API_CleanGhoul2Models (&ghoul2);
				}
			}
		}
	}
...
static qhandle_t UI_RegisterShaderNoMip( const char *name ) {
	if ( *name == '*' ) {
		...
		ui_trap->Cvar_VariableStringBuffer( name+1, buf, sizeof( buf ) );

		if ( buf[0] )
			return ui_trap->R_RegisterShaderNoMip( buf );
	}

	return ui_trap->R_RegisterShaderNoMip( name );
}
...
void UI_Init( qboolean inGameLoad ) {
	...
	uiInfo.languageCount = ui_trap->SE_GetNumLanguages();	// this does a dir scan, so use carefully
	...
	UI_RegisterCvars();
	UI_InitMemory< moduleType::ui >();

	// cache redundant calulations
	ui_trap->GetGlconfig( &uiInfo.uiDC.glconfig );
	...
	uiInfo.uiDC.drawStretchPic					= ui_trap->R_DrawStretchPic;
	uiInfo.uiDC.drawText						= &Text_Paint;
	uiInfo.uiDC.textWidth						= &Text_Width;
	uiInfo.uiDC.textHeight						= &Text_Height;
	uiInfo.uiDC.registerModel					= ui_trap->R_RegisterModel;
	uiInfo.uiDC.modelBounds						= ui_trap->R_ModelBounds;
	uiInfo.uiDC.fillRect						= &UI_FillRect;
	uiInfo.uiDC.drawRect						= &_UI_DrawRect;
	uiInfo.uiDC.drawSides						= &_UI_DrawSides;
	uiInfo.uiDC.drawTopBottom					= &_UI_DrawTopBottom;
	uiInfo.uiDC.clearScene						= ui_trap->R_ClearScene;
	uiInfo.uiDC.drawSides						= &_UI_DrawSides;
	uiInfo.uiDC.addRefEntityToScene				= ui_trap->R_AddRefEntityToScene;
	uiInfo.uiDC.renderScene						= ui_trap->R_RenderScene;
	uiInfo.uiDC.RegisterFont					= ui_trap->R_RegisterFont;
	uiInfo.uiDC.Font_StrLenPixels				= ui_trap->R_Font_StrLenPixels;
	uiInfo.uiDC.Font_StrLenChars				= ui_trap->R_Font_StrLenChars;
	uiInfo.uiDC.Font_HeightPixels				= ui_trap->R_Font_HeightPixels;
	uiInfo.uiDC.Font_DrawString					= ui_trap->R_Font_DrawString;
	uiInfo.uiDC.Language_IsAsian				= ui_trap->R_Language_IsAsian;
	uiInfo.uiDC.Language_UsesSpaces				= ui_trap->R_Language_UsesSpaces;
	uiInfo.uiDC.AnyLanguage_ReadCharFromString	= ui_trap->R_AnyLanguage_ReadCharFromString;
	...
	uiInfo.uiDC.setCVar							= ui_trap->Cvar_Set;
	uiInfo.uiDC.getCVarString					= ui_trap->Cvar_VariableStringBuffer;
	uiInfo.uiDC.getCVarValue					= ui_trap->Cvar_VariableValue;
	uiInfo.uiDC.drawTextWithCursor				= &Text_PaintWithCursor;
	uiInfo.uiDC.setOverstrikeMode				= ui_trap->Key_SetOverstrikeMode;
	uiInfo.uiDC.getOverstrikeMode				= ui_trap->Key_GetOverstrikeMode;
	uiInfo.uiDC.startLocalSound					= ui_trap->S_StartLocalSound;
	uiInfo.uiDC.ownerDrawHandleKey				= &UI_OwnerDrawHandleKey;
	uiInfo.uiDC.feederCount						= &UI_FeederCount;
	uiInfo.uiDC.feederItemImage					= &UI_FeederItemImage;
	uiInfo.uiDC.feederItemText					= &UI_FeederItemText;
	uiInfo.uiDC.feederSelection					= &UI_FeederSelection;
	uiInfo.uiDC.setBinding						= ui_trap->Key_SetBinding;
	uiInfo.uiDC.getBindingBuf					= ui_trap->Key_GetBindingBuf;
	uiInfo.uiDC.keynumToStringBuf				= ui_trap->Key_KeynumToStringBuf;
	uiInfo.uiDC.executeText						= ui_trap->Cmd_ExecuteText;
	uiInfo.uiDC.Error							= Com_Error;
	uiInfo.uiDC.Print							= Com_Printf;
	uiInfo.uiDC.Pause							= &UI_Pause;
	uiInfo.uiDC.ownerDrawWidth					= &UI_OwnerDrawWidth;
	uiInfo.uiDC.registerSound					= ui_trap->S_RegisterSound;
	uiInfo.uiDC.startBackgroundTrack			= ui_trap->S_StartBackgroundTrack;
	uiInfo.uiDC.stopBackgroundTrack				= ui_trap->S_StopBackgroundTrack;
	uiInfo.uiDC.playCinematic					= &UI_PlayCinematic;
	uiInfo.uiDC.stopCinematic					= &UI_StopCinematic;
	uiInfo.uiDC.drawCinematic					= &UI_DrawCinematic;
	uiInfo.uiDC.runCinematicFrame				= &UI_RunCinematicFrame;
	uiInfo.uiDC.ext.Font_StrLenPixels			= ui_trap->ext.R_Font_StrLenPixels;
	...
	uiInfo.uiDC.cursor	= ui_trap->R_RegisterShaderNoMip( "menu/art/3_cursor2" );
	uiInfo.uiDC.whiteShader = ui_trap->R_RegisterShaderNoMip( "white" );
...

	{
		char buf[MAX_NETNAME] = {0};
		Q_strncpyz( buf, UI_Cvar_VariableString( "name" ), sizeof( buf ) );
		ui_trap->Cvar_Register( NULL, "ui_Name", buf, CVAR_INTERNAL );
	}

	Menus_CloseAll();

	ui_trap->LAN_LoadCachedServers();
	...
	uiInfo.currentCrosshair = (int)ui_trap->Cvar_VariableValue("cg_drawCrosshair");
	ui_trap->Cvar_Set("ui_mousePitch", (ui_trap->Cvar_VariableValue("m_pitch") >= 0) ? "0" : "1");
	ui_trap->Cvar_Set("ui_mousePitchVeh", (ui_trap->Cvar_VariableValue("m_pitchVeh") >= 0) ? "0" : "1");

	uiInfo.serverStatus.currentServerCinematic = -1;
	uiInfo.previewMovie = -1;

	ui_trap->Cvar_Register(NULL, "debug_protocol", "", 0 );

	ui_trap->Cvar_Set("ui_actualNetGameType", va("%d", ui_netGametype.integer));
	ui_trap->Cvar_Update(&ui_actualNetGametype);
}
...
void UI_Refresh( int realtime )
{
	...
	//if ( !( ui_trap->Key_GetCatcher() & KEYCATCH_UI ) ) {
	//	return;
	//}

	ui_trap->G2API_SetTime(realtime, 0);
	ui_trap->G2API_SetTime(realtime, 1);
	...
	// draw cursor
	UI_SetColor( NULL );
	if (Menu_Count() > 0 && (ui_trap->Key_GetCatcher() & KEYCATCH_UI)) {
	...
	if (ui_rankChange.integer)
	{
		...
		ui_trap->Cvar_Set("ui_rankChange", "0");
...
void UI_KeyEvent( int key, qboolean down ) {
	if (Menu_Count() > 0) {
		...
		if (menu) {
			if (key == A_ESCAPE && down && !Menus_AnyFullScreenVisible()) {
				...
				Menu_HandleKey< moduleType::ui >(menu, key, down );
			}
		} else {
			ui_trap->Key_SetCatcher( ui_trap->Key_GetCatcher() & ~KEYCATCH_UI );
			ui_trap->Key_ClearStates();
  ...
  //if ((s > 0) && (s != menu_null_sound)) {
	//  ui_trap->S_StartLocalSound( s, CHAN_LOCAL_SOUND );
  //}
}
...
static void UI_DisplayDownloadInfo( const char *downloadName, float centerPoint, float yStart, float scale, int iMenuFont) {
	...
	downloadSize = ui_trap->Cvar_VariableValue( "cl_downloadSize" );
	downloadCount = ui_trap->Cvar_VariableValue( "cl_downloadCount" );
	downloadTime = ui_trap->Cvar_VariableValue( "cl_downloadTime" );
...
void UI_DrawConnectScreen( qboolean overlay ) {
	...
	if ( !overlay && menu ) {
		Menu_Paint< moduleType::ui >(menu, qtrue);
	}
	...
	ui_trap->GetClientState( &cstate );


	info[0] = '\0';
	if( ui_trap->GetConfigString( CS_SERVERINFO, info, sizeof(info) ) ) {
		ui_trap->SE_GetStringTextString("MENUS_LOADING_MAPNAME", sStringEdTemp, sizeof(sStringEdTemp));
	...
	if (!Q_stricmp(cstate.servername,"localhost")) {
		ui_trap->SE_GetStringTextString("MENUS_STARTING_UP", sStringEdTemp, sizeof(sStringEdTemp));
		Text_PaintCenter(centerPoint, yStart + 48, scale, colorWhite, sStringEdTemp, ITEM_TEXTSTYLE_SHADOWEDMORE, FONT_MEDIUM);
	} else {
		ui_trap->SE_GetStringTextString("MENUS_CONNECTING_TO", sStringEdTemp, sizeof(sStringEdTemp));
	...
	case CA_CONNECTING:
		{
			ui_trap->SE_GetStringTextString("MENUS_AWAITING_CONNECTION", sStringEdTemp, sizeof(sStringEdTemp));
			s = va(/*"Awaiting connection...%i"*/sStringEdTemp, cstate.connectPacketCount);
		}
		break;
	case CA_CHALLENGING:
		{
			ui_trap->SE_GetStringTextString("MENUS_AWAITING_CHALLENGE", sStringEdTemp, sizeof(sStringEdTemp));
	...
	case CA_CONNECTED: {
		char downloadName[MAX_INFO_VALUE];
			...
			ui_trap->Cvar_VariableStringBuffer( "cl_downloadName", downloadName, sizeof(downloadName) );
			if (*downloadName) {
				UI_DisplayDownloadInfo( downloadName, centerPoint, yStart, scale, FONT_MEDIUM );
				return;
			}
		}
		ui_trap->SE_GetStringTextString("MENUS_AWAITING_GAMESTATE", sStringEdTemp, sizeof(sStringEdTemp));
...
static void UI_DoServerRefresh( void )
{
	...
	if (ui_netSource.integer != UIAS_FAVORITES) {
		if (ui_netSource.integer == UIAS_LOCAL) {
			if (!ui_trap->LAN_GetServerCount(AS_LOCAL)) {
				wait = qtrue;
			}
		} else {
			if (ui_trap->LAN_GetServerCount(AS_GLOBAL) < 0) {
				wait = qtrue;
			}
		}
	}
	...
	if (ui_trap->LAN_UpdateVisiblePings(UI_SourceForLAN())) {
		uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 1000;
	} else if (!wait) {
...
static void UI_StartServerRefresh(qboolean full)
{
	...
	qtime_t q;
	ui_trap->RealTime(&q);
 	ui_trap->Cvar_Set( va("ui_lastServerRefresh_%i", ui_netSource.integer), va("%s-%i, %i @ %i:%02i", GetMonthAbbrevString(q.tm_mon),q.tm_mday, 1900+q.tm_year,q.tm_hour,q.tm_min));
	...
	ui_trap->LAN_MarkServerVisible(lanSource, -1, qtrue);
	// reset all the pings
	ui_trap->LAN_ResetPings(lanSource);
	//
	if( ui_netSource.integer == UIAS_LOCAL ) {
		ui_trap->Cmd_ExecuteText( EXEC_NOW, "localservers\n" );
		uiInfo.serverStatus.refreshtime = uiInfo.uiDC.realTime + 1000;
		return;
	}
	...
	if( ui_netSource.integer >= UIAS_GLOBAL1 && ui_netSource.integer <= UIAS_GLOBAL5 ) {
		...
		if (strlen(ptr)) {
			ui_trap->Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %s full empty\n", ui_netSource.integer-1, ptr));
		}
		else {
			ui_trap->Cmd_ExecuteText( EXEC_NOW, va( "globalservers %d %d full empty\n", ui_netSource.integer-1, (int)ui_trap->Cvar_VariableValue( "protocol" ) ) );
		}
	}
}
...
uiExport_t* GetUIAPI( int apiVersion, uiImport_t *import )
{
	...
	if ( apiVersion != UI_API_VERSION ) {
		ui_trap->Print( "Mismatched UI_API_VERSION: expected %i, got %i\n", UI_API_VERSION, apiVersion );
		return NULL;
	}
