...
static void UI_UpdateScreenshot( void )
{
	...
	if ( ui_screenshotType.string[0] && isalpha( ui_screenshotType.string[0] ) )
	{
		...
		if ( ssf == -1 )
		{
			ui_trap->Print( "UI Screenshot Format Type '%s' unrecognised, defaulting to JPEG\n", ui_screenshotType.string );
	...
	{
		ui_trap->Print( "ui_screenshotType %i is out of range, defaulting to 0 (JPEG)\n", ui_screenshotType.integer );
		uiInfo.uiDC.screenshotFormat = SSF_JPEG;
		changed = qtrue;
	}
	else {
	...
	if ( changed ) {
		ui_trap->Cvar_Set( "ui_screenshotType", UI_GetScreenshotFormatString( uiInfo.uiDC.screenshotFormat ) );
		ui_trap->Cvar_Update( &ui_screenshotType );
	}
}
...
void UI_RegisterCvars( void ) {
	...
	for ( i=0, cv=uiCvarTable; i<uiCvarTableSize; i++, cv++ ) {
		ui_trap->Cvar_Register( cv->vmCvar, cv->cvarName, cv->defaultString, cv->cvarFlags );
		if ( cv->update )
			cv->update();
	}
}
