...
{//FIXME: these get fucked by vid_restarts
	redSaberGlowShader			= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/red_glow" );
	redSaberCoreShader			= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/red_line" );
	orangeSaberGlowShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/orange_glow" );
	orangeSaberCoreShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/orange_line" );
	yellowSaberGlowShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/yellow_glow" );
	yellowSaberCoreShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/yellow_line" );
	greenSaberGlowShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/green_glow" );
	greenSaberCoreShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/green_line" );
	blueSaberGlowShader			= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/blue_glow" );
	blueSaberCoreShader			= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/blue_line" );
	purpleSaberGlowShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/purple_glow" );
	purpleSaberCoreShader		= ui_trap->R_RegisterShaderNoMip( "gfx/effects/sabers/purple_line" );
}
...
qboolean UI_SaberProperNameForSaber( const char *saberName, char *saberProperName )
{
	...
	{
		ui_trap->SE_GetStringTextString(&stringedSaberName[1], saberProperName, 1024);
	}
	else
	{
...
void UI_SaberLoadParms( void )
{
	...
	WP_SaberLoadParms< moduleType::ui >();
}
...
void UI_DoSaber( vec3_t origin, vec3_t dir, float length, float lengthMax, float radius, saber_colors_t color )
{
	...
	ui_trap->R_AddRefEntityToScene( &saber );
...
//	vec3_t	angles={0};
	...
	{
		ui_trap->Cvar_VariableStringBuffer("ui_saber_color", bladeColorString, sizeof(bladeColorString) );
	}
	else//if ( item->flags&ITF_ISSABER2 ) - presumed
	{
		ui_trap->Cvar_VariableStringBuffer("ui_saber2_color", bladeColorString, sizeof(bladeColorString) );
	}

	if ( !ui_trap->G2API_HasGhoul2ModelOnIndex(&(item->ghoul2),saberModel) )
	{//invalid index!
		return;
	}
	...
	bolt = ui_trap->G2API_AddBolt( item->ghoul2,saberModel, tagName );

	if ( bolt == -1 )
	{
		tagHack = qtrue;
		//hmm, just fall back to the most basic tag (this will also make it work with pre-JKA saber models
		bolt = ui_trap->G2API_AddBolt( item->ghoul2,saberModel, "*flash" );
		if ( bolt == -1 )
		{//no tag_flash either?!!
			bolt = 0;
		}
...
//	angles[ROLL] = 0;
	...
	ui_trap->G2API_GetBoltMatrix( item->ghoul2, saberModel, bolt, &boltMatrix, angles, origin, uiInfo.uiDC.realTime, NULL, vec3_origin );//NULL was cgs.model_draw
...
void UI_GetSaberForMenu( char *saber, int saberNum )
{
	...
	if ( saberNum == 0 )
	{
		ui_trap->Cvar_VariableStringBuffer("ui_saber", saber, MAX_QPATH );
		if ( !UI_SaberValidForPlayerInMP( saber ) )
		{
			ui_trap->Cvar_Set( "ui_saber", DEFAULT_SABER );
			ui_trap->Cvar_VariableStringBuffer("ui_saber", saber, MAX_QPATH );
		}
	}
	else
	{
		ui_trap->Cvar_VariableStringBuffer("ui_saber2", saber, MAX_QPATH );
		if ( !UI_SaberValidForPlayerInMP( saber ) )
		{
			ui_trap->Cvar_Set( "ui_saber2", DEFAULT_SABER );
			ui_trap->Cvar_VariableStringBuffer("ui_saber2", saber, MAX_QPATH );
		}
	}
...
void UI_SaberDrawBlades( itemDef_t *item, vec3_t origin, vec3_t angles )
{
	...
	for ( saberNum = 0; saberNum < numSabers; saberNum++ )
	{
		...
		{
			ui_trap->Cvar_VariableStringBuffer("ui_saber", saber, sizeof(saber) );
			if ( !UI_SaberValidForPlayerInMP( saber ) )
			{
				ui_trap->Cvar_Set( "ui_saber", DEFAULT_SABER );
				ui_trap->Cvar_VariableStringBuffer("ui_saber", saber, sizeof(saber) );
		...
		{
			ui_trap->Cvar_VariableStringBuffer("ui_saber2", saber, sizeof(saber) );
			if ( !UI_SaberValidForPlayerInMP( saber ) )
			{
				ui_trap->Cvar_Set( "ui_saber2", DEFAULT_SABER );
				ui_trap->Cvar_VariableStringBuffer("ui_saber2", saber, sizeof(saber) );
...
void UI_SaberAttachToChar( itemDef_t *item )
{
	...
	if ( ui_trap->G2API_HasGhoul2ModelOnIndex(&(item->ghoul2),2) )
	{//remove any extra models
		ui_trap->G2API_RemoveGhoul2Model(&(item->ghoul2), 2);
	}
	if ( ui_trap->G2API_HasGhoul2ModelOnIndex(&(item->ghoul2),1) )
	{//remove any extra models
		ui_trap->G2API_RemoveGhoul2Model(&(item->ghoul2), 1);
	}
	...
	for ( saberNum = 0; saberNum < numSabers; saberNum++ )
	{
		...
		{//successfully found a model
			int g2Saber = re->G2API_InitGhoul2Model( &(item->ghoul2), modelPath, 0, 0, 0, 0, 0 ); //add the model
			if ( g2Saber )
			{
				int boltNum;
				...
				{
					int g2skin = ui_trap->R_RegisterSkin(skinPath);
					ui_trap->G2API_SetSkin( item->ghoul2, g2Saber, 0, g2skin );//this is going to set the surfs on/off matching the skin file
				}
				else
				{
					ui_trap->G2API_SetSkin( item->ghoul2, g2Saber, 0, 0 );//turn off custom skin
				}
				if ( saberNum == 0 )
				{
					boltNum = ui_trap->G2API_AddBolt( item->ghoul2, 0, "*r_hand");
				}
				else
				{
					boltNum = ui_trap->G2API_AddBolt( item->ghoul2, 0, "*l_hand");
				}
				ui_trap->G2API_AttachG2Model( item->ghoul2, g2Saber, item->ghoul2, boltNum, 0);
			}
		}
	}
}
