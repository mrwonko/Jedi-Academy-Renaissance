...
/*-- do long blocks (if any) --*/
   ...
   for (i = 0; i < n; i++)
   {
      ...
      for (j = 0; j < 9; j++)
      {
	 y[j][i] = x0[j].x + win[btype][j] * x[9 + j].x;
	 y[9 + j][i] = x0[9 + j].x + win[btype][9 + j] * x[17 - j].x;
      }
    /* window x for next time x0 */
	 ...
	 xa = x[j].x;
	 xb = x[8 - j].x;
	 x[j].x = win[btype][18 + j] * xb;
	 x[8 - j].x = win[btype][(18 + 8) - j] * xa;
	 x[9 + j].x = win[btype][(18 + 9) + j] * xa;
	 x[17 - j].x = win[btype][(18 + 17) - j] * xb;
      }
      xa = x[j].x;
      x[j].x = win[btype][18 + j] * xa;
      x[9 + j].x = win[btype][(18 + 9) + j] * xa;
...
/*-- do short blocks (if any) --*/
   ...
   for (; i < n; i++)
   {
      imdct6_3(reinterpret_cast< float*>( x ));
      for (j = 0; j < 3; j++)
      {
	 y[j][i] = x0[j].x;
	 y[3 + j][i] = x0[3 + j].x;

	 y[6 + j][i] = x0[6 + j].x + win[2][j] * x[3 + j].x;
	 y[9 + j][i] = x0[9 + j].x + win[2][3 + j] * x[5 - j].x;

	 y[12 + j][i] = x0[12 + j].x + win[2][6 + j] * x[2 - j].x + win[2][j] * x[(6 + 3) + j].x;
	 y[15 + j][i] = x0[15 + j].x + win[2][9 + j] * x[j].x + win[2][3 + j] * x[(6 + 5) - j].x;
      }
    /* window x for next time x0 */
      for (j = 0; j < 3; j++)
      {
	 x[j].x = win[2][6 + j] * x[(6 + 2) - j].x + win[2][j] * x[(12 + 3) + j].x;
	 x[3 + j].x = win[2][9 + j] * x[6 + j].x + win[2][3 + j] * x[(12 + 5) - j].x;
      }
      for (j = 0; j < 3; j++)
      {
	 x[6 + j].x = win[2][6 + j] * x[(12 + 2) - j].x;
	 x[9 + j].x = win[2][9 + j] * x[12 + j].x;
      }
      for (j = 0; j < 3; j++)
      {
	 x[12 + j].x = 0.0f;
