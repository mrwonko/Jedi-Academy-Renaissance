...
//#endif
	...
	iFrameBytes = head_info3( reinterpret_cast< unsigned char* >( pvData ), iDataLen/2, &head, &iBitRate, &iRealDataStart);
	if (iFrameBytes == 0)
	{
...
//	iFrameBytes = head_info2( pvData, 0, &head, &iBitRate);
	iFrameBytes = head_info3( reinterpret_cast< unsigned char* >( pvData ), iSourceBytesRemaining/2, &head, &iBitRate, &iRealDataStart);
