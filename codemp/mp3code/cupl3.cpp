...
static float yout[18][32];		/* hybrid out, sbt in */	//////////// scratchpad
...
/*====================================================================*/
int hybrid( SAMPLE xin[], SAMPLE xprev[], float y[ 18 ][ 32 ],
...
void ms_process( float x[], int n);	/* sum-difference stereo */
void is_process_MPEG1( float x[],	/* intensity stereo */
		      SCALEFACT * sf,
		      CB_INFO cb_info[2],	/* [ch] */
		      int nsamp, int ms_mode);
void is_process_MPEG2( float x[],	/* intensity stereo */
		      SCALEFACT * sf,
		      CB_INFO cb_info[2],	/* [ch] */
		      IS_SF_INFO * is_sf_info,
		      int nsamp, int ms_mode);

void unpack_huff( int xy[], int n, int ntable);
int unpack_huff_quad( int vwxy[], int n, int nbits, int ntable);
void dequant(SAMPLE sample[], int *nsamp,
	     SCALEFACT * sf,
	     GR * gr,
	     CB_INFO * cb_info, int ncbl_mixed);
void unpack_sf_sub_MPEG1( SCALEFACT sf[], GR * gr,
			 int scfsi,	/* bit flag */
			 int igr);
...
/*--- hybrid + sbt ---*/
   ...
   pMP3Stream->sbt_L3(yout[0], reinterpret_cast< short *>( pcm ), 0);
...
/*--- hybrid + sbt ---*/
   ...
   for (ch = 0; ch < pMP3Stream->nchan; ch++)
   {
	 ...
	 n2 = pMP3Stream->band_limit;
      ...
      pMP3Stream->sbt_L3(yout[0], reinterpret_cast< short *>( pcm ), ch);
   }
...
/*--- hybrid + sbt ---*/
       ...
       && (side_info.gr[igr][1].mixed_block_flag == 0))
   {
	 ...
	 n2 = pMP3Stream->band_limit;
      ...
      if (side_info.gr[igr][0].block_type == 2)
	 n1 = 0;
      sum_f_bands(&pMP3Stream->sample[0][igr][0].x, &pMP3Stream->sample[1][igr][0].x, n2);
...
/*-- right chan --*/
	 ...
	 if (side_info.gr[igr][1].mixed_block_flag)
      ...
      nsamp[igr][1] = hybrid_sum(&pMP3Stream->sample[1][igr][0].x, &pMP3Stream->sample[0][igr][0].x,
...
/*--- huff data ---*/
	 ...
	 n1 = n3;
      nn3 = n3 - n2;
      nn2 = n2 - n1;
      unpack_huff(&pMP3Stream->sample[ch][igr][ 0 ].s, n1, side_info.gr[igr][ch].table_select[0]);
      unpack_huff(&pMP3Stream->sample[ch][igr][ 0 ].s + n1, nn2, side_info.gr[igr][ch].table_select[1]);
      unpack_huff(&pMP3Stream->sample[ch][igr][ 0 ].s + n2, nn3, side_info.gr[igr][ch].table_select[2]);
      qbits = side_info.gr[igr][ch].part2_3_length - (bitget_bits_used() - bit0);
      nn4 = unpack_huff_quad(&pMP3Stream->sample[ch][igr][ 0 ].s + n3, pMP3Stream->band_limit - n3, qbits,
...
/*--- ms stereo processing  ---*/
   if (pMP3Stream->ms_mode)
   {
	 ...
	 m0 = pMP3Stream->sfBandIndex[cb_info[igr][1].cbtype][cb_info[igr][1].cbmax];
      ...
      ms_process(&pMP3Stream->sample[0][igr][ 0 ].x, m0);
   }
...
/*--- is stereo processing  ---*/
   ...
   {
      if (pMP3Stream->id)
	 is_process_MPEG1(&pMP3Stream->sample[0][igr][0].x, &sf[igr][1],
			  cb_info[igr], nsamp[igr][0], pMP3Stream->ms_mode);
      else
	 is_process_MPEG2(&pMP3Stream->sample[0][igr][0].x, &sf[igr][1],
...
/*--- antialias ---*/
   for (ch = 0; ch < pMP3Stream->nchan; ch++)
   {
	 ...
	 n1 = (nsamp[igr][ch] + 7) / 18;
      if (n1 > 31)
	 n1 = 31;
      antialias(&pMP3Stream->sample[ch][igr][0].x, n1);
